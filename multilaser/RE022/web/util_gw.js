...
function saveChanges_basic(form, wlan_id)
{
  ...
  // P2P_SUPPORT
  	if(mode.selectedIndex==0 || mode.selectedIndex==3) 
  	{
		...
		if (ssid.value.trim() == "") {
			alert(ssid_cannot_be_all_spaces);
			ssid.focus();
			return false;
		}
		if (byteLength(ssid.value) > 32){
			alert(install_ssid_length_should_be_maximum_32_octets);
			ssid.focus();
			return false;
		}
		if (includeSpace(ssid.value)) {
			if (!confirm(ssid_contains_spaces_continue_saving)) {
				ssid.focus();
				return false;
			} else {
...
// --------------- USB3G wanType ---------------
   ...
   if (form.wan_macAddr != null){
		if (form.wan_macAddr.value == "") {
			...
			if (!checkMacAddr(form.wan_macAddr)) {
				return false
			}
		}
   }
	...
	if (form.wan_macAddr != null) {
		form.wan_macAddr.value = form.wan_macAddr.value.replace(/:/g, "").replace(/-/g, "");
	}
	if (wanType == 0) {
		form.dnsMode = "dnsManual"
	}
	parent.showWaitDiv()
   return true;
}
...
function check_wpa_psk(form, wlan_id)
{
	...
	if (form.elements["pskFormat"+wlan_id].selectedIndex==1) {
		...
		var VisibleCharacters = /^[!-~]{8,63}$/;
		if (!VisibleCharacters.test(str)) {
			alert(install_invalid_password);
			form.elements["pskValue"+wlan_id].focus();
			return false;
		}
		if (str.length < 8) {
...
function checkMacAddr_is_legal(field)
{
	...
	//mac正则，以-分隔
	var macRegex1 = /^([A-Fa-f0-9]{2}-){5}[A-Fa-f0-9]{2}$/
	//mac正则，不分隔
	var macRegex2 = /^([0-9a-fA-F]{2})(([0-9a-fA-F]{2}){5})$/

	var macstr = field.value;
	if (!macRegex.test(macstr) && !macRegex1.test(macstr) && !macRegex2.test(macstr)) {
		field.focus();
		field.select();
...
{
	if(!field) return false;
	var value = field.value.replace(/:/g, "").replace(/-/g, "");
...
function checkMacAddr_is_muticast(field)
{
	...
	var value = field.value.replace(/:/g, "").replace(/-/g, "");
	if(parseInt(value.substr(0, 2), 16) & 0x01 != 0)
	{
		field.focus();
		field.select();
...
ntp_zone_array[45]=new ntp_entry("<script>dw(util_gw_array45)</script>","-5 4");
ntp_zone_array[46]=new ntp_entry("<script>dw(util_gw_array46)</script>","-6 1");
...
function checkHostIP(ip, checkEmpty)
{
	...
	if (validateKey(ip.value) == 0) {
		alert(util_save_wan11 + util_gw_ipaddr_nodecimal);
		ip.value = ip.defaultValue;
		ip.focus();
		return false;
	}
	...
	if (!checkDigitRange(ip.value, 1, 1, 223)) {
		alert(util_save_wan11 + util_gw_ipaddr_1strange1);
		ip.value = ip.defaultValue;
		ip.focus();
		return false;
	}
	if (!checkDigitRange(ip.value, 2, 0, 255)) {
		alert(util_save_wan11 + util_gw_ipaddr_2ndrange);
		ip.value = ip.defaultValue;
		ip.focus();
		return false;
	}
	if (!checkDigitRange(ip.value, 3, 0, 255)) {
		alert(util_save_wan11 + util_gw_ipaddr_3rdrange);
		ip.value = ip.defaultValue;
		ip.focus();
		return false;
	}
	if (!checkDigitRange(ip.value, 4, 0, 254)) {
		alert(util_save_wan11 + util_gw_ipaddr_4thrange);
		ip.value = ip.defaultValue;
		ip.focus();
		return false;
	}
...
function checkNetmask(netmask, checkEmpty)
{
	...
	for (i=1; i<=4; i++) {
		...
		if( !(d==0 || d==128 || d==192 || d==224 || d==240 || d==248 || d==252 || d==254 || d==255 )) {
			alert(util_gw_mask_invalid + util_gw_mask_rang);
			netmask.focus();
			return false;
		}
	}
...
function showPwdLevel(num, passwordStr) {
	var level = 0;
	if (passwordStr) {
		document.getElementById("level" + num).style.display = "";
		document.getElementById("level" + num + "0").style.borderColor = "#666666";
		document.getElementById("level" + num + "1").style.borderColor = "#666666";
		document.getElementById("level" + num + "2").style.borderColor = "#666666";
		level = checkPwdLevel(passwordStr);
		if (level == 1) {
			document.getElementById("level" + num + "0").style.borderColor = "#ef8218";
			document.getElementById("leveltext" + num).innerText = password_level_weak;
		} else if  (level == 2 || level == 3) {
			document.getElementById("level" + num + "0").style.borderColor = "#ef8218";
			document.getElementById("level" + num + "1").style.borderColor = "#ef8218";
			document.getElementById("leveltext" + num).innerText = password_level_middle;
		} else if  (level == 4) {
			document.getElementById("level" + num + "0").style.borderColor = "#ef8218";
			document.getElementById("level" + num + "1").style.borderColor = "#ef8218";
			document.getElementById("level" + num + "2").style.borderColor = "#ef8218";
			document.getElementById("leveltext" + num).innerText = password_level_powerful;
		}
	} else {
...
function checkPwdLevel(str) {
	...
	var regexp;
	var regexpArr = ['[a-z]+','[A-Z]+','[0-9]+','[,.\/!;<>@#$*%^+&()|:_-]+'];
	for (var i=0; i<4; i++) {
		regexp = new RegExp(regexpArr[i]);
		if (regexp.test(str)) {
			no ++;
		}
	}
...
function Checkvlan(vlan_id)
{
	var index = 0;
	if (vlan_id == 9 || vlan_id == 8)
		index = 1;
	else if (vlan_id <= 1 || vlan_id > 4094)
		index = 2;
	else
		index = 3;
...
function lancheckIp(ip) {
	return ipRegex.test(ip) && ip !== '0.0.0.0' && ip !== '255.255.255.255' && !isSpecialIP(ip)
}

function checkIpMask(ipValue, maskValue) {
	if (!ipRegex.test(maskValue)) return false
	var maskVal = transIp(maskValue)
	var ipVal = transIp(ipValue)
	var result = checkIPNetHost(ipVal, maskVal)
	return result
}

function transIp(val){
	var value = val.split('.')
	return 0x1000000 * value[0] + 0x10000 * value[1] + 0x100 * value[2] + 1 * value[3]
  }

function isSpecialIP(ip) {
	var firstByte = ip.split('.')[0]
	var specialAddrArr = ['0', '255', '127']
	if (specialAddrArr.indexOf(firstByte) > -1) return true
	if (Number(firstByte) > 223) {
		return true
	}
	return false
}

function checkIPNetHost(ipVal, maskVal){
	/* 网络号全0/1 */
...
function lanCheckMask(mask) {
	if (!ipRegex.test(mask)) return false
	var maskNum = getIntIPFromStr(mask)
	if (!maskNum) return false
	var maskString = maskNum.toString(2)
	if (maskString.length !== 32) return false
	if (!maskRegex.test(maskString)) return false
	return true
}

function getIntIPFromStr(ip) {
	var intIP = 0
	if (!ipRegex.test(ip)) {
		console.error('getIntIPFromStr ERROR:', ip, 'is not valid IP')
		return intIP
	}
	var ipArr = ip.split('.')
	var ip0 = Uint32Array.from([Number(ipArr[0]) << 24])[0]
	var ip1 = Uint32Array.from([Number(ipArr[1]) << 16])[0]
	var ip2 = Uint32Array.from([Number(ipArr[2]) << 8])[0]
	var ip3 = Uint32Array.from([Number(ipArr[3])])[0]
	intIP = ip0 + ip1 + ip2 + ip3
	return intIP
}

function calcIPPrefix(ip, mask) {
	var ipNum = getIntIPFromStr(ip)
	var maskNum = getIntIPFromStr(mask)
	ipNum &= maskNum
	ip = getStrIPFromInt(ipNum)
	var ipArr = ip.split('.')
	ipArr.splice(3, 1)
	return ipArr.join('.')
}

function getStrIPFromInt(ip) {
	var errInfo = 'ip should be a int between 0 ~ 4294967295'
	if (isNaN(ip)) {
	...
	dv.setUint32(0, ip)
	var ip0 = dv.getUint8(0)
	var ip1 = dv.getUint8(1)
	var ip2 = dv.getUint8(2)
	var ip3 = dv.getUint8(3)
