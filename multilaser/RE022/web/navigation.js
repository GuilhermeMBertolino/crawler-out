document.write("<script language=javascript src='./macro.js'></script>");

if (document.addEventListener) {                  // For all major browsers, except IE 8 and earlier
	document.addEventListener("click", parent.eventListener);
	document.addEventListener("keypress", parent.eventListener);
} else if (document.attachEvent) {                  // For IE 8 and earlier versions
	document.attachEvent("onclick", parent.eventListener);
	document.attachEvent("onkeypress", parent.eventListener);
}
...
{
	var subhtml_array = [<%getIndex("stats.htm");%>,
						<%getIndex("ddns.htm");%>,
						<%getIndex("ntp.htm");%>,
						<%getIndex("syslog.htm");%>,
						<%getIndex("upload.htm");%>,
...
function init_submenu_tcpip(showLanDev)
{
	if(<%getIndex("tcpiplan.htm")%> == 0){
		if(showLanDev){
			if(<%getIndex("clients.htm");%>){
				init_submenu(menu_lan_client);
				return;
			}
		}
		if(<%getIndex("tcpipwan.htm");%>){
			init_submenu(menu_wan);
			return;
		}
	}else
...
function init_submenu_ipv6(is_mape)
{
	...
	var subnav_array = [menu_ipv6_wan,
						menu_ipv6_lan,
						menu_radvd,
						"Tunnel (6 over 4)"];
...
function init_submenu_firewall(ipv6_enable,route_setup_enable,quagga_enable){
	...
	if(ipv6_enable)
	{
		var subhtml_array = [<%getIndex("macfilter.htm");%>,
							<%getIndex("portfilter6.htm");%>,
							<%getIndex("ip6filter.htm");%>,
							<%getIndex("portfw.htm");%>,
							<%getIndex("urlfilter.htm");%>,
							<%getIndex("dmz.htm");%>,
		...
		var subnav_array = [menu_macFilter,
							menu_portFilter,
							menu_ipFilter,
							menu_portFw,
							menu_urlFilter,
...
function init_submenu_mng(isDisplayCPU, isEnableBT,isDisplayTR069)
{
	var subtml_array1 = [<%getIndex("status.htm");%>,
						<%getIndex("stats.htm");%>,
						<%getIndex("ddns.htm");%>];
	var subnav_array1 = [menu_status,menu_statistics,menu_ddns];
	...
	if(first_show_index1 == -1){
		...
		var subtml_array2 = [<%getIndex("ntp.htm");%>,
							<%getIndex("dos.htm");%>];
		var subnav_array2 = [menu_timeZone,menu_dos];
		var first_show_index2 = index_of(subtml_array2,1); 
		if(first_show_index2 == -1){
			if(isDisplayTR069){
				if(<%getIndex("tr069config.htm");%>){
					init_submenu(menu_tr69_config);
					return;
				}
                           ...
                           isDisplayCPU,isEnableBT,isDisplayTR069, disk_onoff, multi_ap_onoff)
{
  ...
  if (userDegreeFlag == 1 || userDegreeFlag == 0) {
	awaysShow = true
  }

  if (MACRO && MACRO.menuDisplay && (awaysShow || MACRO.menuDisplay.menu3_quick_setup && webAllowStatus[7] == 1)) {
	add_topMenuItem("sub_menu_install.htm",quickSetup_menu);
  }

  add_topMenuItem("sub_menu_status.htm",menu_status);

  if (MACRO && MACRO.menuDisplay.menu3_setup_wizard) {
  ...
  if(userDegreeFlag != 0){
    if(<%getIndex("sub_menu_wlan_support");%> != 0)
    {
		if(<%getIndex("sub_menu_wlan.htm");%>){
			if(show_topmenu_wlan(is8021xClient, isMeshDefined, is80211rSupport, isAirtimeDefined)){
				(awaysShow || webAllowStatus[0] == 1) && add_topMenuItem(get_form("sub_menu_wlan.htm",0),menu_wlan1);
	 			if(wlan_num==2)
					(awaysShow || webAllowStatus[0] == 1) && add_topMenuItem(get_form("sub_menu_wlan.htm",1),menu_wlan2);
			}	
		}
    }

	(awaysShow || webAllowStatus[1] == 1) &&add_topMenuItem("sub_menu_guest_net.htm",wlguest_wlan_guest);

	if(multi_ap_onoff && opmode != 2) {
		(awaysShow || webAllowStatus[2] == 1) && add_topMenuItem("sub_menu_multi_ap.htm","EasyMesh");
	}

	if(<%getIndex("sub_menu_tcpip.htm");%> && <% getIndex("multi_ap_controller"); %>  != 2){
		if(show_topmenu_tcpip(showLanDev)){
			(awaysShow || webAllowStatus[3] == 1) && add_topMenuItem("sub_menu_tcpip.htm",menu_tcpip);
		}
	}

	if(ipv6 && opmode == 0){
		if(<%getIndex("sub_menu_ipv6.htm");%> && <% getIndex("multi_ap_controller"); %>  != 2)
			if(show_topmenu_ipv6(is_mape))
				(awaysShow || webAllowStatus[4] == 1) && add_topMenuItem("sub_menu_ipv6.htm","IPv6");
	}

	if (<%getInfo("show_firewall_all")%>) {
		if(<%getIndex("sub_menu_firewall.htm");%> && <% getIndex("multi_ap_controller"); %>  != 2){
			if(show_topmenu_firewall(ipv6,route_setup_enable,quagga_enable) && (opmode == 0 || opmode == 1))
				(awaysShow || webAllowStatus[5] == 1) && add_topMenuItem("sub_menu_firewall.htm",menu_fireWall);
		}
	}
	if(isDisplayVOIP){
	...
	if(<%getIndex("sub_menu_mng.htm");%>){
		if(show_topmenu_mng(isDisplayCPU,isEnableBT,isDisplayTR069))
			(awaysShow || webAllowStatus[6] == 1) && add_topMenuItem("sub_menu_mng.htm",menu_management);
	}
	...
	if (disk_onoff){
	 ...
	 if(wlan_num==1)
		add_topMenuItem(get_form("sub_menu_wlan.htm",0),menu_wlan2);
	 	...
	 	(awaysShow || webAllowStatus[1] == 1) && add_topMenuItem(get_form("sub_menu_wlan.htm",0),menu_wlan1);
 	 if(wlan_num==2)
	  	(awaysShow || webAllowStatus[2] == 1) && add_topMenuItem(get_form("sub_menu_wlan.htm",1),menu_wlan2);
	
	add_topMenuItem("sub_menu_guest_net",wlguest_wlan_guest);

	 if(multi_ap_onoff && opmode != 2)
	 	(awaysShow || webAllowStatus[3] == 1) && add_topMenuItem("sub_menu_multi_ap.htm","EasyMesh");

	 (awaysShow || webAllowStatus[4] == 1) && add_topMenuItem("sub_menu_tcpip.htm",menu_tcpip);

 	 if(ipv6)
	  	(awaysShow || webAllowStatus[5] == 1) && add_topMenuItem("sub_menu_ipv6.htm","IPv6");
	 if (<%getInfo("show_firewall_all")%>) {
		(awaysShow || webAllowStatus[6] == 1) && add_topMenuItem("sub_menu_firewall.htm",menu_fireWall);
	 }
  	 if( isDisplayVOIP)
  		add_topMenuItem("sub_menu_voip.htm","VoIP");
	 (awaysShow || webAllowStatus[7] == 1) &&add_topMenuItem("sub_menu_mng.htm",menu_management);
	 if (disk_onoff)
  		add_topMenuItem("sub_menu_disk_mng.htm","Storage");
  }
  if (MACRO && MACRO.menuDisplay && MACRO.menuDisplay.menu3_quick_setup) {
	if (webAllowStatus[7] == 1 || awaysShow) {
		init_submenu(quickSetup_menu);
	} else {
...
function draw_subnav_install() {
	add_subMenuItem("install.htm",quickSetup_menu);
	init_submenu(quickSetup_menu);
}

function draw_subnav_status() {
	add_subMenuItem("status.htm",menu_status);
	add_subMenuItem("customerClient.htm",menu_arp_list);
	add_subMenuItem("lan_port_status.htm",lan_port_status);
	init_submenu(menu_status);
}
...
function draw_subnav_tcpip(multiple_wan_enable,showLanDev,userDegreeFlag, ipv6_enable)
{
	if(userDegreeFlag != 0){
		if (<%getInfo("show_opmode_menu")%>==1) {
			if(!multiple_wan_enable){
				...
				show_subnav_element("opmode.htm",menu_operation_mode,1); 
			}
		...
		if (<%getInfo("show_tcpip_wan")%> && opmode == 0) {
			show_subnav_element("tcpipwan.htm",menu_wan,<%getIndex("tcpipwan.htm")%>);
		}
		if (MACRO && MACRO.menuDisplay && MACRO.menuDisplay.qos_menu) {
			if(ipv6_enable){
				opmode == 0 && show_subnav_element("ip6_qos.htm",menu_qos,<%getIndex("ip6_qos.htm");%>);
			} else {
				opmode == 0 && show_subnav_element("ip_qos.htm",menu_qos,<%getIndex("ip_qos.htm");%>);
			}	
		}
		if(<%getIndex("rtk_vlan_support");%> && MACRO && MACRO.menuDisplay &&  MACRO.menuDisplay.menu2_vlan)
			show_subnav_element("vlan.htm","vlan",<%getIndex("vlan.htm");%>);
		if(<%getIndex("linux_vlan_support");%> && MACRO && MACRO.menuDisplay &&  MACRO.menuDisplay.menu2_vlan)
			show_subnav_element("8021q_vlan.htm",menu_vlan,<%getIndex("8021q_vlan.htm");%>);

		if (opmode == 1) {
			show_subnav_element("iptv.htm","IPTV",1); 
		}
		if (opmode == 0) {
			show_subnav_element("upnp.htm","UPNP",1); 
			show_subnav_element("igmp.htm","IGMP",1); 
		}
		
		if (<%getInfo("show_opmode_menu")%>==1) {
			init_submenu(menu_operation_mode);
			return
		}
		if(<%getInfo("show_opmode_menu")%>==2) {
			init_submenu(menu_ulink_operation_mode);
			return
		}
		...
		if (MACRO && MACRO.menuDisplay && MACRO.menuDisplay.qos_menu) {
			if(ipv6_enable){
				opmode == 0 && add_subMenuItem("ip6_qos.htm",menu_qos);
			} else {
				opmode == 0 && add_subMenuItem("ip_qos.htm",menu_qos);
			}
		}
		...
		if (opmode == 1) {
			add_subMenuItem("iptv.htm","IPTV");
		}
		if (opmode == 0) {
			add_subMenuItem("upnp.htm","UPNP");
			add_subMenuItem("igmp.htm","IGMP");
		}
...
function draw_subnav_ipv6(is_mape,is_radvd_wan,userDegreeFlag)
{
	if(userDegreeFlag != 0){
		if (opmode == 0) {
			show_subnav_element("ipv6_wan.htm",menu_ipv6_wan,<%getIndex("ipv6_wan.htm")%>);
		}
  		show_subnav_element("dhcp6s.htm",menu_ipv6_lan,<%getIndex("dhcp6s.htm")%>);
		if (opmode == 0) {
			show_subnav_element("radvd.htm",menu_radvd,<%getIndex("radvd.htm")%>);
		}
		if(is_radvd_wan)
			show_subnav_element("radvd_wan.htm","Wan Radvd",<%getIndex("radvd_wan.htm")%>);

		if (opmode == 0 && MACRO && MACRO.menuDisplay && MACRO.menuDisplay.menu2_tunnel6) {
		...
		if (opmode != 0) {
			init_submenu(menu_ipv6_lan);
		} else {
			init_submenu_ipv6(is_mape);
		}
		
	}else{
		add_subMenuItem("ipv6_wan.htm",menu_ipv6_wan);
  		add_subMenuItem("dhcp6s.htm",menu_ipv6_lan);
  		// add_subMenuItem("radvd.htm",menu_radvd);
		if(is_radvd_wan)
...
function draw_subnav_firewall(ipv6_enable, route_setup_enable,quagga_enable,userDegreeFlag)
{
	if (<%getInfo("show_firewall_all")%>) {
		if(userDegreeFlag != 0){
			...
			if(ipv6_enable)
			{
				if (opmode == 0 && MACRO && MACRO.menuDisplay && MACRO.menuDisplay.menu2_portfilter) {
					show_subnav_element("portfilter6.htm",menu_portFilter,<%getIndex("portfilter6.htm");%>);
				}
				if (opmode == 0 && MACRO && MACRO.menuDisplay && MACRO.menuDisplay.menu2_ipfilter) {
			...
			if (opmode == 0) {
				...
				if (MACRO && MACRO.menuDisplay && MACRO.menuDisplay.menu2_urlfilter) {
			...
			show_subnav_element("macgroup.htm",menu_mac_group_setting,1);
			show_subnav_element("parent_control.htm",menu_parent_control_setting,1);
			
			if (<%getInfo("show_mgt_dos")%> && opmode == 0) {
				show_subnav_element("dos.htm",menu_dos,<%getIndex("dos.htm");%>);    
			}
			if (opmode == 0) {
				if(route_setup_enable && MACRO && MACRO.menuDisplay && MACRO.menuDisplay.menu2_route_set)
					show_subnav_element("route.htm",menu_routeSetup,<%getIndex("route.htm");%>);
				if(quagga_enable && MACRO && MACRO.menuDisplay && MACRO.menuDisplay.menu2_route_set){
			...
			if(ipv6_enable)
			{
				if (MACRO && MACRO.menuDisplay && MACRO.menuDisplay.menu2_portfilter) {
					add_subMenuItem("portfilter6.htm",menu_portFilter);
				}
				if (MACRO && MACRO.menuDisplay && MACRO.menuDisplay.menu2_ipfilter) {
...
{
	if(userDegreeFlag != 0){
		if (MACRO && MACRO.menuDisplay && MACRO.menuDisplay.menu3_statistics) {
		...
		if (<%getInfo("show_mgt_ddns")%> && MACRO && MACRO.menuDisplay &&  MACRO.menuDisplay.menu2_ddns) {
			show_subnav_element("ddns.htm",menu_ddns,<%getIndex("ddns.htm");%>);
		}

    	if(isDisplayTR069 == 1 && userDegreeFlag == 1)
		...
		if (MACRO && MACRO.menuDisplay && MACRO.menuDisplay.menu2_syslog) {
		...
		show_subnav_element("upload_online.htm",online_upgrade_menu,1);
    	show_subnav_element("saveconf.htm",menu_setting,<%getIndex("saveconf.htm");%>);
		if (MACRO && MACRO.menuDisplay && MACRO.menuDisplay.menu3_defsettings && userDegreeFlag == 1) {
			show_subnav_element("preSet.htm",menu_defsettings,1);
		}

		if (MACRO && MACRO.menuDisplay && MACRO.menuDisplay.menu3_mirror) {
			show_subnav_element("mirror.htm",menu_mirror,1);
		}
		
		if(opmode == 0){
			show_subnav_element("remoteManagement.htm",remote_mangement_menu,1);
		}
		...
		if (MACRO && MACRO.menuDisplay && MACRO.menuDisplay.menu3_scheduled_restart) {
			show_subnav_element("scheduled_restart.htm",scheduled_restart,1);
		}
		show_subnav_element("mgm_dev_reboot.htm",device_reboot,1);
		show_subnav_element("diag_ping_admin.htm","ping",1);
		show_subnav_element("diag_tracert_admin.htm","tracert",1);
		if (MACRO && MACRO.menuDisplay && MACRO.menuDisplay.menu3_statistics) {
			init_submenu(menu_statistics);
			return
		}
		if(<%getInfo("show_url_redirect")%>){
			init_submenu(menu_httpRedirect);
			return
		}
		if(isDisplayCPU == 1){
			init_submenu(menu_cpu);
			return
		}
		if (<%getInfo("show_mgt_ntp")%>) {
			init_submenu(menu_timeZone);
			return
		}
		init_submenu(menu_timeZone);
	}
		...
		add_subMenuItem("upload_online.htm",online_upgrade_menu);
  		add_subMenuItem("saveconf.htm",menu_setting);
		if (MACRO && MACRO.menuDisplay && MACRO.menuDisplay.menu3_defsettings) {
			add_subMenuItem("preSet.htm",menu_defsettings);
		}

		if (MACRO && MACRO.menuDisplay && MACRO.menuDisplay.menu3_mirror) {
			add_subMenuItem("mirror.htm",menu_mirror);
		}

		add_subMenuItem("remoteManagement.htm",menu_mirror);
...
function draw_subnav_guest_net(userDegreeFlag)
{
	add_subMenuItem("wlguest.htm",wlguest_wlan_guest);
	init_submenu(wlguest_wlan_guest);
}
