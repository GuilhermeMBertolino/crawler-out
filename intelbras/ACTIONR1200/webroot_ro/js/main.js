var G = {
        ...
        initYunFlag: false,
        wanNum: 0,
        homePage: "http://www.intelbras.com.br",
...
$(function () {
    ...
    //查询是否有IP conflict
    var checkConfilct = window.setInterval(function () {
        $.getJSON("goform/queryConflict?" + Math.random(), function (data) {
            //如果存在冲突，则弹出提示框
            if (data.isConflict === "1") {
                clearInterval(checkConfilct);

                //请求loading.gif
                var img1 = new Image();
                img1.src = "img/loading.gif?0f22ae552ba9649d4434978feddeaf38";

                //提示框添加提示语
                $(".sec-tips").text("Para corrigir esse conflito, o endereço IP da LAN será alterado para " + data.newIP + ".");
                $(".thir-tips").text("A partir de agora, para acessar as configurações do ACtion R1200, utilize o endereço http://" + data.newIP + " ou http://meuintelbras.local.");

                //点击确定跳转
                $("#jump").on("click", function () {
                    clearTimeout(staInfo.time);

                    $.getJSON("goform/enableNewIP?" + Math.random(), function (res) {
                        if (res.errCode === "0") {
                            $(".conflict").addClass("none");
                            //window.location.href = "http://" + data.newIP + "/main.html";
                            top.$("#gbx_overlay_conflict").remove();
                            top.showSaveMsg(0, _("Saving..."), data.newIP, true);
                        } else {
                ...
                top.$("<div id='gbx_overlay_conflict'></div>").appendTo("body");
                $("#noJump").on("click", function () {
                    $(".conflict").addClass("none");
                    top.$("#gbx_overlay_conflict").remove();
                    return false
                })
...
staInfo = {
    ...
    init: function () {
        ...
        if (!staInfo.loading) {
            ...
            $("#notNow").on("click", function () {
                $.post("goform/notNowUpgrade", function () {
            ...
            //跳转到WAN口设置界面，配置单WAN
            $("#wanStatusNumber .show-info-wan,.show-info-wan").on("click", function () {
    ...
    getHomeLink: function () {
        if (B.getLang() == "cn") {
            ...
            G.homePage = "http://www.intelbras.com.br";
            return;
        }
    },
    ...
    setImage: function (obj) {
        ...
        //是否有新版本
        if (obj.newVersionExist === "1") {
            $("#directupgrade").removeClass("none");
            //客户提供的提示升级信息
            $("#directupgrade span").html("Uma nova versão de firmware está disponível. Deseja atualizar?");
        } else {
            $("#directupgrade").addClass("none");
        }
        ...
        $("#curVersion").html(obj.curVersion);

        //AP模式版本号
        $("#apCurVersion").html(obj.curVersion);
        //发现新版本
        /*if (obj.newVersionExist === "1") {
            $(".directupgrade").removeClass("none");
        } else {
    ...
    setRouterMode: function (obj) {
        ...
        for (i = 0; i < G.wanNum; i++) {
            ...
            $("#internetWrap .show-info-wan").data("wan.status", obj.wanInfo[i].wanStatus);
        }
...
ipv6Info = {
    ...
    data: null,
    init: function () {
        $("#ipv6-setting .main-area").on("click", ipv6Info.getIframe);
        ipv6Info.initValue();
    },
    initValue: function () {
        $.getJSON("goform/getIPv6Info?" + Math.random(), ipv6Info.setValue);
    },
    setValue: function (obj) {
        if (obj.ipv6En == "0") {
            $("#ipv6wan .function-status").text(_("off"));
        } else {
            $("#ipv6wan .function-status").text(_("on"));
        }
    },
    getIframe: function () {
        var id = $(this).attr("id");
        switch (id) {
        case "ipv6wan":
            showIframe(_("IPv6 WAN Setup"), "ipv6wan.html", 630, 510);
            break;
        case "ipv6lan":
            showIframe(_("IPv6 LAN Setup"), "ipv6lan.html", 560, 400);
            break;
        case "ipv6status":
            showIframe(_("IPv6 Status"), "ipv6status.html", 630, 510);
            break;
        }
    }
};
...
advInfo = {
    ...
    getIframe: function () {
        ...
        case "adv_led":
            ...
            break;
        case "adv_cloud":
            showIframe(_("INTELBRAS App"), "cloud_managment.html", 620, 450);
            break;
        case "adv_ddns":
...
sysInfo = {
    ...
    setValue: function (obj) {
        ...
        if (obj.firmware == "1") {
            ...
            $("#sys_upgrade .function-status").html("Versão atual: " + obj.firmware);
        }
...
$(".main-section-title").before('<div class="lang-set"><a style="margin-left:10px; color: #8b979f;" href="goform/exit">Exit</a></span></div>')