...
var moduleModel = R.moduleModel({
	...
	getSubmitData: function () {
		var trArry = $("#portBody").children(),
			len = trArry.length,
			i = 0,
			inPort = $("#inPortStart").val() + "-" + $("#inPortEnd").val(),
			outPort = $("#outPortStart").val() + "-" + $("#outPortEnd").val(),
			data = "";
		for (i = 0; i < len; i++) {
...
function initEvent() {
	...
	$("#inPortStart,#outPortStart").addPlaceholder(_("Start port"));
	$("#inPortEnd,#outPortEnd").addPlaceholder(_("End port"));
...
function syncPort() {
	var toSyncLanPort = true,
		toSyncWanPort = true;
	$("#inPortStart").on("keyup", function () {
		if (toSyncLanPort && $(this).val() != "") {
			$("#inPortEnd").val($(this).val()).removeClass("placeholder-text");
		}
	}).on("blur", function () {
		toSyncLanPort = false;
	});

	$("#outPortStart").on("keyup", function () {
		if (toSyncWanPort && $(this).val() != "") {
			$("#outPortEnd").val($(this).val()).removeClass("placeholder-text");
		}
	}).on("blur", function () {
		toSyncWanPort = false;
	});
...
function addList() {
	...
	str += "<td alt='inPort' id='inPort" + (listMax + 1) + "'>" + $("#inPortStart").val() + "-" + $("#inPortEnd").val() + "</td>";
	str += "<td alt='outPort' id='outPort" + (listMax + 1) + "'>" + $("#outPortStart").val() + "-" + $("#outPortEnd").val() + "</td>";
	str += "<td  alt='protocol' id='protocol" + (listMax + 1) + "'>" + $("#protocol").val() + "</td>";
	str += "<td><span class='delete del' title='" + _("Delete") + "'></span></td></tr>";

	$("#portBody").append(str);
	$("#ip").val("");
	$("#inPortStart,#inPortEnd,#outPortStart,#outPortEnd").val("");
	listMax++;
	top.initIframeHeight();
	...
	G.validate = $.validate({
		custom: function () {
			var inPort = "",
				...
				str = "",
				i = 0,
				inPortStart = $("#inPortStart").val(),
				inPortEnd = $("#inPortEnd").val(),
				outPortStart = $("#outPortStart").val(),
				outPortEnd = $("#outPortEnd").val(),
				errMsg;

			ip = $("#ip").val();
			inPort = inPortStart + "-" + inPortEnd;
			outPort = outPortStart + "-" + outPortEnd;
			...
			//test of inPortStart
			if (!(/^[0-9]{1,}$/).test(inPortStart)) {
				$("#inPortStart").focus();
				return _("The LAN port number must consist of digits.");
			} else {
				if (parseInt(inPortStart, 10) > 65535 || parseInt(inPortStart, 10) < 1) {
					$("#inPortStart").focus();
			...
			//test of inPortEnd
			if (!(/^[0-9]{1,}$/).test(inPortEnd)) {
				$("#inPortEnd").focus();
				return _("The LAN port number must consist of digits.");
			} else {
				if (parseInt(inPortEnd, 10) > 65535 || parseInt(inPortEnd, 10) < 1) {
					$("#inPortEnd").focus();
					return _("LAN port range: 1-65535");
				}
			}

			if (parseInt(inPortEnd, 10) < parseInt(inPortStart, 10)) {
				$("#inPortEnd").focus();
				return _("The end port number can not less than starting port number");
			}

			//test of outPortStart
			if (!(/^[0-9]{1,}$/).test(outPortStart)) {
				$("#outPortStart").focus();
				return _("The WAN port must consist of digits.");
			} else {
				if (parseInt(outPortStart, 10) > 65535 || parseInt(outPortStart, 10) < 1) {
					$("#outPortStart").focus();
			...
			//test of outPortEnd
			if (!(/^[0-9]{1,}$/).test(outPortEnd)) {
				$("#outPortEnd").focus();
				return _("The WAN port must consist of digits.");
			} else {
				if (parseInt(outPortEnd, 10) > 65535 || parseInt(outPortEnd, 10) < 1) {
					$("#outPortEnd").focus();
					return _("WAN port range: 1-65535");
			...
			if (parseInt(outPortEnd, 10) < parseInt(outPortStart, 10)) {
				$("#outPortEnd").focus();
				return _("The end port number can not less than starting port number");
			}
			...
			/*判断外网端口是否重复*/
			var outPort = $("#outPortStart").val() + "-" + $("#outPortEnd").val(),
				outPortRepeat = false;
