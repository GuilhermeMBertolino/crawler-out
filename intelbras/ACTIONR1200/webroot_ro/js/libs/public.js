...
function inputValue(obj) {
	var prop;
	for (prop in obj) {
		if (prop == "wanAddr" || prop == "lanAddr" || prop == "lanPrefix") {
	...
	$.progress = {
		showPro: function (str, str2, ip) {
			...
			var html = '<div id="loading_div" >' +
				...
				'</div>' +
				'<div class="load-img"><div class="md-preloader" style="margin-top:15px"><svg xmlns="http://www.w3.org/2000/svg" version="1.1" height="35" width="35" viewbox="0 0 75 75"><circle cx="37.5" cy = "37.5" r="33.5" stroke-width="8" /></svg></div></div><br/>' + str2 + '<span id="load_text"></span>' + '</div>';
			$(html).appendTo("body");
...
function rebooting(time) {
	...
	if (pc <= 100) {
		...
		clearTimeout(rebootTime);
		pc = 1;
		if (ipaddress != "" && (/^([1-9]|[1-9]\d|1\d\d|2[0-1]\d|22[0-3])\.(([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.){2}([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])$/).test(ipaddress) == true || ipaddress == "meuintelbras.local") {
...
function jumpTo(address, callback) {
	var localDomain = "meuintelbras.local";
	//优先域名跳转
...
function translateSpeed(speedKB) {
	var speedKb = Number(speedKB) * 8;

	if (speedKb < 1024) {
		return speedKb.toFixed(1) + "Kbps";
	} else {
		return (speedKb / 1024).toFixed(1) + "Mbps";
	}
}
...
R.extend(R, {
	...
	isTimeOut: function (str) {
		if (str.indexOf("<!DOCTYPE html") === 0 && str.indexOf("login.js?0f22ae552ba9649d4434978feddeaf38") != -1) {
			return true;
		}
