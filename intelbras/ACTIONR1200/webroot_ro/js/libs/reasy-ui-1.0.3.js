...
(function (window, document) {
	...
	valid = {
		...
		ipv6suffix: function (str) {
			var value = Number(str);
			if (isNaN(value)) {
				return _("Please input number");
			} else if (value > 128 || value < 0) {
				return _("Please input valid number");
			}
		},
		ipv6suffixSpecial: function (str) {
			var value = Number(str);
			if (isNaN(value)) {
				return _("Please input number");
		...
		ipv6pool: function (str) {
			var len = str.length,
				reg = /^[0-9a-fA-F]{1,}$/;

			if (!reg.test(str)) {
				return _("Please input valid hex number");
			}

			if (len > 4) {
				return _("Please input hex number range from 1 to ffff");
			}
...
$.extend($.validate.valid, {
	...
	ssid: {
		all: function (str) {
			...
			if (!(/^[ -~]+$/g).test(str)) {
				return _("Há caracteres inválidos no nome da rede Wi-Fi.");
			}
	...
	ssidPwd: {
		...
		specific: function (str) {
			var ret = str;
			if (!(/^[ -~]+$/g).test(str)) {
				return _("Há caracteres inválidos na senha da rede Wi-Fi.");
			}
		}
	}
});