...
function getInitData() {
    ...
    $.getJSON("goform/fast_setting_get" + "?" + Math.random(), function (obj) {
        ...
        解决办法：提取浏览器的输入框域名：如果不是www.meuintelbras.local、meuintelbras.local和管理ip，页面做一个跳转*/
        if ((obj.lanIp !== host) && ('meuintelbras.local' !== host)) {
            location.host = 'meuintelbras.local';
        }
    });
...
function queryConflict() {
    var checkConflict = window.setInterval(function () {
...
function startSet() {
    ...
    $("#net_find,.masthead").removeClass("none");
    //隐藏语言选择框
    $("#lang").addClass("none");
    /*$.ajaxSetup({
        error:function(x,e){
    ...
    $(".control-label,span").css("color", "#fff");
    $.getJSON("goform/fast_setting_get?" + Math.random(), initValue);
...
function showFinish() {
    ...
    if (G.wanStatus == "7") {
        //已联网，
        $(".loadding-ok img").attr("src", "./img/ok_connected.png?0f22ae552ba9649d4434978feddeaf38")
        ...
        if (G.wanStatus == 7) {
            ...
            $(".loadding-ok img").attr("src", "./img/ok.png?0f22ae552ba9649d4434978feddeaf38")
