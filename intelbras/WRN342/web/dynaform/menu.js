...
//	directory level: 0: directory has no branches;  1: directory level 1;  2:level 2;   and so on
	...
	"WzdStartRpm", 				0,		2, 	 		str_menu.wizard,//Rosinei 09-04-2010
//Desativado, porque foi colocado esse menu como sub-menu Wireless 
...
function menuDisplay()
{
	...
	for (var n = 0; n < menuList.length; n +=4  )
	{
		...
		document.write('<ol id=ol'+i+' class='+className+' style="display:'+display+'; background-position:2px;PADDING-LEFT:2px;"><A id=a'+i+' href="/userRpm/'+menuList[n]+'.htm" target=mainFrame class=L1 onClick="doClick('+i+');">'+((power > 0)? '<font color="#e5e5e6"><b>---</b></font>':'<font color="#84c225"><b>.: </b></font>')+'<font color="#344b57">'+menuList[n+3]+'</font></a></ol>');
...
// By Nei	07-04-2010
// Estabelece a cor verde em background para o menu simples!

		  if(document.getElementById('ol'+i).className == "normal")
			{
				document.getElementById('ol'+i).className = "dot1"; 
			}
