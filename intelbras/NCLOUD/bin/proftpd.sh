...
{ 
	...
	do
	ftpuser=`nvram_get 2860 "User"$index"Path"`
	if [ ! -n "$ftpuser" ];	then
	username=`nvram_get 2860 "User"$index""`
	ftpall="$ftpall $username"
	fi
	done	
	if [ ! -n "$ftpall" ]; then
	echo "<Limit LOGIN>" >> $PROFTPD_FILE
	echo "DenyUser $ftpall" >> $PROFTPD_FILE
...
setFtpServer()
{
	...
	echo "ScoreboardFile /tmp/proftpd.scoreboard " >> $PROFTPD_FILE
	...
	admID=`nvram_get 2860 Login`      
    	echo "User $admID" >> $PROFTPD_FILE
    	echo "Group $admID" >> $PROFTPD_FILE
	...
	echo "UseEncoding on" >> $PROFTPD_FILE
	echo "UseEncoding UTF-8 CP860" >> $PROFTPD_FILE
