...
if [ "$host" = "" ]; then
	host="TOTOLINK"
	nvram_set 2860 HostName TOTOLINK
fi
hostname $host
echo "127.0.0.1 localhost.localdomain localhost" > /etc/hosts
echo "$ip $host.com $host" >> /etc/hosts
...
if [ "$dhcp" = "1" ]; then
	...
	#static=`nvram_get 2860 staticDhcpRules | sed 's/;/\n/g' | sed 's/,/ /g'`
	...
	#by luotao
	lktos_networkconfig mtk7620 getdhcpdconfig
...
# radvd
radvd_enable=`nvram_get 2860 radvdEnabled`
if [ "$radvd_enable" != "0" ]; then
	radvd.sh
fi
# pppoe-relay
