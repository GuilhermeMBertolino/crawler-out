...
if [ "$CONFIG_HOTPLUG" == "y" -a "$CONFIG_USB" == "y" -o "$CONFIG_MMC" == "y" ]; then
	...
	echo "# <device regex> <uid>:<gid> <octal permissions> [<@|$|*> <command>]" > /etc/mdev.conf
        ...
        echo "lp0 0:0 0660 */sbin/automount_print.sh \$MDEV" >> /etc/mdev.conf
fi
}
...
if [ "$CONFIG_HOTPLUG" == "y" -a "$CONFIG_USB" == "y" -o "$CONFIG_MMC" == "y" ]; then
	...
	echo "lp0 0:0 0660 */sbin/automount_print.sh \$MDEV" >> /etc/mdev.conf
        ...
        # 3G dial or usb storage
	if [ "$CONFIG_USER_3G" = "y" ]; then
		mode=`nvram_get 2860 3g_mode`
		if [ "$CONFIG_USER_3G_MODE_CHANGE" = "y" -a "$mode" == "1" ]; then
...
current_Bssidnum=`nvram_get 2860 BssidNum`
nvram_set 2860 BssidNum 3
ralink_init make_wireless_config rt2860
nvram_set 2860 BssidNum  $current_Bssidnum
if [ "$CONFIG_RTDEV" != "" -o "$CONFIG_RT2561_AP" != "" ]; then
...
if [ "$opmode" = "0" ]; then
	...
	addWds2Br0
	addMesh2Br0
	APCLI=`nvram_get 2860 ApCliEnable`
