...
function menuAdmin(options) {
   ...
   var isDsl = 0;
	...
	nodeAdvancedSetup = insFld(foldersTree, gFld(getMenuTitle(MENU_ADVANCED_SETUP), 'modeselect.html'));
	insDoc(nodeAdvancedSetup, gFld(getMenuTitle(MENU_LAYER2_INTERFACE), 'modeselect.html'));
	if (atm == '1' ) {
		isDsl = 1;
		if(wanmode=='0'||wanmode=='')
		{
			nodeWANSvrSetup = insFld(nodeAdvancedSetup, gFld(getMenuTitle(MENU_WAN), 'dslatm.cmd'));
			insDoc(nodeWANSvrSetup, gLnk('R', getMenuTitle(MENU_DSL_ATM_INTERFACE), 'dslatm.cmd'));	
		}	
	} 
	if (ptm == '1') {
		isDsl = 1;
		...
		//}
		nodeWANSvrSetup = insFld(nodeAdvancedSetup, gFld(getMenuTitle(MENU_WAN), 'dslptm.cmd'));
		insDoc(nodeWANSvrSetup, gLnk('R', getMenuTitle(MENU_DSL_PTM_INTERFACE), 'dslptm.cmd'));		
	}	   	
	if (ethwan == '1' ) {
		//if (!(atm == '1' || ptm == '1')) {
			//nodeLayer2Inteface = insFld(nodeAdvancedSetup, gFld(getMenuTitle(MENU_LAYER2_INTERFACE), 'modeselect.html'));
		//}
		if(wanmode=='1')
		{
			nodeWANSvrSetup = insFld(nodeAdvancedSetup, gFld(getMenuTitle(MENU_WAN), 'ethwan.cmd'));
			insDoc(nodeWANSvrSetup, gLnk('R', getMenuTitle(MENU_ETH_INTERFACE), 'ethwan.cmd'));
		}
	}
	if (mocawan == '1') {
		...
		//}
		nodeWANSvrSetup = insFld(nodeAdvancedSetup, gFld(getMenuTitle(MENU_WAN), 'mocawan.cmd'));
		insDoc(nodeWANSvrSetup, gLnk('R', getMenuTitle(MENU_MOCA_INTERFACE), 'mocawan.cmd'));
	}  
	
	//if (!(atm == '1' || ptm == '1' || ethwan == '1' || mocawan == '1'))
		//nodeAdvancedSetup = insFld(foldersTree, gFld(getMenuTitle(MENU_ADVANCED_SETUP), 'wancfg.cmd'));
	insDoc(nodeWANSvrSetup, gLnk('R', getMenuTitle(MENU_WAN_PARAM), 'wancfg.cmd'));
   ...
   //dhcp server 
	 if ( 1 ) {        
      nodeDhcp = insFld(nodeAdvancedSetup, gFld(getMenuTitle(MENU_DHCP),'dhcpserver.html'));     
      insDoc(nodeDhcp, gLnk('R', getMenuTitle(MENU_DHCPSERVER),'dhcpserver.html'));    
      insDoc(nodeDhcp, gLnk('R', getMenuTitle(MENU_DHCPCLIENT),'dhcpinfo.html'));
   }
   ...
   if ( proto != 'Not Applicable' && ipExt != '1' ) {
      ...
      if ( urlfilter == '1' )
         insDoc(nodeFirewall, gFld(getMenuTitle(MENU_URLFILTER),'urlfilter.cmd?action=view'));
      ...
      for(i = 0; i < parseInt(numWl); i++)
      {
         ...
         if(numWl == '1')
            //nodeWireless = insFld(foldersTree, gFld(getMenuTitle(MENU_WIRELESS_SETTINGS), wlItemsCgiCmd[0]));
				nodeWireless = insFld(foldersTree, gFld(getMenuTitle(MENU_WIRELESS_SETTINGS), 'wireless_basic.html'));
         else
         ...
         insDoc(nodeWireless, gLnk('R', getMenuTitle(MENU_WL_BASIC), 'wireless_basic.html'));
         insDoc(nodeWireless, gLnk('R', getMenuTitle(MENU_WL_SECURITY), 'wireless_security.html'));
 ...
 /*  if (p8021ag == '1') {
   ...
   //insDoc(nodeAccCntr, gLnk('R', getMenuTitle(MENU_ACC_CNTR_PASSWORD), 'password.html'));
	 insFld(nodeMngr, gFld(getMenuTitle(MENU_TL_SETTINGS_BACKUP), 'backupsettings.html'));
	 insFld(nodeMngr, gFld(getMenuTitle(MENU_TL_SETTINGS_UPDATE), 'updatesettings.html'));
	 insFld(nodeMngr, gFld(getMenuTitle(MENU_TL_SETTINGS_DEFAULT), 'defaultsettings.html'));	
  ...
  // insFld(foldersTree, gFld(getMenuTitle(MENU_EXIT), 'exit'));  
   //Ò×°²×° 
  // nodeWizardInfo = insFld(foldersTree, gFld(getMenuTitle(MENU_WIZARD), 'index.html'));  
}
function preLogout() {	
...
function createBcmMenu(options) {
   ...
   insDoc(nodeSts, gLnk('R', getMenuTitle(MENU_ST_WAN), 'statswan.cmd'));
   if(wanmode=='0')
   {
   ...
   insFld(nodeDeviceInfo, gFld(getMenuTitle(MENU_RT_ARP),'arpview.html'));
   // dhcp info
