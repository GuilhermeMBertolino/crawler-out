...
mesh_hop_check()
{
	...
	limit_5g=`cfm get adv.capacity.en`
#	echo $mppmac
#	echo $limit_5g
	...
	mac_str=$(printf "%s%s%s%s%s%02x\n" $mac0 $mac1 $mac2 $mac3 $mac4 $mac5)
#	echo wifi_5g_mac $mac_str
	hopcount=$((`cat /proc/wlan0/mesh_pathsel_routetable |grep "destMAC: $mac_str" -A 8|grep hopcount|cut -d ' ' -f 6`))
#	echo [$hopcount]
	last_max_5g=$max_5g
...
mesh_sm_normal()
{
	...
	if [[ -z "${is_mesh_group}" ]] || [ $limit_5g -eq 0 ]; then
#		echo "5g enable access state, 5g sta num {$sta_num_total}"
		...
		mesh_hop_check
		#echo max_num $max_5g	
		if [ ${max_change} -eq 1 ];then
		iwpriv wlan0 set_mib td_meshlink_max_sta=$max_5g;
		max_change=0;
		fi
...
mesh_sm_5g_disable()
{
	...
	echo 0 > /proc/hybrid_steering/enable
	state=${MESH_SM_NORMAL}
	en_5g='0'
	dis_5g='1'
}
...
mesh_sm_5g_enable()
{
	...
	echo 1 > /proc/hybrid_steering/enable
	state=${MESH_SM_NORMAL}
	en_5g='1'
	dis_5g='0'
}
