...
{
	local radio_name enrollee_pin enrollee_pin_res
	radio_name=$1
	enrollee_pin=$2

	enrollee_pin_res=`hostapd_cli -i$radio_name wps_check_pin $enrollee_pin`
...
wps_connect_via_pin()
{
	...
	enrollee_pin=`wps_pin_checksum_validity $radio_name $enrollee_pin`
...
cancel_wps()
{
	...
	#echo "cancel_wps: wps_fail: /tmp/hostapd_cli_${radio_name} -i $radio_name wps_cancel" > /dev/console
...
wps_external_done()
{
	...
	then
		...
		[ "$enable_log" = "1" ] && echo "wps_external_done: hostapd_wpa_pairwise=$hostapd_wpa_pairwise" > /dev/console
		[ "$enable_log" = "1" ] && echo "wps_external_done: hostapd_wpa=$hostapd_wpa" > /dev/console
	...
	do
		...
		then
			...
			#	ssid=${hostapd_ssid}_${index}
				if [ "$interface_name_tmp" == "wlan0" ] 
				then
					if [ ${#ssid} -ge 28 ];
					then
						ssid_prefix=${ssid:0:27}
					else
						ssid_prefix=${ssid}
					fi
					ssid_suffix="-2.4G"
				...
				ntgr_ssid=${ssid_prefix}${ssid_suffix}
				[ "$enable_log" = "1" ] && echo -e "\033[33mwps_external_done: interface_name_tmp=$interface_name_tmp ntgr_ssid=$ntgr_ssid\033[0m" > /dev/console
			else
				ntgr_ssid=${ssid}
				[ "$enable_log" = "1" ] && echo -e "\033[33mwps_external_done: interface_name_tmp=$interface_name_tmp ntgr_ssid=$ntgr_ssid\033[0m" > /dev/console
			fi
			...
			ubus_command="${ubus_command}\"pn${param_index}\":\"SSID\",\"pv${param_index}\":\"$ntgr_ssid\","
			param_index=$((param_index+1))
			...
			then
				...
				[ "$enable_log" = "1" ] && echo "wps_external_done: change due to wps_state=2 NoRestart=$NoRestart wps_state_from_conf=$wps_state_from_conf wps_state_val=$wps_state_val" > /dev/console
			fi
