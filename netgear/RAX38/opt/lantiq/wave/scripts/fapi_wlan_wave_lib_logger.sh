...
assign_hw_module_fifo()
{
	...
	if [ "$chip_id" = "$GEN6_CHIP_ID" ] || [ "$chip_id" = "$GEN6D2_CHIP_ID" ]; then
...
assign_hw_module_fifo_gen5()
{
	...
	echo "Logger:assign_hw_module_fifo_gen5" >dev/console
...
assign_hw_module_fifo_gen6()
{
	...
	echo "Logger:assign_hw_module_fifo_gen6" >dev/console
...
get_assigned_fifos()
{
	...
	echo "get_assigned_fifos gen5" >dev/console
...
get_assigned_fifos_gen6()
{
	...
	echo "Logger:get_assigned_fifos gen6" >dev/console
	...
	do
		...
		if [ "$fifo_enabled" = "1" ]
		then
			echo "Logger:$fifo_name is Enabled" >/dev/console
			shift_res=$((1<<i))
			assigned_fifos=$((assigned_fifos|shift_res))
		fi
		i=$((i+1))
	...
	echo "Logger: assigned_fifos final bits: $assigned_fifos" > /dev/console
	echo "$assigned_fifos"
}
...
set_destination_mac()
{
	...
	case $component_name in
		...
		*) object_name="$RADIO_VENDOR_FW_LOGGER_OBJECT";;
	esac
