...
#define BUFFER_SIZE_18 18
#define BUFFER_SIZE_32 32
#define BUFFER_SIZE_64 64
#define BUFFER_SIZE_128 128
#define BUFFER_SIZE_256 256
#define MSG_BUFFER_SIZE 64
#define NDN_STATE_BUFF 32
#define DEFAULT_INDEX_VALUE 256 //!< Stores the value for d2 table index initialization
#define MAX_INDEX_VALUE 255  //!< Last index value in d2 table
#define CD_UPDATE_CLOUD_PASS_ID 10 //!< Value for cloud pass for device details customisation
...
#define MODULENAME "Dalh"


/* Define for the Table Value */
#define RA_DEBUG_TABLE_NAME "RADebugLog"
...
enum featureId {
  ...
  PAUSE_PROFILE = 11,
  UNPAUSE_PROFILE = 12,
  DEV_UPDATE_CD_CUSTOM = 14
};

//CDIL checkpoints index information to debug CDIL failure
enum cdilCheckpointIndex {
  CDIL_PROCESS = 0,
  CDIL_INITIATE = 1,
  CDIL_ENQUEUE = 2,
  CDIL_DALHSUCESS = 3,
...
/** Check for return values used in DALH */
enum dalh_return_codes{
    RET_SUCCESS,
    RET_FAILURE
};
...
struct Queue {
  ...
  uint32_t cdIndexChanged; //!< Stores the customised index
  char bstStatus[10];
...
void getNdnAttributes(uint32_t* index);
void notifyUpagentForNdn(uint32_t index);
...
void getProfileAttributes(void);
void notifyDilForProfileOperation(int profileState);
/*Func defs to get CDIL debug checkpoints message*/
const char* getCdilCheckpointMessage(const enum cdilCheckpointIndex cdilMsg);
