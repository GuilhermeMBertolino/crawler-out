...
/**
 * This buffer size value is depends on CircleDebug table, field rClientStatus
 */
#define RCLIENT_BUFF_SIZE   16
#define SPC_SYNC_DONE_SIZE  16
/**
 * This buffer size value is depends on CircleDebug table, field errorMessage
 */
#define CIRCLE_ERROR_MSG_SIZE 128
...
#define CIRCLECTL_RESET "/circlectl reset | while IFS= read -r line; do printf '[%s][%s]\n' \"($(date '+%Y-%m-%d %H:%M:%S'))\" \"$line\"; done >> /tmp/circlectl.log"
#define CIRCLECTL_STATUS "circlectl status -j"
#define POLL_DELAY 15
#define POLL_DELAY_UUID 5
#define MAX_RETRY 3
#define CA_START_MAX_RETRY 15
#define CA_START_WAIT 2
...
/**
 * This is used to check circle running status either true or false when circle is activated,
 * enabled or disabled under circlectl status -j command
 */
#define CIRCLE_RUNNING "circle-running"
/**
 * This is used to check circle provisioned status either true or false when circle is provisioned
 * under circlectl status -j command
 */
#define CIRCLE_PROVISIONED "circle-provisioned"
/**
 * This is used to check rclient connected status when circle is provisioned, enabled or disabled
 * under circlectl status -j command
 */
#define RCLIENT_CONNECTED "\"rclient\":\"connected\""
/**
 * This is used to check rclient disconnected status when circle is disabled
 * under circlectl status -j command
 */
#define RCLIENT_DISCONNECTED "\"rclient\":\"disconnected\""
#define SPC_AGENT_RUNNING "spcAgentRunning"
#define SPC_PROVISIONED "spcProvisioned"
