...
#define DEVICE_STATE_MAX_LEN 16
...
#define DISPLAY_TYPE_MIN 32
#define SIGNATURE_MIN_BUFF_SIZE 64
#define INIT_FAIL -1
#define MID_MAC_LENGTH 9 // macro is defined to take the half length of MAC-address
#define MAX_DISPLAY_LENGTH 68 //macro to define display_name_netgear buffer length
...
struct postprocessor_args_devicename_devicetype
{
    ...
    char DeviceMacVendor[BUFF_LEN_64+1];
...
extern int update_device_displayname(const char* mac_address, const char* display_name, int name_size, int producer_id);
extern int update_device_displaytype(const char* mac_address, const char* new_display_type, int device_type_size, int producer_id);
...
extern int dil_update_table_field(char* mac_address ,unsigned int field_code,void *new_value, unsigned int field_length, int producerid);
...
extern int dil_update_pause_list(char* mac_address[], bool state, unsigned int mac_count, int producer_id);
extern int check_update_mapping_table (char* Devicemac,int producer_id);
char* unescapeJSON(char* input,char* output, unsigned int length);
extern void check_and_set_default_devicetype( int mac_index );
...
enum error_code
{
	...
	ERROR_NOK_ACL = -21,
        ERROR_PAUSE_NOT_NEEDED = -22,
       ERROR_MAPPING_OP_ONGOING = -23,
       ERROR_INITIALIZATION_FAILURE = -24,
};

/**
 * This producerid contains the predefined producer numbers to identify the consumer of the DIL APIs
 * for example - if the consumer is ODM then while using dilapi, ODM need to pass 50 as producer id
 */
enum producerid {
	dummy_producer1=0,
	dummy_producer2=10,
