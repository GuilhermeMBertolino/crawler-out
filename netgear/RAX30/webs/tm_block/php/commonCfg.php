...
function getMlangIndexByRegion($region)
{
    $index_List = array(
        ...
        "IN" => [ "India", "3G32"],
        "MY" => [ "Malaysia", "3G35"],
        "QA" => [ "Middle East(Iran/Labanon/Qatar)", "SWS083"],
        "KW" => [ "Middle East(Egypt/Tunisia/Kuwait)", "SWS084"],
        "TR" => [ "Middle East(Turkey)", "PCVP_048"],
        "AE" => [ "Middle East(Saudi Arabia/United Arab Emirates)", "PCVP_048"],
        "SG" => [ "Singapore", "3G44"],
        "TW" => [ "Taiwan", "3G49"],
        "HK" => [ "Hong Kong", "3G30"]
    );
...
function getflashUsage()
{
    ...
    $rows = explode("\n", $output);
    $cols = preg_split('/\s+/', trim($rows[1]));
    $used = round($cols[2]/1024)+(256-108);
    return $used."MB/256MB";
}
...
# Prevent OOM, we can only keep one upload file at a time.
function FwUpgradeInit()
{
    $cmd_str= "rm /tmp/multipartFile /tmp/uploadFile /tmp/upload_file";
    exec($cmd_str, $res, $status);
}

function getProtectionEngineVersion()
{
    $lookup_cmd = "cat /opt/bitdefender/bitdefender-release | grep VERSION | awk -F '[=~]' '{printf $2}'";
    $output_armor= shell_exec($lookup_cmd );
    $lookup_cmd = "cat /opt/bitdefender/guster/guster.version";
    $output_guster= shell_exec($lookup_cmd );
    $output=$output_armor."/".$output_guster;
    return $output;
}
?>
