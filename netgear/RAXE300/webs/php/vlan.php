...
function getCheckboxVal_InternetPort()
{
    if (getWanPort() == "1G")
    {
        return "false";
...
function getCheckboxVal_MultigigPort()
{
    if (getWanPort() == "MultiG")
    {
        return "false";
    }
    $portMap = db_get("Device.X_PEGATRON_COM_VLAN.PortMaps");
...
function getCheckboxVal_wifi6g()
{
    $portMap = db_get("Device.X_PEGATRON_COM_VLAN.PortMaps");
    $port_array = explode(",", $portMap);
...
function getVlanRuleTable()
{
    ...
    foreach($data[$db_path] as $inst)
    {
        ...
        foreach($port_array as $port) {
            ...
            if ($port == "oneG") { //Internet Port (1G)
                if (getWanPort() != "1G")
                {
                    $oneG = "checked";
                }
            }
            if ($port == "multiG") { //MultigigPort (MultiG)
                if (getWanPort() != "MultiG")
                {
                    $multiG = "checked";
                }
        ...
        $output_value->{$count}->__enOneG  = $oneG;
        $output_value->{$count}->__enMultiG = $multiG;
        $output_value->{$count}->__enWifi2g = $wifi2g;
        $output_value->{$count}->__enWifi5g = $wifi5g;
        $output_value->{$count}->__enWifi6g = $wifi6g;
...
function getLacpPortName()
{
    if (getLacpEnable() == "true")
    {
        return "Port3+Port4(Aggregated)";
    }
    else
    {
        return "Port3";
...
function getInternetPortStatus()
{
    $status = db_get("Device.Ethernet.Interface.1.Enable");

    return num_to_TrueFalseStr($status);
}
?>
