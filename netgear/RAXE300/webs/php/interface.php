...
function getWanInstanceID()
{
    ...
    {
       $inst_id = 2; //eth0.1 interface id is 2
    }
...
function getWanPort()
{
    $wan_upstream = db_get("Device.Ethernet.Interface.1.Upstream");
...
function getWANPortStatus()
{
  ...
  */
  ...
  /*For multi WAN port*/
  $wanPort1_status = db_get("Device.Ethernet.Interface.1.Status");
  $wanPort2_status = db_get("Device.Ethernet.Interface.2.Status");
  if($wanPort1_status === "Up" || $wanPort2_status === "Up")
  {
      return "up";  
  }
  else
...
function getWizardInternetStatus_autoWan()
{   
    if(getWANPortStatus() ===  "down")
        return "down"; 
    ...
    /*$cmd_str = "ping -A -c 10 netgear.com";
    exec($cmd_str, $res, $status);
    return ($status == 0) ? "up" : "down";*/
...
{
    $wan_inst = getWanEthernetInstanceID();
    $wanport = db_get("Device.Ethernet.Interface.{$wan_inst}.Name");
    ...
    $cmd_str= "wandetect -i {$intf_name} -c {$clientMacAddr} 1<&-";
    system($cmd_str, $ret);

    $cmd_str2= "cat /tmp/WanDetectResult";
    exec($cmd_str2, $res, $output);
