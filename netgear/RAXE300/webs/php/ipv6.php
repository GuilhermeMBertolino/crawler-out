...
function getDHCPV6_IP_InterfaceInstId()
{
    ...
    if($ppp_if_list !== FALSE)
    {
        ...
        foreach($ppp_if_list[$ppp_if_path] as $inst_id)
        {
            if(db_get("Device.IP.Interface.{$inst_id}.Name") == $intf_name)
            {
               return $inst_id;
            }
        }
...
function getConnection_v6AutoDetectResult()
{
...
function getIpVal_gateway()
{
    ...
    $autodetect = getConnection_v6AutoDetectResult();
    $ip_instid = getDHCPV6_IP_InterfaceInstId();
    ...
    {
        $wanInterfaceName = "Device.IP.Interface.{$ip_instid}";
    }
    else if ($wan_type == "AutoDetect")
    {
        if ($autodetect == "PPPoE")
        {
            $wanInterfaceName = "Device.IP.Interface.5";
        }
        else if ($autodetect == "DHCPv6" || $autodetect == "Fixed")
        {
            $wanInterfaceName = "Device.IP.Interface.{$ip_instid}";
        }
        else
...
function getWanIpAddr_For_AdvancedHome()
{
    ...
    foreach($GLOBALS['ip_if_list']["Device.IP.Interface"] as $inst_id)
    {
        ...
        {
            if ($wan_type === "Fixed" || $wan_type === "DHCPv6")
            {
                $ipv6Addr = db_get("Device.IP.Interface.{$inst_id}.X_PEGATRON_COM_StaticIpv6Addr");
                if( $ipv6Addr === "")
                    return $wanIP;
                else
                    return $ipv6Addr;
