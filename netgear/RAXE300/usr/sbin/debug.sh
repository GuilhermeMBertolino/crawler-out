...
function CapturePacket()
{
    ...
    if [ "$NEW_PACKETCAPTURE" == "ENABLE" ]; then
        ...
        tcpdump  -i $WANPort ether[12:2] != 0x8100 and ether[14:2] != 0x001 -w ${STORE_PATH}/ETH_WAN.cap -C 10 &
        #PEGA>>
    else
    ...
    WlGetDriverStats.sh wl0 dhd 20 enable > /tmp/wl0_DrvStats.log 2>&1 &
    WlGetDriverStats.sh wl1 nic 20 enable > /tmp/wl1_DrvStats.log 2>&1 &
    WlGetDriverStats.sh wl2 nic 20 enable > /tmp/wl2_DrvStats.log 2>&1 &

    WlGetDriverCfg.sh wl0 5 dhd > /tmp/wl0_DrvCfg.log 2>&1 &
    WlGetDriverCfg.sh wl1 2 nic > /tmp/wl1_DrvCfg.log 2>&1 &
    WlGetDriverCfg.sh wl2 6 nic > /tmp/wl2_DrvCfg.log 2>&1 &

    wl -i wl0 curpower > /tmp/wl0_Curpower.log 2>&1 &
    wl -i wl1 curpower > /tmp/wl1_Curpower.log 2>&1 &
    wl -i wl2 curpower > /tmp/wl2_Curpower.log 2>&1 &
}
...
function Pega_collect_debug_info()
{
    ...
    wl -i wl2 status >> $outputPath
    wl -i wl2 sta_info all >> $outputPath
    wl -i wl2.1 status >> $outputPath
    wl -i wl2.1 sta_info all >> $outputPath
    ...
    WlGetDriverCfg.sh wl1 5 dhd >> $outputPath
    echo -e "\rcommand: WlGetdriverCfg.sh wl2 6 nic" >> $outputPath
    WlGetDriverCfg.sh wl2 6 nic >> $outputPath
...
function SaveLog()
{
    ...
    if [ -e "/tmp/wl2_DrvStats.log" ]; then
    WL_LOGS="$WL_LOGS /tmp/wl2_DrvStats.log"
    fi
    if [ -e "/tmp/wl0_DrvCfg.log" ]; then
    WL_LOGS="$WL_LOGS /tmp/wl0_DrvCfg.log"
    fi
    if [ -e "/tmp/wl1_DrvCfg.log" ]; then
    WL_LOGS="$WL_LOGS /tmp/wl1_DrvCfg.log"
    fi
    if [ -e "/tmp/wl2_DrvCfg.log" ]; then
    WL_LOGS="$WL_LOGS /tmp/wl2_DrvCfg.log"
    fi
    if [ -e "/tmp/wl0_Curpower.log" ]; then
    WL_LOGS="$WL_LOGS /tmp/wl0_Curpower.log"
    fi
    if [ -e "/tmp/wl1_Curpower.log" ]; then
    WL_LOGS="$WL_LOGS /tmp/wl1_Curpower.log"
    fi
    if [ -e "/tmp/wl2_Curpower.log" ]; then
    WL_LOGS="$WL_LOGS /tmp/wl2_Curpower.log"
    fi
    ...
    zip -9 -q -r -j /${STORE_PATH}/debug_log_1.zip $LOGREAD $BR_PACKET $WAN_PACKET $BOOTUP_MSG $DAL_LOGS $WL_LOGS $BITDEFENDER_LOGS $DAL_DB $RAE_LOG $DEBUG_INFO_PATH $PPP_LOG $SYS_LOG $NTGR_SPC_LOG_ZIP_FILE
...
#<<PEGA Ian: adjustable wan port
function ReadWanPort()
{
    filename='/tmp/WanPorts'
    while read line; do
        WANPort=$line
    done < $filename
}
...
if [ "$FUNC" == "CF4" ]; then
    ...
    if [ ! -z $NEW_RA_SERVERPATH ]; then
        ...
        kill -SIGUSR1 $(pidof puraUpdate) # restart puraUpdate
    fi
    ...
    if [ ! -z $NEW_RA_UPDATEINTERVAL ]; then
        if [ "$NEW_RA_UPDATEINTERVAL" != "$CUR_RA_UPDATEINTERVAL" ]; then
            ...
            sed -i 's/"updaterInternal" : "'$CUR_RA_UPDATEINTERVAL'"/"updaterInternal" : "'$NEW_RA_UPDATEINTERVAL'"/g' /data/rabin/raePolicy.json
            kill -SIGUSR1 $(pidof puraUpdate)
        fi
    fi
fi
...
if [ "$FUNC" == "CF7" ]; then
    if [ ! -z $NEW_ALLOWIPV6PING ]; then
        if [ "$CUR_ALLOWIPV6PING" != "$NEW_ALLOWIPV6PING" ] && [ "$NEW_IPV6MODE" != "Disabled" ]; then
            ...
            #<<PEGA Ian: adjustable wan port
            ip6tables -t filter -w -D FORWARD -i $WANPort.1 -p icmpv6 -jACCEPT
            #PEGA>>
            if [ "$NEW_ALLOWIPV6PING" == "ENABLE" ]; then
                ...
                if [ "$NEW_IPV6MODE" == "PPPoE" ]; then
                    ...
                    ip6tables -t filter -w -I FORWARD -i $WANPort.1 -p icmpv6 -jACCEPT
                fi
            else
