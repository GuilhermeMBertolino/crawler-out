...
start)
  ...
  echo 3 > /proc/sys/vm/drop_caches
	...
	iptables -t mangle -D PREROUTING -i $dev_wan -p udp -j $chain #PEGA JYang, delete rule before add it
...
stop)
	...
	# PEGA, JYang, flush the iptables for BWDPI
	echo "Flush iptable to filter WAN bootp packets..."
	chain=BWDPI_FILTER
	iptables -t mangle -F $chain
...
restart)
	...
	# PEGA JYang, always reload the TDTS module to make enable QoS sequence is consistent
