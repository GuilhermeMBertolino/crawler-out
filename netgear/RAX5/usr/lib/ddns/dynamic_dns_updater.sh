...
# Jinjuan Pei, 20220512, Fix issue: NoIP/DynDNS script file start failed in AP Mode.
# ip_network is local defined network to read IP, but for different operation mode and WAN mode, we should read IP address from different interface
...
# Jinjuan Pei, 20211213, Do not force update DDNS IP address if WAN IP address is not changed and next update time is not timeout yet.
local firstBoot
firstBoot=1
# ddns config is changed and reload, force ip update
[ -f "/var/ddnsconfigchanged" ] && firstBoot=0
# Jinjuan Pei, 20211213, End.

# loop endlessly, checking ip every check_interval and forcing an updating once every force_interval
write_log 6 "Starting main loop at $(eval $DATE_PROG)"
while : ; do
	# Jinjuan Pei, 20211213, Dynamic DNS 'Show Status' feature.
	config_load ddns
	__DdnsUpdate=0
	local updateResult=""
	# Jinjuan Pei, 20211213, End.
	...
	# Jinjuan Pei, 20211213, Do not force update DDNS IP address if WAN IP address is not changed and next update time is not timeout yet.
	# get WAN IP address of lastest successful update
	local wanIpLastUpate currentTime nextUpdateTime

	# get the wan ip address of the lastest successful update
	if [ -f "/etc/wan_ip" ]; then
		wanIpLastUpate=$(cat /etc/wan_ip)
	else
		wanIpLastUpate="0.0.0.0"
	fi

	if [ $firstBoot == 1 -a "$LOCAL_IP" == "$wanIpLastUpate" ]; then
		currentTime=`date +'%s'`
		nextUpdateTime=$(($(cat /etc/ddns_lastupdate)+$FORCE_SECONDS))
		if [ $currentTime -lt $nextUpdateTime ]; then
			local timeDiff
			timeDiff=$(($nextUpdateTime-$currentTime))
			echo "Device boot up and will wait $timeDiff seconds to perform ip address update!" > /dev/console
			sleep $timeDiff &
			PID_SLEEP=$!
	...
	# Jinjuan Pei, 20211213, Perform an update if DDNS config is changed.
	# send update when current time > next time or local ip different from registered ip
	# if [ $CURR_TIME -ge $NEXT_TIME -o "$LOCAL_IP" != "$REGISTERED_IP" ]; then # original
	if [ $CURR_TIME -ge $NEXT_TIME -o "$LOCAL_IP" != "$REGISTERED_IP" -o -f "/var/ddnsconfigchanged" ]; then
		echo "Force update ip address!" > /dev/console
		rm "/var/ddnsconfigchanged"
	# Jinjuan Pei, 20211213, End.
		...
		if [ $ERR_LAST -eq $PROVIDER_ANS_SUCCESSFUL ]; then # Jinjuan Pei, 20211213, Dynamic DNS 'Show Status' feature.
...
# Jinjuan Pei, 20211213, Dynamic DNS 'Show Status' feature.  
			uci set ddns.myddns_ipv4.updatestatus='HostnameUpdateSuccess'
			echo "HostnameUpdateSuccess" > /dev/console
			# record the date/time of update successful
			echo `date +%s` > /etc/ddns_lastupdate
			# record device wan ip address of update successful
			echo $LOCAL_IP > /etc/wan_ip
			__DdnsUpdate=1
			updateResult="successful"
		elif [ $ERR_LAST -eq $PROVIDER_ANS_NOHOST ]; then
			uci set ddns.myddns_ipv4.updatestatus='HostNameError'
			__DdnsUpdate=1
			updateResult="failure"
		elif [ $ERR_LAST -eq $PROVIDER_ANS_BADAUTH ]; then
			uci set ddns.myddns_ipv4.updatestatus='AuthenticationFail'
			__DdnsUpdate=1
			updateResult="failure"
		elif [ $ERR_LAST -eq $PROVIDER_ANS_NOTREACHABLE ]; then
			uci set ddns.myddns_ipv4.updatestatus='ProividerNotReachable'
			echo `date +%s` > /var/run/ddns/ddns_update_notreachable
			__DdnsUpdate=1
			updateResult="failure"
		elif [ $ERR_LAST -eq $PROVIDER_ANS_CLIENTBLOCKED ]; then
			uci set ddns.myddns_ipv4.updatestatus='ClientBlocked'
			__DdnsUpdate=1
			updateResult="failure"
# Jinjuan Pei, 20211213, End.
	...
	# Jinjuan Pei, 20211213, Dynamic DNS 'Show Status' feature.
	if [ $__DdnsUpdate -eq 1 ]; then
		uci commit ddns
	fi
	if [ $updateResult != "" ]; then
		# log DDNS event per NTGR SPEC
		`logger -p local5.warn -t "[RouterOp]" "[Dynamic DNS] host name $lookup_host registration $updateResult"`
	fi
	# Jinjuan Pei, 20211213, End.
