...
local function parseNormalTag(self, xml, f)
    ...
    if (f.endt1=="/") then
        ...
        --PEGA checkEndTag exist, if not ,don't insert table
        --local checkEndTag = '</'..tag.name..'>'
        --if type(string.find(xml, checkEndTag,f.match,true)) == "number" then  --Don't do check, let unpaired tag to be error case and the xpcall to handle error
