...
log.print("[trafficMeter_limited] lua in '/usr/lib/lua/webGetFunc' !")

function M.getSpanVal_monthlyLimit()
	local controlType = uci:get("tm", "tm_control", "volcontrl")
	if (controlType == "true") then
	    return uci:get("tm", "tm_control", "volume_monthly_limit")
	else
	    return uci:get("tm", "tm_control", "conntime_monthly_limit")
	end
end
...
function M.getSpanVal_messageLimitedTime()
	local ret = uci_st:get("tm", "tm_statistics", "limitedTime")
	return ret
end
