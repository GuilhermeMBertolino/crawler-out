...
function Cursor.forkApply(self, configlist, command)
	local fork_exec = require"commonFunc.fork".fork_exec
	configlist = self:_affected(configlist)
	if command then
	    return { "/sbin/luci-reload", unpack(configlist) }
	else
	    return fork_exec("/sbin/luci-reload %s >/dev/null 2>&1" % table.concat(configlist, " "))
	end
end

function Cursor.check_change(self)
    local checkresult = util.exec("/sbin/cgilua_checkchange", "r")
	if #checkresult > 0 then
		local result_table = {}
		string.gsub(checkresult, '[^'.." "..']+', function(token) table.insert(result_table, token) end )
		log.console_r(result_table)
		return result_table
	else 
		log.console("No change!")
		return nil
    end
end
