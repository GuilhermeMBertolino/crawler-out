...
-- Modify for input "function" and "data" parameters, and response format to match RAX30
function handle_netgear(tbl, rawsource, ...)
	local decoder = luci.json.Decoder()
	local stat = luci.ltn12.pump.all(rawsource, decoder:sink())
	local json = decoder:get()
	local response
	...
	if stat then
		if type(json["function"]) == "string" and json.data ~= nil then
			local method = resolve(tbl, json["function"])
			if method then
				response = proxy_netgear(method, json.data)
			else
				response = {status="failure", message="Method not found."}
			end
		else
			response = {status="failure", message="Invalid request."}
		end
	else
		response = {status="failure", message="Parse error."}
	end

	return luci.json.Encoder(response, ...):source()
end

-- Modify for response format to match RAX30
function proxy_netgear(method, ...)
	local res = {luci.util.copcall(method, ...)}
	local stat = table.remove(res, 1)

	if not stat then
		return {status="failure", message="Invalid params."}--[[not match parameter in setting handler--]]
	else
		if #res <= 1 then
			return res[1] or luci.json.null
		else
