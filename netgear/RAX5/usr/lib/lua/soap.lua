...
serialize = function (obj)
	...
	elseif tt == "table" then
		...
		if t ~= nil then
			...
			return strformat ("%s", contents(obj))
		end
	else
		return ""
	end
...
local envelope_template = {
	...
	attr = { "xmlns:soap-env", "soap-env:encodingStyle",
		["xmlns:soap-env"] = nil, -- to be filled
...
local function encode (args)
	...
	if tonumber(args.soapversion) == 1.2 then
		envelope_template.attr["xmlns:soap-env"] = xmlns_soap12
	else
		envelope_template.attr["xmlns:soap-env"] = xmlns_soap
	end
	local xmlns = "xmlns:m"
	...
	local code = (envelope_template[2] and envelope_template[2][2]) or envelope_template[1][2]
	code.tag = args.codeTag
	code[1] = args.codeValue
