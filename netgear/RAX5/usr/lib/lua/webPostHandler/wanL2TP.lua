...
function wanL2tpSetup_validator(parm, value)
...
local wan_l2tp_setup_maps =
    {
		...
		ipAddr	=	{ data_type = "ipv4_addr",     handler = wanL2tpSetup_validator },
		netmask	=	{ data_type = "ipv4_netmask",     handler = wanL2tpSetup_validator },
		gateway	=	{ data_type = "ipv4_addr",     handler = wanL2tpSetup_validator },
		serverAddr	=	{ data_type = "server_address",     handler = nil },
		dnsType	=	{ data_type = "addr_assign",     handler = nil },
		dns1	=	{ data_type = "ipv4_dns",     handler = wanL2tpSetup_validator },
		dns2	=	{ data_type = "ipv4_dns",     handler = wanL2tpSetup_validator },
		macClone	=	{ data_type = "mac_clone",     handler = nil },
		cloneMac	=	{ data_type = "mac_addr",     handler = nil }
    };

---------------------------------------------------------------------------------------------------
function M.wanL2TP_handler(json)
	log.debug(0)
	log.print_r(json)
	--{"function":"wanL2TP","data":{"login_type":"L2TP","username":"l2tpusr","password":"l2tppwd","mode_autoid":"Dail on Demand","mode":"onDemand","idleTimeout":"5","ipType":"fixed","ipAddr":"111.1.2.44","netmask":"255.255.255.0","serverAddr":"111.2.1.4","gateway":"111.2.1.252","dnsType":"dynamic","dns1":"false","dns2":"","macClone":"default","cloneMac":"00:00:00:00:00:00"}}
	log.print("wanL2TP:", json.login_type, json.username, json.password, json.mode_autoid, json.mode, json.idleTimeout, json.ipType, json.ipAddr, json.netmask, json.gateway, json.serverAddr, json.dnsType, json.dns1, json.dns2, json.macClone, json.cloneMac)

	--1. Data validation
	local wan_mode, proto, iptype, ipaddr, netmask, gateway
	local username, password, mode, idle_timeout, server
	local dnstype, dns1, dns2
	local mac_clone, mac_addr
	
	if(json.login_type == "L2TP") then
		wan_mode = "L2TP"
	else
		return {status="failure", message="WAN(L2TP) Setup Failure - login_type error"}
	end
	...
	elseif(iptype ~= "dynamic") then
		return {status="failure", message="WAN(L2TP) Setup Failure - ipType error"}
	end
    ...
    if (validator.post_data_validate(json, wan_l2tp_setup_maps) == false) then
	...
	-- saved config
	uci:set("network", "inet_global", "wan_mode", "L2TP")
	uci:set("network", "inet_l2tp", "proto", proto)
	uci:set("network", "inet_l2tp", "username", username)
	uci:set("network", "inet_l2tp", "password", password)
	uci:set("network", "inet_l2tp", "server", server)
	uci:set("network", "inet_l2tp", "conn_mode", mode)
	if(mode == "onDemand") then
		uci:set("network", "inet_l2tp", "idle_timeout", idle_timeout)
	end

	uci:set("network", "inet_l2tp", "iptype", iptype)
	if(iptype == "fixed") then
		-- saved config
		uci:set("network", "inet_l2tp", "ipaddr",  ipaddr)
		uci:set("network", "inet_l2tp", "netmask", netmask)
		uci:set("network", "inet_l2tp", "gateway", gateway)
	else -- dynamic
		-- saved config
		uci:delete("network", "inet_l2tp", "ipaddr")
		uci:delete("network", "inet_l2tp", "netmask")
		uci:delete("network", "inet_l2tp", "gateway")
		-- set run-time config
	end
	...
	return {status="success", message="Finish WAN(L2TP) Setup"}
end
