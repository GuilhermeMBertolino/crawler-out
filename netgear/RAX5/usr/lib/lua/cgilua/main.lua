...
local function build_library_objects(enviroment, response)
	...
	--[[++PEGA++]]
	---------------------------------------------------------------------------
	-- gets variable from pega table 
	---------------------------------------------------------------------------
	M.getPegaTable = function (key)
		return  L._pega_table[key]
	end;

	---------------------------------------------------------------------------
	-- inserts variable into pega table 
	---------------------------------------------------------------------------
	M.insertPegaTable = function (key,value)
		L._pega_table[key] = value
	end;
	--[[--PEGA--]]
	...
	M.tmpfile = function (dir, namefunction)
		...
		if file then
		  if not M.getPegaTable("formDataNameToTmpNameMap")(tempname) then --[[PEGA]]
	...
	M.doplua = function (filename)
		...
		if not f then
			...
			local ret, content  = xpcall (f, L.errorhandler)
			if ret and type(content) == "string" then
	...
	M.docfg = function (inputFilename)
		if not inputFilename then return end    -- no file
		local f, err = _open(inputFilename)
		if not f then return nil, err end    -- no file (or unreadable file)
		f:close()
		M.header("Content-Disposition", "attachment; filename="..inputFilename)
		return M.doscript (inputFilename)
	end
	--[[--PEGA--]]
	...
	L.default_erroroutput = function (msg)
		...
		msg = gsub (gsub (msg, "\n", "<br>\n"), "\t", "&nbsp;&nbsp;")
		M.Response.contenttype ("text/html")
		M.Response.write ("<html><head><title>Internal Server Error</title></head><body>Plese debug by log</body></html>")
	end
