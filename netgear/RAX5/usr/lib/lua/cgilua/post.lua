...
--[[++PEGA++]]
local getPegaTable = require"cgilua".getPegaTable
local insertPegaTable = require"cgilua".insertPegaTable
--[[--PEGA--]]
...
local function fileupload (filename)
	...
	if getPegaTable("formDataNameToTmpNameMap")[getPegaTable("formDataName")] ~= nil then
      local tmpPath = getPegaTable("tmpPath")
      local getTmpNameFunc = getPegaTable("getTmpNameFunc")
      file, err = tmpfile(tmpPath, getTmpNameFunc)
	else
	  file, err = tmpfile()
	end
	--[[--PEGA--]]
...
local function Main (inputsize, args)
	...
	while true do
		...
		--[[++PEGA++]]
		insertPegaTable("formDataName",name)
		--[[--PEGA--]]
...
return {
	parsedata = function (defs)
		...
		elseif strfind(contenttype, "application/json", 1, true) then
			...
			urlcode.insertfield(defs.args, "inputJson", input)
		elseif strfind(contenttype, "text/xml", 1, true) then --For SOAP
			local input = read(inputsize)
			urlcode.insertfield(defs.args, "inputXml", input)
		else
			local input = read(inputsize)
