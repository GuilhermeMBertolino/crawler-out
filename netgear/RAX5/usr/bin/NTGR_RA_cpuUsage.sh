...
core2Usage=$(mpstat -P 1| tail -n 1 | awk -F ' ' '{printf 100-$11}');
core3Usage=$(mpstat -P 2| tail -n 1 | awk -F ' ' '{printf 100-$11}');
core4Usage=$(mpstat -P 3| tail -n 1 | awk -F ' ' '{printf 100-$11}');
...
if [ "$exist" -ge 1 ]; then
    sed -i '/^'"$targetRow"':/c '"$targetRow"': '"$core1Usage"' '"$core2Usage"' '"$core3Usage"' '"$core4Usage"'' /tmp/ra/cpuUsage.dat;
else
    echo  "$targetRow: $core1Usage $core2Usage $core3Usage $core4Usage" >> /tmp/ra/cpuUsage.dat;
fi
