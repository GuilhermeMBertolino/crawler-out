...
if [ -e /etc/rabin/raePolicy.json ]; then
    highCpuThres=$(cat /etc/rabin/raePolicy.json | grep "cpuThresholdHigh" | cut -d'"' -f4 | xargs echo -n);
else
    highCpuThres=90;
fi
...
core4AveUtilization=$(cat /tmp/ra/cpuUsage.dat | awk -F ' ' '{core4Total += $5; count++;} END{print core4Total/count}' | awk '{printf("%.2f", $1)}');
cpuAllAveUtilization=$(cat /tmp/ra/cpuUsage.dat | awk -F ' ' '{cpuTotal += $2 + $3 + $4 + $5; count++;} END{print cpuTotal/count/4}' | awk '{printf("%.2f", $1)}');
...
#echo "\$core4AveUtilization=$core4AveUtilization" 2>/dev/console;
...
if [ "$(grep 'cpuAveByCores=' /tmp/ra/NTGR_RA_cpuAveAll.txt -c)" -ge 1 ]; then
    sed -i 's/cpuAveByCores=*.*/cpuAveByCores='"$core1AveUtilization"','"$core2AveUtilization"','"$core3AveUtilization"','"$core4AveUtilization"'/g' /tmp/ra/NTGR_RA_cpuAveAll.txt;
else
    echo "cpuAveByCores=$core1AveUtilization,$core2AveUtilization,$core3AveUtilization,$core4AveUtilization" >> /tmp/ra/NTGR_RA_cpuAveAll.txt;
fi
