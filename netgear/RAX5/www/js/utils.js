...
function fillOutTableData(tableId, json, tableTemplate) {
  ...
  for (index in tableData) {
    ...
    for (key in data) {
      ...
      var newStr = tableStr.replace(regex, data[key]);
      tableStr = newStr;
    }
...
function spaceToNbsps(str) {
  return str.replace(/\s/g, "&nbsp;");
}

function fillOutData(formId, json) {
  var data;console.log(json);console.log(typeof json);
  if(json)
  {
      try{
          if(typeof json == "string")
    ...
    if ( type == undefined || type == "input") {
      ...
      if (data[key]['value'] == $('#' + formId + ' [name=' + key + ']').val()) {
        ...
        var trRow = '<tr class="basic-text">\r\n' + mlangTd +
                    '<td class="basic-text-content" >' + spaceToNbsps(data[key]['value']) + '</td>\r\n\
                    </tr>\r\n';
