...
proto_l2tp_setup() {
	...
	if [ "${demand:-0}" -gt 0 ]; then
		#For the idle timeout and dial on demand, correct the filter criteria for PPP connection.
		#The "+ipv6 ipv6 ,::123" is for pppd bug, it needs LL ip in demand mode.
		#demand="precompiled-active-filter /etc/ppp/filter demand idle $demand"
		demand="replacedefaultroute ipcp-accept-remote ipcp-accept-local demand idle $demand +ipv6 ipv6 ,::123"
		ip_default_ttl="$(sysctl -n net.ipv4.ip_default_ttl)"
		activeFilter="outbound and ip[8] != $ip_default_ttl"
	...
	#For the idle timeout and dial on demand, add the "active-filter" option. If "activeFilter" variable be defined, add the "active-filter"(and the filter rule) option to bring to pppd command line.
	#Also comment the "nodefaultroute" and add "defaultroute" for connection trigger up.
	mkdir -p /tmp/l2tp
	cat <<EOF >"$optfile"
usepeerdns
#nodefaultroute
