...
ppp_generic_setup() {
	...
	local usev4=$(uci get network.wan_v6.use_v4)
	...
	# for pppv6 use the same pppv4 information
	if [ "$conntype" = "pppoe" ] && [ "$ipv6" = 1 ]; then
		ipv6=1
		autoipv6=1
	else
		ipv6=""
	...
	# need to disable v4 if use 2 ppp session(v4 only for wan, v6 only for wan2)
	local noIpv4=""
	if [ "$usev4" = "false" ] && [ "$config" = "wan2" ]; then
		noIpv4=1
	fi
