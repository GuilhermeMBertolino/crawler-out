...
local ntgr_models = "RAX5" --if more models, extend this define to "RAX5 model_A model_B model_C"
...
function add_vif_into_lan(vif)
    ...
    local productModel = mtkwifi.__trim( mtkwifi.read_pipe("uci -P /var/state get netgear.board.model"))

    if string.match(ntgr_models, esc(productModel)) then
            --PegaBU6, Yocheng Lian, 2022.08.03, Remove the "not" in condition judgement.
            --Because we separate the "network" and "wifi" reload behavior(ucitrack + /etc/init.d/),
            --We don't need here to fill-out "remaining" wireless interfaces in br-lan bridge group.
            --And the "network" or "wifi" may be reload/restart independently, so the thinking logic becomes straight forward rather than complementary.
            if string.match(brvifs, esc(vif)) then
               ...
               nixio.syslog("debug", vif.." is NOT the interface that should be in br-lan bridge.")
            end
    else --MTK original implementation. --The "network" and "wifi" service were combined in ucitrack.
    ...
    if l1parser and devs then
        ...
        do
            ...
            then
                ...
                if string.match(vif, esc(dev.apcli_ifname).."[0-9]+") and cfgs.ApCliSsid ~= "" then
...
function mt7615e_reload(devname)
    ...
    if not devname then
        ...
        if normal_reload then
            ...
            if string.sub(dev.INDEX, -1) ~= "D" then
            ...
            -- If the reloaded device belongs to dbdc, then another device on dbdc also need to be reloaded
                ...
                for devname, dev in pairs(devs.devname_ridx) do
                    if string.sub(dev.INDEX, -1) == "D" and dev.init_compatible == compatname then
                        mt7615e_down(devname)
                    end
                end
                for devname, dev in mtkwifi.__spairs(devs.devname_ridx) do
