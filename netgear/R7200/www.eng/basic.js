...
function basic_menu_class_default()
{
	...
	var menu_btns = $("div#menu div[class^='basic_button']",parent.document);
	var max_len = 20;

	if(top.gui_region == "French" || top.gui_region == "Italian" || top.gui_region == "Dutch" || top.gui_region == "Greek" || top.gui_region == "Polish")////special handling for FR and IT and DU GR...
		max_len = 21;
	...
	if(top.enabled_wds == 1 || (top.enable_ap_flag ==0 && top.enabled_modem == 1))
	{
	...
	if((top.enable_ap_flag ==0 && top.enabled_modem == 1) || top.enabled_wds == 1)
	{
		var par_div = top.document.getElementById("parental");
		par_div.className = par_div.className + "_grey";
	}
	
	if(top.enable_ap_flag ==0 && top.enabled_modem == 1)
	{
		var wire_div = top.document.getElementById("wireless");
		wire_div.className = wire_div.className + "_grey";
	}
	if(top.funjsq_flag == "1" && (top.enable_ap_flag == 1 || top.enabled_modem == 1))
	{
		var funjsq_div = top.document.getElementById("funjsq");
		funjsq_div.className = funjsq_div.className + "_grey";
	}
}
...
function click_action(id)
{
		...
		else if( id == "no_internet" && (top.enable_ap_flag == 1 || top.enabled_modem != 1))
		{
			basic_menu_color_change('internet');
			if(portstatus == 0)
