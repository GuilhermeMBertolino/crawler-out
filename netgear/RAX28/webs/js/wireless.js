...
function generate_2GOpMode(ax_en, ax_en_256qam)
{
  ...
  var ax_speed_256qam = [54, 230, 460];
  ...
  $.each($("select[name=2GMode] option"), function (index) {
    if (ax_en == "false") {
      ...
      if (ax_en_256qam == "1")
      {
        if (ax_speed_256qam[index] == 0) {
          this.hidden = true;
        }
        this.text = mlang_upto + ax_speed_256qam[index] + " " + mlang_mbps;
      }
      else
      {
        if (ax_speed[index] == 0) {
...
function reGenerate_OpModeSelectMenu()
{
    ...
    var ax_en_256qam = $("[name=enableAX256QAM]").val();
    var country = $("select[name=country]").val();
...
function checkSecurityType(radioOn_2g, radioOn_5g, isGuest)
{
    var security_2g = $("input[name='2GSecurity']:checked").val();
    var security_5g = $("input[name='5GSecurity']:checked").val();
    //var security_5g1 = $("input[name='5G1Security']:checked").val();
