...
#define CJSON_MAX_MAC_UUID 256
//a stringified JSON object containing the device identifier data of Multiple MAC designated with same UUID
//(Code:CJSON_MULTI_MAC_ONE_UUID)
struct device_list_multimac_oneuuid {
    struct device_mac_info dev_mac[CJSON_MAX_MAC_UUID];
    char device_did[CJSON_BUFF_64 + 1];
    char displayname[CJSON_BUFF_32 + 1];
    char devicetype[CJSON_BUFF_32 + 1];
    char device_manufacturer[CJSON_BUFF_32 + 1];
    char deviceos[CJSON_BUFF_32 + 1];
    char deviceosversion[CJSON_BUFF_32 + 1];
    int traffic_blocked;
    int macs_mapped_count;
}__attribute__((packed));
...
enum cjson_structure_code {
    ...
    CJSON_MULTI_MAC_ONE_UUID,                // 16-struct device_list_multimac_oneuuid (array)
    CJSON_LAST_TAG,
};
