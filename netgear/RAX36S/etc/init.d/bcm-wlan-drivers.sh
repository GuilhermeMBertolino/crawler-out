...
if [ "x$(/bin/config get factory_mode)" = "x1" ]; then
	if [ "x$(/bin/config get cal)" = "x0" ]; then
		is_mfg=2
	fi
fi
...
if [ ! -z "$2" ]; then
    ...
    if [ ! -z $CPEROUTER ]; then
        if [ -f /lib/modules/$KERNELVER/kernel/net/wireless/cfg80211.ko ]; then
            if [ "x$(cat /module_name)" == "xRAX10" -o "x$(cat /module_name)" == "xRAX10v2" -o "x$(cat /module_name)" == "xR6700AXv2" -o "x$(cat /module_name)" == "xRAX36S" ]; then
                all_wlan_modules="firmware_class cfg80211 hnd emf igs wl"
            else
