...
then
	...
	# DEV_CREAT_PATH is used for mtlk_cli & drvhlpr.
	...
	# DEV_CREAT_PATH is used for dwpal_cli, mtlk_cli & drvhlpr.
	# In PUMA /dev/ is RO so use default path (/tmp) for creating the dev
...
export VENDOR_PATH=/opt/${vendor_name}
...
#syslog trace priority
export SYS_LOG_EMERG=0
export SYS_LOG_ALERT=1
export SYS_LOG_CRIT=2
export SYS_LOG_ERR=3
export SYS_LOG_WARNING=4
export SYS_LOG_NOTICE=5
export SYS_LOG_INFO=6
export SYS_LOG_DEBUG=7

#do the actual trace output to Syslog or Console or both
sendTrace2Output()
{
	# Define local parameters
	local msg log_output priority

	msg=$1
	log_output=$2
	priority=$3

        if [ "$log_output" == "Syslog" ] || [ "$log_output" == "Both" ]
        then
                logger -t FAPIWAVESCRIPTS -p $priority "$msg"
        fi

	if [ "$log_output" == "Console" ] || [ "$log_output" == "Both" ]
	then
		echo "FAPIWAVESCRIPTS $log_type $msg" > /dev/console
	fi
}
...
print2log()
{
	...
	then
		log_output="Syslog"
		sendTrace2Output "$log_type WaveScriptsDebugOutput parameter is missing in DB, printing to $log_output" "$log_output" "$SYS_LOG_ERR"
	fi

	if [ -z "$log_level" ]
	then
		log_level=0
		sendTrace2Output "$log_type WaveScriptsDebugLevel parameter is missing in DB, using value of $log_level" "$log_output" "$SYS_LOG_ERR"
	fi

	case $log_type in
	ERROR)
		[ $log_level -ge 0 ] && sendTrace2Output "$msg" "$log_output" "$SYS_LOG_ERR"
	;;
	WARNING)
		[ $log_level -ge 1 ] && sendTrace2Output "$msg" "$log_output" "$SYS_LOG_WARNING"
	;;
	INFO)
		[ $log_level -ge 2 ] && sendTrace2Output "$msg" "$log_output" "$SYS_LOG_INFO"
	;;
	DEBUG)
		[ $log_level -ge 3 ] && sendTrace2Output "$msg" "$log_output" "$SYS_LOG_DEBUG"
	;;
	*)
		sendTrace2Output "$log_type $msg" "$log_output" "$SYS_LOG_ERR"
	;;
	esac
...
save_db_params_local()
{
	...
	echo "save_db_params_local: dump /opt/${vendor_name}/wave/confs/fapi_wlan_wave_profile_conf" > /dev/console
	cat /opt/${vendor_name}/wave/confs/fapi_wlan_wave_profile_conf > /dev/console
}
...
db2fapi_cw_exponent_convert()
{
	# Define local parameters
	local param interface_index cw_value

	param=$1
	interface_index=$2

	cw_value=`db2fapi_regular_convert $param $interface_index`
	cw_value=$((2**cw_value-1))
	echo "$cw_value"
}
...
set_drv_param()
{
	...
	then
		...
		then
		echo "iw dev $interface_name iwlwav $param_name $value" >> $conf_file_path
		check_and_write_to_otf_file $otf "iw dev $interface_name iwlwav $param_name $value" $interface_name
	else # iwpriv
...
set_conf_param()
{
	...
	# Using "echo" to clear leading spaces. if $8=1, do not clear leading spaces for SSID and passphrase
	if [ "$8" = "1" ]
	then
		value=$7

	else
#netgear fix rax40-570 & rax40-588. start
	# value=`echo $7`
	if [ "$param_name" = "wpa_passphrase" ] || [ "$param_name" = "ssid" ]
	then
		value=`echo "$7"`
	else
	value=`echo $7`
	fi
#netgear fix rax40-570 & rax40-588. end
	fi	
...
# 1 = certifiction on , 0 = certification off 
is_certifiction()
{
	local ret_val 

	[ -e ${WAVE_DIR}/SIGMA_CERTIFICATION_ON ] && ret_val="1" || ret_val="0"
...
update_mac_address()
{
	...
	then
		...
		# MBSSID UCC certification W/A(WLANRTSYS-8966)
		if [ "$(is_certifiction)" = "1" ]; then
			vap_index=$((vap_index+1))
		else
        #NTGR,RAX40,mac utilization. 
		vap_index=$((vap_index+1))
		fi
	else
		vap_index=0
    ...
    #NTGR,RAX40,mac utilization. 
	[ "$radio_name" = "wlan0" ] && phy_offset=8
	[ "$radio_name" = "wlan2" ] && phy_offset=10
	[ "$radio_name" = "wlan4" ] && phy_offset=12
	...
	then
        ##PegaCVP#,merge Intel patch to fix ethaddr mac get failure occasionally to let wifi bring up fail
		# board_mac=`upgrade mac_get 0`
		board_mac=`sed -e 's/.*\(ethaddr=.*\)/\1/g' -e 's/ .*//g' -e 's/.*=\(.*\)/\1/g' /proc/cmdline`
...
check_interface_is_ready()
{
	...
	then
		echo "check_interface_is_ready: $interface_name interface not up." > /dev/console
		echo 1
	else
		echo 0
	fi
...
build_wlan_notification()
{
	...
	case "$fapi_rpc" in
		...
		"true")
			...
			if [ "$next" -gt 12 ]
			then
				echo "FAPI NOTIFY ERROR, notify greater than limit using default name" > /dev/console
...
check_wave_chip_id()
{
	...
	if [ "${CONFIG_BUILD_SUFFIX//gen6emu/""}" != "$CONFIG_BUILD_SUFFIX" ] || \
		[ "${CONFIG_BUILD_SUFFIX//fpga/""}" != "$CONFIG_BUILD_SUFFIX" ]
	then ## of PDXP or FPGA we will have only 1 interface.
...
check_wave_logger_sid()
{
	...
	[ "$chip_id" = "$GEN6_CHIP_ID" ] || [ "$chip_id" = "$GEN6D2_CHIP_ID" ] && logger_sid=255
...
#netgear fix rax40-591 & new setup. start
			cur_wpa_passphrase=`grep "current_wpa_passphrase" $current_values_file`
			echo "$conf_file_prefix" >> /tmp/test.log
      if [ ! -z "$cur_wpa_passphrase" ]
      then
        # Remove current_ssid=" prefix and " suffix
        cur_wpa_passphrase=${cur_wpa_passphrase##current_wpa_passphrase=\"}
        #echo "$conf_file_prefix" >> /tmp/test.log
				#echo "$cur_wpa_passphrase" >> /tmp/test.log
				#echo "$current_values_file" >> /tmp/test.log
        cur_wpa_passphrase=${cur_wpa_passphrase%\"}
        # Convert to hex value and update the current_values_file
        hex_wpa_passphrase=`ascii2hex "$cur_wpa_passphrase"`	
				#echo -e "$hex_wpa_passphrase" >> /tmp/test.log
				#echo "$cur_wpa_passphrase" > /tmp/test_tmp.log
				cur_wpa_passphrase=`echo -e  "$hex_wpa_passphrase" | sed 's/"/\\\\"/g'`
				#cur_wpa_passphrase=`cat /tmp/test_tmp.log | sed 's/"/\\\\"/g'`
				#echo "$cur_wpa_passphrase" >> /tmp/test.log
        sed -i '/current_wpa_passphrase=/d' $current_values_file && echo "current_wpa_passphrase=\"$cur_wpa_passphrase\"" >> $current_values_file
      fi
#netgear fix rax40-591 & new setup. end	
		;;
...
#netgear fix rax40-591 & new setup. start
		new_value=`echo "$new_value"`
#netgear fix rax40-591 & new setup. end
		[ "$parameter" = "sFWRecovery" ] && continue
		if [ "$parameter" = "ssid" ]
		then
...
extract_stats_from_dwpal_cli()
{
	# Define local parameters
	local dwpal_cli_output post_stat_text pre_stat_text

	dwpal_cli_output=$1
	post_stat_text=$2
	pre_stat_text=$3

	stat_value=${dwpal_cli_output%%$post_stat_text*}
...
up_copy_enabled_confs()
{
	...
	then
		...
		if [ "$interface_type" != "$STA" ]
		then
			if [ "$interface" = "$radio_name" ]
			then
				accesspoint_disabled=1
			else
	...
	if [ "$interface_type" = "$AP" ]
	then
		echo "$accesspoint_disabled"
	elif [ "$interface_type" = "$STA" ]
	then
		echo "$endpoint_disabled"
	else
		echo ""
	fi
}
...
{
	# Define local parameters
	local radio_name interface_type endpoint_name accesspoint_disabled endpoint_disabled \
	line init_or_rt status interface interface_no_dot enable_online ret
	...
	do
		...
		# Handle only the needed interface type
		if [ "$interface_type" = "$STA" ]
		then
		...
		then
			ret=`up_copy_enabled_confs "no" $interface $interface_type $endpoint_name`
			if [ "$interface_type" = "$STA" ]
			then
				endpoint_disabled="$ret"
			elif [ "$interface" = "$radio_name" ]
			then
				accesspoint_disabled="$ret"
			fi
			continue
		fi
		...
		then
			...
			then
				ret=`up_copy_enabled_confs "yes" $interface $interface_type $endpoint_name`
				if [ "$interface_type" = "$STA" ]
				then
					endpoint_disabled="$ret"
				elif [ "$interface" = "$radio_name" ]
...
up_add_start_endpoint_to_runner()
{
	...
	then
		...
		then
			echo "echo \"$script_name: Start wpa_supplicant_${endpoint_name} in -${supplicant_log_str}t debug mode to syslog (/var/log/wpa_supplicant_${endpoint_name})\" > /dev/console" >> $start_supplicant_conf
			...
			echo "echo \"$script_name: Start wpa_supplicant_${endpoint_name} in -${supplicant_log_str}t debug mode to ${path}/wpa_supplicant_${endpoint_name}.log\" > /dev/console" >> $start_supplicant_conf
	...
	then
		echo "echo \"$script_name: ${endpoint_name}; Start fapi_wlan_daemon (supplicant) listener\" > /dev/console" >> $start_supplicant_conf
...
up_add_start_ap_to_runner()
{
	...
	then
		...
		then
			...
			then
				echo "echo \"$script_name: Start hostapd_${radio_name} in ${hostapd_log_str} debug mode to syslog (/var/log/hostapd_${radio_name})\" > /dev/console" >> $start_hostapd_conf
				...
				echo "echo \"$script_name: Start hostapd_${radio_name} in -${hostapd_log_str}t debug mode to ${path}/hostapd_${radio_name}.log\" > /dev/console" >> $start_hostapd_conf
		...
		if [ "$fapi_daemon_enabled" = "1" ]
		then
			echo "echo \"$script_name: ${radio_name}; Start fapi_wlan_daemon (hostapd) listener\" > /dev/console" >> $start_hostapd_conf
...
up_add_wait_for_hostapd_to_runner()
{
	...
	echo "			echo \"$script_name: ERROR hostapd_${radio_name}\" > /dev/console" >> $wait_for_hostapd_conf
...
{
	# Define local parameters
	local radio_name accesspoint_disabled restart_radio externally_managed \
	...
	then
		...
		# If the radio AP is disabled, remove ppacmd from post-up
		sed -i '/ppacmd/d' ${CONF_DIR}/${DRIVER_POST_UP_CONF_PREFIX}_${radio_name}.conf
	fi
}
...
init_check_interfaces_order()
{
	...
	name_radio_51 status_radio_51 name_sta_51 name_radio_52 status_radio_52 name_sta_52 phy_index supported_frequency_bands ret phy phy_type
	...
	iw dev > ${TEMP_DIR}/iw_dev_output
	chmod 777 ${TEMP_DIR}/iw_dev_output
	...
	do
		...
		then
			...
			if [ "$phy_type" = "AP" ]
			then
				echo "${phy}" >> ${TEMP_DIR}/iw_dev_phy_list
			else
				phy=""
				phy_type=""
			fi
		fi
	done < ${TEMP_DIR}/iw_dev_output
	chmod 777 ${TEMP_DIR}/iw_dev_phy_list
	phy_list=`cat ${TEMP_DIR}/iw_dev_phy_list | sort -n`

	rm -f ${TEMP_DIR}/iw_dev_phy_list ${TEMP_DIR}/iw_dev_output
...
is_cdb_dual()
{
	local cdb_dual_enable

	cdb_dual_enable=`iw dev wlan0 iwlwav gCdbCfg`
	cdb_dual_enable="${cdb_dual_enable##*:}"
	echo "$cdb_dual_enable"
}
...
{
	local ret_val interfaces_init indx pci_interfaces hstpd_cli_cnt

	interfaces_init=$1

	ret_val="0"

	hstpd_cli_cnt=$(ps | grep -c hostapd_cli)
	...
	## WA:for PDXP (5G i/f only) in case  hostapd_cli is not up
	if [ "${CONFIG_BUILD_SUFFIX//gen6emu/""}" != "$CONFIG_BUILD_SUFFIX" ]; then
		if [ $hstpd_cli_cnt -eq 1 ] && [ "$TOTAL_WLAN_COUNT" = "1" ]; then
	...
	if [ $(ps | grep -c hostapd_cli) -gt 1 ]; then
		...
		# remove all spaces and special chars that will prevent source
		sed -r 's/ //g' $interfaces_init  > ${interfaces_init}_no_spaces
		sed -i 's/`//g' ${interfaces_init}_no_spaces
		sed -i 's/\"//g' ${interfaces_init}_no_spaces
		# make the ssid a string to handle special chars
		sed -i 's/$/"/' ${interfaces_init}_no_spaces
		sed -i 's/D=/D="/g' ${interfaces_init}_no_spaces
...
## if chipset HW ID is gen6b(0x910) return true
## input param: interface name (wlan0/wlan2/wlan4).
is_gen6b()
{
	local ret_val interface_name line index temp_interface_id_list \
	wlan0_id wlan2_id wlan4_id tmp_param tmp_val
	...
	[ ! -s ${EEPROM_INFO} ] && echo "is_gen6b:interfaces are not up cannot read chipset id" > /dev/console && return

	if [ ! -e ${CONF_DIR}/WAVE_GEN6B_CHIPSET_${interface_name} ]; then

		cat ${EEPROM_INFO} | grep "HW ID" > /tmp/temp_interface_id_list

		let index=0
		while read -r line || [ -n "$line" ]
		do
			line=${line/"HW ID         : 0x1a30,"/}
			line=${line/",0x0,0x0"/}
			tmp_param="wlan$index"
			echo "${tmp_param}_id"="$line" >> /tmp/interfaces_chipset_id
			let index=index+2
		done < /tmp/temp_interface_id_list
		source /tmp/interfaces_chipset_id
		rm /tmp/interfaces_chipset_id
		tmp_param="${interface_name}_id"
		tmp_val=$(eval echo \"\${$tmp_param}\")
		[ "$tmp_val" = "0x910" ] && ret_val=1 || ret_val=0

		[ "$ret_val" = "1" ] && touch ${CONF_DIR}/WAVE_GEN6B_CHIPSET_${interface_name}

	else ## flag is on, retrun 1
			ret_val=1
	fi
	echo "$ret_val"
}

## if chipset HW ID is gen6d2(0x980) return true
## input param: interface name (wlan0/wlan2/wlan4).
is_gen6d2()
{
	local ret_val interface_name line index temp_interface_id_list \
	...
	[ ! -s ${EEPROM_INFO} ] && echo "is_gen6d2:interfaces are not up cannot read chipset id" > /dev/console && return

	if [ ! -e ${CONF_DIR}/WAVE_GEN6D2_CHIPSET_${interface_name} ]; then
		cat ${EEPROM_INFO} | grep "HW ID" > /tmp/temp_interface_id_list
		...
		tmp_param="${interface_name}_id"
		tmp_val=$(eval echo \"\${$tmp_param}\")
		[ "$tmp_val" = "0x980" ] && ret_val=1 || ret_val=0

		[ "$ret_val" = "1" ] && touch ${CONF_DIR}/WAVE_GEN6D2_CHIPSET_${interface_name}
	else
		ret_val=1
	fi
...
then
	...
	cpu_type=`cat /proc/cpuinfo | grep "^system type" | awk '{ print $4 }'`
	cpu_type_short=${cpu_type:0:6}
	if [ "$cpu_type_short" = "GRX350" ]
	then
	     cpu_type_short="GRX500"
	fi
	export CONFIG_IFX_CONFIG_CPU=$cpu_type_short
