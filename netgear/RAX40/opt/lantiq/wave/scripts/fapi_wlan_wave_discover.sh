...
LTQ_CODE_WAVE600="8086:09"
LTQ_CODE_WAVE600D2="8086:0d5a"
...
PCI_LTQ_COUNT_WAVE600=`grep -c $LTQ_CODE_WAVE600 $PCI_FILENAME`
PCI_LTQ_COUNT_WAVE600D2=`grep -c $LTQ_CODE_WAVE600D2 $PCI_FILENAME`
PCI_LTQ_COUNT=$((PCI_LTQ_COUNT_WAVE300+PCI_LTQ_COUNT_WAVE500+PCI_LTQ_COUNT_WAVE600+PCI_LTQ_COUNT_WAVE600D2))
...
then
	...
	# check that it is not !GRX550, in this case we will check if AHB is supported and enabled it.
	...
	then
		...
		then
			...
			WAVE_DRIVER_PATH=`find /opt/ -name mtlk.ko`
		...
		# if GEX550 then AHB remain disabled 
		echo "fapi_wlan_wave_discover.sh: ##########################################" > /dev/console
...
## TBD need to disable AHB only for gen6 driver all others no change
##For WAVE600 no AHB
echo "$script_name:TBD disable AHB only for gen6 driver- WAVE600:AHB_WLAN_COUNT=0" > /dev/console
AHB_WLAN_COUNT=0

TOTAL_WLAN_COUNT=$((PCI_WLAN_COUNT+AHB_WLAN_COUNT))
echo "fapi_wlan_wave_discover.sh ( WAVE600 driver has no AHB support): TOTAL WLAN COUNT: $TOTAL_WLAN_COUNT "
...
echo "TOTAL_WLAN_COUNT=$TOTAL_WLAN_COUNT" >> /tmp/wlan_wave/fapi_wlan_wave_discover.txt
