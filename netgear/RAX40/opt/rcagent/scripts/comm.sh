...
URL=`/opt/lantiq/usr/sbin/readycloud_nvram get readycloud_fetch_url`

# auth data
NAS_NAME=`/opt/lantiq/usr/sbin/readycloud_nvram get readycloud_hostname`
NAS_PASS=`/opt/lantiq/usr/sbin/readycloud_nvram get readycloud_password`
...
#    [ -z "${1}" ] && return $ERROR
    if [ -z "${2}" ];
    then
	FULL_EXEC="\`echo '$1' | ${COMM_EXEC} -X POST --data-binary @- 2>/dev/null\`"
# 	FULL_EXEC="\`cat "${1}" | ${COMM_EXEC} -X POST --data-binary @- \`"
    else
# 	FULL_EXEC="\`cat "${1}" | ${COMM_EXEC} -X POST --data-binary @- > "${2}"\`"
	FULL_EXEC="\`echo '$1' | ${COMM_EXEC} -X POST --data-binary @- 2>/dev/null -o '${2}'\`"
    fi
#     echo "${FULL_EXEC}"
...
do_register()
{
	...
	USER_PASS=$2
	temp_dir=$3
	XAGENT_ID=$(d2 xagentcfg[0].x_agent_id | awk '{print $2}' | tr -d '[[:space:]]')
...
#	echo "Registering with following string (credentials are ${NAS_NAME} ${NAS_PASS}): ${DATA}" > ${LOG_FILE}
	comm_post "${DATA}" && {
	   if [ "xSUCCESS" = "x${COMM_RESULT}" ]; then
		/opt/lantiq/usr/sbin/readycloud_nvram set readycloud_registration_owner=${USER_NAME}
        /opt/lantiq/usr/sbin/readycloud_nvram set leafp2p_run="1"
        /opt/lantiq/usr/sbin/readycloud_nvram set x_force_connection="1"
...
do_unregister()
{
	...
	comm_post "${DATA}" && {
	if [ "xSUCCESS" = "x$COMM_RESULT" ]; then
            /opt/lantiq/usr/sbin/readycloud_nvram unset readycloud_registration_owner
            /opt/lantiq/usr/sbin/readycloud_nvram set leafp2p_run="0"
            /opt/lantiq/usr/sbin/readycloud_nvram unset x_force_connection
	    d2 -c xagentcfg[0].x_force_connection 0
            /opt/lantiq/usr/sbin/readycloud_nvram unset leafp2p_username
            /opt/lantiq/usr/sbin/readycloud_nvram unset leafp2p_password
            /opt/lantiq/usr/sbin/readycloud_nvram commit
