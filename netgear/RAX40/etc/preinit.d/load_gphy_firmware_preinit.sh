...
gphy_fw_load() {
	...
	if [ "$platform" = "GRX350" -o "$platform" = "GRX550" -o "$platform" = "grx350" -o "$platform" = "grx550" ]; then
		...
		#make sure coefficients written by adding delay 
		sleep 1
		#gphy_fw_load the SSC enable with no init and disable, instead
		#of relying the reset state value
