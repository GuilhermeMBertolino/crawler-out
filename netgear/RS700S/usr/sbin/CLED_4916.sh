...
# CONFIGADDRESS=("0xFF803020" "0xFF803024" "0xFF803028" "0xFF80302C")
CONFIGADDRESS="\
...
function SetConfig() {
	...
	if [ "x$CONFIGVALUE" == "x" ]; then
		...
		/opt/scripts/sw ${ADDR} ${CONFIGVALUE}
	fi
}

function HardwareLed() {
	if [ "x$ACTIVE_VALUE" == "x" ]; then
		echo "dw 0xFF803004"
		/opt/scripts/dw 0xFF803004
	else
		echo "sw 0xFF803004 ${ACTIVE_VALUE}"
		/opt/scripts/sw 0xFF803004 $ACTIVE_VALUE
	fi
}

function SoftLEDSetLed() {
	if [ "x$ACTIVE_VALUE" == "x" ]; then
		echo "dw 0xFF803010"
		/opt/scripts/dw 0xFF803010
	else
		echo "sw 0xFF803010 ${ACTIVE_VALUE}"
		/opt/scripts/sw 0xFF803010 $ACTIVE_VALUE
	fi
}

function InputPolarityLed() {
	if [ "x$ACTIVE_VALUE" == "x" ]; then
		echo "dw 0xFF803014"
		/opt/scripts/dw 0xFF803014
	else
		echo "sw 0xFF803014 ${ACTIVE_VALUE}"
		/opt/scripts/sw 0xFF803014 $ACTIVE_VALUE
	fi
}
...
function ActiveLed() {
	if [ "x$ACTIVE_VALUE" == "x" ]; then
		echo "dw 0xFF80301c"
		/opt/scripts/dw 0xFF80301c
	else
		echo "sw 0xFF80301c ${ACTIVE_VALUE}"
		/opt/scripts/sw 0xFF80301c $ACTIVE_VALUE
	fi
}
...
function ErrorPrint() {
	echo Please try:
	echo "${0} [Action] [Value]"
	echo "${0} [Cz number(0-31)] [Config Bank(0-3)] [Value]"
