...
CONFIG_FILE=/tmp/arno-iptables-firewall/firewall.conf
# ------------------------------------------------------------------------------
...
bd_restart()
{
    ...
    # To check armor enable
    if [ "x`d2 armorstatus[0].protectionStatus | awk '{print $2}'`" = "xtrue" ]; then
        armorEable=1
    else    
        armorEable=0
    fi
    echo "armorEable = $armorEable" >&2 
    
    if [ -e "/tmp/iptable_run_once" ]; then
      if [ "x$armorEable" = "x1" ]; then       
          echo " " >&2
          echo "!!!! bd stop !!!!!" >&2    
          /opt/bitdefender/bin/bd stop
          # It will kill wbd_master and wbd_slave and this should be done by bd stop
          # ps |grep bd | grep -v grep | grep -v khubd  | awk '{print $1}' | xargs kill -9
          iptables -X GUSTER
          bd_addchain
          echo "!!!! bd start !!!!!" >&2  
          /opt/bitdefender/bin/bd start
          echo "!!!! bd start done !!!!!" >&2 
      fi
    else
      echo " " >&2
      echo "!!!! iptable applied first time !!!!!" >&2
      # to check Chain BD_FILTER
      #/usr/share/armor/add_chain.sh &
      #if [ "x$armorEable" = "x1" ]; then
        bd_addchain
      #fi  
      touch /tmp/iptable_run_once
      echo "!!!! iptable applied first time done !!!!!" >&2
    fi
}    
