...
do
  is_10G=`ethctl $ethx media-type | grep -c 10G`
  is_5G=`ethctl $ethx media-type | grep -c -e \ 5G -e \|5G`
  is_2G=`ethctl $ethx media-type | grep -c 2.5G`
  is_1G=`ethctl $ethx media-type | grep -c 1G`
  if [ $is_10G -gt 0 ]; then
    speed=10GE
