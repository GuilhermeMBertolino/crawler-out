...
//.o
	...
	var rootPath = this.r();
	var nickName = this.aNodes[1].name.split(':')[0];
	var lastPath = echo_info[3].replace(rootPath, nickName + ":").replace(/\//g,"\\");

	try{
		opener.document.getElementById('folder').value = lastPath;
	}catch(e){}
...
dTree.prototype.r = function() {
	var rpStr = eval('node1');
	var rp = rpStr.split('*');
	return rp[3];
}
