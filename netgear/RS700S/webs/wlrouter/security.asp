...
function update_ui_ctrls_for_enterprise_security(wpa_state)
{
	...
	document.forms[0].wl_saepk_enable.disabled = wpa_state;
...
function update_ui_ctrl_for_owe(six_g_state)
{
	...
	var saepk = document.forms[0].wl_saepk_enable[document.forms[0].wl_saepk_enable.selectedIndex].value;
	...
	if (owe == "enabled") {
		...
		document.forms[0].wl_saepk_enable.disabled = 1;
		document.forms[0].wl_akm_wpa3.disabled = 1;
		document.forms[0].wl_akm_dpp.disabled = 1;
		...
		/* Do not disable SAE for 6G */
		document.forms[0].wl_akm_sae.disabled = 0;
		document.forms[0].wl_saepk_enable.disabled = 0;
		...
		if ((wpa2 == "enabled") || (psk2 == "enabled") ||
			(sae == "enabled") || (wpa3 == "enabled") || (saepk == "enabled") ||
			(dpp == "enabled") || (wpa3_suite_b == "enabled")) {
			document.forms[0].wl_akm_owe.disabled = 1;
		} else {
			document.forms[0].wl_akm_owe.disabled = 0;
		}
	}
}
function wl_saepk_change()
{
	var saepk = document.forms[0].wl_saepk_enable[document.forms[0].wl_saepk_enable.selectedIndex].value;
	if (saepk == "enabled") {
...
function wl_akm_change(initial_load)
{
	...
	var is_mfp_required, is_mfp_enabled;
	var i;
	var j;
	var mfp_options;
	var osen = <% nvram_get_bitflag("wl_hsflag", 1); %>
	...
	if (!<% wl_cap("suite-b"); %>) {
		document.getElementById("wl_akm_wpa3_suite_b").style.display = "none";
	}
	...
	if (band == "Current: 6 GHz") {
		...
		document.forms[0].wl_wpa_psk.disabled = 0;
		if ((sae != "enabled") || (saepk == "enabled")) {
			document.forms[0].wl_wpa_psk.disabled = 1;
		}
		update_ui_ctrl_for_owe(1);
	} else {
		...
		if (wpa2 == "enabled" || wpa3 == "enabled" || wpa3_suite_b == "enabled" || osen == "1") {
		...
		update_ui_ctrl_for_owe(0);
	}

	mfp_options = [{name:"Required", val:"2"}, {name:"Capable", val:"1"}, {name:"Off", val:"0"}];
	is_mfp_required = (sae == "enabled" && psk2 != "enabled") ||
		(wpa3_suite_b == "enabled" && wpa2 != "enabled" && wpa3 != "enabled") ||
		(owe == "enabled") || (dpp == "enabled");
	is_mfp_enabled = is_mfp_required || wpa2 == "enabled" || wpa3 == "enabled" || psk2 == "enabled" || osen != "1";
	mfp = <% nvram_get("wl_mfp"); %>;
	document.forms[0].wl_mfp.disabled = 0;
	i = mfp_options.length-1;
	j = 0;
	k = 0;
	if ((is_mfp_required == true) &&
		((initial_load == false) || (mfp == '2'))) {
		/* Create single option 'Required' */
		i = 0;
	} else if ((is_mfp_enabled == false) &&
		((initial_load == false) || (mfp == '0'))) {
		/* Create single option 'Off' */
		i = 2;
		k = 2;
	}
	...
	document.forms[0].wl_mfp.length = j;
	if (j == 1) {
		/* select the only option 'Required' or 'Off' */
		document.forms[0].wl_mfp[0].selected = true;
	} else if (initial_load == false) {
		/* select option based on current security settings */
		if (wpa2 == "enabled" || wpa3 == "enabled" || psk2 == "enabled") {
			/* select option 'Capable' */
		...
		/* select option based on current nvram setting */
		if (mfp == "0") {
			document.forms[0].wl_mfp[0].selected = true;
		} else if (mfp == "1") {
			document.forms[0].wl_mfp[1].selected = true;
		} else if (mfp == "2") {
			document.forms[0].wl_mfp[2].selected = true;
		}
	}
	...
	if (<% wl_corerev(); %> >= 3) {
		if (wl_ibss == "1") {
			...
			if ((document.forms[0].wl_akm_wpa.disabled == 0 &&
				...
				algos = ["Suite-B"];
			else if ((document.forms[0].wl_akm_sae.disabled == 0 &&
                                document.forms[0].wl_akm_sae[document.forms[0].wl_akm_sae.selectedIndex].value == "enabled"))
				if(<% wl_cap("gcmp"); %>) {
					if (document.forms[0].wl_akm_psk2.disabled == 0 &&
						document.forms[0].wl_akm_psk2[document.forms[0].wl_akm_psk2.selectedIndex].value == "enabled")
						algos = ["AES", "GCMP256", "AES+GCMP256"];
					else
						algos = ["AES", "GCMP256"];
				}
				else
...
<table border="0" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td>&nbsp;&nbsp;</td>
    <td>
	<select name="wl_akm_wpa" onChange="wl_akm_change(false);">
  ...
  <tr>
    ...
    <td>&nbsp;&nbsp;</td>
    <td>
	<select name="wl_akm_psk" onChange="wl_akm_change(false);">
  ...
  <tr>
    ...
    <td>&nbsp;&nbsp;</td>
    <td>
	<select name="wl_akm_wpa2" onChange="wl_akm_change(false);">
  ...
  <tr>
    ...
    <td>&nbsp;&nbsp;</td>
    <td>
	<select name="wl_akm_psk2" onChange="wl_akm_change(false);">
  ...
  <tr style="display:<% nvram_ternary("hapd_enable", "1", "table-row", "none"); %>;">
    ...
    <td>&nbsp;&nbsp;</td>
    <td>
	<select name="wl_akm_sae" onChange="wl_akm_change(false);">
    ...
    <tr>
    <th width="310"
	onMouseOver="return overlib('Enables/Disables SAE-PK Authenticated Key Management suite.', LEFT);"
	onMouseOut="return nd();">
	WPA3-SAE-PK:&nbsp;&nbsp;
    </th>
    <td>&nbsp;&nbsp;</td>
    <td>
	<select name="wl_saepk_enable" onChange="wl_saepk_change();">
	  <option value="enabled" <% nvram_inlist("wl_saepk_enable", "1", "selected"); %>>Enabled</option>
	  <option value="disabled" <% nvram_inlist("wl_saepk_enable", "0", "selected"); %>>Disabled</option>
	</select>
    </td>
  </tr>
  <tr>
    ...
    <td>&nbsp;&nbsp;</td>
    <td>
	<select name="wl_akm_wpa3" onChange="wl_akm_change(false);">
  ...
  <tr>
    ...
    <td>&nbsp;&nbsp;</td>
    <td>
	<select name="wl_akm_owe" onChange="wl_akm_change(false);">
  ...
  <tr>
    ...
    <td>&nbsp;&nbsp;</td>
    <td>
	<select name="wl_akm_dpp" onChange="wl_akm_change(false);">
  ...
  <tr id=wl_akm_wpa3_suite_b>
    ...
    <td>&nbsp;&nbsp;</td>
    <td>
	<select name="wl_akm_wpa3_suite_b" onChange="wl_akm_change(false);">
...
<table border="0" cellpadding="0" cellspacing="0">
  <tr>
	...
	<select name="wl_crypto">
	  ...
	  <option value="gcmp256" <% nvram_match("wl_crypto", "gcmp256", "selected"); %>>GCMP256</option>
	  <option value="aes+gcmp256" <% nvram_match("wl_crypto", "aes+gcmp256", "selected"); %>>AES+GCMP256</option>
