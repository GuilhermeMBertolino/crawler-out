...
if  [[ $SYSTEM_UBOOT -gt 0 ]]; then 
    _root_fs_dev=`/etc/get_rootfs_dev.sh`
...
case "$1" in
	...
	if  [[ $SYSTEM_UBOOT -gt 0 ]]; then 
	   image_default_nvram_file="/etc/wlan/nvram/"`cat /proc/environment/boardid`".nvm"
	   if [ -f /proc/environment/wlnvram ]; then
 		image_default_nvram_file="/etc/wlan/nvram/"`cat /proc/environment/wlnvram`
	   fi
	else
