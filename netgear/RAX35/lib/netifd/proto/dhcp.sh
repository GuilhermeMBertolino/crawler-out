...
proto_dhcp_setup() {
	...
	#20181129, Support dhcp option 61 for udhcpc. Send default clientid ignore -C.
	#[ -n "$clientid" ] && clientid="-x 0x3d:${clientid//:/}" || clientid="-C"
	#[ -n "$clientid" ] && clientid="-x 0x3d:${clientid//:/}" || clientid=""
	#End of 20181129.
	[ -n "$clientid" ] && clientid="${clientid//:/}"
	[ -n "$vendorid" ] && vendorid="${vendorid//:/}"
	[ -n "$iface6rd" ] && proto_export "IFACE6RD=$iface6rd"
	[ "${iface6rd:-0}" != 0 -a -f /lib/netifd/proto/6rd.sh ] && append dhcpopts "-O 212"
	...
	echo "$clientid"
	echo "$clientid" > /dev/console
	echo "$vendorid"
	echo "$vendorid" > /dev/console

    #20180621, Add the -I option for the Internet Test feature.
	proto_export "INTERFACE=$config"
	proto_run_command "$config" udhcpc \
		-a \
		${internetTest:+-I $internetTest} \
