...
function PHYINF_getdevdatamac($name)
{
	...
	$mac = PHYINF_getrandommac();
	if ($mac != "")
	{
		set("/runtime/devdata/".$name, $mac);
		TRACE_error("[PHYINF_getdevdatamac] generate random mac. name=".$name.", mac=".$mac);
		return $mac;
	}
	...
	else if	($name=="wlanmac")	$mac = "00:de:fa:1a:01:00";
	else if	($name=="wlan5mac")	$mac = "00:de:fa:1a:01:02";
	else if	($name=="wlanmac2")	$mac = "00:de:fa:1a:01:02";
	else if	($name=="wlanmac3")	$mac = "00:de:fa:1a:01:04";
	if ($mac=="") $mac = "00:de:fa:00:01:00"; /* Always return somethying. */

	set("/runtime/devdata/".$name, $mac);
	TRACE_error("[PHYINF_getdevdatamac] generate random mac FAILED. using fixed mac address. name=".$name.", mac=".$mac);
