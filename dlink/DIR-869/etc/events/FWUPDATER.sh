...
# do not change LED status when firmware upgrade
# event STATUS.CRITICAL 
sleep 1

echo "kill servd and xmldb load.sh updatewifistats"
killall -9 servd
killall -9 xmldb
...
LOADSH=`ps | grep "noise_watchdog.sh" | grep -v "grep"`
if [ -n "$LOADSH" ]; then
LOADSHPID=`echo $LOADSH | cut -d" " -f1`
kill $LOADSHPID
fi
LOADSH=`ps | grep "wifi_watchdog.sh" | grep -v "grep"`
if [ -n "$LOADSH" ]; then
LOADSHPID=`echo $LOADSH | cut -d" " -f1`
kill $LOADSHPID
fi
# kill hostapd_loop.sh
ps | grep hostapd_loop.sh | grep -v "grep" | awk '{print $1}' | xargs kill -SIGTERM
