...
global=$global"x_"$fwcheckparameter
else
...
if [ -f $fwinfo ]; then
	...
	LatestFWVersionDate=`grep Date /tmp/fwinfo.xml | sed -r 's/.*<Date>//' | sed -r 's/<\/Date>.*//'`
	if [ "$new_major" != "" ] || [ "$new_minor" != "" ]; then
		...
		xmldbc -s /runtime/firmware/LatestFWVersionDate $LatestFWVersionDate
