...
function do_wps($uid, $method)
{
	...
	echo "echo $! > ".$pidfile."\n";
	*/
	if ($method == "pbc") $cmd = "wps_pbc";
	else return;	
	
	echo "hostapd_cli -i ".$dev." ".$cmd."\n";
}

function do_wps_sta($uid)
{
	$p = XNODE_getpathbytarget("", "phyinf", "uid", $uid, 0);
	...
	echo "wpa_cli wps_pbc\n";
}

function set_wps($uid, $mode)
{
	...
	{
		anchor("/runtime/wps_sta/setting");
	}
	else
	{
		anchor("/runtime/wps/setting");
	...
	if($uid == "BAND24G-1.1" || $uid == "BAND5G-1.1")	
		set($p."/ssid",		$ssid);
	else if($uid == "WIFISTA-1.1" || $uid == "WIFISTA-2.1")
		set($p."/ssid",         $ssid);
	set($p."/authtype",	$atype);
	set($p."/encrtype",	$etype);
/*marco*/
/*	$2_or_5g=query("/runtime/wps_sta/enable");
	if($2_or_5g==2)
	{//2.4G
		$sta_p = XNODE_getpathbytarget("", "phyinf", "uid", $uid, 0);
		set($sta_p."/media/freq","2.4");
		set($sta_p."/media/wlmode","bgn");
		set($sta_p."/media/dot11n/bandwidth","20+40");
	}
	else if($2_or_5g==1)
	{//5g
	$sta_p = XNODE_getpathbytarget("", "phyinf", "uid", $uid, 0);
		set($sta_p."/media/freq","5");
		set($sta_p."/media/wlmode","ac");
		set($sta_p."/media/dot11n/bandwidth","20+40+80");
	}*/
	if ($etype=="WEP")
	{
		foreach ("key")
	...
	if( $uid!="WIFISTA-1.1" || $uid!="WIFISTA-2.1" )
	{
		set($p."/wps/locksecurity","1");
	}
}
...
function stop_wps($uid)
{
	...
	if ($p == "") return;
        anchor($p);
	$dev = query("name");
	//wpa_supplicant stop wps command
	echo "hostapd_cli -i ".$dev." wps_cancel\n";
}
...
function stop_wps_sta($uid)
{
	...
	$dev = query("name");
	//wpa_supplicant stop wps command
	echo "wpa_cli wps_cancel\n";
	echo "killall wpa_supplicant\n";
	echo "wpa_supplicant -b br0 -i ".$dev." -c /var/run/wpa_supplicant.conf &\n";
}
...
else if ($PARAM1=="restartap")
{
	...
	//XNODE_set_var("hostapd_restartap", "1");
	set("/runtime/hostapd_restartap", "1");
	
}
else if($PARAM1=="pbc_sta")
{
	TRACE_debug("STA_PBC:".$PARAM1);
	do_wps_sta($UID3);
	do_wps_sta($UID4);
//	set("/runtime/wps_sta/enable","1");
//	echo 'service PHYINF.WIFI restart\n';
}
else if($PARAM1=="restartsta")
{
	set("/runtime/wps_sta/state", "WPS_SUCCESS");
	event("WPS.SUCCESS");
	set_wps($UID3, "sta");
	set_wps($UID4, "sta");
	event("DBSAVE");
//	set("/runtime/wps_sta/enable","0");
	echo 'xmldbc -t "WPS:1:service PHYINF.WIFI restart"\n';
}
else if($PARAM1=="stop")
{
...
else if ($PARAM1=="WPS_SUCCESS")
{
	...
	set("/runtime/wps/state", "WPS_SUCCESS");
	event("WPS.SUCCESS");
	//kill_wpatalk($UID1);
	//kill_wpatalk($UID2);
/*	$dosave1 = update_locksecurity($UID1);
	$dosave2 = update_locksecurity($UID2);
	if($dosave1 == 1 || $dosave2 == 1)
	{
		event("DBSAVE");
		echo 'xmldbc -t "WPS:1:service PHYINF.WIFI restart"\n';
	}*/
}
else if($PARAM1=="WPS_WLAN_CONNECTED")
{
	echo 'echo "hostapd success process in eap identity" > /dev/console';
	set("/runtime/wps/state","WPS_CONNECTED");
}
else if($PARAM1=="WPS_STA_WLAN_CONNECTED")
{
	echo 'echo "wpa_supplicant success process in eap identity" > /dev/console';
	set("/runtime/wps_sta/state","WPS_CONNECTED");
}
else
{
