...
function infsvcs_setup($name, $cfg, $sts, $ipv6)
{
	...
	/* stunnel service */
	if ($stunnel==1)	startcmd("service STUNNEL restart");
...
function infsvcs_wan($index)
{
	...
	$upnp_active     = query("/upnpav/dms/active");
	...
	if ($lower=="")
	{
		if ($ipv6>0)
		{
			...
			stopcmd("service IP6TINGRESSFILTER restart");		
	...
	if ($upper=="")
	{
		if ($ipv6>0)
		{
			...
			startcmd("service IP6TINGRESSFILTER restart");
	...
	if($index == 1 && isfile("/usr/sbin/tr069c") == 1){
        startcmd("service TR069 start");
        stopcmd("service TR069 stop");
    }

	//Kwilt
	if ($uid == "WAN-1" && query("/kwcfg/enable") == 1 ) {
		startcmd("/etc/scripts/kwilt_api.sh set wan_up"); 
		
		if ($upnp_active!="0") //DLNA launch so KWILT start
		{		
			startcmd("echo KWILT_start");
			startcmd("service KWILT start &"); 
		} 
		
		stopcmd("echo KWILT_stop");
		stopcmd("/etc/scripts/kwilt_api.sh set wan_down");
		stopcmd("service KWILT stop "); 
	}
...
//in webUI the information will be wrong.
    ...
    if($index==1 && isfile("/etc/services/IPSEC.php")==1)
    {
        startcmd("service IPSEC restart\n");
    }
...
function infsvcs_lan($index)
{
	...
	if(query("/tr069/external/enable_multilogin/enable")=="1")
    {
        $multilogin_username = query("/tr069/external/enable_multilogin/username");
        $multilogin_password = query("/tr069/external/enable_multilogin/password");
        startcmd("echo ".$multilogin_username.":".$multilogin_password.",::: > /var/run/tr069_account_root");
    }
