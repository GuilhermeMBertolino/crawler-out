...
//marco, for cone	
	...
	startcmd("iptables -A FORWARD -o ".$dev." -m state --state INVALID -j DROP");

	/* Get NAT */
	$nat = query($infp."/nat");
	if ($nat=="")
	{
		// HuanYao @ 2015/10/21, if WAN mode is ds-lite, still needs ip_forward.
		$ipv4in6mode = get("",$stsp."/inet/ipv4/ipv4in6/mode");
		if ($ipv4in6mode == "dslite")
		{
			startcmd("\n# ds-lite needs to enable ip_forward.");
			startcmd("echo -n \"--proto ICMP\" > /proc/nf_conntrack_flush");
			startcmd("echo -n \"--dport 53\" > /proc/nf_conntrack_flush");
					
			startcmd("echo 1 > /proc/sys/net/ipv4/ip_forward\n");
			stopcmd("echo 0 > /proc/sys/net/ipv4/ip_forward\n");
		}
		return;
	}
	...
	startcmd("echo -n \"--proto ICMP\" > /proc/nf_conntrack_flush");
	startcmd("echo -n \"--dport 53\" > /proc/nf_conntrack_flush");
	stopcmd("echo -n \"--dip ".$ipaddr."\" > /proc/nf_conntrack_flush");
			
	startcmd("echo 1 > /proc/sys/net/ipv4/ip_forward");
	/* prevent private ip on wan when renew ip */
	stopcmd("echo 0 > /proc/sys/net/ipv4/ip_forward");
}
	...
	while ($i>0)
	{
		...
		if ($stsp!="")
		{
			...
			if ($waninf!="")
			{
				...
				$addrtype = query("addrtype");
				if ($addrtype=="ipv4" && query("ipv4/valid")=="1")
					startcmd("echo 0 > /proc/sys/net/ipv4/ip_forward");
				else if	($addrtype=="ppp4" && query("ppp4/valid")=="1")
					startcmd("echo 0 > /proc/sys/net/ipv4/ip_forward");
			}
		}
		$i++;
...
if ($layout == "router")
{
	...
	/* Guest Zone */
	$path_phyinf_wlan_guest24 = XNODE_getpathbytarget("", "phyinf", "uid", $WLAN1_GZ, 0);
	$path_phyinf_wlan_guest5 = XNODE_getpathbytarget("", "phyinf", "uid", $WLAN2_GZ, 0);
	if (query($path_phyinf_wlan_guest24."/active")==1 || query($path_phyinf_wlan_guest5."/active")==1)
	{
		startcmd("# GuestZone enabled");
		startcmd("echo 1 > /proc/sys/net/ipv4/ip_forward");
		stopcmd("echo 0 > /proc/sys/net/ipv4/ip_forward");
	}
