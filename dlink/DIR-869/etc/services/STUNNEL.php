...
if ($stunnel==1 || $wfa_stunnel==1 || $mydlink==1)
{
	...
	fwrite("w",$stunnel_conf,"cert = /tmp/server.crt\n");
	fwrite("a",$stunnel_conf,"key = /tmp/server.key\n");
	...
	if($stunnel==1)
	{
		...
		//traveller: stunnel for Link-local IPv6 and global IPv6 bridge
		if($wirelessmode=="WirelessAp")
		{
			stunnel_setup($stunnel_conf,"BRIDGE-3","443","127.0.0.1:80");
			stunnel_setup($stunnel_conf,"BRIDGE-3","4433","127.0.0.1:8181");		
			stunnel_setup($stunnel_conf,"BRIDGE-4","443","127.0.0.1:80");
			stunnel_setup($stunnel_conf,"BRIDGE-4","4433","127.0.0.1:8181");	
		}

		// HuanYao: stunnel for Link-local IPv6 LAN.
		stunnel_setup($stunnel_conf,"LAN-3","443","127.0.0.1:80");
		stunnel_setup($stunnel_conf,"LAN-3","4433","127.0.0.1:8181");
		// HuanYao: stunnel for global IPv6 LAN.
		stunnel_setup($stunnel_conf,"LAN-4","443","127.0.0.1:80");
		stunnel_setup($stunnel_conf,"LAN-4","4433","127.0.0.1:8181");
	...
	fwrite("a", $START, "/etc/scripts/updatessl.sh\n");
