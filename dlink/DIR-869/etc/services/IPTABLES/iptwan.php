...
function IPTWAN_build_command($name)
{
	...
	if ($path!="")
	{
		...
		$ipsec_entry = query("/vpn/ipsec/enable");
		if ($vpn_entry != "" || $ipsec_entry != "")
		{
		...
		//For TR069
		if(query("/tr069/enable") == 1){
			$cpe_connreq_port = query("/tr069/misc/connectionrequestport");
			if($cpe_connreq_port > 0) fwrite("a",$_GLOBALS["START"], $iptcmd." -p tcp --dport ".$cpe_connreq_port." -j ACCEPT\n");
			$ftp_data_port = query("/tr069/misc/ftpdataport");
			if($ftp_data_port > 0) fwrite("a",$_GLOBALS["START"], $iptcmd." -p tcp --dport ".$ftp_data_port." -j ACCEPT\n");
			$udp_echo_port = query("/tr069/tr143/udpechoconfig/udpport");
			if($udp_echo_port > 0) fwrite("a",$_GLOBALS["START"], $iptcmd." -p udp --dport ".$udp_echo_port." -j ACCEPT\n");
		}

		//Kwilt
		if($name == "WAN-1" && query("/kwcfg/enable") == 1 ){
			$kw_https_port=query("/kwcfg/https_port"); //42443    
			
			fwrite("a",$_GLOBALS["START"], "echo $0\n"); 
			fwrite("a",$_GLOBALS["START"], $iptcmd." -p tcp --dport ".$kw_https_port." -j ACCEPT\n");
		}		
	...
	fwrite("a",$_GLOBALS["START"], "echo -n \"--proto ICMP\" > /proc/nf_conntrack_flush\n");
	fwrite("a",$_GLOBALS["START"], "echo -n \"--dport 53\" > /proc/nf_conntrack_flush\n");
