...
if($layout=="router") { $INF = $LAN1; }
else { $INF = $BR1; }

$wan1_infp = XNODE_getpathbytarget("", "inf", "uid", "WAN-1", 0);
$runtime_wan1_infp = XNODE_getpathbytarget("runtime", "inf", "uid", "WAN-1", 0);
$opendns_type = query($wan1_infp."/open_dns/type");
$lan1_infp = XNODE_getpathbytarget("", "inf", "uid", $INF, 0);
...
$RouterLANIP = query($path_run_inf_lan1."/inet/ipv4/ipaddr");

if($RouterLANIP=="")
{
	$RouterLANIP = query($path_run_inf_lan1."/ipalias/ipv4/ipaddr");
}
...
$devconfsize = get("", "/runtime/device/devconfsize");
$fresetwizard_support = get("", "/device/fresetwizard");
$countrycode = get("", "/runtime/devdata/countrycode");
$wandhcpcip = get("", $runtime_wan1_infp."/udhcpc/ip");
$ReturnLANIP="0";
if($devconfsize=="0" && $fresetwizard_support!="0")
{
	if($countrycode=="RU") {$ReturnLANIP="1";}
	else
	{
		if($wandhcpcip=="")	{$ReturnLANIP="1";}
	}
}
if($ReturnLANIP=="1")
{
