...
IP6T_newchain($START, "filter", "FIREWALL_POLICY");//rbj

/* Add chain for Outbound filter */
IP6T_newchain($START, "filter", "FWD.OBFILTER");
IP6T_newchain($START, "filter", "INP.OBFILTER");

IP6T_newchain($START, "filter", "INGRESSFILTER");

/* Create new chain for IPv6 Simple Security, by Jerry Kao. */
IP6T_newchain($START, "filter", "FWD.SMPSECURITY");
IP6T_newchain($START, "filter", "INP.SMPSECURITY");
IP6T_newchain($START, "filter", "OUT.SMPSECURITY");
$New_SECURITY_chain   = 'SMPSECURITY.';
...
while ($i>0)
{
	...
	/* IPv6 Simple Security chains */	
	IP6T_newchain($START, "filter", "FWD.".$New_SECURITY_chain.$ifname);
	IP6T_newchain($START, "filter", "INP.".$New_SECURITY_chain.$ifname);
	IP6T_newchain($START, "filter", "OUT.".$New_SECURITY_chain.$ifname);	
	
	/* define chain */
	IP6T_newchain($START, "filter", "FWD.".$ifname);
	IP6T_newchain($START, "filter", "INP.".$ifname);
	IP6T_newchain($START, "filter", "MACF.".$ifname);
...
/* SPI */
/* Because nf_conntrack_ipv6 is module, move it to IP6TDEFCHAIN.php
$drpcmd = "-j DROP";
$logcmd	= "-m limit --limit 10/m -j LOG --log-level info --log-prefix";
