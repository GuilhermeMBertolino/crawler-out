...
include "/etc/services/BWC/bwclib.php";
...
function bwc_tc_spq_2013gui_start($rtbwcp, $name, $ifname)
{
	...
	if ($name == "WAN-1") { startcmd("sh /var/run/BWC_LAN1.sh"); }	//run BWC_LAN1.sh in BWC.WAN-1_start.sh
	/* TC fw policy will be:
		0: check skb->mark only. This is linux native default.
		1: check connection->mark only.
		2: Prefer connection->mark, if connection->mark==0, then check skb->mark later.
		...
		4: check connection->mark only, and speed up TCP small packets.
	  */
	startcmd("echo 1 > /proc/sche/fw_policy");
	...
	if ($autowan_enable == "1")
	{
		startcmd('upspeed=`cat tmp/speedtest_upload_result | sed "s/[^0-9]//g"`');
		startcmd('upsp=$upspeed"kbps"\n');
		startcmd($tc_class_add." parent 1:0 classid 1:1 htb rate $upsp ceil $upsp");
	}
	else
	{
		startcmd($tc_class_add." parent 1:0 classid 1:1 htb rate ".$trate." ceil ".$trate." burst 10000 cburst 10000");
	}
	/* add qDisc PRIO */
	...
	foreach($rtbwcp."/rules/entry")
	{
		if (query("enable")=="1")
		{
			...
			{
				startcmd("#=========================================================================================");
				startcmd($ipt_add_prefix." -m mac --mac-source ".$macaddr." -j CONNMARK --set-xmark ".$hex_mark_base."/0xFF00");
				startcmd($ipt_add_prefix." -m mac --mac-source ".$macaddr." -j RETURN");				
			}
			if( $name == "LAN-1" ) /* Download bandwidth control */
			{
				//startcmd("#=========================================================================================");
...
function bcm_iqos_start($rtbwcp, $name, $ifname)
{
	if ($name == "WAN-1")
	{
		startcmd("echo 0 > /proc/sys/net/ipv4/netfilter/ip_conntrack_fastnat");
		/* Retrieve WAN/LAN interface */
		$waninf = $ifname;
		$laninf = PHYINF_getruntimeifname("LAN-1");
		
		/* set nvram for bcmiqosd */
		//Comment below two lines to make bandwidth control can work.
		//startcmd("nvram set wan_ifname=".$waninf);
		startcmd("nvram set lan_ifname=".$laninf);
		$autowan_enable = query($rtbwcp."/autobandwidth");
		if ($autowan_enable == "1") startcmd("nvram set broadstream_iqos_wan_bw_auto=1");
		/*
			iQoS init procedure:
			1. Check Internet(speedtest server) is reachable
			2. Run speed test and wait it done, write download/upload result to qosd.conf 
			3. Wait step2 done, generate qosd.conf 
			4. Run setup.sh to set iptables, insert kernel modules, and enable qosd
		*/
		//auto_wan_bw($name);
		gen_iqos_conf($name, $waninf, $laninf);
		startcmd("sh /var/run/gen_conf.sh");
	}
}
function bcm_iqos_stop($rtbwcp, $name, $ifname)
{
	if ($name == "WAN-1")
	{
		...
		$waninf = query($stsp."/devnam");
		$stsp = XNODE_getpathbytarget("/runtime", "inf", "uid", "LAN-1", 0);
		$laninf = query($stsp."/devnam");
		
		/* 1. stop iqos */
		stopcmd("sh /tmp/trend/qos.sh stop");
		stopcmd("sh /tmp/trend/setup.sh stop ".$waninf." ".$laninf);
		/* 2. remove gen config script */
		stopcmd("rm /var/run/gen_conf.sh");
	}
}
...
function bwc_setup($name)
{
    ...
    stopcmd("echo ".$name." Stop Traffic Control system: TC_SPQ_2013GUI ...");
	    ...
	    /* run speedtest when autobandwidth enabled */
		if ($name == "WAN-1")
		{
			$autowan_enable = query($bwcp."/autobandwidth");
			if ($autowan_enable == "1") startcmd("speedtest_cli 1 3 1 2");
		}
		...
		if( query($bwcp."/enable") == "1")
		{
			...
			if ($name == "LAN-1") 
			{
				bwc_tc_spq_2013gui_lanup_start($bwcp, $name, $ifname);//run BWC.LAN-1 when BWC.WAN-1 start 
			} 
			else 
			{ 
	...
	else if($bwcp_flag == "BROADSTREAM_iQoS")
	{
		startcmd("echo ".$name." Start Traffic Control system: Broadcom iQoS ...");
    	stopcmd("echo ".$name." Stop Traffic Control system: Broadcom iQos ...");
    	...
    	if( query($bwcp."/enable") == "1")
		{
			startcmd("service HW_NAT stop"); // Stop ralink HW_NAT
   			...
   			/* ex: $name=> WAN-1, $ifname=>ppp0 */
			bcm_iqos_start($bwcp, $name, $ifname);
			bcm_iqos_stop($bwcp, $name, $ifname);
			
			stopcmd("service HW_NAT restart"); // Restart ralink HW_NAT

			/* shortcut-fe */
			if (isdir("/sys/module/shortcut_fe_cm") == 1) {
				startcmd("rmmod shortcut-fe-cm shortcut-fe");
				stopcmd("insmod /lib/modules/shortcut-fe.ko");
				stopcmd("insmod /lib/modules/shortcut-fe-cm.ko");
			}
		...
		{
			startcmd("echo ".$name." Traffic Control Broadcom iQoS is disabled.\n"); 
			stopcmd("echo ".$name." Traffic Control Broadcom iQoS disabled.\n"); 
			return; 
		}			
	}	
	else
