...
function pppoptions($inf, $ifname, $profile, $default, $mode , $addrtype)
{
	...
	if($mode=="ondemand")
	{
		...
		//+++ Jerry Kao, pick a fake [IPv6] IP for WAN port.
		$ip6addr = "::cafe:beef,::cafe:beaf";
		fwrite("a",$optfile, "ipv6 ".$ip6addr."\n");	
	...
	if($ipaddr == "")
	{
		//+++ Jerry Kao, supports a unique persistent id, such as EUI-48 derived from 
		//               the Ethernet MAC addressreference to pppd(8) man.		
		fwrite("a",$optfile, "ipv6cp-use-persistent\n");				
	}
	else
	{
...
//+++ Jerry Kao, added for Enable new crested IPv6 interface.
if($dial == "ondemand") fwrite(a, $dialupsh, 'echo 0 > /proc/sys/net/ipv6/conf/default/disable_ipv6\n');
...
fwrite(a, $START,
      ...
      'cp /etc/scripts/ppp6-status /etc/ppp/.\n'.
