...
function lower_static($inetp, $l_infp, $l_inetp)
{
	$inf = $_GLOBALS["LOWER"];
	anchor($l_inetp."/ipv4");
	$ipaddr	= query("ipaddr");
...
function lower_dhcp($inetp, $l_infp, $l_inetp)
{
	$inf = $_GLOBALS["LOWER"];
	$phyinf = query($l_infp."/phyinf");
	$ifname = PHYINF_getifname($phyinf);
	...
	/* Get DNS setting */
	$cnt = query($inetp."/ppp4/dns/count")+0;
	foreach($inetp."/ppp4/dns/entry")
	{
		if ($InDeX > $cnt) break;
		if ($dns=="") $dns = $VaLuE." ";
		else $dns = $dns.$VaLuE." ";
...
else
{
	if (query($L_INETP."/ipv4/static")=="1")
		lower_static($INETP, $L_INFP, $L_INETP);
	else  lower_dhcp($INETP, $L_INFP, $L_INETP);
