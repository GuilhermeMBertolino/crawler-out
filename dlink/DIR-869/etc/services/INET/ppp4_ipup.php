...
$nativephyinf = query($infp."/phyinf");
del($stsp."/inet");
...
set($stsp."/nativephyinf",		$nativephyinf);
...
if($child!="")
{
	...
	if($childip!="")
	{
		...
		XNODE_set_var($child."_NATIVEPHYINF", $nativephyinf);
		echo "event ".$PARAM.".UP\n";
		echo "echo 1 > /var/run/".$PARAM.".UP\n";
		//echo "phpsh debug /etc/scripts/IPV6.INET.php ACTION=ATTACH INF=".$PARAM;
		
		// HuanYao: workaround for IPCP rejected. Related: ppp4_ipup.php
		echo "xmldbc -k ".$PARAM.".wait_ipcp \n";
	}
	else
	{
		echo "echo IPV6CP is not ready!! WAIT --- > /dev/console\n";
		
		// HuanYao: workaround for IPv6CP rejected. Related: ppp6_ipup.php
		echo "xmldbc -t '".$PARAM.".wait_ip6cp:5:event ".$PARAM.".UP; echo 1 > /var/run/".$PARAM.".UP'\n";
