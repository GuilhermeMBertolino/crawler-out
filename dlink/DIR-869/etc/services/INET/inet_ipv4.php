...
function inet_ipv4_static($inf, $ifname, $inet, $inetp, $default)
{
	...
	if(strstr($inf, "WAN") != "")
	{
		$event_add_WANPORTLINKUP = "event WANPORT.LINKUP insert WANPORTLINKUP:'phpsh /etc/events/WANPORT_LINKUP.sh'";
		$event_remove_WANPORTLINKUP = "event WANPORT.LINKUP remove WANPORTLINKUP";
	}
	else
	{
		$event_add_WANPORTLINKUP = "";
		$event_remove_WANPORTLINKUP = "";
	}
...
function inet_ipv4_dynamic($inf, $ifname, $inet, $inetp)
{
	...
	//add by Vic, do not add dhcp renew and release to event WANPORT.LINKUP and WANPORT.LINKDOWN when repeater mode
	$existingnetwork = query("/device/existingnetwork");
	if($inf=="BRIDGE-1")
	{		
		if($existingnetwork=="wifi")
		{
			$WANPORTLINKUP = 'true';
			$WANPORTLINKDOWN = 'true';
		}
	...
	TRACE_info("  ++ $WANPORTLINKUP  = ".$WANPORTLINKUP);
	TRACE_info("  ++ $WANPORTLINKDOWN  = ".$WANPORTLINKDOWN);

	/* Get Setting */
	$hostname_dhcpc	= get("s", "/device/hostname_dhcpc"); //The hostname for DHCP client.
	...
	startcmd(
		...
		'event WANPORT.LINKUP          insert "IPv4Renew:'.$WANPORTLINKUP.'"\n'.			// HuanYao: To capality with IPv6 WAN port linkup DAD.
		'event WANPORT.LINKDOWN        insert "IPv4Renew:'.$WANPORTLINKDOWN.'"\n'.
	...
	stopcmd(
		...
		'event WANPORT.LINKUP remove IPv4Renew\n'.	// HuanYao : Needs to remove the event tag when stop services.
		'event WANPORT.LINKDOWN remove IPv4Renew\n'.
		'event WANPORT.LINKUP remove SMART404\n'.
		'event WANPORT.LINKDOWN remove SMART404\n'.
...
if($mode != "")
{
	...
	if($mode=="dslite") 
	{
		fwrite(a,$START, "phpsh /etc/scripts/IPV4.DSLITE.php ACTION=DSLITE_START INF=".$inf." IFNAME=".$ifname." INET=".$inet." INETP=".$inetp." INFPREV=".$infprev." &\n");
		fwrite(a,$STOP,  "phpsh /etc/scripts/IPV4.DSLITE.php ACTION=DSLITE_STOP INF=".$inf." IFNAME=".$ifname." INET=".$inet." INETP=".$inetp." INFPREV=".$infprev." &\n");
		//inet_ipv4_dslite($inf, $ifname, $inet, $inetp, $infprev);
	}
}
else
{
