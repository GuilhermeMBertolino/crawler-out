		...
		if ($mac!="")
		{
			if ($policy == "DROP")
			{
				if ($sch_uid=="" || $sch_path=="")
				{
					...
					// HuanYao: Add IPv6 control.
					fwrite("a", $START, "ip6tables -A ".$CHAIN." -m mac --mac-source ".$mac." -j LOG --log-level notice --log-prefix 'DRP:004:' \n");
				...
				else
				{
					...
					// HuanYao: Add IPv6 control.
					IPT_fwrite_schedule("a", $START, "ip6tables -A ".$CHAIN." -m mac --mac-source ".$mac." -j LOG --log-level notice --log-prefix 'DRP:004:'", $sch_uid);
