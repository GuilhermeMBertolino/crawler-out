...
foreach($node_info) //Prevent the security issue of Command Injection
{
	...
	$port = query("ExternalPort");
		
	if($protocol == "Other")
	{
		if(isdigit(get("x", "ProtocolNumber"))==0)
		{
			$result = "ERROR";
			break;
		}
	...
	//check Port compare with PortForward_port
	$conflict_value = nat_pfport_conflict($port,  $protocol );
	if($conflict_value==1)
	{
		$result = "ERROR";
		TRACE_error("[ERROR] Virtual Server port is conflicted with PF port");
		break;
	}
	
	//check port compare with remotePort
	$check_remote_conflict = nat_remoteport_conflict($port,  $protocol ); 
	if ($check_remote_conflict==1)
	{
		$result = "ERROR";
		TRACE_error("[ERROR] Virtual Server port is conflicted remote port");
		break;
	}
}
...
if($result == "OK")
{
	...
	foreach($node_info)
	{
		...
		if ($protocol == "TCP") 
			...
			{$protocolnum = get("x", "ProtocolNumber");}
