...
if($Address != "" && INET_validv6addr($Address)==0)                    { $result = "ERROR"; }
else if($DefaultGateway != "" && INET_validv6addr($DefaultGateway)==0) { $result = "ERROR"; }
else if($PrimaryDNS != "" && INET_validv6addr($PrimaryDNS)==0)         { $result = "ERROR"; }
else if($SecondaryDNS != "" && INET_validv6addr($SecondaryDNS)==0)     { $result = "ERROR"; }
else if($LanAddress != "" && INET_validv6addr($LanAddress)==0)         { $result = "ERROR"; }
else if($SubnetPrefixLength != "" && isdigit($SubnetPrefixLength)==0)  { $result = "ERROR"; }
...
if($result == "OK")
{
	...
	if ($Layout == "bridge")
	{
		set(INF_getinfpath($WAN3)."/inet",   "INET-8"); 
	...
	/*<--PreWAN()  */
		...
		set($WANllact_infp."/childgz", "");	// HuanYao (2016.3): Missing 6in4 DHCP-PD enabled mode.
		...
		set("/autodetect/active", "0");

	// HuanYao: We needs to check the IPv4 WAN is DS-lite.
		if($dslite_mode=="dslite")
		{ set($WAN1_infp."/infprevious", $WAN); }
		else
		{
		...
		if($ConnectionType == "IPv6_Static")
		{
			...
			{
				set($WANact_infp."/infnext", "");
				set($WANllact_infp."/infnext", $WAN4);
			}

		}
		else if($ConnectionType == "IPv6_AutoDetection")
		{
			$ppp4 = query($WAN1_inetp."/addrtype");

			if ($ppp4 == "ppp4" || $ppp4 == "ppp10") 
			{ /*PPPoE*/
			...
			$idletimeout_v4 = query($WAN1_inetp."/ppp4/dialup/idletimeout");
			...
			set($WAN1_inetp."/ppp6/dns/count", "0");
			set($WAN1_inetp."/ppp6/mtu", $MTU_v4);
			set($WAN1_inetp."/ppp6/dialup/mode", $dialup_v4);
			set($WAN1_inetp."/ppp6/dialup/idletimeout", $idletimeout_v4);
			set($WANllact_inetp."/addrtype", "ipv6");
			...
			{			/*Autoconfigure*/
				...
				set($WANact_infp."/child", "LAN-4");
				set($WANact_infp."/childgz", "LAN-5");
				set($WANact_inetp."/ipv6/dhcpopt", "IA-NA+IA-PD");
				set($WANact_inetp."/ipv6/dns/entry:1", "");
				set($WANact_inetp."/ipv6/dns/entry:2", "");
				set($WANact_inetp."/ipv6/dns/count", "0");
			}
			...
			set($WAN1_inetp."/ppp6/dns/count", $dnscnt);


			set("/autodetect/active", "1");
			set("/autodetect/change", "0");
			set("/autodetect/inf4", $WAN1);
			set("/autodetect/inf6", $WAN4);
			set("/autodetect/infLL", $WAN3);
		...
		else if($ConnectionType == "IPv6_AutoConfiguration") 
		{
			...
			// HuanYao: set infnext/infprefious.
			set($WANllact_infp."/infnext", $WAN4);
			set($WANact_infp."/infprevious", $WAN3);
		...
		else if($ConnectionType=="IPv6_IPv6InIPv4Tunnel")
		{
			...
			if($DhcpPd == "Enable")
			{
				/* WAN-1 (IPv4) -> WAN-4 (6in4 tunnel) -> WAN-3 (DHCP-PD) -child-> LAN-4 and LAN-5 */
	...
	/*<--PreLAN()  */
		...
		else if($ConnectionType=="IPv6_AutoDetection" || $ConnectionType=="IPv6_AutoConfiguration")
		{
			if($DhcpPd != "Enable")
			{
				...
				// HuanYao: bug fix for restart LAN will not start LAN-4.
...
if($result == "OK")
{
	...
	fwrite("a",$ShellPath, "xmldbc -k autodetect\n"); // HuanYao: kill the timer of autotect to prevent timing issue.
