...
//only check IP format
function validaddr($ipaddr)
{
	$host = ipv4hostid($ipaddr, 0);
	if ($host == ""||$host == 0) return 0;
	$network = ipv4networkid($ipaddr, 8);
	if (cut($network, 0, ".") > 255) return 0;
	return 1;
}


foreach($node_rule) //Avoid the security issue of Command Injection
{
	if(INET_validv4addr(get("", "SrcIPv4AddressRangeStart"))==0 || INET_validv4addr(get("", "DestIPv4AddressRangeStart"))==0)
	{
		$result = "ERROR";
		break;
	}
	
	if(get("", "SrcIPv4AddressRangeEnd")!="" && INET_validv4addr(get("", "SrcIPv4AddressRangeEnd"))==0)
	{
		$result = "ERROR";
		break;
	}
	
	//if(get("", "DestIPv4AddressRangeEnd")!="" && INET_validv4addr(get("", "DestIPv4AddressRangeEnd"))==0)
	if(get("", "DestIPv4AddressRangeEnd")!="")
	{
		$IPcheck = get("", "DestIPv4AddressRangeEnd");
		if(validaddr($IPcheck)==0) // just check format
		{ 
			$result = "ERROR"; 
		}
		break;
	...
	{
		if(get("", "PortRangeStart")!="" && isdigit(get("", "PortRangeStart"))==0)
		{
			$result = "ERROR";
			break;
		}
		if(get("", "PortRangeEnd")!="" && isdigit(get("", "PortRangeEnd"))==0)
		{
			$result = "ERROR";
			break;
		}
...
if($result == "OK")
{
	...
	*/
	...
	foreach($node_rule)
	{
		...
		else 								{set("/acl/firewall/entry:".$InDeX."/protocol", "ALL");}
