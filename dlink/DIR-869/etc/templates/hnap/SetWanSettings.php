...
if(isempty($MTU) == "1"){ $MTU="0"; }
$mac = "";
$rlt = "OK";

//Prevent the security issue of Command Injection
//Work around the issue from QRS mobile: when set DHCPPPPoE mode without DHCP server (ex, Hinet's PPPoE), QRS modbile will set ipaddress to 0.0.0.0, this situation ignore the error check of ipaddress.
if($IPAddress !="" && $IPAddress != "0.0.0.0" && INET_validv4addr($IPAddress)==0)           { $rlt = "ERROR"; }
else if($Gateway != "" && $Gateway != "0.0.0.0" && INET_validv4addr($Gateway)==0)           { $rlt = "ERROR"; }
else if($MacAddress != "" && PHYINF_validmacaddr($MacAddress)!=1)  { $rlt = "ERROR"; }
else if($HostName != "" && isdomain($HostName)==0)                 { $rlt = "ERROR"; }
else if($ServiceName != "" && isdomain($ServiceName)==0)           { $rlt = "ERROR_SERVICENAME"; }
...
if($rlt == "OK")
{
	...
	$current_wirelessmode = query("/device/wirelessmode");
	...
	if($layout=="bridge")
	{
		if(strstr($Type, "Static")!="")
			{$new_type="1";}
		else if (strstr($Type, "Dynamic")!="")
			{$new_type="0";}
		$old_type=get("x", $path_br1_inet."/ipv4/static");
	...
	{
		$previous_addrtype = get("", $path_wan1_inet."/addrtype");
		if ($previous_addrtype == "ppp10")
		{
			set($path_wan1_inet."/addrtype", "ppp10");
		}
		else
		{
		...
		//Work around the issue from QRS mobile Android v1.5.1.0: MTU set to 1500 will cause setting error, set MTU to 1492 if MTU is over 1492.
		if($Type == "DHCPPPPoE" && $MTU > 1492) { $MTU = "1492"; }
...
/*Joe H. If PPPoEv6 is share with PPPoEv4, change ppp6 configuration to be the same with ppp4*/
		if ($previous_addrtype == "ppp10") {
			set($path_wan1_inet."/ppp6/username", $Username);
			set($path_wan1_inet."/ppp6/password", $Password);
			set($path_wan1_inet."/ppp6/pppoe/servicename", $ServiceName);
			if ($MTU == "0")
				set($path_wan1_inet."/ppp6/mtu", "1492");
			if ($MTU >= 200 && $MTU <= 1492)
				set($path_wan1_inet."/ppp6/mtu", $MTU);
			set($path_wan1_inet."/ppp6/dialup/idletimeout", $MaxIdleTime);
			if ($AutoReconnect == "false") {
				if ($MaxIdleTime > 0)
					set($path_wan1_inet."/ppp6/dialup/mode", "ondemand");
				else
					set($path_wan1_inet."/ppp6/dialup/mode", "manual");
			} else {
				set($path_wan1_inet."/ppp6/dialup/mode", "auto");
			}
		}
	}
	...
	if ($layout == "router" && $autodetect == "1") {
		if($Type == "StaticPPPoE" || $Type == "DHCPPPPoE") {
			$dialup_v4 = query($path_wan1_inet."/ppp4/dialup/mode");
			$idletimeout_v4 = query($path_wan1_inet."/ppp4/dialup/idletimeout");
			set($path_inf_wan1."/child", "WAN-3");
			set($path_inf_wan1."/infnext", "");
			set($path_inf_wan3."/infnext", "WAN-4");
			set($path_inf_wan3."/inet", "");
			set($path_inf_wan4."/infprevious", "WAN-3");
			set($path_inf_wan4."/defaultroute", "0");
			set($path_wan1_inet."/addrtype", "ppp10");
			set($path_wan1_inet."/ppp6/username", $Username);
			set($path_wan1_inet."/ppp6/password", $Password);
			set($path_wan1_inet."/ppp6/pppoe/servicename", $ServiceName);
			set($path_wan1_inet."/ppp6/over", "eth");
			set($path_wan1_inet."/ppp6/static", "0");
			del($path_wan1_inet."/ppp6/ipaddr");
			set($path_wan1_inet."/ppp6/dns/entry:1","");
			set($path_wan1_inet."/ppp6/dns/entry:2","");
			set($path_wan1_inet."/ppp6/dns/count", "0");
			set($path_wan4_inet."/ipv6/dhcpopt", "IA-PD");
			if($MTU == "0")
				set($path_wan1_inet."/ppp6/mtu", "1492");
			else
				set($path_wan1_inet."/ppp6/mtu", $MTU);

			set($path_wan1_inet."/ppp6/dialup/mode", $dialup_v4);
			set($path_wan1_inet."/ppp6/dialup/idletimeout", $idletimeout_v4);
			set($path_wan4_inet."/ipv6/mode", "AUTO");
		} else {
			$defaultroute = query($path_inf_wan4."/defaultroute");
			if (query($path_inf_wan4."/defaultroute") == "0") //last ipv4 mode is ppp
				$autodetect_reload = "1";
			set($path_inf_wan1."/child", "");
			set($path_inf_wan3."/infnext", "");
			set($path_inf_wan3."/inet", "INET-8");
			set($path_inf_wan4."/defaultroute", "1");
			set($path_inf_wan4."/infprevious", "");
			set($path_wan4_inet."/ipv6/dhcpopt", "IA-NA+IA-PD");
			set($path_wan4_inet."/ipv6/mode", "AUTO");
		}
		set("/autodetect/change", "0");
	}
...
{
	fwrite("a",$ShellPath, "event DBSAVE > /dev/console\n");
	if ($autodetect_reload == "1") {	/*Joe H. ppp chang to dhcp need to reparser the following php before restart WAN*/
		fwrite("a",$ShellPath, "xmldbc -P /etc/services/WAN.php -V STOP=/var/servd/WAN_stop.sh\n");
		fwrite("a",$ShellPath, "xmldbc -P /etc/services/INFSVCS.WAN-3.php -V STOP=/var/servd/INFSVCS.WAN-3_stop.sh\n");
	}
...
//	fwrite("a",$ShellPath, "service WAN restart > /dev/console\n");
	...
	if($Type == "StaticBridged" || $Type == "DynamicBridged"  || $Type == "StaticAp" || $Type == "DynamicAp" || $Type == "StaticExtender" || $Type == "DynamicExtender")
