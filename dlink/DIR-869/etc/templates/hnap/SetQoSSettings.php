...
$node_appinfo = $nodebase."/QoSInfoData/QoS_AppInfo";
$path_dev_log = "/device/log";
$result = "OK";
$rule_count = 0; //QoS rules count on UI
$autobandwidth = get("x", $nodebase."/QoSInfoData/autodetect");
$nodebase_clientinfo= "/runtime/hnap/SetClientInfo";
...
if($result=="OK" || $result=="REBOOT")
{
	if($FEATURE_IQOS==1)
	{
		$rule_type = get("x", $nodebase."/rule_type");
		set("/bwc/rule_type", $rule_type);
	}
	//Set bandwidth is manual from web
	if ($autobandwidth == "0" || $autobandwidth == "")
	{	
		set("/bwc/entry:1/autobandwidth","0");
		set("/bwc/entry:2/autobandwidth","0");
		$uplink = get("x", $nodebase."/uplink");
		$downlink = get("x", $nodebase."/downlink");
		
		if ($bwc1link == "up")
		{
			set("/bwc/entry:1/bandwidth", $uplink); //upstream
			set("/bwc/entry:2/bandwidth", $downlink); //downstream
		}
		else if ($bwc1link == "down")
		{
			set("/bwc/entry:1/bandwidth", $downlink); //downstream
			set("/bwc/entry:2/bandwidth", $uplink); //upstream
		}
	}
	else
	{
		set("/bwc/entry:1/autobandwidth","1");
		set("/bwc/entry:2/autobandwidth","1");
	}
	...
	foreach($entry_bwc1)
	{
		...
		if($FEATURE_IQOS==1)
		{
			set($entry_bwc1.":".$InDeX."/deviceos", "");
			set($entry_bwc1.":".$InDeX."/devicetype", "");
			set($entry_bwc1.":".$InDeX."/devicefamily", "");
		}
	}
	foreach($entry_bwc2)
	{
		...
		if($FEATURE_IQOS==1)
		{
			set($entry_bwc2.":".$InDeX."/deviceos", "");
			set($entry_bwc2.":".$InDeX."/devicetype", "");
			set($entry_bwc2.":".$InDeX."/devicefamily", "");
		}
	}

	foreach($node_info)
	{
		set($path_bwcf."/seqno", $InDeX+1);
		set($path_bwcf."/count", $InDeX);
		...
		if($FEATURE_IQOS==1)
		{
			$DEVOS = query("DeviceOS");
			$DEVTYPE = query("DeviceType");
			$DEVFAMILY = query("DeviceFamily");
		}
		...
		if($FEATURE_IQOS==1)
		{
			TRACE_debug("$DEVOS=".$DEVOS);
			TRACE_debug("$DEVTYPE=".$DEVTYPE);
			TRACE_debug("$DEVFAMILY=".$DEVFAMILY);
		}
		
		foreach("/dhcps4/entry:1/staticleases/entry") 
		{
			if(PHYINF_macnormalize($MACADDR) == PHYINF_macnormalize(get("", "macaddr")))
			{
				$ipaddr = ipv4ip(INF_getcurripaddr($LAN1), INF_getcurrmask($LAN1), get("", "hostid"));
		...
		if($FEATURE_IQOS==1)
		{
			set($entry_bwc1.":".$InDeX."/deviceos", $DEVOS);
			set($entry_bwc1.":".$InDeX."/devicetype", $DEVTYPE);
			set($entry_bwc1.":".$InDeX."/devicefamily", $DEVFAMILY);
		}
		...
		if($FEATURE_IQOS==1)
		{
			set($entry_bwc2.":".$InDeX."/deviceos", $DEVOS);
			set($entry_bwc2.":".$InDeX."/devicetype", $DEVTYPE);
			set($entry_bwc2.":".$InDeX."/devicefamily", $DEVFAMILY);
		}

		//only available TC_SPQ_2013GUI for new UI QoS now
		if($FEATURE_IQOS==1) $TYPE = "BROADSTREAM_iQoS";
		else $TYPE = "TC_SPQ_2013GUI";
	...
	// set application info to db
	if($FEATURE_IQOS==1)
	{
		foreach($node_appinfo)
		{
			...
			$bwcrulep =  XNODE_getpathbytarget("/bwc/entry:1/app_rules", "entry", "description", $APPNAME, 0);
			set($bwcrulep."/enable", "1");
			if ($PRIORITY == "0") { set($bwcrulep."/bwcqd", "BWCQD-4"); }
			else if ($PRIORITY == "1") { set($bwcrulep."/bwcqd", "BWCQD-3"); }
			else if ($PRIORITY == "2") { set($bwcrulep."/bwcqd", "BWCQD-2"); }
			else if ($PRIORITY == "3") { set($bwcrulep."/bwcqd", "BWCQD-1"); }
	...
	if($FEATURE_IQOS==1)
	{
		$QoS_enable = get("x", $nodebase."/QoSInfoData/enable");
		if( query("/bwc/entry:1/enable")!= $QoS_enable || query("/bwc/entry:2/enable")!= $QoS_enable)
		{
			set("/bwc/entry:1/enable", $QoS_enable);
			set("/bwc/entry:2/enable", $QoS_enable);
			$result="OK";
		}
	}
	else
	...
	if($rule_count > 0)
	{
		...
		if($en_bwc1 == 1 && $en_bwc2 == 1)      {$result = "OK";}
	}
	else
	{
		set("/bwc/entry:1/enable", "0");
		set("/bwc/entry:2/enable", "0");
		//Clean the /bwc/bwcf entries when QoS disable
		foreach($entry_bwcf)
		{
			set($entry_bwcf.":".$InDeX."/ipv4/start", "");
			set($entry_bwcf.":".$InDeX."/ipv4/end", "");
		...
		if($en_bwc1 == 0 || $en_bwc2 == 0)      {$result = "OK";}
	}
}
}
