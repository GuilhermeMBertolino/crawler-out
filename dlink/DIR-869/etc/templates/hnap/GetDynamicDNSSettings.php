...
//else if($DDNS4_ServerAddress=="ORAY")			{$DDNS4_ServerAddress="oray.cn";}
else if($DDNS4_ServerAddress=="ORAY")			{$DDNS4_ServerAddress = get("x", "/ddns4/entry:1/oray-type");}
else if($DDNS4_ServerAddress=="NOIP")			{$DDNS4_ServerAddress = get("x", "/ddns4/entry:1/noip-type");}
...
{	
	if($DDNS4_ServerAddress != "oray.cn" && $DDNS4_ServerAddress != "www.oray.com")
	{
	...
	else //oray
	{
		$status = get("x", $path_run_inf_wan1."/ddns4/status");
		if($status == "successed")	
			$DDNS4_Status = "Connected";
		else if($status == "connecting")
			$DDNS4_Status = "Connecting";	
		else
			$DDNS4_Status = "Disconnected";
	}
}
