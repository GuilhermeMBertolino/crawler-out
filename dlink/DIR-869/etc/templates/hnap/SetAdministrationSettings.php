...
$inf_wan4 = XNODE_getpathbytarget("", "inf", "uid", $WAN4, 0);
$inf_lan1 = XNODE_getpathbytarget("", "inf", "uid", $LAN1, 0);
$inf_bridge1=  XNODE_getpathbytarget("", "inf", "uid", $BR1, 0);
...
$conflict_value = nat_pfport_conflict($RemoteMgtPort,  "TCP"); //compare PF ports
if( $conflict_value==1)
{
	$result = "ERROR";
	TRACE_error("[ERROR] Remote Port is conflicted with PF_port");
}		

$conflict_value = nat_vsport_conflict( $RemoteMgtPort, "TCP"); //compare VS ports
if( $conflict_value==1)
{
	$result = "ERROR";
	TRACE_error("[ERROR] Remote Port is conflicted with VS_port");
}		
...
if($RemoteMgtHTTPS=="true")
{
	if($HTTPS == 0) $RemoteMgtHTTPS = 0;
	else $RemoteMgtHTTPS = 1;
}
...
if($result=="OK")
{
	...
	else
	{
	  ...
	  set($inf_wan4."/web",$RemoteMgtPort);
	  set($inf_wan1."/inbfilter",$InboundFilter);
	  del($inf_wan1."/https_rport",$RemoteMgtPort);
	  del($inf_wan4."/https_rport",$RemoteMgtPort);
	}
}
...
if($result=="OK")
{
	...
	fwrite("a",$ShellPath, "service HTTP.WAN-4 restart > /dev/console\n");
