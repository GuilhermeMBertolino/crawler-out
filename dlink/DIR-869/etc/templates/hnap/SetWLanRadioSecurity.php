...
{
	bsd_wificonfig_from_24g($WLAN2_UID, $WLAN1_UID);
	if ($_GLOBALS['FEATURE_TRI_BAND'] == 1) bsd_wificonfig_from_24g($WLAN3_UID, $WLAN1_UID);
}
...
if ($wps_enable == "false") $wps=0;
else $wps=1;

//Add by Vic, move to here to global setting
...
if (query($encr_check_wlan1."/active")==1)
{
	...
	if (query($encr_wifi_wlan1."/encrtype")=="WEP" || query($encr_wifi_wlan1."/encrtype")=="TKIP" || query($encr_wifi_wlan1."/ssidhidden")==1) {set($encr_wifi_wlan1."/wps/enable", 0);}
}
if (query($encr_check_wlan2."/active")==1 && $smart_en == "0")
{
	...
	if (query($encr_wifi_wlan2."/encrtype")=="WEP" || query($encr_wifi_wlan2."/encrtype")=="TKIP" || query($encr_wifi_wlan2."/ssidhidden")==1) {set($encr_wifi_wlan2."/wps/enable", 0);}
}
if (query($encr_check_wlan3."/active")==1 && $smart_en == "0")
{
	...
	if (query($encr_wifi_wlan3."/encrtype")=="WEP" || query($encr_wifi_wlan3."/encrtype")=="TKIP" || query($encr_wifi_wlan3."/ssidhidden")==1) {set($encr_wifi_wlan3."/wps/enable", 0);}
}
//Marked by Vic and move ahead
//if($encr_wifi_wlan1!=""){set($encr_wifi_wlan1."/wps/enable", $wps);}
//if($encr_wifi_wlan2!=""){set($encr_wifi_wlan2."/wps/enable", $wps);}
//if($encr_wifi_wlan3!=""){set($encr_wifi_wlan3."/wps/enable", $wps);}
//----longlay(20131113)
