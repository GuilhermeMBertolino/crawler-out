...
function SiteSurveyInfo($path)
{
	...
	if(strip(get("x","ssid"))=="") {return;} //Don't show the wireless information if it's SSID is empty or blank.
	
	//skip WEP mode
	$encrtype = get("x","encrtype");
	if($encrtype=="WEP")	{return;}
	...
	echo "						<WirelessMode>".get("x","wlmode")."</WirelessMode>\n";
...
else if($RadioID=="5GHZ_2" || $RadioID=="RADIO_5GHz_2")
{
	$sitesurvey_path = $sitesurvey_node."/sitesurvey_5G/entry";
	$mode = "5G2";
}
