...
$hnap_action="SetMultipleActions_GuestZoneRouterSettings";
foreach("/runtime/hnap/SetMultipleActions/SetGuestZoneRouterSettings")
{
	$nodebase="/runtime/hnap/SetMultipleActions/SetGuestZoneRouterSettings:".$InDeX."/";
	if(exist($nodebase)==1)
	{
		TRACE_debug("SetMultipleActions.php: execute ".$hnap_action);
		include "etc/templates/hnap/SetMultipleActions_GuestZoneRouterSettings.php";
		if($result!="OK")
		{
			TRACE_error($hnap_action." is not OK ret=".$result);
		}
...
if(exist("/runtime/hnap/SetMultipleActions/SetOperationMode")==1)
{
	$WirelessMode = query("/runtime/hnap/SetMultipleActions/SetOperationMode/CurrentOPMode");

	if($WirelessMode=="WirelessAp")  // must same as SetWanSetting.php
	{
	    $layout = "bridge";
	}
	...
	else	 if($WirelessMode=="WirelessRepeaterExtender")
	{
		$layout = "bridge";
		//set("/device/wirelessmode", "manual");
		}
	else if($WirelessMode=="WirelessRouter")
	{
	    $layout = "router";
