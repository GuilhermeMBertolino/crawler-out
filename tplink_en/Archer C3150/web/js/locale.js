...
//            把lan作为一个片段来匹配
    ...
    /**
     * get language from DM, then $.locale.load.
     * @param callbackSuccess
     * @param callbackFailed
     */
    $.locale.get = function(callbackSuccess, callbackFailed) {
        var match;
        if ($.sim || $.local) {
        ...
        if (!$.exe()) {
            $.locale.load(match.language,callbackSuccess, callbackFailed)
        }
    };

    /**
     * check language is available or not,then $.locale.changeType
     * @param language value from DM or locale_language
     * @param callbackSuccess
     * @param callbackFailed
     */
    $.locale.load = function(language, callbackSuccess, callbackFailed, strOnly) {
        //check the language in DM whether it is support or not
        if (language && (language = this._findLocale(language))) {
                $.locale.changeType({
                locale: language,
                strOnly: strOnly
                });
        ...
        } else if ((language = this._findLocale(navigator.language || navigator.browserLanguage))){                //赋值
            //check the language in browser whether it is support or not
                $.locale.changeType({
                locale: language,
                strOnly: strOnly
                });
            ...
            $.locale.changeType({
                locale: $.locale.DEFAULT_LAN_TYPE,
                strOnly: strOnly
            });
            if (callbackFailed) {
                callbackFailed();
