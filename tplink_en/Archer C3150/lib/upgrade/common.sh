...
run_ramfs() { # <command> [...]
	install_bin /bin/busybox /bin/ash /bin/sh /bin/mount /bin/umount        \
		/sbin/pivot_root /usr/bin/wget /sbin/reboot /bin/sync \
		/bin/grep /bin/cp /bin/mv /usr/bin/md5sum "/usr/bin/[" \
		/bin/vi /bin/ls /bin/cat /usr/bin/awk /usr/bin/hexdump          \
		/bin/sleep /usr/bin/printf /usr/bin/wc \
		/bin/sed /bin/rm /bin/mkdir /bin/echo /bin/touch /bin/ps
...
kill_remaining() { # [ <signal> ]
	...
	for stat in /proc/[0-9]*/stat; do
		...
		case "$name" in
			...
			*sh*|*ash*|*init*|*watchdog*|*ssh*|*dropbear*|*telnet*|*login*|*hostapd*|*wpa_supplicant*) : ;;
