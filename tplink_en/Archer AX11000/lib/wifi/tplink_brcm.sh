...
#vifs is wl01 wl02 wl03 wl04 wl11 wl12 wl13 wl14 wl21 wl22 wl23 wl24
				for vif in $vifs; do
					...
					config_get ifname $vif ifname  
#ifname is eth6 wl0.2 wl0.3 eth7 wl1.2 wl1.3 eth8 wl2.2 wl2.3				
...
init_nvram_radio() {
	...
	for idx in `seq 0 1 2`; do
		...
		nvram unset wl${idx}_cfg_maxassoc
...
init_nvram_lan() {
	...
	nvram set lan_ifnames="eth1 eth2 eth3 eth4 eth5 eth6 eth7 eth8"
...
#add by wanghaisen 
#disable backup queue
init_nvram_bkq() {
	nvram set dhd0_rnr_flowring_physize="0"
	nvram set dhd1_rnr_flowring_physize="0"
	nvram set dhd2_rnr_flowring_physize="0"
}

# Fix DVT 200pcs
init_nvram_ax11000_only() {
	nvram set 1:mcs1024qam5ghpo=0xEEBBBBBB
	nvram set 1:mcs1024qam5gx1po=0xEEBBBBBB
	nvram set 1:mcs1024qam5gx2po=0xEEBBBBBB
	nvram set 1:mcsbw1605ghpo=0xA9864200
	nvram set 1:mcsbw1605gx1po=0xA9864200
	nvram set 1:mcsbw1605gx2po=0xA9864200
	nvram set 2:mcs1024qam2gpo=0x6666
	nvram set 2:mcsbw202gpo=0x32100000
	nvram set 2:mcsbw402gpo=0x32100000
	nvram set 3:mcs1024qam5glpo=0xEECCCCCC
	nvram set 3:mcs1024qam5gmpo=0xEECCCCCC
	nvram set 3:mcsbw1605glpo=0xA9864200
	nvram set 3:mcsbw1605gmpo=0xA9864200
...
# Wireless phy_ed_thresh for AX11000 AC-PHY
# For (wl is_edcrs_eu)
# 	None-EU: ed_thresh2g, ed_thresh5g
#   EU: eu_edthresh2g, eu_edthresh5g
init_nvram_phy_ed_thresh() {
	nvram set 1:ed_thresh5g="-40"
	nvram set 2:ed_thresh2g="-40"
	nvram set 3:ed_thresh5g="-40"
	nvram set 1:eu_edthresh5g="-40"
	nvram set 2:eu_edthresh2g="-40"
	nvram set 3:eu_edthresh5g="-40"
}
...
init_nvram_routermini() {
	...
	for idx in 0 1 2; do
		nvram set wl${idx}_maxassoc="96"
		nvram set wl${idx}_bss_maxassoc="96"
...
# MBO
# Wi-Fi Agile Multiband AP
init_nvram_mbo() {
	for idx in 0 1 2; do
			nvram set wl${idx}_mbo_enable="1"
	done
}
...
init_nvram_run() {
	...
	nvram set landevs="eth1 eth2 eth3 eth4 eth5 eth6 eth7 eth8"
	nvram set wfi_cmd=""
	nvram set wfi_error=""
	nvram set coma_sleep=""
	nvram set partialboots="0"
	nvram set br0_ifname="br-lan"
	nvram set br0_ifnames="eth1 eth2 eth3 eth4 eth5 eth6 eth7 eth8"
}
...
init_nvram_router() {
	...
	#add by wanghaisen for backup queue
	init_nvram_bkq
}
...
wifi_chanspec_config() {
	...
	if [ $channel = "auto" ]; then
		...
		116|124)
			...
			#for country AU donot support weather channel, ch116 only match 20M
...
wifi_basic_config() {
	...
	elif [ "$tmpCode" = "NZ" ]; then
		nvram set ${HOME_WIFI}_country_code="$tmpCode"
		nvram set ${HOME_WIFI}_country_rev="$COUNTRYCODE_NZ"
	else
	...
	if [ "$eth_enable" = "1" ]; then
		...
		#add by zhangshengbo, for tri-band:5G1 disable band3/4; 5G2 disable band1/2
		#dis_ch_grp:bit 0-ch36-48; bit 1-ch52-60; bit2-ch64; bit3-ch100-140; bit5-ch149-165
		nvram set 1:dis_ch_grp=0x7
		nvram set 3:dis_ch_grp=0x18
...
wifi_security_config() {
	...
	if [ "$eth_enable" = "1" ]; then
		...
		#changed by wanghaisen for AX6000 only support wpa/wpa2 security config as follows:
		#1.wpa2-psk + aes
		...
		#
			...
			#changed by wanghaisen for AX6000 only support wpa/wpa2-enterprise security config as follows:
			#1.wpa2 + aes
			#2.wpa/wpa2 + aes/tkip
			#3.wpa/wpa2 + aes
			#
			...
			#add by zhangshengbo for wpa3 personal	
				...
				#add by wanghaisen, set wlx_preauth=0 temporarily
...
wifi_wds_config() {
	...
	if [ "$eth_enable" = "1" -a "$wds_enable" = "1" ]; then
		...
		nvram set ${vap_name}_bss_maxassoc="96"
...
wifi_backhaul_config() {
	...
	if [ "$backhaul" == "on" ]; then
		...
		nvram set ${WIFI_BACKHAUL}_bss_maxassoc="96"
...
wifi_guest_config() {
	...
	if [ "$eth_enable" = "1" -a "$guest_enable" = "1" ]; then
		...
		nvram set ${GUEST_WIFI}_bss_maxassoc="96" #need to check guestNet num
		...
		#if in WDS mode,do not set vifs
		local cur_mode=`nvram get ${WIFI_DEV}_mode`
		local wifi_state=`cat /tmp/wifi_state`
		if [ "$cur_mode" = "ap" -o "$wifi_state" != "inited" ]; then
...
wifi_driver_startup(){
	...
	if [ "$rftestflag" = "1" -a "$btntestflag" = "1" ]; then
		...
		grep -q '^'$module_name_4360'\>' /proc/modules || insmod $module_name_4360
		...
		grep -q '^'$module_name_4366'\>' /proc/modules || insmod $module_name_4366
		grep -q '^'$module_name_4366'\>' /proc/modules || echo "insmod wltest dhd.ko failed." >$STDOUT ;
		grep -q '^'$module_name_4360'\>' /proc/modules || insmod $module_name_4360
		grep -q '^'$module_name_4360'\>' /proc/modules || echo "insmod wltest wl.ko failed." >$STDOUT ;
	fi
	wifi_features_set
}
...
wifi_features_set(){
	...
	for band in "2g" "5g" "5g_2"; do
		...
		if [ "$home" = "on" ]; then 
			...
			# Add for Evalulate, set amsdu_aggsf=4, if channel width is 160M
			local evaluate_flag=`nvram get tp_evaluateflag`
			if [ "$evaluate_flag" = "1" ]; then
				config_get htmode $vap0 htmode
				if [ "$band" != "2g" -a "$htmode"  = "160" ]; then 			
					wl -i $vap0 amsdu_aggsf 4
				else
					wl -i $vap0 amsdu_aggsf 2
				fi
			fi		
	...
	#move start_hosapd after wlconf start
	start_hosapd
...
wifi_driver_stop(){
	...
	# for ifd in "eth6" "wl0.1" "eth7" "wl1.1" "eth8" "wl2.1"; do 
	# 		wlconf "$ifd" down
	# 		ifconfig "$ifd" down
...
wifi_nvram_config() {
	local rftestflag=`nvram get rftestflag`
	local btntestflag=`nvram get btntestflag`
	# load onemesh first for the following backhaul configuration
	config_get onemesh_enable onemesh enable "on"

	for dev in ${DEVICES}; do  # eth6 eth7
...
wifi_start_forcecal(){
	if [ "$( nvram get rftestflag )" != "1" -o "$( nvram get btntestflag )" != "1" ] ; then
