...
tp_check_nvram()
{
	...
	if	[ -e $kernel_nvram_file ]; then
		echo  "*** CHECK: NVRAM file checking... ***"
#nvram is incompeleted,rm it
		cat $kernel_nvram_file | grep -q tp_nvram_end_flag || needToDel="1"
		
		if	[ "$needToDel" = "0" ]; then
			echo "*** CHECK: NVRAM file is ok... ***"
		else
			rm -f $kernel_nvram_file
			echo "*** CHECK: NVRAM file is wrong, del it... ***"
		fi
	fi
	return 0
