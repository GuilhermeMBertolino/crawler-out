...
# get the real switch port id for vlan base on LAN_PHY_PORT_SET.
# $1: index
get_lan_switch_port()
{
	index=$1
	port_id=`echo $LAN_PHY_PORT_SET | cut -d " " -f$index`
	echo $port_id
}
...
add_br_member()
{
	...
	[ "$1" == "br-wan" ] && { echo 1 > /proc/sys/net/ipv6/conf/$dev/disable_ipv6; }
...
del_br_member()
{
	...
	if [ $(is_in_br $2 $1) -eq "1" ]; then
		echo "del $2 from $1"
		[ -n "$vid" ] && dev=$2
		[ "$1" == "br-wan" ] && { echo 0 > /proc/sys/net/ipv6/conf/$dev/disable_ipv6; }
		brctl delif $1 $dev
	fi
}
