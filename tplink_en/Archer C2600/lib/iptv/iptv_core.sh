...
iptv_init()
{
	...
	config_get lan_dft_cpu_port "switchport_map" "lan_cpu"
	config_get wan_dft_cpu_port "switchport_map" "wan_cpu"
	config_get WAN_PHY_PORT_SET "switchport_map" "wan"

	for i in `seq 4`; do
		config_get lan_port "switchport_map" "lan$i"
		append LAN_PHY_PORT_SET "$lan_port"
	done
	echo "WAN_PHY_PORT_SET $WAN_PHY_PORT_SET"
	echo "LAN_PHY_PORT_SET $LAN_PHY_PORT_SET"
	...
	export "LAN_CPU_PORT"="${lan_dft_cpu_port:-6}"
	export "WAN_CPU_PORT"="${wan_dft_cpu_port:-0}"
...
iptv_bridge_mode_ex()
{
	...
	for netp in $net_ports; do
		...
		switch_port=`get_lan_switch_port $pid`
		create_vdevice $LAN_PHY_IF $vid $switch_port "*" $LAN_CPU_PORT "t"
	...
	fw add 4 n postrouting_rule ACCEPT ^ { -m physdev --physdev-is-bridged }
}
...
bridge_wan()
{
	...
	[ $wan_vid -gt 0 -a -n "$ports" ] && {
		...
		for port in $ports; do
			...
			switch_port=`get_lan_switch_port $pid`
			create_vdevice $LAN_PHY_IF $lan_vid $switch_port "*" $LAN_CPU_PORT "t"
...
iptv_reset_lan()
{
	...
	for vid in $lan_dft_vid; do
		if [ $DEFALUT_MULTI_LAN_DEV == 1 ];then
			...
			switch_port=`get_lan_switch_port $vid`
			set_pvid $switch_port $vid
		else
	...
	case $mode in
		Bridge)
			for v in $viface; do
				...
				destroy_vif "$vif"
				switch_port=`get_lan_switch_port $pid`
				del_vlan_member $vid $switch_port
				del_vlan_member $vid ${LAN_CPU_PORT}
				[ $vid -ne 1 ] && delete_vlan_entry $vid
				# which case would run this branch?
				if [ $vid == ${WAN_DFT_ID} ]; then
					set_pvid $switch_port ${WAN_DFT_ID:-4094}
				else
					if [ $DEFALUT_MULTI_LAN_DEV == 1 ]; then
						# set port vid:
						# vlan1 - port1 - 1
						# vlan2 - port2 - 2
						set_pvid $switch_port $pid
					else
						set_pvid $switch_port ${LAN_DFT_ID:-1}
					fi
				fi
			done
			[ -n "$br_wan_ports" ] && {
...
iptv_stop()
{
    ...
    [ "$iptv_en" == "on" ] || iptv_is_loaded && iptv_disconnect_ifs
