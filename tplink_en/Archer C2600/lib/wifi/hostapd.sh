...
hostapd_set_bss_options() {
	...
	[ -n "$wps_possible" -a -n "$config_methods" ] && {
		...
		config_get model_url "$vif" model_url "http://192.168.1.1/"
		config_get manufacturer_url "$vif" wps_manufacturer_url "openwrt.org"
		config_get os_version "$vif" os_version "1.0"
		config_get wps_rf_bands "$vif" wps_rf_bands "ag"
		config_get wps_uuid "$vif" wps_uuid "87654321-9abc-def0-1234"
		...
		append "$var" "manufacturer_url=$manufacturer_url" "$N"
		append "$var" "model_url=$model_url" "$N"
		...
		[ -n "$uuid" ] && {
			append "$var" "uuid=$wps_uuid-$uuid" "$N"
		}
	...
	then
		case "$enc" in
			*psk*) append "$var" "wpa_key_mgmt=WPA-PSK" "$N";;
			*wpa*) append "$var" "wpa_key_mgmt=WPA-EAP" "$N";;
		esac
	fi
...
EOF
	...
	if [ -n "$wps_possible" -a -n "$config_methods" ]; then
		pid=/var/run/hostapd_cli-$ifname.pid
			hostapd_cli -i $ifname -P $pid -a /lib/wifi/tplink-update-uci -p /var/run/hostapd-$device -B
	fi
