	...
	create: function(defaults, options){
			...
			for (var index = 0; index < columns.length; index++){
				...
				switch(column.xtype){
					case "checkcolumn":
						...
						addOn +=		"</div>";
						addOn +=	"</div>";
						column.text = column.text || $.su.CHAR.GRID.SELECTED;
		...
		me.delegate("a.grid-content-btn.grid-content-btn-status", "click", function(e){
			...
			var btn = $(this),
				...
				td = $(this).closest("td.grid-content-td");
			...
			if(tar.statusValidate){
				var result = tar.statusValidate(dOld,store.data) || false;
				if(!result){
					tar.statusValidMsg.msg("show");
					return ;
				}
			}
			...
			if (/grid-content-td-(\d+)/.test(td.attr("class")))
			{
				op = me.get(0).columns[parseInt(RegExp["$1"])].op || null;
			}
		...
		for (var kndex = 0; kndex < len; kndex++){
			...
			switch (column.xtype){
				...
				case "statuscolumn":
					...
					msgInHTML +=			"<span class=\"text\">"+obj.statusValidMsgText+"</span>";
					msgInHTML +=		"</h4>";
					msgInHTML += 	"</div>";
					
					var statusValidMsg = $("<div class=\"grid-max-rules-msg\"></div>").msg({
						type: "alert",
						cls: "grid-popup-msg",
						innerHTML: msgInHTML
					});

					obj.statusValidMsg = statusValidMsg;
				...
				case "actioncolumn":
					...
					break;
				default:
					inHTML += 	"<td class=\"grid-content-td grid-content-td-"+kndex+" grid-content-td-"+column.name+" "+fst+" "+lst+"\" name=\""+column.name+"\">";
