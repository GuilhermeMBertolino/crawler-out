	...
	create: function(defaults, options){
		...
		me.each(function(i, obj){
			...
			if (this.fieldLabel !== null){
				...
				inHTML +=				"<input class=\"combobox-text "+obj.inputCls+"\" type=\"text\"  UNSELECTABLE =\"on\" tabindex=\"-1\" readonly=\"true\" value=\""+obj.noneSelectedText+"\" />";	//目前先做成readonly
		...
		container.delegate("div.combobox-wrap", "click", function(e){
			...
			if (_selectType !== "multiple"){	//单选框的情况
				//重置所有
...
//			e.stopPropagation();
	...
	setValue: function(me, value){
		...
		container.find("input.combobox-value, select.combobox-value").val(value);
	...
	disableItem: function(me, valueArray){	//实现对单个对象的启用禁用
		var me = me || this,
			container = me.combobox("getContainer"),
