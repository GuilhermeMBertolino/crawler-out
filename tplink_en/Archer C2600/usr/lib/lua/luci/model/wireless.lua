...
function Apcfg:scan_driver()
    ...
    self.uci:foreach(self.module, "wifi-device",
        function(s)
            if dev2g or (s['band'] == '5g' and not dev5g) then
                dev5g = s['.name']
            else
                dev2g = s['.name']
            end
...
function Apcfg:get_capability(secs)
    if not self.capability then
        ...
        if not cap or cap.country ~= country or cap.dev_loaded ~= "loaded" then
            ...
            if load_2g and load_5g then
                cap.dev_loaded = "loaded"
            end
...
function Apcfg:get_extinfo(secs)
    ...
    extinfo = {support_band = band, wds_guest_compatible = wds_guest, wds2g_wds5g_compatible = wds2g_wds5g,}
    return extinfo
end
...
function Apcfg:set_country(val)
    local ret = val and val:match("^[A-Z][A-Z]$")
    if ret then
        local value = self.uci:get(self.module, "acsblock2g", val) or ""
        for _, vap in pairs(self.section[CFG_HST_2G]) do
            self.uci:set(self.module, vap, "block_acs_channel", value)
        end
        local value = self.uci:get(self.module, "acsblock5g", val) or ""
        for _, vap in pairs(self.section[CFG_HST_5G]) do
            self.uci:set(self.module, vap, "block_acs_channel", value)
        end
    end
    return ret
