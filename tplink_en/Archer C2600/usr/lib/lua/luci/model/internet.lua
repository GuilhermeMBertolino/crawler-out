...
local ifaces = {"wan", "internet", "wanv6", "internetv6"}
...
function Internet:up(iface)
    ...
    if util.contains(ifaces, iface) then
...
function Internet:up_vif(iface)
	local args = {interface = iface}

	if util.contains(ifaces, iface) then
		log(nlog.UP_VIF_WAN, iface)
		self:invoke("up_vif", args)
	end
end
...
function Internet:restart(wait)
    log(nlog.RESTART)
    sys.fork_exec("/etc/init.d/network restart")
    if wait then
        -- Wait 2 seconds specially after fork network restart
        nixio.nanosleep(2, 0)
    end
end

function Internet:xxtp_status()
    local r1 = self:invoke("status", {interface = "wan"})
    local r2 = self:invoke("status", {interface = "internet"})

    if r1 == nil then
        return "disconnected"
    elseif r2 == nil then
        if r1.state == "connected" then
            return "connecting"
        else
            return r1.state
        end
    else
        if r1.state == "connected" and r2.state == "connected" then
            return "connected"
        elseif r1.state == "disconnected" and r2.state == "disconnected" then
            return "disconnected"
        elseif r1.state == "connecting" or r2.state == "connecting" then
            return "connecting"
        elseif r1.state == "disconnecting" or r2.state == "disconnecting" then
...
function Internet:status(v6)
    ...
    elseif iface == "internetv6" and result == nil then
        result = self:invoke("status", {interface = "wanv6"})
    end
...
function Internet:start_netctrl()
    self:invoke("start", {}, "network.connctl")
end

function Internet:stop_netctrl()
    self:invoke("stop", {}, "network.connctl")
end
