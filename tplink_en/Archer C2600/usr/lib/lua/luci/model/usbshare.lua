...
local sharecfglock = "/var/run/sharecfg.lck"
local usb_lock

local function lock(w)
    usb_lock = nixio.open(sharecfglock, "w", 600)
    usb_lock:flock(w and "ex" or "sh")
end
...
function apply()
    ...
    if uci_r:get(CFG_UCI, "global", "ftp") == "on" or uci_r:get(CFG_UCI, "global", "ftpex") == "on" then
