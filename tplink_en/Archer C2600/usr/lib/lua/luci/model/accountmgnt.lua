...
function get_password(username)
    ...
    uci_r:foreach(ACCCFG, "account",
        function(section)
            config_username = section["username"]
            if username == config_username then
                password = aes_decrypt(section["password"])
            end
        end
    )
    
    if config_username == "admin" and (not password or password == "") then
        reset()
        return "admin"
    end

    --if username == "admin" and (not password or password == "") then 
        --reset()
        --return "admin"
    --end
...
function update_no_encrypt(newname, newpasswd, oldname, oldpasswd)
    ...
    local b64pwd = aes_encrypt(newpasswd)
...
function reset()
    ...
    timeset.ts_commit_neglect()
    uci_r:commit(ACCCFG)
    timeset.ts_commit_normal()
