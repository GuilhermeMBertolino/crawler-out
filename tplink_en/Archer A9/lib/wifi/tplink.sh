...
wifi_fixup_mode() {
	...
	config_get_bool mumimo "$dev" mumimo 1
	for vif in $(config_get "$dev" vifs); do 
		config_get isolate "$vif" isolate "$isolate"
                config_set "$vif" vhtmubfer $mumimo
                config_set "$vif" vhtmubfee $mumimo
...
config_vap() {
	...
	config_get_bool atf "$vif" atf 
	if [ "$atf" = "1" ]; then
		iwpriv $vif commitatf 1
	else
		iwpriv $vif commitatf 0
	fi
    ...
    if [ $suite_b -ne 192 ]
    then
        echo "$suite_b bit security level is not supported for SUITE-B" >&2
        destroy_vap $ifname
        continue
    fi
    #[lixiangkui end]
	...
	case "$enc" in
		...
		wep*)
            ...
            then
                echo "With SAE/OWE enabled, wep enc is not supported" >&2
                destroy_vap $ifname
                continue
            fi
            #[lixiangkui end]
        ...
        ccmp*|gcmp*)
            ...
            then
                ...
                then
                    echo "key/sae_password are NULL" >&2
                    destroy_vap $ifname
                    continue
                fi
            fi
...
wifi_smart() {
	...
	config_get_bool smart_enable smart smart_enable 0
	if [ "$smart_enable" -ne "1" ]; then
		[ -f "$SMARTC_RUN_FILE" ] && wifi_smart_stop
		return
	fi
	wifi_smart_start
}
...
wifi_vap() {
	...
	#Restart Smart connect temporarily, as lbd seems to have some problem in dealing with ssid change event
...
wifi_radio() {
	...
	for dev in ${1:-$DEVICES}; do
		...
		[ "$action" = "reload" ] && {
			...
			#mask 5G 4th antenna off
			[ "$band" = "5g" ] && {
				iwpriv $dev txchainmask 0x7
				iwpriv $dev rxchainmask 0x7
			}
...
wifi_reload() {
	...
	wifi_radio "$1" "txchainmask rxchainmask AMPDU ampdudensity !AMSDU AMPDULim AMPDUFrames AMPDURxBsize !bcnburst:set_bcnburst=0 \
		...
		!led_enable promisc block_interbss aggr_burst set_pmf txbf_snd_int=100 mcast_echo obss_rssi_th=35 obss_rx_rssi_th=35 discon_time=10 reconfig_time=60" \
		&>$STDOUT
