...
hostapd_set_bss_options() {
	...
	[ -n "$wps_possible" -a -n "$config_methods" ] && {
		...
		device_name=$(getfirm HOSTNAME_NO_BLANK) && [ -z "$device_name" ] && device_name="OpenWrt AP"
		...
		serial_number=$(getfirm HOSTNAME_BLANK) && [ -z "$serial_number" ] && serial_number="12345"
		...
		[ "$wps_label" -gt 0 ] && append "$var" "ap_pin=$wps_pin" "$N"
		...
		then
			...
			case "$enc" in
				...
				*psk*)
					if [ "${ieee80211w}" -eq 0 ]
					then
						config_set "$vif" ieee80211w 1
						config_set "$vif" sae_require_mfp 1
...
EOF
	...
	if [ $wps_vap_tie_dbdc -ne 0 ]; then
		...
		/bin/nice -n -10 /usr/sbin/hostapd -P /var/run/wifi-$ifname.pid -B /var/run/hostapd-$ifname.conf
		touch /var/run/hostapd-$ifname.lock
		if [ -n "$wps_possible" -a -n "$config_methods" ]; then
			pid=/var/run/hostapd_cli-$ifname.pid
			/bin/nice -n -10 /usr/sbin/hostapd_cli -i $ifname  -P $pid -a /lib/wifi/wps-hostapd-update-uci -p /var/run/hostapd-$device -B
		fi
	fi
}
