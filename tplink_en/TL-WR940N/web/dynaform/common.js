...
function macverify(mac_string) {
	...
	if (lcMac == "ff-ff-ff-ff-ff-ff") {
		alert(js_broadcast_mac = "The MAC address is a broadcast MAC address, please input another mac address!");
		return false;
	}
	...
	if (mac_string.length != 17) {
		alert(js_bad_mac_format = "The MAC address format is invalid! The valid format is 'XX-XX-XX-XX-XX-XX' (X is any hexadecimal digit).");
		return false;
	}
...
var pureApMode = 7; /* pure ap */
var repeaterMode = 6; /* Repeater */
var clientMode = 3; /* Client */
	...
	for (i = startIdx; i <= endIdx; i++) {
		...
		switch (i) {
			...
			case wiredIdx:/*非AP路由器去掉这一步*/
				if ((sysMode != apMode) && (sysMode != pureApMode) && (sysMode != repeaterMode) && (sysMode != clientMode)) {
					nextLoop = 1;
				}
				break;
			default:
...
serialize = function (form,filter) {
	...
	for(var i=0;i<form.elements.length;i++){
		...
		if (!name || disabled || !type || (/^reset|image$/i.test(type)) || (/^checkbox|button|radio$/i.test(type) && !ele.checked) || (filter!==undefined && type === filter)) {
			continue;
		}
		...
		if (type !== 'select-multiple') {
			...
			var optionCon=ele.querySelectorAll('option');
			for(var i=0;i<optionCon.length;i++){
				var option=optionCon[i];
				var optionValue = encodeURIComponent(option.value);
				if (option.selected) {
					if (objFormData[name]) {
...
function submitForm(form, document, type) {
	var target = transferURL(form.getAttribute("action"));
	var encrypt = getEncrypt();
	var encryptor = encrypt.encryptManager.getEncryptor();
	var data = encryptor.dataEncrypt(serialize(form,type));
	doSubmit_gdpr({target:target,data: data},document);
}
function doSubmit_gdpr(opts,document){
	var method = opts.data===null?"GET":"POST"
	...
	form.attributes["enctype"].value="multipart/form-data";
	document.forms[document.forms.length - 1].submit();
}
function createDom(name, opts) {
...
function handleString(str) {
	...
	if (str.indexOf("new") !== -1) {
		if (str.toLowerCase().indexOf("array") !== -1) {
			var start = str.indexOf("(") + 1;
			var end = str.lastIndexOf(")");
			var content = str.slice(start, end);
			var resultArr = [];
			var tmp = content.split(",");
			for (var i = 0; i < tmp.length; i++) {
				if (tmp[i].indexOf('"') !== -1 || tmp[i].indexOf("'") !== -1) {
					tmp[i] = tmp[i].slice(1, -1);
				}
				if (tmp[i].indexOf("true") !== -1 || tmp[i].indexOf("false") !== -1 || tmp[i] - 0 === tmp[i] - 0) {
					if (tmp[i] === "" || tmp[i] === " " || tmp[i] === null || tmp[i] === undefined) {
						resultArr[i] = tmp[i];
					} else {
						resultArr[i] = JSON.parse(tmp[i]);
					}
				} else {
					resultArr[i] = tmp[i];
				}
...
function parse(data) {
	...
	var encrypt = getEncrypt();
	var encryptor = encrypt.encryptManager.getEncryptor();
	var data = encryptor.dataDecrypt(data);
...
function parseString(data){
	...
	for (var i = 0; i < dataArr.length; i++) {
		...
		dataArr[i][0] = dataArr[i][0].replace("var", "").trimStr();
		window[dataArr[i][0]] = handleString(dataArr[i][1]);
	}
}
function transferURL(url){
	var currentHref=location.href;
	var sessionId=localStorage.getItem("sessionId");
	if(currentHref.indexOf(sessionId)!==-1){
		return url;
	}
	var urlArr=url.split("/");
	var sessionId=localStorage.getItem("sessionId");
	if(urlArr.length!==1){
		urlArr.splice((urlArr.length-2),0,sessionId);
		url=urlArr.join("/");
	}else {
		url="../"+sessionId+"/userRpm/"+url;
	}
	return url;
}
function handleURL(url, document) {
	if (url.indexOf("?") !== -1) {
		var urlArr = url.split("?");
		var finalURL = urlArr[0];
		var oriData = urlArr[1];
	} else {
		var finalURL = url;
		var oriData = null;
	}
	var target = transferURL(finalURL);
	var encrypt = getEncrypt();
	var encryptor = encrypt.encryptManager.getEncryptor();
	var data = oriData === null ? null : encryptor.dataEncrypt(oriData);
	doSubmit_gdpr({ target: target, data: data }, document);
}
function getElesByName(doc, Name) {
	var i = doc.getElementsByName(Name)
	if (i > 0) {
		return i;
	} else {
		var aEle = doc.getElementsByTagName('script');
		var arr = [];
