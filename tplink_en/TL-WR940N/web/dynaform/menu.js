...
//	directory level: 0: directory has no branches;  1: directory level 1;  2:level 2;   and so on
	...
	"WlanNetworkRpm_RootAP",	0,		2,			str_menu.wlanNetwork_RootAP,
	"WlanNetworkRpm_ExtNetwork",	0,	2,			str_menu.wlanNetwork_ExtNetwork,
...
/*end add*/
	...
	"LedCtrlRpm",				0,		2,  	    str_menu.ledCtrl,
	...
	"SysRebootAutoRpm",				0,		2,  	    str_menu.sysRebootAuto,
	...
	"WlanThroughputRpm",0,0,'wlanThrough'
);                            
...
function menuDisplay()
{
	...
	for (var n = 0; n < menuList.length; n +=4  ){
		if(menuList[n+1]==1){
			menuListTmp=menuListTmp.concat(menuList.slice(n,n+4));
		}
	}
	...
	for (var n = 0; n < menuList.length; n +=4  )
	{
		...
		else
		{
			className = "dot2";
			display = "none";
			if(menuList[n+6]<menuList[n+2]){
				className = "\"dot2 dot2-last\"";
			}
		}
		var power = (menuList[n+2] > 0)?(menuList[n+2] - 1):0;
		document.write('<ol id=ol'+i+' class='+className+' style="display:'+display+'; background-position:2px;"><li><a id=a'+i+' href="../userRpm/'+menuList[n]+'.htm" target=mainFrame class=L1 onClick="doClick('+i+');"><span>'+((power > 0)? '- ':'')+menuList[n+3]+'</span></a></li></ol>');
