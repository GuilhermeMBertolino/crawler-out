...
{
    "workingMode": "maxis_1",
    "displayName": n_str.ethIptv.t_maxis_mode_1,
    "vlanSetting": {
        "internet": {
            "enable": true,
            "vlanID": 621,
...
{
    "workingMode": "maxis_2",
    "displayName": n_str.ethIptv.t_maxis_mode_2,
    "vlanSetting": {
        "internet": {
            "enable": true,
            "vlanID": 11,
...
{
    "workingMode": "maxis_celcom_west_biz",
    "displayName": "Malaysia-CELCOM WEST(BIZ)",
    "vlanSetting": {
        "internet": {
            "enable": true,
            "vlanID": 626,
    ...
    "lan_mode": ["internet", "internet", "internet", "ipphone"],
    "lan_edit": [false, false, false, false]
},
{
    "workingMode": "maxis_celcom_west_home",
    "displayName": "Malaysia-CELCOM WEST(HOME)",
    "vlanSetting": {
        "internet": {
            "enable": true,
            "vlanID": 623,
...
{
    "workingMode": "maxis_celcom_east_biz",
    "displayName": "Malaysia-CELCOM EAST(BIZ)",
    "vlanSetting": {
        "internet": {
            "enable": true,
            "vlanID": 601,
...
{
    "workingMode": "maxis_celcom_east_home",
    "displayName": "Malaysia-CELCOM EAST(HOME)",
    "vlanSetting": {
        "internet": {
            "enable": true,
            "vlanID": 101,
...
{
    "workingMode": "maxis_digi_tm",
    "displayName": "Malaysia-Digi-TM",
    "vlanSetting": {
        "internet": {
            "enable": true,
            "vlanID": 629,
...
{
    "workingMode": "maxis_digi_time",
    "displayName": "Malaysia-Digi-TIME",
    "vlanSetting": {
        "internet": {
            "enable": true,
            "vlanID": 221,
...
{
    "workingMode": "maxis_digi_digi",
    "displayName": "Malaysia-Digi-DIGI",
    "vlanSetting": {
        "internet": {
            "enable": true,
            "vlanID": 1001,
...
{
    "workingMode": "maxis_digi_cts",
    "displayName": "Malaysia-Digi-CTS",
    "vlanSetting": {
        "internet": {
            "enable": true,
            "vlanID": 150,
...
{
    "workingMode": "maxis_digi_allo",
    "displayName": "Malaysia-Digi-ALLO",
    "vlanSetting": {
        "internet": {
            "enable": true,
            "vlanID": 1600,
...
{
    "workingMode": "maxis_digi_sacofa",
    "displayName": "Malaysia-Digi-SACOFA",
    "vlanSetting": {
        "internet": {
            "enable": true,
            "vlanID": 1000,
...
if (1) {
    default_mode_data.splice(4, 0, {
        "workingMode": "deTele",
        "displayName": "Deutsche Telekom",//n_str.ethIptv.t_maxis_mode_1,
        "vlanSetting": {
            "internet": {
                "enable": true,
                "vlanID": 7,
...
var services = new Array(
{
    ...
    displayNameDe: n_str.ethIptv.t_internet_vlan + "/" + n_str.ethIptv.t_iptv_vlan,
...
function initLanOption(obj)
{
	...
	for(var lanIdx in lanset)
	{
        ...
        var sel = $.c("select");
        if (obj.workingMode === "deTele") {
            sel.setAttribute("class","M");
        } else {
		...
		for (var servIdx in services)
	    {
            ...
            if (obj.workingMode === "deTele") {
                opt.text = services[servIdx].displayNameDe || services[servIdx].displayName;
            } else {
...
function initISPService()
{
	for (var servIdx in services)
    {
        ...
        var select = $.c("select");
        select.setAttribute("id", services[servIdx]["name"] + "_vlanid_select");
        select.setAttribute("class", "S nd");
        select.setAttribute("onChange", "vlanIDOnchange(event)");
        ...
        else
        {
            for(var idx = 0; idx < ispService.length; idx++)
            {
                if (services[servIdx]["name"] == ispService[idx].name)
                {
                    ...
                    vlanIDSel = $.id(services[servIdx]["name"] + "_vlanid_select");
                    if (vlanIDSel) {
                        vlanIDSel.value = ispService[idx].vlanID;
                        if ($.id("iptv_mode").value === "deTele" && services[servIdx]["name"] === "iptv") {
                            changeLanText(ispService[idx].vlanID);
                        }
                    }
...
function showDefaultData(obj)
{
    ...
    for(var servIdx in services)
    {
        ...
        vlanSettings = obj.vlanSetting[services[servIdx].name];
        vlanIDSel = $.id(services[servIdx]["name"] + "_vlanid_select");
        vlanIDText = $.id(services[servIdx]["name"] + "_vlanid");
        if (typeof vlanSettings.vlanID !== "number") {
            vlanIDSel.innerHTML = "";
            for (var i = 0, len = vlanSettings.vlanID.length; i < len; i++) {
                vlanIDOpt = $.c("option");
                vlanIDOptVal = vlanSettings.vlanID[i];
                vlanIDOpt.setAttribute("value", vlanIDOptVal);
                vlanIDOpt.text = vlanIDOptVal;
                vlanIDSel.add(vlanIDOpt);
            }
            $.removeClass(vlanIDSel, "nd");
            $.addClass(vlanIDText, "nd");
        } else {
            vlanIDText.value = vlanSettings.vlanID;
            $.addClass(vlanIDSel, "nd");
            $.removeClass(vlanIDText, "nd");
        }
        $.id(services[servIdx]["name"] + "_vlanprio").value = vlanSettings.priority;
        if (vlanSettings.vlanIdEditEnable == false)
        {
        	$.addClass($.id("div_serv_" + services[servIdx].name), "vidDisable");
        	vlanIDText.disabled = true;
        }
        else
        {
...
function getDefaultCfg(mode){
    ...
    for (var servIdx in services)
    {
        settingName = services[servIdx].name;
        vlanSettings = defaultVlanCfg.vlanSetting[settingName];
        result["vlanSetting"][settingName] = {};
        if (mode === "deTele" && settingName === "iptv" && $.id(settingName + "_vlanid_select").value == 7) {
            result["vlanSetting"][settingName].enable = false;
        } else {
            result["vlanSetting"][settingName].enable = vlanSettings.enable;
        }
        result["vlanSetting"][settingName].vlanID = vlanSettings.vlanID;
        result["vlanSetting"][settingName].priority = vlanSettings.priority;
        result["vlanSetting"][settingName].extendID = vlanSettings.extendID;
        result["vlanSetting"][settingName].untag = vlanSettings.untag;
        result["vlanSetting"][settingName].show = vlanSettings.show;
        result["vlanSetting"][settingName].lanPort = vlanSettings.lanPort ? vlanSettings.lanPort : 0;
    }
...
function doCollectData(obj)
{
    ...
    for(var servIdx in services)
    {
        ...
        if (services[servIdx].showEnableOption == true)
        {
            vlanSettings.enable = $.id(services[servIdx].name + "vlanEnable").checked;
        }

        vlanSettings.vlanID = typeof vlanSettings.vlanID === "number" ? $.id(services[servIdx].name + "_vlanid").value : $.id(services[servIdx].name + "_vlanid_select").value;
        vlanSettings.priority = $.id(services[servIdx].name + "_vlanprio").value;

        if (services[servIdx].showTagOption == true)
        {
            vlanSettings.untag = $.id(services[servIdx].name + "tagEnable").checked ? 0: 1;
        }
    }
    for(var lanIdx in lanset)
    {
...
function vlanIDOnchange(event) {
    var value = (event.target || event.srcElement).value;
    if (value == 7) {
        $.id("lan4").value = "internet";
        changeLanText(7);
    } else {
        $.id("lan4").value = "iptv";
        changeLanText();
...
function changeLanText(value) {
    if (value == 7) {
        $.id("lan1_internet").text = n_str.ethIptv.t_internet_vlan + "/" + n_str.ethIptv.t_iptv_vlan;
        $.id("lan2_internet").text = n_str.ethIptv.t_internet_vlan + "/" + n_str.ethIptv.t_iptv_vlan;
        $.id("lan3_internet").text = n_str.ethIptv.t_internet_vlan + "/" + n_str.ethIptv.t_iptv_vlan;
    } else {
        $.id("lan1_internet").text = n_str.ethIptv.t_internet_vlan;
        $.id("lan2_internet").text = n_str.ethIptv.t_internet_vlan;
        $.id("lan3_internet").text = n_str.ethIptv.t_internet_vlan;
    }
}