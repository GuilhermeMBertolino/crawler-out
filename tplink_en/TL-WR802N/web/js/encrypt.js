...
(function(){
	$.rsa = $.rsa || {};
	$.rsa.encrypt = function (val, nn, ee, rsaBits, flag){	//n, e, 
		...
		//calculate  rsa value
		function calculateRsaValue(val, strEnlen, flag)
		{
			...
			var result = rsaObj.encrypt(val, flag);
			//var result = linebrk(res, 64);

			if(result.length != strEnlen ){
				//$.su.encrypt(n,e,val);
				var l = Math.abs(strEnlen - result.length);
				for (var i = 0; i < l; i++){
					result = "0" + result;
				};
			}
		...
		function sEncrypt(data, RSA_BIT, flag){
			/**
			**	RSA_BIT	STR_EN_LEN	STR_DE_LEN	STR_DE_11PADDING_LEN
			**	512		128			64			64-11=53
			**	1024	256			128			128-11=117
			**
			**/
			var STR_EN_LEN   = RSA_BIT/4 ;
			var STR_DE_LEN   = RSA_BIT/8 ;
			
			var STR_DE_LEN_11PADDING   =  STR_DE_LEN- 11 ;
			
			var step = flag ? STR_DE_LEN_11PADDING : STR_DE_LEN;
			...
			while(startlength < data.length)
			{
				endlength = endlength < data.length ? endlength : data.length;
				tmp=data.substring(startlength, endlength);
				tmpen += calculateRsaValue(tmp, STR_EN_LEN, flag);
				startlength += step;
				endlength += step;
			}
		...
		var result = sEncrypt(val, rsaBits || 512,  (flag != null )?  flag :1);
