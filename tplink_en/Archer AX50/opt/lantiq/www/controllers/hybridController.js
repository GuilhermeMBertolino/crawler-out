				...
				swapArrayElements(finalobjects,2,3);
                    ...
                    angular.forEach(finalobjects, function (key, keyindex) {
                        ...
                        angular.forEach($scope[key.replace(/[^a-zA-Z0-9_-]/g, '')], function (value, key1) {
                            newpost += "&" + key1 + "=" + $sanitize(encodeURIComponent(value)) + "";
                        })
                        newpost += "&";
                    })
                }
...
//                        $route.reload();
    ...
    $scope.DefaultValuesFormApply = function ($event) {
            ...
            angular.forEach(objectParameterList, function (object) {
                ...
                if (objectName !== "temp") {
                    ...
                    angular.forEach(parameters, function (parameter) {
                        var currentElement = angular.element('[ng-model="' + objectName.replace(/\./g, '') + '.' + parameter + '"]');
                        if (currentElement.length > 0) {
                            var elementType = currentElement[0].getAttribute('type');

                            if (elementType === null) {
                                var elementType = currentElement[0].type;
                            }
                            if ((elementType !== null && elementType !== undefined) && elementType !== "Button") {
                                ...
                                if ($route.current.pathParams.param2 === 'home_wifi_ire' && parameter == "Onboarding") {
                                    if ($scope[objectName.replace(/\./g, '')][parameter] == true) {
                                        post = post + "false";
                                    }
                                } else {
                                    paramValue = $scope[objectName.replace(/\./g, '')][parameter] == undefined ? "" : $scope[objectName.replace(/\./g, '')][parameter];
