	...
	do
		...
		if [ "$file" = "card0" ] || [ "$file" = "card1" ]; then
			...
			if [ ! -z "$fw_files_list" ]; then
				echo "### Compress from $trace_out" > $TRACE_OUT
				for fw_file in $fw_files_list
				do
					fw_dump_file=/$dump_file/$fw_file
					echo "### Now Compressing: $fw_dump_file" > $TRACE_OUT
					gzip -c $fw_dump_file > $dump_dir/${fw_file}.gz
				done
			else
				echo "### FW directory $trace_out is empty ###" > $TRACE_OUT
			fi
			...
			if [ -e $dump_file ]; then
				...
				echo "###[  ${dump_file##*/}  ]###-> doesn't exist skip it" > $TRACE_OUT
			fi
		fi
	done
