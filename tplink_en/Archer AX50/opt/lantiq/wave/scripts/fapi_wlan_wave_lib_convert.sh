...
convert_ht_capab()
{
	...
	channel_width=""
	if [ "$auto_channel" = "1" ] && [ "$channel_bonding" != "20MHz" ]
	then
		channel_width="[HT40+]"
	elif [ "$channel_bonding" != "20MHz" ]
	then
...
{
	# Define local parameters
	local interface_index interface_name is_wave500 is_wave600 channel_bandwidth phy_name driver_ldpc driver_tx_stbc driver_rx_stbc driver_no_tx_stbc driver_no_rx_stbc \
	...
	vht_txop_ps ampdu_max_len_exp bf_mode
	...
	[ "$chip_id" = "$GEN5B_CHIP_ID" ] && is_wave500="yes"
	[ "$chip_id" = "$GEN6_CHIP_ID" ] && is_wave600="yes"
	...
	then
		...
		# SU-BEAMFORMEE GEN5 only
		[ "$is_wave500" = "yes" ] && [ "$su_beamformee" = "1" ] && vht_capab_value="$vht_capab_value[SU-BEAMFORMEE]"
		
		# MU-BEAMFORMER GEN5/gen6b(is_gen6b) enable & GEN6a(is_wave600) disable 
		[ "$is_wave600" != "yes" ] && vht_capab_value="${vht_capab_value}[MU-BEAMFORMER]"
		
		[ "$is_wave500" = "yes" ] && vht_capab_value="${vht_capab_value}[MU-BEAMFORMEE]"
	fi
	...
	if [ "$channel_bandwidth" = "160MHz" ] || [ "$channel_bandwidth" = "Auto" ]; then
		...
		[ "$is_wave600" = "yes" ] && vht_capab_value="${vht_capab_value}[VHT160]"
	fi
...
# Read DB values for he capabilities (ieee80211ax draft3.0) and create string for hostapd.
# Flags set in the he_capab are:
# [PLUS_HTC_HE_SUPPORT]
...
#[4X_HE_LTF_AND_08US_GI_SUPPORT_FOR_HE_PPDUS]
#[TRIGGERED_SU_BEAMFORMING_FEEDBACK]
#[TRIGGERED_CQI_FEEDBACK]
#[SU_PPDU_AND_HE_MU_PPDU_WITH_4X_HE_LTF_AND_08US_GI]
#[EXT-NSS-BW1] or [EXT-NSS-BW2] or [EXT-NSS-BW3]
#[SU_BEAMFORMEE_CAPABLE]
convert_he_capab()
{
	local interface_index interface_name he_capab_value plus_htc_he_support ack_enabled_aggregation_support a_msdu_in_a_mpdu_support device_class \
	ppe_threshold_present ldpc_coding_in_payload su_beamformer_capable mu_beamformer_capable \
	triggered_su_beamforming_feedback triggered_cqi_feedback su_ppdu_and_he_mu_ppdu_with_4x_he_ltf_and_08us_gi ext_nss_bwx \
	index is_wave600 chip_id
	...
	plus_htc_he_support=`db2fapi_convert boolean HePlusHtcHeSupport $interface_index`
	[ "$plus_htc_he_support" = "1" ] && he_capab_value="[PLUS_HTC_HE_SUPPORT]"
	ack_enabled_aggregation_support=`db2fapi_convert boolean HeAckEnabledAggregationSupport $interface_index`
	[ "$ack_enabled_aggregation_support" = "1" ] && he_capab_value="$he_capab_value[ACK_ENABLED_AGGREGATION_SUPPORT]"
	a_msdu_in_a_mpdu_support=`db2fapi_convert boolean HeAMsduInAMpduSupport $interface_index`
	[ "$a_msdu_in_a_mpdu_support" = "1" ] && he_capab_value="$he_capab_value[A_MSDU_IN_A_MPDU SUPPORT]"
	device_class=`db2fapi_convert boolean HeDeviceClass $interface_index`
	[ "$device_class" = "1" ] && he_capab_value="$he_capab_value[DEVICE_CLASS]"
	ppe_threshold_present=`db2fapi_convert boolean HePpeThresholdPresent $interface_index`
	[ "$ppe_threshold_present" = "1" ] && he_capab_value="$he_capab_value[PPE_THRESHOLD_PRESENT]"
	ldpc_coding_in_payload=`db2fapi_convert boolean HeLdpcCodingInPayload $interface_index`
	[ "$ldpc_coding_in_payload" = "1" ] && he_capab_value="$he_capab_value[LDPC_CODING_IN_PAYLOAD]"
	su_beamformer_capable=`db2fapi_convert boolean HeSuBeamformerCapable $interface_index`
	[ "$su_beamformer_capable" = "1" ] && he_capab_value="$he_capab_value[SU_BEAMFORMER_CAPABLE]"
	# MU-BEAMFORMER GEN5/gen6b(is_gen6b) enable & GEN6a(is_wave600) disable
	if [ "$is_wave600" != "yes" ]; then
		mu_beamformer_capable=`db2fapi_convert boolean HeMuBeamformerCapable $interface_index`
		[ "$mu_beamformer_capable" = "1" ] && he_capab_value="$he_capab_value[MU_BEAMFORMER_CAPABLE]"
	fi
	triggered_su_beamforming_feedback=`db2fapi_convert boolean HeTriggeredSuBeamformingFeedback $interface_index`
	[ "$triggered_su_beamforming_feedback" = "1" ] && he_capab_value="$he_capab_value[TRIGGERED_SU_BEAMFORMING_FEEDBACK]"
	triggered_cqi_feedback=`db2fapi_convert boolean HeTriggeredCqiFeedback $interface_index`
	[ "$triggered_cqi_feedback" = "1" ] && he_capab_value="$he_capab_value[TRIGGERED_CQI_FEEDBACK]"
	su_ppdu_and_he_mu_ppdu_with_4x_he_ltf_and_08us_gi=`db2fapi_convert boolean HeSuPpduAndHeMuPpduWith4xHeLtfAnd08usGi $interface_index`
	[ "$su_ppdu_and_he_mu_ppdu_with_4x_he_ltf_and_08us_gi" = "1" ] && he_capab_value="$he_capab_value[SU_PPDU_AND_HE_MU_PPDU_WITH_4X_HE_LTF_AND_08US_GI]"
	for index in 1 2 3
	do
		ext_nss_bwx=`db2fapi_convert boolean HeExtNssBw$index $interface_index`
		[ "$ext_nss_bwx" = "1" ] &&  break
	done
	[ "$ext_nss_bwx" = "1" ] && he_capab_value="$he_capab_value[EXT-NSS-BW$index]"
...
convert_fw_recovery()
{
	...
	fw_recovery="${fw_recovery} ${recovery_timer} ${recovery_num_of_dumps}"
...
convert_txop_enbale()
{
	...
	[ "$chip_id" = "$GEN6_CHIP_ID" ] && sta_id=511
...
{
	# Define local parameters
	local interface_index sFixedRateCfg station_index auto_rate band_width bw phy_mode phym nss mcs cp_mode chip_id
	...
	[ "$chip_id" = "$GEN6_CHIP_ID" ] && station_index=511
	...
	sFixedRateCfg="$station_index $auto_rate $bw $phym $nss $mcs $cp_mode"
...
convert_aggregation_config()
{
	...
	[ "$window_size" = "Default" ] && window_size="0"
...
convert_static_planner_common()
{
	...
	local interface_index sMuStaticPlann_common txop_com_max_tx_op_duration sequence_type txop_com_start_bw_limit dl_com_phases_format		\
		...
		rcr_com_he_sig_a_spatial_reuse rcr_com_stbc operation_mode
