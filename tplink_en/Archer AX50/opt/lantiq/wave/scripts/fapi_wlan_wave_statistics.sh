...
statistics() {
	cmd="mtdump $wave $STATISTICS -r /tmp/mhi_stats_temp -f $mtidlcpath/mtdump_$wave.mtidlc"
	#echo $cmd
	if [ $ID != "NULL" ]
	then
					cmd="$cmd | grep \" \[$ID\]\""
...
then
	...
	vendor_name="puma"
	echo "fapi_wlan_wave_statistics.sh: TODO: add support for puma in script" > $TRACE_OUT
	exit 0
fi
...
count=1
type_to_extract=""
mtidlcpath="/opt/${vendor_name}/wave/images"
print_desc_or_all="0"
shift 1
# init-END
...
while true; do
	...
	cmd="rm -f /tmp/mhi_stats_temp; cp -f /proc/net/mtlk/$wlan/mhi_stats /tmp/mhi_stats_temp; chmod 777 /tmp/mhi_stats_temp;"
	eval $cmd
