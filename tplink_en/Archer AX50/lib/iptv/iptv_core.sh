	...
	[ $iptv_vid -gt 0 -a $mcast_vid -gt 0 -a -n "$ports" ] && {
		...
		for wanif in $WAN_PHY_IF; do
			...
			if [ $index -eq 1 ]; then
				...
				iptvname="iptv_wan${index}"
				mcastname="mcast_wan${index}"
			fi
			create_vdevice_interface "$iptvname" "$wanif" $iptv_vid
			create_vdevice_interface "$mcastname" "$wanif" $mcast_vid

			tmpvif="${wanif}.${iptv_vid}"
			append env_vif "$tmpvif"
			append vifaces "${wanport}-${tmpvif}"
		...
		index=1
		for port in $ports; do
			lan_vid=$(get_vid_by_port $port)
			tmpvif=$(eval "echo \"${ifaces}\" | awk '{print \$${index}}'")
			[ -z "$tmpvif" ] && continue

			log "mcast_vlan_do_ex port($port) vid($lan_vid)"
			append lan_vid_map $lan_vid
			append env_port $port
			append env_vif "${tmpvif}"
			append vifaces "${pid}-${tmpvif}"
		done
...
iptv_notify_internet_vlan()
{
	local iptvc_setvalue=""

	[ -n "$(which iptvc)" ] || return

	if [ "$iptv_enable" = "off" ]; then
		iptvc_setvalue="0:$WAN_DFT_ID"
	else
		if [ "$iptv_mode" = "Bridge" ]; then
			iptvc_setvalue="0:$WAN_DFT_ID"
		else
			if [ "$internet_tag" = "on" ]; then
				iptvc_setvalue="1:$internet_vid"
			else
				iptvc_setvalue="0:$internet_vid"
			fi
		fi
	fi

	iptvc -m notify_internet_vlan -c set "$iptvc_setvalue"
}
