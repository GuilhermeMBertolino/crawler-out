...
$.su.func.setIframeSrc = function(module){
	if(module){
		var url = $.su.url.cloudOrigin + "/cloud_ui_newVI/pages/device/index.html?module=" + module;
...
$.su.func.tokenRead = function(module, tryTime, proxy){
    proxy.read({},function(data){
        $.su.url.token = data.token;
        if(!$.su.url.token && ++tryTime < 3){
...
$.su.func.getToken = function(module, updateFlag, hasStok){//login.html don't have stok,means it's proxy is different with index.html
    $.su.url = $.su.url || {};
    var proxyToken;
    if(hasStok === false){
    	proxyToken = new $.su.Proxy({
	        url: $.su.url("/login?form=get_token")
	    });
    }else{
	    proxyToken = new $.su.Proxy({
...
$.su.func.getDeviceInfo = function(hasStok){//login.html don't have stok,means it's proxy is different with index.html
    ...
    if($.su.deviceInfo.cloudUserName){
...
$.su.cloudErrorCode = $.su.cloudErrorCode || [
	...
	"E22007","E22008","E22010","E50101","E50102","E50103","E50111",
