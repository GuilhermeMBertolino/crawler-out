...
enable_mac80211() {
	...
	for vif in $vifs_mac80211; do
		...
		case "$mode" in
			...
			sta)
				...
				# fix 4addr support
				[ "$wds" -gt 0 ] && iw dev "$ifname" set 4addr on
	...
	[ -n "$start_hostapd" ] && {
		local try_count=0
		while true; do
			hostapd -P /var/run/wifi-$phy.pid -B /var/run/hostapd-$phy.conf && break
		...
		(
			sleep 2
			local vif_type=$(iw dev $vif info | grep type | awk '{print $2}')
			[ managed = "$vif_type" ] && {
				[ -n "$RESTART_HOSTAPD_COUNT" ] || RESTART_HOSTAPD_COUNT=1
				[ "$RESTART_HOSTAPD_COUNT" -ge 5 ] && exit
				export RESTART_HOSTAPD_COUNT=$((RESTART_HOSTAPD_COUNT+1))
				wifi radio "$device"
			}
		) &
		#sleep 2
