...
wifi_fixup_led()
{                                                                                     
        type ledgeneral_onoff &>/dev/null || return             
        type ledpm_onoff &>/dev/null || return        
                                                   
        local gnr_en=$(ledgeneral_onoff)
        #echo "just a test!!!" >/dev/console
                                                             
        #echo $gnr_en >/dev/console                          
                                                       
        if [ "$gnr_en" = "off" ]; then                         
               config_set "$1" led_enable off                
        else                                                   
                                                                                           
         local en=$([ -f /tmp/ledpm_enable ] && ledpm_onoff)                                                         
         if [ "$en" = "on" ]; then                                                         
                 config_set "$1" led_enable off                                       
         else                                                   
...
wifi_fixup_config() {
	...
	ls -d /sys/class/net/${brname} &>$STDOUT || for brname in $(cd /sys/class/net && ls -d br* 2>$STDOUT); do break; done
	...
	for dev in ${1:-DEVICES}; do
		...
		[ "$(config_get "$dev" type)" = "mac80211" ] && wifi_fixup_config_wigig "$dev"
	done
}

wifi_fixup_config_wigig() {
	local vifs vif macaddr
	config_get vifs "$1" vifs
	config_get macaddr "$1" macaddr
	[ -n "$macaddr" ] && config_set "$1" macaddr "${macaddr//-/:}"
	local removed vif_ap
	for vif in $(config_get "$1" vifs); do
		[ "$(config_get "$vif" enable)" = "off" ] && {
			list_remove vifs "$vif"
			removed=y
			continue
		}
		[ "$(config_get "$vif" mode)" = "ap" ] && vif_ap="$vif"
	done
	[ "${removed}" != "y" ] && list_remove vifs "$vif_ap"
	config_set "$1" vifs "$vifs"
}
...
config_vap_mode() {
	local cfg cfgs="rts frag pureg puren pure11ac deny11ab chwidth disablecoext vht_11ng vht_11ng_intop"
...
config_vap_vlan() {
	local vlanid=1 gvlan=2
	for brname in $(cd /sys/class/net && ls -d br* 2>$STDOUT); do break; done
	for port in $(brctl show "$brname" | grep eth | cut -f 6-8); do
		brctl setifvlan "$brname" "$port" "$vlanid" 1
	done
	for dev in $DEVICES; do
		for vif in $(config_get "$dev" vifs); do 
			local ifname=$(config_get "$vif" ifname)
			[ -n "$ifname" ] && vif="$ifname"
			if [ -d /sys/class/net/$vif ]; then
				...
				vlanid=1
				[ "$guest" = "1" ] && [ "$isolate" = "1" ] && gvlan=$((gvlan + 1))
				[ "$access" = "0" ] && fw_action="block" && vlanid="$gvlan"
...
create_vap() {
	...
	local cfg="!shortgi !disablecoext chwidth !wds TxBFCTL bintval !countryie !uapsd=1 powersave !ant_ps_on ps_timeout \
			...
			encap_type decap_type rawsim_txagr clr_rawsim_stats rawsim_debug osen"
...
kill_pid() {
	for f in wifi; do
		...
		while [ -f "/var/run/$f-$1.pid" -a "$n" -ge 0 ]; do 
...
wifi_vap() {
	...
	for vap in $1; do
		...
		[ "$(config_get "$device" type)" = "mac80211" ] && {
			wifi_radio_wigig "$device"
			continue
		}
...
wifi_country() {
	...
	devs="${1:-$DEVICES}"
	for dev in $devs; do
		[ "$(config_get "$dev" type)" = "mac80211" ] && {
			iw reg set "$(config_get "$dev" country)"
			list_remove devs "$dev"
		}
	done
	wifi_mode "$devs" "country" &>$STDOUT
}
...
wifi_radio_wigig() {
	...
	config_get_bool disabled_all "$1" disabled_all 0
	for vif in $vifs; do ifconfig "$vif" down &>$STDOUT; done
	if [ "$disabled" != "1" -a "$disabled_all" != "1" ]; then
		enable_mac80211 "$1"
	fi
}

wifi_radio() {
	local dev action="update"
	[ -n "$2" ] && action="reload"
	wifi_led "$1"
	for dev in ${1:-$DEVICES}; do
		[ "$(config_get "$dev" type)" = "mac80211" ] && {
			wifi_radio_wigig "$dev"
			continue
		}
		local max_sta
...
wifi_led() {
    for dev in ${1:-$DEVICES}; do
        ...
        config_get_bool dev_disabled "$dev" disabled
        config_get_bool dev_disabled_all "$dev" disabled_all
        config_get band "$dev" "band"
        iwpriv $dev led_enable $led_en
        [ "$dev_disabled" = "1" -o "$dev_disabled_all" = "1" ] && led_en=0
        echo "$led_en" > "/sys/devices/platform/leds-gpio/leds/wlan${band}:blue/brightness"
    done
}
