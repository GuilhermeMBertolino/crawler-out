...
function wireless_statistics(formvalue)
    ...
    local map      = {"2.4GHz", "5GHz", "Guest 2.4GHz", "Guest 5GHz", "60GHz", "Guest 60GHz"}
    local ap       = wlan.Apcfg()
    local sta_list = {}
...
-- Process 2.4/5/60G survey HTTP request.
-- @param    survey["2g"]    Do survey on 2G radio.
-- @param    survey["5g"]    Do survey on 5G radio.
-- @param    survey["60g"]    Do survey on 60G radio.
-- @return N/A
function wireless_survey(formvalue, survey)
    local ap = wlan.Apcfg()
    local ap_list = ap and ap:scanlist(survey)
    return ap_list
end
...
function tmp_pick_data(wifi_type, wifi_2G_5G, status_all)
    ...
    if wifi_type == "wireless" then
        data["ssid"] = nixio.bin.b64encode(data["ssid"])
    end
    
    data["mac"] = status_all[prefix .. "_macaddr"]
    data["channel"] = status_all[prefix .. "_current_channel"]
    data["hidden"] = status_all[prefix .. "_hidden"]
...
function tmp_read_wifi(wifi_type, has_5G, has_60G)
    ...
    local wifi_60g = has_60G and tmp_pick_data(wifi_type, "60g", status_all) or nil
    local data    = {basicCfg = {wifi_2g, wifi_5g, wifi_60g}}
    ...
    local wifi_60g_disabled = has_60G and status_all[wifi_type .. "_60g_disabled"] or "on"

    if wifi_2g_disabled == "on" or has_5G and wifi_5g_disabled == "on" or has_60G and wifi_60g_disabled == "on" then
    ...
    local enable_60G = has_60G and status_all[wifi_type .. "_60g_enable"] or "off"
    ...
    --60g
    if has_60G and enable_60G == "on" then
        data["enableBand"] = data["enableBand"] .. "1"
    else
        data["enableBand"] = data["enableBand"] .. "0"
    end
...
function wireless_tmp_read()
    return tmp_read_wifi("wireless", true, true)
end


function wireless_tmp_read_guest()
    return tmp_read_wifi("guest", true, false)
end


function tmp_fill_value(lua_form, prefix, wifi_type)
    local values = {
        [prefix .. "_encryption"] = lua_form[prefix .. "_encryption"],
        [prefix .. "_ssid"]       = lua_form[prefix .. "_ssid"]
    }
    if wifi_type == "wireless" then
        values[prefix .. "_ssid"] = nixio.bin.b64decode(lua_form[prefix .. "_ssid"])
    end
...
function wireless_tmp_set(lua_form)
    local formvalue = {}
    formvalue["form"]                     = {"wireless_2g", "wireless_5g", "wireless_60g"}
    ...
    formvalue["wireless_60g_enable"]       = lua_form.wireless_60g_enable or "off"
    formvalue["wireless_60g_disabled_all"] = lua_form.wireless_60g_enable == "on" and "off" or "on"

    local values = {}
    values[1] = tmp_fill_value(lua_form, "wireless_2g", "wireless")
    values[2] = tmp_fill_value(lua_form, "wireless_5g", "wireless")
    values[3] = tmp_fill_value(lua_form, "wireless_60g", "wireless")
...
function wireless_tmp_set_guest(lua_form)
    ...
    local values = {}
    values[1] = tmp_fill_value(lua_form, "guest_2g", "guest")
    values[2] = tmp_fill_value(lua_form, "guest_5g", "guest")
...
function wireless_status_all(formvalue)
    local form = {"wireless_2g", "wireless_5g", "wireless_60g", "guest_2g"
...
local wireless_form = {
    ...
    wireless_2g  = common_action,
    wireless_5g  = common_action,
    guest_2g     = common_action,
    guest_5g     = common_action,
    syspara_2g   = common_action,
    syspara_5g   = common_action,
    region       = common_action,
    guest        = common_action,
    syspara_wps  = common_action,
    macfilter    = common_action,
    hw_info      = common_action,
    wireless_60g = common_action,
    syspara_60g  = common_action,
    wps_pin = {
    ...
    survey_2g = {
        [".super"] = {cb = wireless_survey, args = {["2g"] = true}}
    },
    survey_5g = {
        [".super"] = {cb = wireless_survey, args = {["5g"] = true}}
    },
    survey_60g = {
        [".super"] = {cb = wireless_survey, args = {["60g"] = true}}
    },
    statistics = {
