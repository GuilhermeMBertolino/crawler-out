...
local CFG_VAP_60  = CFG_HST_60 * CFG_GST_60 * CFG_STA_60
local CFG_VAP_AL  = CFG_VAP_2G * CFG_VAP_5G -- not used now
local CFG_DEV_AL  = CFG_DEV_2G * CFG_DEV_5G * CFG_DEV_60
local CFG_HST_AL  = CFG_HST_2G * CFG_HST_5G * CFG_HST_60
local CFG_GST_AL  = CFG_GST_2G * CFG_GST_5G * CFG_GST_60
local CFG_STA_AL  = CFG_STA_2G * CFG_STA_5G -- not used now
...
local APCFG_DATA = {
    hw_info = {
        {cfg = CFG_DEV_AL, act = ACT_NOTHING, cvt = "band"},
    },
    ...
    wireless_2g = {
        ...
        {cfg = CFG_DEV_2G, act = ACT_DEVMODE, opt = "hwmode",       val = {"b", "g", "bg", "n", "gn", "bgn"}},
    ...
    wireless_5g = {
        ...
        {cfg = CFG_DEV_5G, act = ACT_DEVMODE, opt = "hwmode",       val = {"a_5", "n_5", "ac_5", "an_5", "nac_5", "anac_5"}},
    ...
    wireless_60g = {
        {cfg = CFG_DEV_60, act = ACT_NOTHING, opt = "macaddr"},
        {cfg = CFG_DEV_60, act = ACT_DEV_CFG, opt = "disabled",     cvt = "on_off"},
        {cfg = CFG_DEV_60, act = ACT_DEV_CFG, opt = "disabled_all", cvt = "on_off"},
        {cfg = CFG_DEV_60, act = ACT_NOTHING, cvt = "extinfo"},
        {cfg = CFG_STA_60, act = ACT_NOTHING, cvt = "wds_status"},        
        {cfg = CFG_DEV_60, act = ACT_DEV_CFG, opt = "channel",      cvt = "channel_60g"},
        {cfg = CFG_HST_60, act = ACT_VAP_CFG, opt = "enable",       cvt = "on_off"},
        {cfg = CFG_HST_60, act = ACT_VAP_CFG, opt = "ssid"},
        {cfg = CFG_HST_60, act = ACT_VAP_CFG, opt = "hidden",       cvt = "on_off"},
        {cfg = CFG_HST_60, act = ACT_VAP_CFG, opt = "wps_state"},
        {cfg = CFG_HST_60, act = ACT_VAP_CFG, opt = "encryption",   val = {"none", "psk", "wpa"}},
        {cfg = CFG_HST_60, act = ACT_VAP_CFG, opt = "psk_key"},
        {cfg = CFG_HST_60, act = ACT_VAP_CFG, opt = "server"},
        {cfg = CFG_HST_60, act = ACT_VAP_CFG, opt = "port"},
        {cfg = CFG_HST_60, act = ACT_VAP_CFG, opt = "wpa_key"},
        --{cfg = CFG_DEV_60, act = ACT_DEV_CFG, opt = "txpower",      val = {"low", "middle", "high"}},
        {cfg = CFG_VAP_60, act = ACT_NOTHING, cvt = "current_channel"},
        --{cfg = CFG_HST_60, act = ACT_VAP_CFG, opt = "psk_version",  val = {"auto", "rsn"}},
        --{cfg = CFG_HST_60, act = ACT_VAP_CFG, opt = "psk_cipher",   val = {"auto", "gcmp"}},
        --{cfg = CFG_HST_60, act = ACT_VAP_CFG, opt = "wpa_version",  val = {"auto", "rsn"}},
        --{cfg = CFG_HST_60, act = ACT_VAP_CFG, opt = "wpa_cipher",   val = {"auto", "gcmp"}},
    },
    syspara_60g = {
        {cfg = CFG_DEV_60, act = ACT_NOTHING, opt = "disabled"},
        {cfg = CFG_DEV_60, act = ACT_NOTHING, cvt = "extinfo"},
        {cfg = CFG_DEV_60, act = ACT_DEV_CFG, opt = "rts"},
        {cfg = CFG_DEV_60, act = ACT_DEV_CFG, opt = "dtim_period"},
        {cfg = CFG_DEV_60, act = ACT_DEV_CFG, opt = "wpa_group_rekey"},
        {cfg = CFG_DEV_60, act = ACT_DEV_CFG, opt = "wmm",         cvt = "on_off"},
        --{cfg = CFG_DEV_60, act = ACT_DEV_CFG, opt = "beacon_int"},
        --{cfg = CFG_DEV_60, act = ACT_DEV_CFG, opt = "frag"},
        {cfg = CFG_DEV_60, act = ACT_DEV_CFG, opt = "isolate",     cvt = "on_off"},
        {cfg = CFG_STA_60, act = ACT_VAP_CFG, opt = "enable",      cvt = "on_off"},
        {cfg = CFG_STA_60, act = ACT_VAP_CFG, opt = "ssid",        cvt = "root_ssid"},
        {cfg = CFG_STA_60, act = ACT_VAP_CFG, opt = "bssid"},
        {cfg = CFG_STA_60, act = ACT_VAP_CFG, opt = "encryption",  val = {"none", "psk"}},
        {cfg = CFG_STA_60, act = ACT_VAP_CFG, opt = "psk_key"},
    },
...
local function wps_do_cmd(vap, str, arg)
    local res  = {}
    local iwinfo = get_iwinfo()
    local iftype = iwinfo.type(vap)
    local iw = iftype and iwinfo[iftype]
    local cmd = APCFG_SHELL.." wps %s %s %s" % {vap, str or "", arg or ""}

    if iw and iw.wps then
        cmd = "cat<<EOF\n%sEOF\n" % {iw.wps(vap, str or "", arg or "")}
    end

    printf(cmd)

    local util = require("io").popen(cmd)
...
function Apcfg:wps_vap_cmd(vap, cmd, data)
        ...
        if data.wps_status ~= "success" and data.wps_status ~= "failed" then
            if res["Peer Address"] then
                ...
                if status and (data.wps_status ~= "timeout" or data.wps_status ~= "overlap") then
                    local map = {["Active"] = "ok", ["Overlap"] = "overlap"
                        , ["Timed-out"] = "timeout", ["Disabled"] = "na"}
    ...
    elseif cmd == "ap_pin" then
        if data.wps_label == "on" then
            res = wps_do_cmd(vap, "ap_pin", data.wps_pin)
        else
            res = wps_do_cmd(vap, "ap_pin", "disable")
        end
    ...
    elseif cmd == "pin_lock" then
        ...
        for _, sec in pairs(self.section[CFG_HST_2G]) do
            if vap == sec then
                data.lock_2g = string.lower(res["LockDown"] or "")
            end
        end

        for _, sec in pairs(self.section[CFG_HST_5G]) do
            if vap == sec then
                data.lock_5g = string.lower(res["LockDown"] or "")
            end
        end
        return true
    end
...
-- Check if vap can execute the command
-- @param    vap     Virtual ap interface name
-- @param    cmd     WPS command name.
-- @return bool 
function Apcfg:can_do_wps_cmd(vap, cmd)
	-- when 60G WDS is on, the 60G vap work on sta mode, it cannot execute wps cmd
	local is_support = true
	local device = self.uci:get(self.module, vap, "device")
	local cfg = self.uci:get_all(self.module, device)
	if cfg.band == "60g" and cfg.type == "mac80211" then
		self.uci:foreach(self.module, "wifi-iface", function(s)
			if s.device == device and s.mode == "sta" and s.enable == "on" then
				is_support = false
			end
		end)
	end
...
function Apcfg:wps_cmd(cmd, data)
    for _, vap in pairs(self.section[CFG_HST_AL]) do
        ...
        if cfg and cfg.enable == "on" and cfg.wps == "on" and cfg.hidden == "off"
            and (cfg.encryption == "none" or (cfg.encryption == "psk" 
            and cfg.psk_cipher ~= "tkip" and cfg.psk_version ~= "wpa")) 
	    and self:can_do_wps_cmd(vap, cmd)
        then
            if not cmd then
                return true
            elseif not self:wps_vap_cmd(vap, cmd, data) then
                return false
            end
        end
    end
...
function Apcfg:maclist_check(data)
    if data.mac and data.mac:match("^%x[02468aceACE]%-%x%x%-%x%x%-%x%x%-%x%x%-%x%x$")
        ...
        and (not data.note or #data.note < 32) 
    then
        return true
    end
    return false
...
function sort_aplist(ap_list)
    ...
    if a.signal == nil then
        return false
    elseif b.signal == nil then
        return true
    else
...
-- @param    survey["2g"]    Do survey on 2G radio.
-- @param    survey["5g"]    Do survey on 5G radio.
-- @param    survey["60g"]    Do survey on 60G radio.
-- @return table All scan results
function Apcfg:scanlist(survey)
    local iwinfo  = get_iwinfo()
    local ap_list = {}
    local map     = {
        [CFG_DEV_2G] = (survey["2g"] and CFG_VAP_2G) or nil,
        [CFG_DEV_5G] = (survey["5g"] and CFG_VAP_5G) or nil,
        [CFG_DEV_60] = (survey["60g"] and CFG_HST_60) or nil,
    }
...
function Apcfg:assoclist()
    local sta    = {}
    local map    = {CFG_HST_2G, CFG_HST_5G, CFG_GST_2G, CFG_GST_5G, CFG_HST_60, CFG_GST_60}
    local iwinfo = get_iwinfo()
    ...
    for id, sec in ipairs((iwinfo and map) or {}) do
        for _, vap in pairs(self.section[sec]) do
            local iftype = iwinfo.type(vap)
            local iw = iftype and iwinfo[iftype]
            -- FIXME tempory fix for 60G
            if iw and iw.mode(vap) ~= "Client" then
            --if iw then
...
function Apcfg:scan_driver()
    ...
    self.uci:foreach(self.module, "wifi-device",
        function(s)
            if (dev2g and dev5g) or (s['band'] == '60g' and not dev60) then
                dev60 = s['.name']
            elseif dev2g or (s['band'] == '5g' and not dev5g) then
                dev5g = s['.name']
            else
                dev2g = s['.name']
            end
    ...
    self.uci:foreach(self.module, "wifi-iface",
        function(s)
            ...
            elseif s['device'] == dev60 then
                if s['mode'] ~= 'ap' then
                    sta60 = s['.name']
                elseif hst60 or s['guest'] then
                    gst60 = s['.name']
                else
                    hst60 = s['.name']
                end
    ...
    return {
        ...
        [CFG_HST_60] = {hst60}, [CFG_GST_60] = {gst60}, [CFG_STA_60] = {sta60},
        [CFG_DEV_2G] = {dev2g}, [CFG_DEV_5G] = {dev5g}, [CFG_DEV_60] = {dev60},
        [CFG_DEV_AL] = {dev2g, dev5g, dev60},
        [CFG_HST_AL] = {hst2g, hst5g, hst60},
        [CFG_GST_AL] = {gst2g, gst5g, gst60},
        [CFG_STA_AL] = {sta2g, sta5g, sta60},
        [CFG_VAP_2G] = {hst2g, gst2g, sta2g},
        [CFG_VAP_5G] = {hst5g, gst5g, sta5g},
        [CFG_VAP_60] = {hst60, gst60, sta60},
        [CFG_VAP_AL] = {hst2g, gst2g, sta2g, hst5g, gst5g, sta5g, hst60, gst60, sta60}
    }
end
...
function Apcfg:get_option(cfg, option, convert)
    ...
    printf("get %s %s = %s" % {self.section[cfg][1] or "", option or convert or ""
    ...
    if changes then
        for i, item in pairs(self.data) do
            for _, section in pairs(self.section[item.cfg]) do
                if (self.change % item.cfg > 0) and changes[section] 
                    and changes[section][item.opt] 
                then
        ...
        for _, sec in ipairs({CFG_HST_2G, CFG_HST_5G}) do
            if self:get_option(sec, "wps_state") ~= "configured" 
                and self.change % sec == 0 
            then 
        ...
        if self.action % act == 0 then
            ...
            if apply_cmd then
                if (CFG_VAP_2G * CFG_DEV_2G) % self.change == 0 then
                    apply_cmd = "%s %s %s" % {APCFG_SHELL, APCFG_ACTION[ACT_RE_LOAD].cmd
                        , (self.section[CFG_DEV_2G][1] or "")}
                elseif  (CFG_VAP_5G * CFG_DEV_5G) % self.change == 0 then
                    apply_cmd = "%s %s %s" % {APCFG_SHELL, APCFG_ACTION[ACT_RE_LOAD].cmd
                        , (self.section[CFG_DEV_5G][1] or "")}
                elseif  (CFG_VAP_60 * CFG_DEV_60) % self.change == 0 then
                    apply_cmd = "%s %s %s" % {APCFG_SHELL, APCFG_ACTION[ACT_RE_LOAD].cmd
                        , (self.section[CFG_DEV_60][1] or "")}
                else
...
function Apcfg:get_wds_status(sec)
    ...
    for _, vap in pairs(self.section[sec]) do
        -- ifname maybe different form vap
        vap = self.uci:get(self.module, vap, "ifname") or vap
        local iftype = iwinfo.type(vap)
        local iw = iftype and iwinfo[iftype]

        if iw and iw.mode(vap) == "Client" then
            if iw.bssid(vap) and iw.bssid(vap) ~= "00:00:00:00:00:00" and iw.channel(vap) then
                local enc = iw.encryption(vap)
                if enc.enabled then
...
--GET available wireless band on this board.
function Apcfg:get_band(secs)
    return {
        support_2g = next(self.section[CFG_DEV_2G]) and "yes" or "no",
        support_5g = next(self.section[CFG_DEV_5G]) and "yes" or "no",
        support_60g = next(self.section[CFG_DEV_60]) and "yes" or "no"
    }
end
...
function Apcfg:get_capability(secs)
    if not self.capability then
        ...
        if not cap or cap.country ~= country or cap.dev_loaded ~= "loaded" then
            local iwinfo  = get_iwinfo()
            local success = false
            cap = {
                country = country,
            ...
            for _, dev in pairs(self.section[secs or CFG_DEV_AL]) do
                local htmode  = {}
                local hwmode  = {}
                local channel = {}
                local ht20    = nil
                ...
                local is60g   = (self.section[CFG_DEV_60][1] == dev)
                local vaps    = self.section[(is60g and CFG_HST_60) or (is5g and CFG_VAP_5G) or CFG_VAP_2G]
                local iftype  = iwinfo and iwinfo.type(dev)
                local iw      = iftype and iwinfo[iftype]

                if not iw then
                    cap.dev_loaded = "unloaded"
                else
                    ...
                    elseif is5g then
                        ...
                        --    hwmode[#hwmode + 1] = "n_5" 
                        --end
                        if hw.a and hw.ac then 
                            hwmode[#hwmode + 1] = "ac_5" 
                        end
                        if hw.n and hw.ac then 
                            hwmode[#hwmode + 1] = "nac_5" 
                        end
                        if hw.a and hw.n and hw.ac then 
                        ...
                        if hw.g and hw.b and hw.n then 
                            hwmode[#hwmode + 1] = "bgn" 
                        end
                    end
                    ...
                    for _, chan in ipairs(iw.freqlist(dev) or {}) do
                        ...
                        elseif is5g == (chan.mhz > 4000) then 
                    ...
                    if is60g then
                        htmode[#htmode + 1] = "2160" 
                    end

                    -- Is any channel supported in country?
                    if #channel > 0 then
                        if is60g then
                            cap.htmode_60g  = htmode
                            cap.hwmode_60g  = hwmode
                            cap.channel_60g = channel
                        elseif is5g then
        ...
        self.capability = {
            ...
            htmode_60g  = cap.htmode_60g  or {},
            hwmode_60g  = cap.hwmode_60g  or {},
            channel_60g = cap.channel_60g or {},
        }
    end
...
function Apcfg:get_extinfo(secs)
    ...
    local wds_guest = self.uci:get_profile(self.module,"wds_guest_compatible")
    local wds2g_wds5g = self.uci:get_profile(self.module,"wds2g_wds5g_compatible")
    local support_wds60g = self.uci:get_profile(self.module,"support_wds60g")
    extinfo = {support_band = band, wds_guest_compatible = wds_guest, wds2g_wds5g_compatible = wds2g_wds5g, support_wds60g = support_wds60g,}
    return extinfo
end
...
--GET check 60G channel
function Apcfg:set_channel_60g(val)
    if val == "auto" then 
        return val 
    end

    for _, i in ipairs(self:get_capability().channel_60g) do
        if val == i then 
            return val    
        end
    end
...
--GET check 60G channel
function Apcfg:get_channel_60g(val)
    if val == "auto" then 
        return val 
    end
...
function Apcfg:set_country(val)
    ...
    return (pro_region == "UN" or pro_region == val) and val and val:match("^[A-Z][A-Z]$")
end
