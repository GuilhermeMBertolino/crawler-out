...
function TschedConf:show()
    ...
    uci_r:foreach(mname, SETTING,
                function(section)
                    print(section.need_time_sync)
                end)
    uci_r:foreach(mname, STYPE,
                function(section)
...
function TschedConf:del_all()
    ...
    uci_r:delete_all(mname, SETTING)
    return true
end

--- Add setting to a module.
-- @param need_time_sync	If module need time sync before running.
-- @return			        Boolean
function TschedConf:add_setting(need_time_sync)
    check_args(need_time_sync)
    local mname = self.mname
    local uci_r = self.uci_r
    ...
    uci_r:section(mname, SETTING, nil, value_setting)
    return true
end
...
function TschedConf:add_entry(entry_arg, ranges)
    ...
    -- FIXME: May be better return a UCI error string.
    uci_r:section(mname, STYPE, nil, values)
    return true
end

--- Delete setting from a module.
-- @param need_time_sync	If module need time sync before running.
-- @return			Boolean
function TschedConf:del_setting(need_time_sync)
    check_args(need_time_sync)
    local mname = self.mname
    local uci_r = self.uci_r
    ...
    -- FIXME: delete_all don't raise any error, is it safe?
    uci_r:delete_all(mname, SETTING, values)
    return true
end
