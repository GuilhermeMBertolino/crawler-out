...
local MIR_ENABLE = "0x8000"
local MIR_DISABLE = "0x0"
...
local function ping_action(http_form)
    ...
    if my_ip and my_ip:match("^[a-zA-Z0-9%-%.:_]+$") then
        ...
        while true do
            ...
            -- ping one time
            t_count  = t_count + 1
            local lines = util.exec("ping -c 2 -W %d -s %d %q 2>&1" % {t_timeout, t_pkt, my_ip})
            if lines then
...
local function found_pid(my_cmd)
    ...
    local ps_info = io.popen("ps | grep %q | grep -v grep" % my_cmd)
    local pid = false
    local tbl
...
--            if cmd and cmd:match("^" .. my_cmd) then
            if line and line:find(my_cmd, 1, true) then
                pid = tbl[1]
                --dbg(pid)
                break
            end
...
local function get_port_cfg()
    local uci_r = uci.cursor()
    local data = {
        enable   = uci_r:get("diagnostic", "port_mirror", "enable") or "off",
        wan   = uci_r:get("diagnostic", "port_mirror", "mirror_port_wan") or "0",
        lan1   = uci_r:get("diagnostic", "port_mirror", "mirror_port_lan1") or "0",
        lan2   = uci_r:get("diagnostic", "port_mirror", "mirror_port_lan2") or "0",
        lan3   = uci_r:get("diagnostic", "port_mirror", "mirror_port_lan3") or "0",
        lan4   = uci_r:get("diagnostic", "port_mirror", "mirror_port_lan4") or "0",
        mirroring_port   = uci_r:get("diagnostic", "port_mirror", "monitor_port") or "1"
    }
    return data
end

local function set_port_cfg(enabled, mirror_port, monitor_port)
    local uci_r = uci.cursor()
    uci_r:set("diagnostic", "port_mirror", "enable", enabled)
    uci_r:set("diagnostic", "port_mirror", "mirror_port_wan", mirror_port[1])
    uci_r:set("diagnostic", "port_mirror", "mirror_port_lan1", mirror_port[2])
    uci_r:set("diagnostic", "port_mirror", "mirror_port_lan2", mirror_port[3])
    uci_r:set("diagnostic", "port_mirror", "mirror_port_lan3", mirror_port[4])
    uci_r:set("diagnostic", "port_mirror", "mirror_port_lan4", mirror_port[5])    
    uci_r:set("diagnostic", "port_mirror", "monitor_port", monitor_port)
    uci_r:commit("diagnostic")
end

local function reset_port_cfg()
    local dflt = {
        enabled = "off",
        mirror_port = {"1", "0", "0", "0", "0"},
        monitor_port = "1"
    }
    return set_port_cfg(dflt.enabled, dflt.mirror_port, dflt.monitor_port)
end

local function get_port_cfg_dflt()
    reset_port_cfg()
    return get_port_cfg()
end
...
local function util_exec(page, reg, val)
    util.exec("et robowr 0x%x 0x%x 0x%x 2>&1" % {page, reg, val})
    local lines = util.exec("et robord 0x%x 0x%x 2>&1" % {page, reg})
    local val = tenToSixteen(val)
    if lines:match(val) then
        return
    else
        return false, "set register fail"
    end
end

local function stop_port_mirror()
    local monitor_port_val = MIR_DISABLE
    local mirror_port_val = 0x0

    reset_port_cfg()
    util_exec(MIRCAPCTL.page, MIRCAPCTL.reg, monitor_port_val)     
    util_exec(IGMIRCTL.page, IGMIRCTL.reg, mirror_port_val)
    util_exec(EGMIRCTL.page, EGMIRCTL.reg, mirror_port_val)
end

local function set_port_mirror(http_form)

    local enabled = http_form.enable or ""
    local mirror_port = {
        http_form.wan or "0",
        http_form.lan1 or "0",
        http_form.lan2 or "0",
    ...
    local monitor_port = http_form.mirroring_port or "1"
    
    if enabled == "" then
        local monitor_port_val = MIR_ENABLE + monitor_port
        local mirror_port_val = 0
        local port = 0
        for i in pairs(mirror_port) do
            if mirror_port[i] == "1" then
                mirror_port_val = mirror_port_val + 2^port
            end
            port = port + 1
        end
 
        enabled  = "on" 
        set_port_cfg(enabled, mirror_port, monitor_port)

        util_exec(MIRCAPCTL.page, MIRCAPCTL.reg, monitor_port_val)     
        util_exec(IGMIRCTL.page, IGMIRCTL.reg, mirror_port_val)
        util_exec(EGMIRCTL.page, EGMIRCTL.reg, mirror_port_val) 
    elseif enabled == "on" then
        set_port_cfg(enabled, mirror_port, monitor_port)
...
local dispatch_tbl = {
    ...
    port_mirror = {
        ["read"]      = {cb = get_port_cfg},
        ["write"]     = {cb = set_port_mirror}
    }
}
