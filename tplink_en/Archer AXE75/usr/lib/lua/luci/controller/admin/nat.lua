...
--[[
local function disable_conflict_settings()
    -- for qos
    local qos = require "luci.controller.admin.qos"
    local qos_form = qos.read_qos_settings()
    qos_form.enable = "off"
    qos.write_qos_settings(qos_form)

    -- for dos
    local dos = require "luci.controller.admin.security_settings"
    dos.write_security_settings_enable({enable = "off"})

    -- for traffic statistics
    local tfs = require "luci.controller.admin.tfstats"
    tfs.tf_save_status({enable = "off"})
end
]]--
...
local function set_alg(formvalue)
    ...
    local log = logm.Logn("nat")

    if not old then
        return false, "Unknown error"    
    end

    if formvalue["ftp"] ~= old.ftp then
        if formvalue["ftp"] == "on" then 
            log(log.logid.NAT_ALG_ENABLED, "ftp")
        else
            log(log.logid.NAT_ALG_DISABLED, "ftp")
        end
    end
    if formvalue["tftp"] ~= old.tftp then
        if formvalue["tftp"] == "on" then 
            log(log.logid.NAT_ALG_ENABLED, "tftp")
        else
            log(log.logid.NAT_ALG_DISABLED, "tftp")
        end
    end
    if formvalue["h323"] ~= old.h323 then
        if formvalue["h323"] == "on" then 
            log(log.logid.NAT_ALG_ENABLED, "h323")
        else
            log(log.logid.NAT_ALG_DISABLED, "h323")
        end
    end
    if formvalue["rtsp"] ~= old.rtsp then
        if formvalue["rtsp"] == "on" then 
            log(log.logid.NAT_ALG_ENABLED, "rtsp")
        else
            log(log.logid.NAT_ALG_DISABLED, "rtsp")
        end
    end
    if formvalue["sip"] ~= old.sip then
        if formvalue["sip"] == "on" then 
            log(log.logid.NAT_ALG_ENABLED, "sip")
        else
            log(log.logid.NAT_ALG_DISABLED, "sip")
        end
    end
    if formvalue["pptp"] ~= old.pptp then
        if formvalue["pptp"] == "on" then
            log(log.logid.NAT_ALG_ENABLED, "pptp")
        else
            log(log.logid.NAT_ALG_DISABLED, "pptp")
        end
    end
    if formvalue["l2tp"] ~= old.l2tp then
        if formvalue["l2tp"] == "on" then
            log(log.logid.NAT_ALG_ENABLED, "l2tp")
        else
            log(log.logid.NAT_ALG_DISABLED, "l2tp")
        end
    end
    if formvalue["ipsec"] ~= old.ipsec then
        if formvalue["ipsec"] == "on" then
            log(log.logid.NAT_ALG_ENABLED, "ipsec")
        else
            log(log.logid.NAT_ALG_DISABLED, "ipsec")
        end
    end
    ...
    local sip   = formvalue["sip"] or old.sip
    ...
    local cfg = { ["ftp"] = ftp, ["tftp"] = tftp, 
        ["h323"] = h323, ["rtsp"] = rtsp, ["sip"] = sip,["pptp"] = pptp, 
        ["l2tp"] = l2tp, ["ipsec"] = ipsec }

    return Nat:operate("alg", "write", cfg)
end

local function get_access_devices(http_form)
    local data = {}
    -- get online device
    local clist = require("luci.model.tm_clientmgmt").get_access_device_list(1) or {}
...
local dispatch_tbl = {
    ...
    pt = {
        ...
        ["load"] = {cb = get_pt_rules, others = max_pt_rules},
