...
local ERR_TBL = {
    ...
    ["#004"] = "DHCP reservation duplication"
}
...
local function blst_load()
    local bindlist = Imb:bandlist_read()
    local access_list = get_access_devices() or {}
    local client_list = {}

    for _, client in ipairs(access_list) do
        client_list[change_mac(client.mac)] = client
    end

    for i, v in ipairs(bindlist) do
        local device = client_list[change_mac(v.mac)]
        if device then
            bindlist[i].name = device.name
        else
            bindlist[i].name = ""
        end
    end
...
local function blst_insert(formvalue)
    ...
    if form[1].enable == "on" then
        if dhcps.dhcplease_check_dup(form[1]) then
            return false, ERR_TBL["#004"]
        end
    end
    ...
    for _, e in ipairs(arp) do
        if e.flags and bit_isset(e.flags, ATF_COM) then
            if e.ip and lan_subnet(lanip, e.ip, mask) then
                local tmp = {ipaddr = "", mac = "", enable = "", name = ""}
                local device = client_list[change_mac(e.mac)]
