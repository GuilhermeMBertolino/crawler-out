...
function disk_status()
    ...
    for _, disk in pairs(disks) do 
        ...
        data.storage_vendor = data.storage_vendor .. disk.vendor .. " " .. disk.model .. "; "
    end
...
function printer_status()
    ...
    local printer_enable = uci:get(UCICFG, "global", "printer")
    if printer_enable == "on" then
	data["printer_name"] = printsvr.printinfo()
	data["printer_count"] = printsvr.printcnt()
    else
	data["printer_name"] = "None"
	data["printer_count"] = 0
    end
    return data
end
