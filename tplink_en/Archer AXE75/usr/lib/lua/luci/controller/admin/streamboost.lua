...
function index()
	entry({"admin","streamboost"}, call("_index")).leaf = true
end
...
function dispatch(http_form)
    return appflow_operate(http_form)
end
...
local function get_appflow_settings(http_form)
	local appflow_data = {
		timeout = false,
		success = true,
		data = {}
...
local function set_appflow_settings(http_form)
	local uplimit, downlimit, enable, auto_update, auto_bw

        enable      = http_form["enable_streamboost"] or "0"
	auto_bw     = http_form["enable_auto"] or "0"
	auto_update = http_form["auto_update"] or "0"
	uplimit     = http_form["uplimit"]
	downlimit   = http_form["downlimit"]
...
function appflow_operate(http_form)
    local operate = http_form["operation"]
    local ret

	if operate == "read" then
		get_appflow_settings(http_form)
	elseif operate == "write" then
		set_appflow_settings(http_form)		
	else
		write_json(err_data)
	end
   return
