...
local function pptpd_cfg_set(form)
    ...
    local account_num = form_tools:count("pptpd", "login")
...
function pptpd_account_min_check()
    local cur = form:count("pptpd", "login")
    local enabled = uci:get("pptpd", "pptpd", "enabled")
...
function remove_pptpd_account(http_form)
    ...
    if pptpd_account_min_check() then
        return false, "cant remove the last pptpd account when pptpd enable"
    end
