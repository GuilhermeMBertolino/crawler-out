...
local function iface_get_network(iface)
    ...
    state:foreach("network", "interface",
        function (section)
            if not string.match(section['.name'], "v6") then
                local ifname = state:get("network", section['.name'], "ifname")
                if iface == ifname then
                    table.insert(net , section[".name"])
                end
            end
        end
        )
...
local CHECK_RULE_TBL = {
    ...
    {
        field = "gateway", canbe_absent = true, canbe_empty = true,
        check = {
            ctypes.check_ipv4
        }
    },
...
function route_sys_list()
    ...
    for k, rt in ipairs(routes) do
        ...
        if isSupportFFS == "yes" then
            if data.interface ~= "wifi" then
                sys_routes[#sys_routes + 1] = data
            end
        else
            sys_routes[k] = data
        end
...
--- Convert given address to network address
-- @param address   IPv4 address in dotted quad
-- @param netmask   IPv4 netmask in dotted quad notation
-- @return  Converted net address in dotted quad
function route_get_net_address(address, netmask)
    ...
    a[1], a[2], a[3], a[4] = address:match("^(%d+)%.(%d+)%.(%d+)%.(%d+)$")
    m[1], m[2], m[3], m[4] = netmask:match("^(%d+)%.(%d+)%.(%d+)%.(%d+)$")

    for i = 4, 1, -1 do
        if tonumber(m[i]) < 255 then
            a[i] = bit.band(tonumber(a[i]), tonumber(m[i]))
            a[i] = tostring(a[i])
            changed = true
        else
            break
...
function route_insert(http_form)
    ...
    -- gateway 0.0.0.0 will cause problem in netifd, while nil not
    if new.gateway == "0.0.0.0" then
        new.gateway = nil
    end

    local tmp_target = route_get_net_address(new.target, new.netmask)
    if tmp_target then
        new.target = tmp_target
    else
        return false, "invalid args"
    end
...
function route_update(http_form)
    ...
    -- handle old value start
    -- If webpage not refreshed after insert/update, old.gateway/old.target from http_form maynot changed, so convert it
    if old.gateway == "0.0.0.0" then
        old.gateway = nil
    end

    local tmp_target = route_get_net_address(old.target, old.netmask)
    if tmp_target then
        old.target = tmp_target
    else
        return false, "invalid args"
    end
    -- handle old value end
