...
-----------------------------------------------------------------------------------
-----------------------------------------------------------------------------------
function new_read_security_settings_enable()
    local ret = {}
    ret.enable = uci_r:get("basic_security", "settings", "spi_enable")
    ret.wan_ping = uci_r:get("basic_security", "settings", "wan_ping")
    ret.lan_ping = uci_r:get("basic_security", "settings", "lan_ping")
    return ret
end

function new_write_security_settings_enable(http_form)
    if http_form.enable ~= nil then
        uci_r:set("basic_security", "settings", "spi_enable", http_form.enable)
    end
    if http_form.wan_ping ~= nil then
        uci_r:set("basic_security", "settings", "wan_ping", http_form.wan_ping)
    end
    if http_form.lan_ping ~= nil then
        uci_r:set("basic_security", "settings", "lan_ping", http_form.lan_ping)
    end
    uci_r:commit("basic_security")
    return new_read_security_settings_enable()
end
-----------------------------------------------------------------------------------
-----------------------------------------------------------------------------------
...
local dispatch_tbl = {
	...
	new_enable = {
        ["read"]  = { cb  = new_read_security_settings_enable },
        ["write"] = { cb  = new_write_security_settings_enable,
                      cmd = "/etc/init.d/basic_security reload" }        
    },
