...
local function recalc_pool(change_ip_pool)
    ...
    if not cidr_ip or not cidr_broad then
        ...
        if tonumber((cidr_broad - cidr_ip)[2][2]) ~= 255 then
			...
			if true == change_ip_pool then -- ip&mask changed or mask changed, set default ip_pool
				uci_r:set("dhcp", "lan", "start", 100)
				uci_r:set("dhcp", "lan", "limit", 150)
			end
			return true
        end
    end 
...
local function calc_ipaddr_pool()
    ...
    if sysmode == "router" then
        ...
        lan_ip = net:ipaddr()
        netmask = net:netmask()
    elseif sysmode == "ap" then
        lan_ip = uci_r:get("network", "lan", "ipaddr")
        netmask = uci_r:get("network", "lan", "netmask")
    end
...
local function get_dhcp_options()
    ...
    if sysmode == "ap" then
        local gw = uci_r:get("network", "lan", "gateway")
        if gw ~= "255.255.255.255" then
            data.gateway = gw
        end
    end
...
function dhcp_opt_update(ipaddr, change_ip_pool)
    if not dtype.ipaddr(ipaddr) then
        return false
    end

    local chg = recalc_pool(change_ip_pool) -- Reset start ipaddr and range limit
...
function is_match_static_entry(macaddr)
    local ret = false
    local leases = {}
    ...
    for k, rt in ipairs(leases) do
        ...
        local enable = rt.enable

        if enable == "on" and macaddr == mac then
		    ret = true
			break
		end
    end
...
function load_static_leases()
    ...
    for i=1,#leases do
        for k, client in pairs(table) do
            if client.mac == leases[i].mac:gsub(":", ""):upper() then
                leases[i].hostname = client.nickname or client.hostname
                break
            end
        end
    end
    ...
    for k, rt in ipairs(leases) do
        ...
        data.ip   = rt.ip
        data.mac  = mac:upper()
        data.comment = rt.hostname or ""
        data.enable = (rt.enable == "on") and "on" or "off"
        data.hostname = rt.hostname or ""
...
function load_active_leases()
    ...
    local lan_ip_addr = uci_r:get("network", "lan", "ipaddr")
    local lan_mask_addr =  uci_r:get("network", "lan", "netmask")
    
	local table = uci_r:get_all("history_list")
	for i=1,#leases do
		for k, client in pairs(table) do
			if client.mac == leases[i].macaddr:gsub(":", ""):upper() then
				leases[i].hostname = client.nickname or leases[i].hostname
				break
			end
		end
    end
    ...
    for k, rt in ipairs(leases) do
        ...
        if isSupportFFS == "yes" then
            if data.ipaddr ~= nil and ctypes.check_same_network(lan_ip_addr, data.ipaddr, lan_mask_addr) then
                client_list[#client_list + 1] = data
            end
        else
            client_list[k] = data
        end
...
function get_dhcp_settings()
	...
	local status = uci_r:get("dhcp", "lan", "ignore")
	if "0" == status then
		settings.enable = "on"
	elseif "1" == status then
		settings.enable = "off"
	elseif "2" == status then
		settings.enable = "auto"
	end
...
function set_dhcp_settings(http_form)
	...
	if status == "auto" then
		uci_r:set("dhcp", "lan", "ignore", "2")
		uci_r:commit("dhcp")
		return get_dhcp_settings()
	end

    if not start_ip and status == "on" then
...
function update_static_lease(http_form)
    ...
    if ret then
        local table = uci_r:get_all("history_list")
        for k, client in pairs(table) do
            if client.mac == ret.mac:gsub("-", ""):upper() then
                ret.hostname = client.nickname or client.hostname
                break
            end
        end
    end
...
function change_lease_items(http_form)
    local ip,now,tmp,num,leasetime,cmd
    ...
    for i, v in ipairs(indexes) do
		ip = uci_r:get("dhcp", items[v + 1], "ip")
        for k, rt in ipairs(leases) do
        	if ip == rt.ipaddr then
				local fdtime=io.open(uptimefile, "r")
				if fdtime then
					num = fdtime:read("*n")
					if not num then
					    fdtime:close()
						return false, "read uptime failed"
					else
						tmp = tonumber(num)
						now = math.floor(tmp)
					end
				...
				leasetime = uci_r:get("dhcp", "lan", "leasetime") or "120m"
				leasetime = string.match(leasetime, "(%d+)")
				leasetime = now + 60 * tonumber(leasetime)
				cmd = "awk '{if (/" .. rt.macaddr .. "/) {$1=" .. leasetime .. "}print}' /var/dhcp.leases > /var/dhcp.leases.tmp;cp /var/dhcp.leases.tmp /var/dhcp.leases;rm /var/dhcp.leases.tmp"
				os.execute(cmd)
			end
		end
    end
...
function insert_static_lease(http_form)
    ...
    if ret then
        ...
        for k, client in pairs(table) do
            if client.mac == ret.mac:gsub("-", ""):upper() then
                ret.comment = client.nickname or client.hostname
                break
            end
        end
    end
