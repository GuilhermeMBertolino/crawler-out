...
local function provider_get()
    ...
    local enable = uci_c:get("ddns", "tp-link", "enabled")
    if enable == "on" then
        data.provider = "tp-link"
    else
	    data.provider = uci_c:get("ddns", "provider", "provider")
    end
...
local function tp_check()
    local uci_r = uci.cursor()
    if uci_r:get_profile("cloud", "https_client") ~= 1 then
        return cloud_account:check_login()
    else
        return true
    end
end
