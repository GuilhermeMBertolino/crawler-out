...
function CfgParser:__init__(path)
    ...
    function(section)
        ...
        volumn.label = volumn.label or ("volume" .. volcnt)
        volcnt = volcnt + 1
...
-- @param N/A
-- @return N/A
function account_update(username, password, account_type)
    local password = password:gsub("[']", "'\\''")
    local cmd = {"usbuser", username, password, account_type}
    --debug(cmd)
    subprocess.call(cmd)
end

-- Stop Samba/FTP/SFTP/DLNA Service
-- @param N/A
-- @return N/A
function stop(wait)
    local uci_r = uci:cursor()
...
function apply()
    local uci_r = uci:cursor()
    local log   = logm.Log(SHARE_ID)
    local ftp = uci_r:get(CFG_UCI, "global", "ftp") or "off"
    local ftpex = uci_r:get(CFG_UCI, "global", "ftpex") or "off"
    local samba = uci_r:get(CFG_UCI, "global", "samba") or "off"
    local sftp = uci_r:get(CFG_UCI, "global", "sftp") or "off"
    local sftpex = uci_r:get(CFG_UCI, "global", "sftpex") or "off"
    stop(true)
    -- Wait 500ms
    nixio.nanosleep(0, 50000000)

    if (ftp == "on" or ftpex == "on" or sftp == "on" or sftpex == "on") and samba == "on" then
        sys.fork_exec("/etc/init.d/proftpd start;/etc/init.d/samba start;/etc/init.d/minidlna start")
        log(LOGX_ID_SAMBA_START)
	log(LOGX_ID_PROFTPD_START)
	log(LOGX_ID_MINIDLNA_START)
    elseif samba == "on" then
	sys.fork_exec("/etc/init.d/samba start;/etc/init.d/minidlna start")
	log(LOGX_ID_SAMBA_START)
	log(LOGX_ID_MINIDLNA_START)
    elseif (ftp == "on" or ftpex == "on" or sftp == "on" or sftpex == "on") then
        sys.fork_exec("/etc/init.d/proftpd start;/etc/init.d/minidlna start")
        log(LOGX_ID_PROFTPD_START)
	log(LOGX_ID_MINIDLNA_START)
    else
