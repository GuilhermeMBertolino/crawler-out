...
--- Disable all invalid ip&mac binding entries
-- @param N/A
function IMB_INST:arplist_disinvalid()
    --uci update
    self.uci:foreach(self.config, self.stype,
        function(section)
            if section["enable"] == "on" then
                if not ctypes.check_ip_in_lan(section["ipaddr"]) then
                    self.uci:set(self.config, section[".name"], "enable", "off")
                end
            end
        end)
    self.uci:commit(self.config)
...
--- Check duplication
-- @param new
function IMB_INST:arplist_check_dup(new)
    if self:imb_enable() then
        local old_lst = self:bandlist_read()
        ...
        for _, e in ipairs(old_lst) do
            if e.enable == "on" then
                if new.ip == e.ipaddr or new.mac:gsub('-', ':'):upper() == e.mac:gsub('-', ':'):upper() then
                    return true
                end
            end
        end
