...
function check_ascii_visible(v)
    if not v or v == "" then 
        return false
    end

    for i = 1, #v do
        if not check_range(string.byte(v, i), 33, 126) then
            return false 
        end
    end
    return true
...
function check_unicast_ipv6_gw(ip)
	local head = ip:match("^[23]") or ip:match("^[fF][eE]80")  -- start with '2' or '3' or 'fe80', ignore uppper-case and lower-case 
    return check_ipv6(ip)
        and head
        and not ip:match("::$")   -- can not end with '::'
        and not ip:match("/%d*$") -- can not end with '/$num'
end
...
function check_ip_in_lan(ip)
    ...
    return check_same_network(ip, lan_ipaddr, lan_netmask)
        ...
        return check_same_network(ip, lan_ipaddr, lan_netmask) and not vpn.check_ip_is_pptp_client(ip)
    end
end
