...
--- Decrypt message and reverse it by private key (n, d)
-- @class function
-- @name  luarsa.decrypt_reverse
-- @param message Message (in hex format) to be decrypted and reversed.
-- @param n       Modulus, part of private key.
-- @param e       Public exponent.
-- @param d       Private exponent.
-- @return        String after decrypted and reversed.
...
--- Decrypt message and reverse it by using private key.
-- @param hexcode   Message (in hex format) to be decrypted and reversed.
-- @param prikey    Private key.
-- @return #string String after decrypted and reversed.
function rsa_decrypt_reverse(hexcode, prikey)
    local luarsa = require "luarsa"
    if not hexcode then 
        return nil 
    end
    ...
    prikey.e = prikey.e or ""
    prikey.d = prikey.d or ""
    local str = luarsa.decrypt_reverse(hexcode, prikey.n, prikey.e, prikey.d)
    return str
end
...
function Crypto:__init__(type)  
    if not type or string.upper(type) == "RSA" then
        ...
        self.decrypt_reverse = rsa_decrypt_reverse
