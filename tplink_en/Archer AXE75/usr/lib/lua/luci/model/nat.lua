...
--- set hnat on/off in config and excute it
-- @param enable:true/false
-- @return nothing
local function hnat_set_onoff(enable)
	local tmp 
	if (enable == true) then
		tmp = "on"
	else
	...
	-- step1: save hnat status to config...
	local uci = nat_uci_cursor()
	local hnat_en = uci:get("nat", "nat_global", "hw_enable") or "off"

    if hnat_en ~= tmp then
		uci:set("nat", "nat_global", "hw_enable",tmp)
		uci:commit(module_name)
    else
       dbg.printf("[nat]WARNING!!! hnat status not changed!" ..hnat_en .. " v.s."..tmp)
		return 0
    end

    --step 2:excute HNAT start/stop function.
    if (enable == true) then
		dbg.printf("[nat]Enable HNAT")
		os.execute("[ -f '/etc/init.d/hnat-start' ] && /etc/init.d/hnat-start start > /dev/console")
	else
		dbg.printf("[nat]Disable HNAT")
		os.execute("[ -f '/etc/init.d/hnat-start' ] && /etc/init.d/hnat-start stop > /dev/console")	
	end
...
local function nat_get(ctype)
    local uci = nat_uci_cursor()
    local data = { enable = "", boost_enable = "", reboot_time = uci:get_profile("global", "reboot_time") or 75, 
                   boost_support = "", boost_qos_compatible = "", boost_traffic_compatible = ""}
    ...
    for _, item in ipairs(cfg) do
        ...
        data.boost_enable = item.boost_enable or "on"
        break
    end
    
    data.boost_support = nat_profile_get("boost_support") or "no";
    data.boost_qos_compatible = nat_profile_get("boost_qos_compatible") or "yes";
    data.boost_traffic_compatible = nat_profile_get("boost_traffic_compatible") or "yes";
...
local function nat_set(cfg,...)
    ...
    local boost_en = (cfg.boost_enable == "on") and "on" or "off"
    ...
    for _, item in ipairs(config) do
        if item.enable ~= en or item.boost_enable ~= boost_en then
            uci:set(module_name, item[".name"], "enable", en)
            uci:set(module_name, item[".name"], "boost_enable", boost_en)
            uci:commit(module_name)
            ...
            -- disable hnat when nat is disabled;enable hnat (if needed) when nat is enabled;
            if "off" == en then
				-- disable SFE and HNAT
				dbg.printf("[nat] no need to disable HNAT and SFE")
				--os.execute("[ -f '/etc/init.d/shortcut-fe' ] && /etc/init.d/shortcut-fe stop > /dev/console")
                --hnat_set_onoff(false)
			else			
				-- enable SFE, if qos off, traffic statistics off, then enalbe HNAT
				dbg.printf("[nat] no need to Enable SFE")
				--os.execute("[ -f '/etc/init.d/shortcut-fe' ] && /etc/init.d/shortcut-fe start > /dev/console")				
				--NAT_INST:sync_hnat_status()
			end

           -- if item.boost_enable ~= hw_en then
           --     os.execute("[ -f '/usr/sbin/qca-hnat' ] && (sleep 3;reboot)&")
           -- end

            return nat_get("nat_global")
        end
    end
        ...
        if cfg.ftp ~= c.ftp or cfg.tftp ~= c.tftp 
            or cfg.h323 ~= c.h323 or cfg.rtsp ~= c.rtsp 
                or cfg.pptp ~= c.pptp or cfg.l2tp ~= c.l2tp 
                    or cfg.ipsec ~= c.ipsec or cfg.sip ~= c.sip
        then
            ...
            uci:set(module_name, c[".name"], "sip", nat_on_off(cfg.sip))
...
function NAT_INST:check_hnat_status()
	local uci = nat_uci_cursor()
    -- qos status
	local qos_en = uci:get("qos_v2", "settings", "enable") or "off"
	
	-- traffic statistics status
	local tfs_en = uci:get("tfstats", "switch", "enable") or "off"

	-- parental controls status, mtk hwnat support pctlv2
	--	local pc_count = 0
	--	uci:foreach("parental_control_v2", "owner",
	--		function(section)
	--			pc_count = pc_count + 1
	--		end
	--	)
	--	if "off" == qos_en and "off" == tfs_en and 0 == pc_count then
	local wan_type = uci:get("network", "wan", "wan_type") or "dhcp"
	if ("off" == qos_en or ( wan_type == "v6plus" or wan_type == "dslite")) and "off" == tfs_en  then
		return true
	end
	return false
end

function NAT_INST:sync_hnat_status()
	local en_hnat = NAT_INST:check_hnat_status()
	hnat_set_onoff(en_hnat)
end
