...
fw_openvpn_access(){
    ...
    [ -e /proc/fcache/ ] && {
		local skip_rule=$(fw list 4 m FORWARD | grep -i "BLOGSKIP" | grep "tun")
		[ -z "${skip_rule}" ] && {
			fw_s_add 4 m FORWARD BLOGSKIP 1 { "-i tun+" }
			fw_s_add 4 m FORWARD BLOGSKIP 1 { "-o tun+" }
		}
    }
}
...
fw_openvpn_block(){
    ...
    if [ "$enabled" = "off" -o "$vpntype" != "openvpn" ]; then
        [ -e /proc/fcache/ ] && {
            local skip_rule=$(fw list 4 m FORWARD | grep -i "BLOGSKIP" | grep "tun")
            [ -n "${skip_rule}" ] && {
                fw_s_del 4 m FORWARD BLOGSKIP { "-i tun+" }
                fw_s_del 4 m FORWARD BLOGSKIP { "-o tun+" }
            }
        }
    fi
}
