...
. /lib/bcmenet/bcmenet_rtl8367s.sh
...
iptv_init()
{
	...
	# for bcm models which have rtl8367s as external switch
...
{	
	...
	for i in ${portSeq} ; do
		if [ $((bridgePortMask&(1<<(i-1)))) -ne 0 ] ; then
			continue
		fi
		...
		local vid=""
		eval "vid=\${vid_p$k}"
		create_vdevice $LAN_PHY_IF $vid ${int_lan}.$vid 0 "all" "t"
		
		echo vlanset $vid $((65536+(1<<(i-1)))) $((1<<(i-1))) > /proc/driver/phy/rtl8367s
		echo pvidset $((i-1)) $vid 0 > /proc/driver/phy/rtl8367s 
	done

	set_iface_mode "$LAN_PHY_IF" 3 1 1
}
...
iptv_load()
{
	...
	[ "$iptv_en" = "on" ] && {
		...
		#add by wanghao, create lan vlan devices
		create_lan_vlan_ifs
...
iptv_unload()
{
	...
	#add by wanghao
	echo vlanreset > /proc/driver/phy/rtl8367s
	echo vlaninit > /proc/driver/phy/rtl8367s
	#add end
...
snooping_is_loaded()
{
	local en=$(uci_get_state iptv core snooping) 
	return $((! ${en:-0}))
}
#add end
...
iptv_stop()
{
	...
	#add by wanghao
	! iptv_is_loaded && snooping_is_loaded && {
		clear_all_rules ${lan_ifs}
		echo vlanreset > /proc/driver/phy/rtl8367s
		echo vlaninit > /proc/driver/phy/rtl8367s
		iptv_reset_lan
		
		uci_toggle_state iptv core snooping 0
		reload=1
	}
	#add end
...
iptv_start()
{
	...
	#add by wanghao, create lan vlan devices
	config_get snooping_en "iptv" "igmp_snooping_enable"
	[ "$snooping_en" = "on" ] && {
		restart=$(uci_get_state iptv core snooping)
		if [ -z "$restart" ]; then
			restart=1
		fi
		...
		uci_revert_state iptv
		uci_set_state iptv core "" state
		uci_set_state iptv core snooping 1
		iptv_set_device_attr keepup 1
		...
		#during startup, netifd start before iptv and it runs fail cause the devices in lan.ifname have not been created yet,
		#so we must restart netifd to make it work.
		if [ "$restart" = "1" ]; then
			/etc/init.d/network restart
		else
			/etc/init.d/network reload
