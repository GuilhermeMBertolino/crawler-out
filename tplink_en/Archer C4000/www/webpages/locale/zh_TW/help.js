(function($){
 ...
 $.su.CHAR.HELP ={
       ...
       content: [ 
                "1. 點選 增加。",
                "2. 點選掃描從列表選擇一項服務以自動填寫適當的通訊埠號碼到外部通訊部與內部通訊部欄位。如果服務不在列表上請輸入外部通訊部號碼(例如：21)或通訊部範圍(例如：21-25)。如果內部通訊部與外部通訊部相同，該欄位可以保留空白，否則如果外部通訊部是單一埠，請輸入特定的通訊部號碼(例如：21)。請輸入運行這些服務應用的電腦的IP位址到內部IP欄位。（IP位址格式為x.x.x.x，x為數字並且在0-255之間。）",
       ...
       content: [{                      
                ...
                "1. 點選 增加。",
                "2. 點選掃描從列表中選擇一個應用以自動填寫預設值到適當欄位。如果您要增加不在列表中的應用，請手動輸入應用、觸發通訊埠、觸發協定、外部通訊部與外部協定。",
                "3. 選擇 啟用。",
                "4. 點選儲存。"]
} ,{                      
                           type: "note",
       ...
       content: [ 
                ...
                "註：當超過5磁碟區連接到路由器時，會出現共享圖示。您可以點選圖示來停用不需要共享磁碟區。"]
}]
},
       ...
       children: [{                      
                           ...
                           type: "name",
                           title: "SFTP",
                           content: "選擇此選項以啟用SFTP伺服器功能，允許SFTP用戶端和網路上的使用者透過下面位址欄位顯示的SFTP位址存取USB儲存設備。"
                       }]
} ,{                      
                           type: "name",
                           title: "位址",
       ...
       BASIC_FOLDER_SHARING :{ 
               ...
               CONTENT: [{
                           ...
                           type: "note",
                           title: "註",
                           content: "當超過5磁碟區連接到路由器時，會出現共享圖示。您可以點選圖示來停用不需要共享磁碟區。"
                       }]
},
       FOLDER_SHARING :{ 
               TITLE:"共享內容與安全性",
       ...
       content: [ 
                ...
                "密碼應該包含1-15個字元，可以輸入英文字母、數字或下底線。"]
                           ...
                           type: "name",
                           title: "允許訪客網路存取",
                           content: "切換至「開」，以允許訪客網路使用者可以存取所有的共享資料夾。"
                       } ,{                      
                           type: "name",
                           title: "啟用媒體共用",
