...
dlogin_iface_event()
{
    ...
    then
        ...
        insmod $DLOGIN_KER_PATH/domain_dns.ko lan_ip=$_ipaddr dut_domain="\"$_domains\"" ignore_me=0 not_router=1
    fi
    insmod $DLOGIN_KER_PATH/domain_redirect.ko
    [ "$sysmode" == "router" ] && dlogin_dnsmasq_restart
