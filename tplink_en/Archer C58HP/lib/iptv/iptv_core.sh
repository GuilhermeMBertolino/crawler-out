...
iptv_init()
{
	...
	export "LAN_CPU_PORT"="${LAN_DFT_CPU:-0}"
...
internet_vlan_do_ex()
{
	...
	[ $internet_vid -ge 0 ] && {
		...
		if [ "$internet_tag" = "on" ]; then
			create_vif $WAN_PHY_IF $internet_vid
			up_iface "$WAN_PHY_IF.$internet_vid"
...
bridge_wan()
{
	...
	[ $wan_vid -ge 0 -a -n "$ports" ] && {
		...
		if [ ${GMAC_NUM:-1} -eq 2 -a "${type_name}" = "wan" ];then		
			ip link set br-wan address 00:0a:e7:2c:44:99
		fi 
		...
		if [ ${GMAC_NUM:-1} -eq 2 -a "${type_name}" = "wan" ];then
			...
			create_vif $WAN_PHY_IF $wan_vid
			up_iface "$WAN_PHY_IF.$wan_vid"
...
iptv_load()
{
	...
	# for vlan tag passthrough functionality
	echo 1 > /proc/ag71xx_switch/iptv
	
	[ "$iptv_en" != "on" ] && mode="Bridge"
	uci_set_state iptv core mode "$mode"
	for i in `seq $PHY_LAN_PORT_NUM`; do
		config_get port "iptv" "lan""$i"
		# refer to C25, let all port as internet port when igmp snooping enabled but iptv disabled.
	...
	#it seems ARP reply packet lost in internal switch, force to update DUT LAN MAC to hosts in the LAN subnet before find out root cause.
	#local lan_ip

	#network_get_ipaddr lan_ip "lan"
	#arping -I br-lan -U $lan_ip &
...
iptv_unload()
{
	...
	case $mode in
		Bridge)
			...
			[ -n "$br_wan_ports" ] && {
				del_br_member "br-wan" "${WAN_PHY_IF}"
			...
			#fw del 4 n postrouting_rule ACCEPT { -m physdev --physdev-is-bridged }
			fw del 4 n POSTROUTING ACCEPT { -m physdev --physdev-is-bridged }
...
iptv_stop()
{
	...
	# for vlan tag passthrough functionality
	echo 0 > /proc/ag71xx_switch/iptv
	...
	iptv_is_loaded && {
		...
		#Need to destroy br-wan here because of current network restart mechanism.
		down_iface "br-wan"
		destroy_br "br-wan"
	...
	#remove it after fixing ARP reply issue 
	#killall -9 arping
...
iptv_start()
{
	...
	config_get iptv_en "iptv" "enable"
	config_get igmp_snooping_en "iptv" "igmp_snooping_enable"
	config_get mcwifi_enable "iptv" "mcwifi_enable"
	
	remove_wifi_mc_drop_rules
	if [ "$mcwifi_enable" = "off" ]; then
	...
	[ "$iptv_en" = "on" -o "$igmp_snooping_en" = "on" ] && {
		...
		[ "$igmp_snooping_en" = "on" ] && iptv_igmp_snooping_set "lan" 1
		iptv_load $1
		return
	}
