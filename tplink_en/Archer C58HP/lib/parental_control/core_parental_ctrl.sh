...
# $3:chain
# $4:key-word
fw_parental_check_chain() {
	local list=
	[ -n "$4" ] && list=$(fw list $1 $2 $3 |grep -- $4 | wc -l) || list=$(fw list $1 $2 $3 |wc -l)
    [ $list == 0 ] && echo "0" || echo "1"    
}
...
fw_exit_parental_ctrl(){
    ...
    fw flush 6 f parental_ctrl
    fw flush 4 f parental_ctrl_list
    fw flush 6 f parental_ctrl_list
    fw flush 4 f parental_ctrl_block
    fw flush 6 f parental_ctrl_block
  
    local rule_tcp="-p tcp -m multiport --dports 80"
    local rule_udp="-p udp -m multiport --dports 53,443"
    ...
    fw s_del 6 f zone_lan parental_ctrl_block { "$rule_webserver" }

    fw s_del 4 f zone_lan_forward parental_ctrl_list { "$rule_tcp" }
    fw s_del 6 f zone_lan_forward parental_ctrl_list { "$rule_tcp" }

    fw s_del 4 f zone_lan_forward parental_ctrl_list { "$rule_udp" }
    fw s_del 6 f zone_lan_forward parental_ctrl_list { "$rule_udp" }
    ...
    if [[ $nss_custom == "on" ]]; then
       ...
       fw s_del 6 f zone_lan_forward "$target"
    fi

    fw s_del 4 f INPUT parental_ctrl_list { "$rule_udp" }
    fw s_del 6 f INPUT parental_ctrl_list { "$rule_udp" }

    fw del 4 f parental_ctrl
    fw del 6 f parental_ctrl
    fw del 4 f parental_ctrl_list
    fw del 6 f parental_ctrl_list
    fw del 4 f parental_ctrl_block
    fw del 5 f parental_ctrl_block

    fw s_del 4 f FORWARD ACCEPT { "-o br-lan -m conntrack --ctstate RELATED,ESTABLISHED" }
    fw s_del 6 f FORWARD ACCEPT { "-o br-lan -m conntrack --ctstate RELATED,ESTABLISHED" }
    fw s_add 4 f FORWARD ACCEPT 1 { "-m conntrack --ctstate RELATED,ESTABLISHED" }
    fw s_add 6 f FORWARD ACCEPT 1 { "-m conntrack --ctstate RELATED,ESTABLISHED" }

    #remove chains for ap mode
    local sysmode=$(uci get sysmode.sysmode.mode)
    if [ "$sysmode" == "ap" ]
    then
	#for ipv4 firewall
	[ $(fw_parental_check_chain 4 f forward zone_lan_forward) == "1" ] && fw del 4 f forward zone_lan_forward
	[ $(fw_parental_check_chain 4 f zone_lan_forward) == "1" ] && fw del 4 f zone_lan_forward
	[ $(fw_parental_check_chain 4 f FORWARD forward) == "1" ] && fw del 4 f FORWARD forward
	[ $(fw_parental_check_chain 4 f forward) == "1" ] && fw del 4 f forward
	#for ipv6 firewall
        [ $(fw_parental_check_chain 6 f forward zone_lan_forward) == "1" ] && fw del 6 f forward zone_lan_forward
        [ $(fw_parental_check_chain 6 f zone_lan_forward) == "1" ] && fw del 6 f zone_lan_forward
        [ $(fw_parental_check_chain 6 f FORWARD forward) == "1" ] && fw del 6 f FORWARD forward
        [ $(fw_parental_check_chain 6 f forward) == "1" ] && fw del 6 f forward
    fi
...
fw_load_global() {
    ...
    fw add 6 f parental_ctrl
    fw add 6 f parental_ctrl_list
    fw add 6 f parental_ctrl_block
    ...
    if [[ "$global_enable" == "on" ]]; then
        ...
        fw s_del 6 f FORWARD ACCEPT { "-m conntrack --ctstate RELATED,ESTABLISHED" }
        fw s_add 6 f FORWARD ACCEPT 1 { "-o br-lan -m conntrack --ctstate RELATED,ESTABLISHED" }

        if [[ "$global_dns_filter" == "on" ]]; then
            fw s_add 4 f INPUT parental_ctrl_list 1 { "$rule_udp" }
            fw s_add 6 f INPUT parental_ctrl_list 1 { "$rule_udp" }
        fi
    ...
    #create chains for ap mode
    local sysmode=$(uci get sysmode.sysmode.mode)
    if [ "$sysmode" == "ap" ]
    then
	#for ipv4 firewall
	[ $(fw_parental_check_chain 4 f zone_lan_forward) == "0" ] && fw add 4 f zone_lan_forward
	[ $(fw_parental_check_chain 4 f forward) == "0" ] && fw add 4 f forward
	[ $(fw_parental_check_chain 4 f FORWARD forward) == "0" ] && fw add 4 f FORWARD forward
	[ $(fw_parental_check_chain 4 f forward zone_lan_forward) == "0" ] && fw add 4 f forward zone_lan_forward
	#for ipv6 firewall
        [ $(fw_parental_check_chain 6 f zone_lan_forward) == "0" ] && fw add 6 f zone_lan_forward
        [ $(fw_parental_check_chain 6 f forward) == "0" ] && fw add 6 f forward
        [ $(fw_parental_check_chain 6 f FORWARD forward) == "0" ] && fw add 6 f FORWARD forward
        [ $(fw_parental_check_chain 6 f forward zone_lan_forward) == "0" ] && fw add 6 f forward zone_lan_forward
        ...
        if [[ "$global_dns_filter" == "on" ]]; then
            if [[ "$global_dns_forward" == "on" ]]; then
                ...
                fw s_add 6 f zone_lan_forward parental_ctrl_list 1 { "$rule_udp" }
            fi
        fi
...
fw_load_white_list() {
    ...
    if [[ $nss_custom == "on" ]]; then
        ...
        fw s_add 6 f parental_ctrl "$target"
    fi
    ...
    [ $global_filter_mode == "simple" ] && {
        ...
        fw s_add 6 f parental_ctrl RETURN { "$rule" }
    ...
    fw add 6 f parental_ctrl "REJECT --reject-with tcp-reset" { "-p tcp" }
    fw add 6 f parental_ctrl DROP
...
fw_load_black_list() {
    ...
    [ $global_filter_mode == "simple" ] && {
        ...
        fw s_add 6 f parental_ctrl DROP { "$rule" }
    ...
    fw add 6 f parental_ctrl RETURN
}
...
fw_load_device() {
    ...
    if [ $device_enable == 'on' ]; then
         ...
         fw s_add 6 f parental_ctrl_block DROP { "-m mac --mac-source ${mac//-/:}" }
