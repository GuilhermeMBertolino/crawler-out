...
wpa_supplicant_setup_vif() {
	...
	# These fields must be local, otherwise they reserve the values in the previous
	# interface, which results in the wrong configuration file.
	local key_mgmt
	local proto
	local freq
	local ieee80211w
	...
	[ -n "$config_methods" ] && {
		...
		wps_config_methods="config_methods=$config_methods display"
		update_config="update_config=1"
		wps_device_name="device_name=$device_name"
		wps_manufacturer="manufacturer=$manufacturer"
		wps_model_name="model_name=$model_name"
		wps_model_number="model_number=$model_number"
		wps_serial_number="serial_number=$serial_number"
		wps_device_type="device_type=1-0050F204-1"
...
EOF
	...
	then
		wpa_supplicant ${bridge:+ -b $bridge} -B $wait_for_wrap -P "/var/run/wifi-${ifname}.pid" -D ${driver:-wext} -i "$ifname" -c /var/run/wpa_supplicant-$ifname.conf $options
                wpa_cli -p "/var/run/wpa_supplicant-$vif" -a /lib/wifi/wpa_supplicant-update-led -B
                ledcli RE_INPROGRESS
	else
		kill -HUP $(cat /var/run/wifi-${ifname}.pid)
	fi
}
