...
proto_lan_setup() {
	...
	local ip_mode
	local ip_config
	local prefix prefixlen proto wan_proto
	...
	#check if pppoeshare
	wan_type=$(uci get network.wan.wan_type)
	if [ "$wan_type" == "pppoeshare" ]
	then
		ip_mode=$(uci get network.internet.ipv6_mode)
		wan_proto=$(uci get network.internet.proto)
		ip_config=$(uci get network.wan.ip_config)
		[ "$ip_mode" == "prefix" ] && {
			[ "$(uci -p /var/state get network.wan.up 2>/dev/null)" = "1" ] || {
				prefix=""
			}
			[ "$wan_proto" == "dhcp6c" -o "$wan_proto" == "pppoeshare" ] && [ "$ip_config" != "specified" ] && prefix=`cat /tmp/dhcp6c/prefix.info`
		}
	else
		ip_mode=$(uci get network.wanv6.ip_mode)
		wan_proto=$(uci get network.wanv6.proto)
		ip_config=$(uci get network.wanv6.ip_config)
		[ "$ip_mode" == "prefix" ] && {
			[ "$(uci -p /var/state get network.wanv6.up 2>/dev/null)" = "1" ] || {
				prefix=""
			}
			[ "$wan_proto" == "dhcp6c" -o "$wan_proto" == "pppoev6" ] && [ "$ip_config" != "specified" ] && prefix=`cat /tmp/dhcp6c/prefix.info`
		}
	fi
	...
	[ -z "$prefix" ] && {
        ...
        [ "$ula_enable" = "on" ] || return
        local ula_prefix=$(uci get protocol.ula.prefix)
        local ula_prefixlen=$(uci get protocol.ula.prefix_len)
        [ -n "$ula_prefix" ] || return

        prefix="$ula_prefix"
        prefixlen="$ula_prefixlen"
	}
	...
	# [ "$wanv6_type" == "6rd" ] && ip6addr="${prefix}:1::1"
