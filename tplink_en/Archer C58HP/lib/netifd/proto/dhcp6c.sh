...
proto_dhcp6c_create_conffile(){
	...
	[ "$ip_mode" == "non_temp" ] && {
		...
		[ "$dns_mode" == "dynamic" -o "$ip_config" == "slaac" ] && echo -e "\trequest domain-name-servers;" >> "${conffile}"
		echo -e "\tscript \"/lib/netifd/dhcp6c.script\";" >> "${conffile}"
...
proto_dhcp6c_teardown() {
	...
	[ -n "$ifname" ] && {
		...
		#echo "dhcp6c.sh [route -A inet6 del ::/0 dev $ifname]" > /dev/console
		route -A inet6 del ::/0 dev "$ifname"
	}
