...
fw_start() {
	local operationMode
	config_load sysmode
	config_get operationMode sysmode mode router
    ...
    then
        ...
        # tpsdk custom cmd by huluyao 20140423
        echo "Loading tpcmd"
        fw_load_tpcmd
...
fw_stop() {
	...
	env -i ACTION=stop /sbin/hotplug-call firewall
...
fw_restart() {
	fw_stop
	fw_start
	env -i ACTION=start /sbin/hotplug-call firewall
}
