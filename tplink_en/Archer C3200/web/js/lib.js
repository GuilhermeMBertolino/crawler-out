...
//                reset isComplete. beacuse it has benn already completed, it don't need to judge again.
    ...
    goToOtherPage: function(page, navName) {
        ...
        var timer2 = setInterval(function(){
            var li = $('#menuTree').find('a[url="' + page + '"]').parent('li');
            if (li.length === 0) {
                return;
            }
...
*/
	        ...
	        switch (type) {
	            case "title":
                    ...
                    if (!!obj.title) {
                        ...
                        if (!!obj.content) {
                            htmlStr += "<p>" + (obj.content ? obj.content : '') + "</p>";
                        }
                    } else {
	                htmlStr += "<h5 class=\"title title-title\">" + (obj.content ? obj.content : '') + "</h5>";
                    }
                    break;
                case "title2":
                    htmlStr += "<div class=\"title-container\">";
                    htmlStr += "<h5 class=\"title title-title2\">" + (obj.content ? obj.content : '') + "</h5>";
	                break;
	            case "name":
                    if (index >= 2) {
                        var level = index;
	            ...
	            case "paragraph":
	                ...
	                htmlStr += "<p class=\"paragraph\">" + (obj.content ? obj.content : '') + "</p>";
	                break;
                case "step":
	            case "note":
	...
	*/
    ...
    toastGlobal: function(options) {
        ...
        var html = '<div id="global-toast-container"><div class="global-toast-wrapper"><span id="global-toast-text"></span><span class="icon-close"></span> </span></div></div>';
        var toast_container = $('#global-toast-container').length > 0 ? $('#global-toast-container') : $(html).appendTo('body');
        toast_container.removeClass().fadeIn(400);
        ...
        $('#global-toast-container .icon-close').off('click').on('click', function() {
            $('#global-toast-container').fadeOut(400);
