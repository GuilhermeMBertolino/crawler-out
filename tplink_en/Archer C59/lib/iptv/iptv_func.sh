...
add_vlan_member()
{
	...
	local port=$2
	if [ $BOARD_TYPE = "qca" ]; then
		local port_var=REL_ID_PORT$2
		local port_id=$(eval echo \$$port_var)
		ssdk_sh vlan member add $vid $port_id unmodified
		case "$3" in
			t) ssdk_sh portVlan egress set $port_id tagged ;;
			*) ssdk_sh portVlan egress set $port_id untagged ;;
		esac
	else
		et robowr $PAGE_VTBL $REG_VTBL_INDX_5395 $vid
...
del_vlan_member()
{
	...
	if [ $BOARD_TYPE = "qca" ]; then
		local port_var=REL_ID_PORT$2
		local port_id=$(eval echo \$$port_var)
		ssdk_sh vlan member del $1 $port_id
	else
...
{
	[ $# != 2 ] && return
	local port_var=REL_ID_PORT$1
	local port_id=$(eval echo \$$port_var)
	case "$2" in
		t) ssdk_sh portVlan egress set $port_id tagged ;;
		u) ssdk_sh portVlan egress set $port_id unmodified ;;
		*) ssdk_sh portVlan egress set $port_id untagged ;;
	esac
}
...
set_pvid()
{
	...
	if [ $BOARD_TYPE = "qca" ]; then
		local port_var=REL_ID_PORT$1
		local port_id=$(eval echo \$$port_var)
		ssdk_sh portVlan defaultCVid set $port_id $2
		ssdk_sh portVlan defaultSVid set $port_id $2
	else
...
{
	[ $# != 1 ] && return
	echo "disable port $1 learn"
	local port_var=REL_ID_PORT$1
	local port_id=$(eval echo \$$port_var)
	ssdk_sh fdb portLearn set $port_id disable
}

enable_port_learn()
{
	[ $# != 1 ] && return
	echo "enable port $1 learn"
	local port_var=REL_ID_PORT$1
	local port_id=$(eval echo \$$port_var)
	ssdk_sh fdb portLearn set $port_id enable
}
...
create_vdevice()
{
	...
	if [ "$cpu_tag" = "t" ]; then
		disable_port_learn $port
		disable_port_learn $cpu_port 
...
create_hybrid_vlan()
{
	...
	for lan in $LAN_PHY_PORT_SET;do
		[ -z "$lan" ] && continue
		create_vdevice "eth1" $lan $lan "*" "0" "t"
	done
	flush_fdb
}
