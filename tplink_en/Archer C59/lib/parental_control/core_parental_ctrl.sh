...
fw_exit_parental_ctrl(){
    ...
    local rule_tcp="-p tcp -m multiport --dports 80,443"
...
fw_load_global() {
    ...
    if [[ "$global_enable" == "on" ]]; then
        ...
        fw s_add 4 f zone_lan_forward parental_ctrl_list 1 { "$rule_tcp" }
        ...
        fw s_add 4 f zone_lan parental_ctrl_block 1 { "$rule_webserver" }
...
fw_load_white_list() {
    ...
    [ $global_filter_mode == "simple" ] && [ -n "$urls" ] && {
...
fw_load_black_list() {
    ...
    [ $global_filter_mode == "simple" ] && [ -n "$keys" ] && {
