...
(function(){
		...
		*/
		...
		transHelp: function(helpUrl) {
			...
			for (var k in tags)
			{
				...
				{
					if (name_str[title][elem[i].className.split(" ")[0]])
						elem[i].innerHTML = name_str[title][elem[i].className.split(" ")[0]];
				}
			}
		},

        styleInputElements: function() {
            function updateElemFace(elem, elemFace) {
                if (elem.type != "radio" && elem.type != "checkbox") {
                    return;
                }
                if (!elem.checked) {
                        elemFace.className = elem.type + "_face unchecked";
                } else {
                        elemFace.className = elem.type + "_face checked";
                }
                if (!elem.disabled) {
                    elemFace.className += " enabled";
                } else {
                    elemFace.className += " disabled";
                }
            }
            function addElemFace(elem) {
                var elemParent = elem.parentNode;
                var elemNextSibling = elem.nextSibling;
                var elemFace = document.createElement("span");
                switch(elem.type) {
                case 'radio':
                    elemFace.onclick = 
                        (function() {
                            /*
                             * 因为addElemFace函数是被循环调用的，所以如果直接在onclick函数中引用elem，则会
                             * 引用到退出循环时的elem 值，因此，这里要创建一个闭包来保存此时的elem值。
                             */
                            var ef = elemFace;
                            return function(e) {
                                var elemFace = ef;
                                var elem = elemFace.previousSibling;
                                var name = elem.name;
                                if (!elem.disabled && !elem.checked) {
                                    elemFace.className = "radio_face checked enabled"
                                    var elemArray = document.getElementsByName(elem.name);
                                    for (var i in elemArray) {
                                        if (elemArray[i] != elem && elemArray[i] != undefined) {
                                            if (elemArray[i].nextSibling != undefined) {
                                                elemArray[i].nextSibling.className = "radio_face unchecked enabled";
                                            }
                                        }
                                    }
                                    elem.click();
                    ...
                    if (elemParent != undefined) {
                        if (elemNextSibling != undefined) {
                            elemParent.insertBefore(elemFace, elemNextSibling);
                        } else {
                            elemParent.appendChild(elemFace);
                        }
                    }
                    break;
                case 'checkbox':
                    elemFace.onclick = 
                        (function(){
                            ...
                            return function(e) {
                                ...
                                if (!elem.disabled) {
                                    if (!elem.checked) {
                                        elemFace.className = "checkbox_face checked enabled"
                                    } else {
                                        elemFace.className = "checkbox_face unchecked enabled"
                                    }
                                }
                                elem.click();
                            }
            ...
            function updateStyle() {
                ...
                var numObjs = inputObjs.length;
                for (var i = 0; i < numObjs; i++) {
                    var elem = inputObjs[i];
                    var elemFace = elem.nextSibling;
                    if (elemFace == undefined || (!$.hasClass(elemFace, "radio_face") && 
                                                    !$.hasClass(elemFace, "checkbox_face"))) {
                        elemFace = addElemFace(elem);
                    }
                    updateElemFace(elem, elemFace);
                }
	...
	(function(){
		...
		*/
		...
		IGD_DEV_INFO = (typeof IGD_DEV_INFO == "undefined" ? "IGD_DEV_INFO" : IGD_DEV_INFO);
		ETH_SWITCH = (typeof ETH_SWITCH == "undefined" ? "ETH_SWITCH" : ETH_SWITCH);
		MULTIMODE = (typeof MULTIMODE == "undefined" ?  "MULTIMODE" : MULTIMODE );
		SYS_MODE = (typeof SYS_MODE == "undefined" ?  "SYS_MODE" : SYS_MODE );
		...
		var currLang = $.act(ACT_GET, IGD_DEV_INFO, null, null, ["X_TP_SupportedLanguages", "X_TP_CurrentLanguage", "X_TP_LanguageSetByUser"]);
