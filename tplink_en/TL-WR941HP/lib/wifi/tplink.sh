...
wifi_fixup_mode() {
	...
	local pureg=0 puren=0 pure11ac=0 deny11ab=0 disablecoext=1
	...
	if [ "$htmode" = "auto" ]; then
		...
		case $htmode in
			...
			40)  chwidth=1; htmode=HT40;;
			80)  chwidth=2; htmode=HT80;;
		esac
	fi
...
wifi_radio() {
	...
	for dev in ${1:-$DEVICES}; do
		...
		for vif in $vifs; do [ -d /sys/class/net/$vif ] && kill_pid "$vif" &>$STDOUT; done
