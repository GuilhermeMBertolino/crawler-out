...
_s_add() {
	...
	if [ "$fam" == e ]; then
		fw_check $fam $tab $chn "${2:+ $2} -j $tgt"
	else
...
fw_ap_re_rules(){
    ...
    fw policy i f INPUT ACCEPT
    fw policy i f OUTPUT ACCEPT
    fw policy i f FORWARD ACCEPT
    
    fw add i f INPUT ACCEPT
    fw add i f INPUT ACCEPT { -i lo }
    
    fw add i f OUTPUT ACCEPT
    fw add i f OUTPUT ACCEPT { -o lo }
    
    # Access control chain
    fw add i r zone_lan_notrack
}
