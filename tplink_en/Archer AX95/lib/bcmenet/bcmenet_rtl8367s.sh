...
#RGMII,cpuport=(1<<17)=131072
#SGMII,cpuport=(1<<16)=65536
cputag_8367s="131072"
...
roboswitch_passthrough_vlan_mode_config ()
{
	...
	for port in ${portSeq} ; do
		...
		if [ $port -lt 4 ]
		then
			eval "realIf=\${int_lan_8367s}"
		else
			eval "realIf=\${int_lan4}"
		fi
		...
		if [ "${internet_tag}" = "on" ] ; then
			...
			then
				#add by wanghao
				echo vlanset ${internet_vid} $(($cputag_8367s+(1<<(port-1)))) 0 > /proc/driver/phy/rtl8367s
			...
			then
				#add by wanghao
				echo vlanset ${internet_vid} $(($cputag_8367s+(1<<(port-1)))) $((1<<(port-1))) > /proc/driver/phy/rtl8367s
		...
		if [ "${iptv_vid}" != "0" ] ; then
			...
			if [ "${mciptv_enable}" = "on" ] ; then
				...
				then
					#add by wanghao
					echo vlanset ${iptv_vid} $(($cputag_8367s+(1<<(port-1)))) 0 > /proc/driver/phy/rtl8367s
					echo vlanset ${mciptv_vid} $(($cputag_8367s+(1<<(port-1)))) 0 > /proc/driver/phy/rtl8367s
					#add end
				fi
			else
		...
		if [ "${ipphone_vid}" != "0" ] ; then
			...
			then
				#add by wanghao
				echo vlanset ${ipphone_vid} $(($cputag_8367s+(1<<(port-1)))) 0 > /proc/driver/phy/rtl8367s
				#add end
			fi
		fi
...
get_lan_wan_ifs()
{
    ...
    local lan_ifname=$(uci get switch.${lan_sec}.ifname)
    lan_ifs=${lan_ifname}
    
    local lan4_sec=$(uci get switch.lan4.switch_port)
    local lan4_ifname=$(uci get switch.${lan4_sec}.ifname)
    lan_ifs=${lan_ifs}" "${lan4_ifname}

    int_lan4=${lan4_ifname}
    
    #echo "ALL lan: $lan_ifs" >/dev/console
}
...
rtl8367s_get_interface_group ()
{
	...
	if [ ${iptv_enable} == "on" ] ; then
		for i in ${portSeq} ; do
				...
				if [ $i -lt 4 ]
				then
					eval "${portgroup}=\"\${${portgroup}}\${int_lan_8367s}.\${vid_p$((i-1))}\""
				else
					eval "${portgroup}=\"\${${portgroup}}\${int_lan4}\""
				fi
			#fi
		done
	#add by wanghao
...
iptv_lan_snooping_change()
{
	...
	if [ "$igmp_snooping_en" = "off" -a "$lan_snooping" = "0" -a "$iptv_en" = "off" ]; then
...
c5400s_interface_init_config ()
{
	...
	if [ "$mode" = "router" ] ; then
		...
		for ent in staticv6 dhcpv6 pppoev6 pppoeshare 6to4 passthrough dslite 6rd ; do
			if ifname=`uci get protocol.$ent.ifname 2> /dev/null` ; then
				if [ $ifname != "${int_wan}" ] ; then
					protchange=1
					uci set protocol.$ent.ifname="${int_wan}"
...
# switch phy power up
# $1 switch port bitmap
bcm_6750_port_power_up()
{
	local wandev=${int_wan}
	local lan4dev=${int_lan4}
	local apmode=$1
	
	if [ $apmode -eq 1 ]; then
		ethctl $wandev phy-power up
	fi

	ethctl $lan4dev phy-power up
	echo physet 1 > /proc/driver/phy/rtl8367s
}

# switch phy power down
# $1 switch port bitmap
bcm_6750_port_power_down()
{
	local wandev=${int_wan}
	local lan4dev=${int_lan4}
	local apmode=$1
	...
	if [ $apmode -eq 1 ]; then
		ethctl $wandev phy-power down
	fi

	ethctl $lan4dev phy-power down
