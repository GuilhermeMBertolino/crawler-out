...
vifs_to_dev()
{
	...
	for vif in $*; do
		...
		if [ "${found}" = "true" ];then
			echo "vif="${vif}" device="${device} >$STDOUT
			echo ${CUR_DEVICE} | grep -q ${device} || CUR_DEVICE="${CUR_DEVICE} ${device}"
			found="false"
		else
			echo "vif="${vif}" not valid..." >$STDOUT
		fi
	done
	
	echo "CUR_DEVICE="${CUR_DEVICE} >$STDOUT
}
...
wifi_country() {
	...
	get_wlan_ini COUNTRYCODE_RU_2G
	get_wlan_ini COUNTRYREV_RU_2G
	get_wlan_ini COUNTRYCODE_RU_5G
	get_wlan_ini COUNTRYREV_RU_5G
	get_wlan_ini BAND_DIFFCOUNTRY_RU
	...
	if [ "$cmd_flag" = "0" ]; then
		...
		for dev in ${devices}; do
			...
			if [ "${FEATURE_HP_FLAG}" = "y" ]; then
				wifi_add_hp_flag
			else
				echo "=====>>>>> NO FEATURE_HP_FLAG" >$STDOUT
			fi
...
wifi_portal_config() {
	...
	if [ -n "${PORTAL_IFNAMES}" ];then
		...
		echo "PORTAL_IFNAMES ${PORTAL_IFNAMES}" > $CONSOLE
		echo "${PORTAL_IFNAMES}" > /proc/portal_filter/portal_interface_list
	else
		if ps | grep -v grep| grep 'wifidog' > /dev/null;then
			/etc/init.d/wifidog stop
			echo "no portal ifnames, wifidog end" > $CONSOLE
		fi
	fi
}
...
wifi_vlan() {
	...
	if [ $1 = "wdsup" ];then
		echo "[debug.ljx] brctl setifvlan $brname $2 $hvlan 1" > $STDOUT
		brctl setifvlan "$brname" "$2" "$hvlan" 1
		return;
	fi
...
init_nvram_defaults() {
	...
	for ifname in ${ifname_list}; do
		...
		nvram set ${ifname}_wmf_mdata_sendup="0"
...
wifi_acs_config()
{
	...
	local ACS_BW_80UP160=""
	local ACS_TP_INCL_20_CHANS=""
	local ACS_TP_INCL_40_CHANS=""
	local ACS_TP_INCL_80_CHANS=""
	local ACS_TP_INCL_160_CHANS=""
	...
	case $band in
		2g)
			get_wlan_ini ACS_TP_INCL_20_CHANS_2G
			get_wlan_ini ACS_TP_INCL_40_CHANS_2G
			
			HOME_WIFI=${NAME_HOME_2G}
			
			ACS_TP_INCL_20_CHANS=${ACS_TP_INCL_20_CHANS_2G}
			ACS_TP_INCL_40_CHANS=${ACS_TP_INCL_40_CHANS_2G}
		;;
		5g)
			get_wlan_ini ACS_BW_80UP160_5G
			get_wlan_ini ACS_TP_INCL_20_CHANS_5G
			get_wlan_ini ACS_TP_INCL_40_CHANS_5G
			get_wlan_ini ACS_TP_INCL_80_CHANS_5G
			get_wlan_ini ACS_TP_INCL_160_CHANS_5G
			
			HOME_WIFI=${NAME_HOME_5G}
			
			ACS_BW_80UP160=${ACS_BW_80UP160_5G}
			ACS_TP_INCL_20_CHANS=${ACS_TP_INCL_20_CHANS_5G}
			ACS_TP_INCL_40_CHANS=${ACS_TP_INCL_40_CHANS_5G}
			ACS_TP_INCL_80_CHANS=${ACS_TP_INCL_80_CHANS_5G}
			ACS_TP_INCL_160_CHANS=${ACS_TP_INCL_160_CHANS_5G}
		;;
		5g_2)
			get_wlan_ini ACS_BW_80UP160_5G2
			get_wlan_ini ACS_TP_INCL_20_CHANS_5G2
			get_wlan_ini ACS_TP_INCL_40_CHANS_5G2
			get_wlan_ini ACS_TP_INCL_80_CHANS_5G2
			get_wlan_ini ACS_TP_INCL_160_CHANS_5G2
			
			HOME_WIFI=${NAME_HOME_5G2}
			
			ACS_BW_80UP160=${ACS_BW_80UP160_5G2}
			ACS_TP_INCL_20_CHANS=${ACS_TP_INCL_20_CHANS_5G2}
			ACS_TP_INCL_40_CHANS=${ACS_TP_INCL_40_CHANS_5G2}
			ACS_TP_INCL_80_CHANS=${ACS_TP_INCL_80_CHANS_5G2}
			ACS_TP_INCL_160_CHANS=${ACS_TP_INCL_160_CHANS_5G2}
		;;
		6g)
			get_wlan_ini ACS_TP_INCL_20_CHANS_6G
			get_wlan_ini ACS_TP_INCL_40_CHANS_6G
			get_wlan_ini ACS_TP_INCL_80_CHANS_6G
			get_wlan_ini ACS_TP_INCL_160_CHANS_6G
			
			HOME_WIFI=${NAME_HOME_6G}
			
			ACS_TP_INCL_20_CHANS=${ACS_TP_INCL_20_CHANS_6G}
			ACS_TP_INCL_40_CHANS=${ACS_TP_INCL_40_CHANS_6G}
			ACS_TP_INCL_80_CHANS=${ACS_TP_INCL_80_CHANS_6G}
	...
	if [ "${FEATURE_ACS_CONFIG}" = "y" ]; then
		...
		nvram set ${HOME_WIFI}_acs_tp_incl_20_chans=${ACS_TP_INCL_20_CHANS}
		nvram set ${HOME_WIFI}_acs_tp_incl_40_chans=${ACS_TP_INCL_40_CHANS}
		nvram set ${HOME_WIFI}_acs_tp_incl_80_chans=${ACS_TP_INCL_80_CHANS}
...
wifi_security_config() {
	...
	if [ "$eth_enable" = "1" ]; then
		...
		#	
			...
			wifi_security_nvram "$dev" "$vif" "${WIFI_PRE}" 
			if [ "$encryption" == "psk" -a "$psk_cipher" == "tkip"] || [ "$encryption" == "wep" ]; then
				close11N="1"
			fi
...
wifi_wps_triband()
{
	get_wlan_ini FEATURE_TRIBAND
	get_wlan_ini FEATURE_FOURBAND
	if [ "${FEATURE_TRIBAND}" = "y" -o "${FEATURE_FOURBAND}" = "y" ]; then
		config_get_bool wifi_disabled_5g1 ${NAME_HOME_5G} disabled_all
	 	config_get_bool wifi_disabled_5g2 ${NAME_HOME_5G2} disabled_all
		if [ "$wifi_disabled_5g1" = "0" -a "$wifi_disabled_5g2" = "0" ]; then
			wps_5g1_tmp="`nvram get ${NAME_HOME_5G}_wps_mode`"
			wps_5g2_tmp="`nvram get ${NAME_HOME_5G2}_wps_mode`"
			if [ "$wps_5g1_tmp" = "enabled" -a "$wps_5g2_tmp" = "enabled" ]; then
				echo "=====>>>>> close 5g2 wps_mode " >$CONSOLE
				nvram set ${NAME_HOME_5G2}_wps_mode="disabled"
			fi
		fi
	fi
}
...
wifi_backhaul_config() {
	...
	if [ "$backhaul" == "on" ]; then
		...
		wifi_security_nvram "$dev" "$vif" "${WIFI_BACKHAUL}" 
		if [ "$encryption" == "psk" -a "$psk_cipher" == "tkip" ]; then
...
wifi_security_nvram(){
	...
	if [ "$encryption" == "none" ]; then
		nvram set ${WIFI_NAME}_preauth=""
		nvram set ${WIFI_NAME}_wep="disabled"
		nvram set ${WIFI_NAME}_auth="0"
		nvram set ${WIFI_NAME}_akm=""
		nvram set ${WIFI_NAME}_mfp="0"
	elif [ "$encryption" == "psk" ]; then
		nvram set ${WIFI_NAME}_preauth=""
		if [ "$psk_version" == "wpa" ]; then
			nvram set ${WIFI_NAME}_akm="psk"
		elif [ "$psk_version" == "rsn" ]; then
			nvram set ${WIFI_NAME}_akm="psk2"
		else
			nvram set ${WIFI_NAME}_akm="psk psk2"
		fi
		if [ "$psk_cipher" == "aes" ]; then
			nvram set ${WIFI_NAME}_crypto="aes"
		elif [ "$psk_cipher" == "tkip" ]; then
			nvram set ${WIFI_NAME}_crypto="tkip"
		else
			if [ "$hwmode" == "11ax" -o "$hwmode" == "11ax_5" ]; then
				#ax only mode donnot support tkip
				nvram set ${WIFI_NAME}_crypto="aes"
			else
				nvram set ${WIFI_NAME}_crypto="tkip+aes"
			fi
		fi

		config_get psk_key $vif psk_key
		nvram set ${WIFI_NAME}_wpa_psk="$psk_key"
		config_get wpa_group_rekey $dev wpa_group_rekey
		if [ -z "$wpa_group_rekey" ]; then
			nvram set ${WIFI_NAME}_wpa_gtk_rekey="0"
		else
			nvram set ${WIFI_NAME}_wpa_gtk_rekey="$wpa_group_rekey"
		fi
		nvram set ${WIFI_NAME}_wep="disabled"
		nvram set ${WIFI_NAME}_auth="0"
		nvram set ${WIFI_NAME}_mfp="0"
	#add by zhangshengbo for wpa3 personal	
		...
		if [ "$psk_version" == "sae_transition" ]; then
			nvram set ${WIFI_NAME}_akm="psk2 sae"
		else
		#psk_version==sae_only
			nvram set ${WIFI_NAME}_akm="sae"
		fi
		nvram set ${WIFI_NAME}_crypto="aes"
	...
	#add by xuyaming for owe
		...
		nvram set ${WIFI_NAME}_akm="owe"
		nvram set ${WIFI_NAME}_crypto="aes"
		nvram set ${WIFI_NAME}_mfp="2"
		nvram set ${WIFI_NAME}_wep="disabled"
		nvram set ${WIFI_NAME}_auth="0"
		...
		nvram set ${WIFI_NAME}_preauth="0"

		config_get wpa_version $vif wpa_version
		config_get wpa_cipher $vif wpa_cipher

		if [ "$wpa_version" == "wpa" ]; then
			nvram set ${WIFI_NAME}_akm="wpa"
		elif [ "$wpa_version" == "rsn" ]; then
			nvram set ${WIFI_NAME}_akm="wpa2"
		else
			nvram set ${WIFI_NAME}_akm="wpa wpa2"
		fi
		if [ "$wpa_cipher" == "aes" ]; then
			nvram set ${WIFI_NAME}_crypto="aes"
		...
		nvram set ${WIFI_NAME}_radius_ipaddr="$server"
		config_get port $vif port
		nvram set ${WIFI_NAME}_radius_port="$port"
		config_get wpa_key $vif wpa_key
		nvram set ${WIFI_NAME}_radius_key="$wpa_key"
		config_get wpa_group_rekey $dev wpa_group_rekey
		if [ -z "$wpa_group_rekey" ]; then
			nvram set ${WIFI_NAME}_wpa_gtk_rekey="0"
		else
		...
		nvram set ${WIFI_NAME}_preauth=""
		nvram set ${WIFI_NAME}_wep="enabled"
		config_get wep_mode $vif wep_mode
		if [ "$wep_mode" == "shared" ]; then
			nvram set ${WIFI_NAME}_auth="1"
		else
			nvram set ${WIFI_NAME}_auth="0"
		fi

		config_get wep_key1 $vif wep_key1
		nvram set ${WIFI_NAME}_key1="$wep_key1"
		nvram set ${WIFI_NAME}_key="1"
		nvram set ${WIFI_NAME}_akm=""
		nvram set ${WIFI_NAME}_mfp="0"
	else
		nvram set ${WIFI_NAME}_wep="disabled"
...
wifi_guest_config() {
	...
	if [ "$eth_enable" = "1" -a "$guest_enable" = "1" ]; then
		...
		wifi_security_nvram "$dev" "$vif" "${GUEST_WIFI}" 
...
start_hostapd(){
	...
	if [ $( nvram get hapd_enable ) == "1" ]; then
		for dev in ${1:-$devices}; do
			...
			case "$band" in
				2g)
					get_wlan_ini HOSTAPD_ORDER_2G
					index=${HOSTAPD_ORDER_2G}
				;;
				5g)
					get_wlan_ini HOSTAPD_ORDER_5G
					index=${HOSTAPD_ORDER_5G}
				;;
				5g_2)
					get_wlan_ini HOSTAPD_ORDER_5G2
					index=${HOSTAPD_ORDER_5G2}
				;;
				6g)
					get_wlan_ini HOSTAPD_ORDER_6G
					index=${HOSTAPD_ORDER_6G}
				;;
			esac
		
		hapdsupport -n$index >$STDOUT
...
wifi_smart_config()
{
	...
	if [ "${FEATURE_FOURBAND}" = "y" ] || [ "${FEATURE_TRIBAND}" = "y" -a "${FEATURE_6G}" != "y" ]; then
...
wifi_led_set_lp5523()
{
    ...
    if [ "$lp5523_flag" == "chip-on" ]; then
		...
		if [ "${FEATURE_TRIBAND}" != "y" -a "${FEATURE_FOURBAND}" != "y" -a "$wifi_2g_off" = "1" -a "$wifi_5g_off" = "1" ] ; then
			ubus send leds '{"action" : "2", "status" : "0"}'
		elif [ "${FEATURE_TRIBAND}" = "y" -a "${FEATURE_6G}" = "y" -a "$wifi_2g_off" = "1" -a "$wifi_5g_off" = "1" -a "$wifi_6g_off" = "1" ] ; then
			ubus send leds '{"action" : "2", "status" : "0"}'
		elif [ "${FEATURE_TRIBAND}" = "y" -a "${FEATURE_6G}" != "y" -a "$wifi_2g_off" = "1" -a "$wifi_5g_off" = "1" -a "$wifi_5g_2_off" = "1" ] ; then
			ubus send leds '{"action" : "2", "status" : "0"}'
		elif [ "${FEATURE_FOURBAND}" = "y" -a "$wifi_2g_off" = "1" -a "$wifi_5g_off" = "1" -a "$wifi_5g_2_off" = "1" -a "$wifi_6g_off" = "1" ] ; then
			ubus send leds '{"action" : "2", "status" : "0"}'
		else
...
wifi_reload() {
	...
	get_wlan_ini FEATURE_WIFI_PORTAL
	...
	if [ "${FEATURE_WIFI_PORTAL}" = "y" ]; then
		wifi_portal_config
	else
		echo "=====>>>>> NO WIFI_PORTAL" >$STDOUT
	fi
