...
found_unuse_vid()
{
	local vid=$LAST_USE_VID
	while [ $vid -le 4095 ]; do
		if [ "$vid" = "$WAN_HNAT_ID" ]; then
			vid=$((vid+1))
			continue
		fi
		if [ "$vid" = "$LAN_HNAT_ID" ]; then
			vid=$((vid+1))
			continue
		fi
		...
		if [ "$vid" = "$iptv_vid" ]; then
			vid=$((vid+1))
			continue
		fi
		if [ "$vid" = "$mciptv_vid" ]; then
			vid=$((vid+1))
			continue
		fi
		if [ "$vid" = "$ipphone_vid" ]; then
			vid=$((vid+1))
			continue
		fi
		break
