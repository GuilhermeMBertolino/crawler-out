...
wifi_fixup_mode() {
	...
	local pureg=0 puren=0 pure11ac=0 deny11ab=0 disablecoext=0 mu_mimo=0
	...
	case $htmode in
		...
		80_80) chwidth=3; htmode=HT80_80; disablecoext=1;;
	...
	for vif in $(config_get "$dev" vifs); do 
		...
		[ "$hwmode" = "11ng" ] && {
			config_set "$vif" vht_11ng 1
			config_set "$vif" vht_11ng_intop 1
		}
...
wifi_fixup_config() {
	...
	ls -d /sys/class/net/${brname} &>$STDOUT || for brname in $(cd /sys/class/net && ls -d br-lan* 2>$STDOUT); do break; done
	...
	for dev in ${1:-DEVICES}; do
		...
		for vif in $(config_get "$dev" vifs); do 
			...
			[ "$wps" = "1" ] && {
				...
				#config_set "$vif" pbc_in_m1 1
				[ "$band" = "2g" ] && config_set "$vif" wps_rf_bands ag
				[ "$band" = "5g" ] && config_set "$vif" wps_rf_bands ag
			}
			...
			config_set "$vif" metimeout 260000
...
config_country() {
	...
	config_get country_id "$1" country_id 0	
	config_get_bool blockdfs "$1" blockdfslist 0
	iwpriv "$1" blockdfslist "$blockdfs"
	iwpriv "$1" blockdfschlist "$blockdfs"
	if [ "$country_id" -ne 0 ]; then
		iwpriv "$1" setCountryID "$country_id"
	elif [ -n "$country" ]; then
...
config_vap_mode() {
	local cfg cfgs="rts frag pureg puren pure11ac deny11ab disablecoext vht_11ng vht_11ng_intop"
	...
	[ -n "$vht_11ng_intop" ] && iwpriv "$1" 11ngvhtintop "$vht_11ng_intop"
	[ -n "$disablecoext" ] && iwpriv "$1" disablecoext "$disablecoext"
	[ -n "$frag" ] && iwconfig "$vif" frag "$frag"
	[ -n "$rts" ] && iwconfig "$vif" rts "$rts"
	if [ x$mode = x"11ACVHT80_80" ]; then
		local chan_freq1=`echo $chan  | awk '{split($0,arr,":");print arr[1]}'`
		local chan_freq2=`echo $chan  | awk '{split($0,arr,":");print arr[2]}'`
		chan="$chan_freq1"
		echo "=======>111111chan=$chan">/dev/console
		if [  x$chan_freq1 != x"auto" -a x$chan_freq2 != x"auto" ];then
			iwpriv "$1" cfreq2 "$chan_freq2"
			echo "=======>222222222chan_freq2=$chan_freq2">/dev/console
		fi
		echo "=======>333333333chan=$chan,chan_freq2=$chan_freq2">/dev/console
	fi	
...
create_vap() {
	...
	local b c v n
	local cfg="!shortgi !disablecoext !wds=1 TxBFCTL bintval !countryie !uapsd=1 powersave !ant_ps_on ps_timeout \
			mcastenhance metimer metimeout !medropmcast me_adddeny vap_ind extap scanband periodicScan cwmin cwmax aifs \
			txoplimit noackpolicy !wmm !doth !ignore11d doth_chanswitch quiet mfptest noedgech ps_on_time inact wnm=1 ampdu amsdu maxampdu vhtmaxampdu \
			...
			!send_add_ies !ext_ifu_acs rrm=1 !rrmslwin !rrmstats rrmdbg acparams setwmmparams !qbssload !proxyarp !l2tif !dgaf_disable setibssdfsparam \
	...
	#TP IE will use MAC addr on wifi0(2G)
        config_get tp_ie_hw_addr "wifi0" macaddr
	...
	gp_id_rand="0x${gp_id_rand:0:2} 0x${gp_id_rand:2:2}"
	...
	if [ $htmode = "HT80_80" ];then
			config_get secondary_channel "$dev" secondary_channel 0
			channel="$channel":"$secondary_channel"
			echo "=============>create_vap:htmode=$htmode,channel=$channel" > /dev/console
	fi
	config_get_bool guest "$vif" guest 0
	# Comments:guestnetwork
	# following settings may incur restart of main network, so don't config them
	# when dealing with guestnetwork in order to avoid main network connection
	# interruption when changing guestnetwork config.
	if [ "$guest" = "0" ]; then
	...
	[ "$onemesh_enable" = "on" ] && [ "$onemesh_ie" = "on" ] && [ "$sysmode" = "router" ] && {
		iwpriv "$vif" setiebuf 0x0 0x0 0x0 0x0 0x0 0x0 0x0 0x20 0x8 0xdd 0x1e 0x0 0x1d 0x0f 0x10 0x1 0x7 0x0 0x0 $tpie_mac $tpie_mac $random_suffix 0x0 0x0 ${tpie_mac:21:9} 0x0 0x1 0x0 0x0
		iwpriv "$vif" setiebuf 0x0 0x0 0x0 0x2 0x0 0x0 0x0 0x20 0x8 0xdd 0x1e 0x0 0x1d 0x0f 0x10 0x1 0x7 0x0 0x0 $tpie_mac $tpie_mac $random_suffix 0x0 0x0 ${tpie_mac:21:9} 0x0 0x1 0x0 0x0
	}
	
	config_get smart_enable smart smart_enable "off"
	if [ "$sysmode" = "router" -a "$onemesh_enable" = "off" -a "$smart_enable" = "off" ] || [ "$sysmode" = "ap" -a "$smart_enable" = "off" ];then
		iwpriv "$vif" rrm 0
		iwpriv "$vif" wnm 0
		#iwpriv "$vif" wds 0
	fi

	[ "$mode" = "ap" ] && iwpriv "$vif" maxsta "$max_sta" &>$STDOUT
...
config_vap() {
	...
	config_get txbf_enable "$vif" txbf_enable "none"
	...
	sysmode=$(uci get sysmode.sysmode.mode)
	config_get onemesh_enable onemesh enable "on"
	if [ "$sysmode" != "router" -o "$onemesh_enable" != "on" ];then
		if [ "$vif" = "ath03" -o "$vif" = "ath13" ];then 
			[ -n "$hasvif" ] && destroy_vap "$vif" &>$STDOUT
			return
		fi
	fi
	...
	[ -n "$hasvif" -a "$action" = "update" ] &&  {
		...
		iwpriv "$vif" vhtmubfer "$vhtmubfer"
		iwpriv "$vif" vhtmubfee "$vhtmubfee"
		iwpriv "$vif" wmm "$wmm"
	...
	tpdbg "txbf_enable $txbf_enable."
	if [ "$txbf_enable" = "on" ]; then
		iwpriv "$1" vhtsubfer 1
		iwpriv "$1" vhtsubfee 1
	elif [ "$txbf_enable" = "off" ]; then
		iwpriv "$1" vhtsubfer 0
		iwpriv "$1" vhtsubfee 0
	fi
...
wifi_mode() {
	...
	for dev in ${1:-$DEVICES}; do
		...
		config_get band "$dev" band
		if $(wireless_schedule_disable_wifi "$band") || [ "$disabled" = "1" ] || [ "$disabled_all" = "1" ]; then
			for vif in $vifs; do if [ $vif = "ath03" -o $vif = "ath13" ]; then continue; fi; destroy_vap "$vif" &>$STDOUT; done
			continue
		fi
		config_get hwmode "$dev" hwmode auto
		config_get htmode "$dev" htmode auto
		...
		if [ $htmode = "HT80_80" ];then
			config_get secondary_channel "$dev" secondary_channel 0
			channel="$channel":"$secondary_channel"
			echo "=============>wifi_mode:htmode=$htmode,channel=$channel" > /dev/console
		fi
		config_get txpower "$dev" txpower
...
wifi_vap() {
	local device vifs disabled dev_disabled dev_disabled_all band
	for vap in $1; do
		...
		# Don't bring down interfaces when configuring guestnetwork, otherwise
		# main network connection will get reset. 
		if [ "$guest" = "0" ]; then
...
wifi_radio() {
	...
	for dev in ${1:-$DEVICES}; do
		...
		if $(wireless_schedule_disable_wifi "$band") || [ "$disabled" = "1" ] || [ "$disabled_all" = "1" ]; then
			for vif in $vifs; do if [ $vif = "ath03" -o $vif = "ath13" ]; then continue; fi; config_set "$vif" disabled "on"; [ -d /sys/class/net/$vif ] && destroy_vap "$vif" &>$STDOUT; done
		fi
		...
		for vif in $vifs; do 
			config_get mode "$vif" mode
			[ "$mode" = "ap" ] && config_vap "$vif" "$action" &>$STDOUT;
			#wifi_fixup_config "$phy"
		done
		[ -n "$tpscale" ] && iwpriv "$dev" tpscale "$tpscale" &>$STDOUT
	done
	config_vap_vlan up &>$STDOUT
...
led_ctrl() {
	...
	if [ "$band" = "2g" ];then
		if $(wireless_schedule_disable_wifi "$band") || [ "$disabled_all" = "on" -o "$disabled" = "on" ];then
			ledcli WIFI2G_OFF
		else
			ledcli WIFI2G_ON
		fi
...
wifi_onemesh() {
	...
	rm /tmp/sync-server/onemesh_client_list 
...
wifi_reload() {
	...
	wifi_radio "$1" "txchainmask rxchainmask AMPDU ampdudensity !AMSDU AMPDULim AMPDUFrames AMPDURxBsize !bcnburst:set_bcnburst=0 \
		...
		dcs_usermaxc:set_dcs_usermaxc dcs_debug:set_dcs_debug set_ch_144:setCH144 eppovrd_ch_144:setCH144EppOvrd !ani_enable !acs_bkscanen acs_scanintvl acs_rssivar \
		...
		!rst_tso_stats !rst_lro_stats !rst_sg_stats set_fw_recovery=1 !allowpromisc set_sa_param !aldstats \
		!led_enable promisc block_interbss aggr_burst set_pmf txbf_snd_int=100 mcast_echo obss_rssi_th=35 obss_rx_rssi_th=35 obss_long_slot" \
		&>$STDOUT
