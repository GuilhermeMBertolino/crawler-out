...
//	directory level: 0: directory has no branches;  1: directory level 1;  2:level 2;   and so on
	...
	"IGMPCfgRpm",					0,		2,			str_menu.ntwIgmp,
...
function menuDisplay()
{
	...
	for (var n = 0; n < menuList.length; n +=4  )
	{
		...
		document.write('<ol id=ol'+i+' class='+className+' style="display:'+display+'; background-position:2px;PADDING-LEFT:2px;"><A id=a'+i+' href="/userRpm/'+menuList[n]+'.htm" hidefocus="true" target=mainFrame class=L1 onClick="doClick('+i+');">'+((power > 0)? '- ':'')+menuList[n+3]+'</a></ol>');
