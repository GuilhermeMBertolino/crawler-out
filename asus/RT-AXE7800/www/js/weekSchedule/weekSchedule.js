...
$("#" + weekScheduleApi.obj_id + "").find("#timezone_hint").html("* <#2745#>");
}
else
return;
}
...
if(_type == "start")
$time_setting_title.html("<span><#4433#></span>");
else
$time_setting_title.html("<span><#4434#></span>");
var get_time_setting_component = function(_type, _unit){
var time_unit_parm = {
"text":{"hour":"<#2425#>", "minute":"<#2822#>"},
...
show_valid_hint("* <#2205#>", $(this));
return false;
}
if((end_hour*60+end_min) > 1440){//24:00
show_valid_hint("* <#4434#> : <#4450#>", $(this));
return false;
}
if((start_hour*60+start_min) == (end_hour*60+end_min)){
...
$all_day_text.html("<#4432#>");
$edit_hour_minute_bg.append($("<div>").addClass("horizontal_line"));
var $routine_schedule_title = $("<div>").html("<#4446#>").appendTo($edit_hour_minute_bg);
...
return $("<div>").addClass("offtime_schedule_text dis").html("<#1629#> " + weekScheduleApi.text_Offtime_Sche);
},
...
$("<div>").addClass("btn_icon_bg tips_icon").attr("title", "<#660#>").unbind("click").click(function(e){
e = e || event;
e.stopPropagation();
...
weekScheduleApi.text_Offtime_Sche = "<#4440#>";
else if(weekScheduleApi.PC_schedule_mode == "M")
weekScheduleApi.text_Offtime_Sche = "<#4443#>";
}
},
...
$no_data_bg.append($("<div>").addClass("no_data_text").html("<#2554#>"));
return $no_data_bg;
},
...
.replace("MON", "<#1801#>").replace("TUE", "<#1805#>").replace("WED", "<#1806#>")
.replace("THU", "<#1804#>").replace("FRI", "<#1800#>").replace("SAT", "<#1802#>")
.replace("SUN", "<#1803#>");
else
display_title = week_group.replace("Daily", "<#4454#>").replace("Weekdays", "<#4453#>").replace("Weekend", "<#4452#>");
...
var $content_bg = $("<div>");
$("<div>").addClass("edit_offtime_title").html("<#4439#>").appendTo($content_bg);
...
$content_bg.append("<br>");
$("<div>").addClass("change_mode_hint").html("<#4448#>").appendTo($content_bg);
...
$("<div>").html("<#4442#>").appendTo($mode_desc);
$("<div>").html("<#4441#>").appendTo($mode_desc);
...
$("<div>").html("<#4445#>").appendTo($mode_desc);
$("<div>").html("<#4444#>").appendTo($mode_desc);
...
$("<div>").addClass("button_gen edit_done_btn").html("<#1773#>").appendTo($content_bg).unbind("click").click(function(e){
e = e || event;
e.stopPropagation();
...
var $action_cancel = $("<input/>").attr({"id":"action_cancel", "type":"button", "value":"<#289#>"}).addClass("button_gen");
...
var $start_bg = $("<div>").addClass("start_bg");
$("<div>").addClass("icon").appendTo($start_bg);
$("<div>").addClass("btn").html("<#793#>").unbind("click").click(function(e){
e = e || event;
e.stopPropagation();
$(this).closest("#tutorial_guide_bg").fadeOut(function(){
$(this).remove();
...
$("<div>").addClass("menu_title").html("<#1327#>").appendTo($left_bg);
