...
var str_vpnc_ip_allowed = stringSafeGet("<#3877#>");
var str_vpnc_keepalive = stringSafeGet("<#3879#>");
var wgs_faq_href = {
...
var $faq_setup_wgs = $("<a/>").attr({"target":"_blank", "href":wgs_faq_href.setup_wgs}).html("<#3860#>");
$("<div>").addClass("step_text faq hyperlink").append($faq_setup_wgs).appendTo($step_text_container);
var $faq_site_to_site = $("<a/>").attr({"target":"_blank", "href":wgs_faq_href.site_to_site}).html("<#3861#>");
$("<div>").addClass("step_text faq hyperlink").append($faq_site_to_site).appendTo($step_text_container);
var $faq_setup_wgce = $("<a/>").attr({"target":"_blank", "href":wgs_faq_href.setup_wgc}).html("<#3862#>");
...
$("<div>").addClass("title").html("<#3893#>").appendTo($popup_title_container);
var $close_btn = $("<div>").addClass("vpn_icon_all_collect close_btn");
$close_btn.appendTo($popup_title_container);
$close_btn.unbind("click").click(function(e){
e = e || event;
...
if(show_config){
var wgs_export_switch_parm = {"options": [{"text":"QR code","value":"1"},{"text":"<#1631#>","value":"2"}]};/* untranslated */
...
$("<div>").addClass("notice_title").html("<#2515#>").appendTo($notice_container);
...
$("<div>").addClass("qr_text").html(htmlEnDeCode.htmlEncode("<#4487#>")).appendTo($qr_code_cntr);
return $container
}
...
$("<div>").addClass("text").html(htmlEnDeCode.htmlEncode("<#1631#>")).appendTo($export_btn);
...
var wgsc_name_parm = {"title":"<#3695#>", "type":"text", "id":"wgsc_name", "need_check":true, "maxlength":64};
if(!show_config){
...
var wgsc_aips_parm = {"title":str_vpnc_ip_allowed+" (<#2720#>)", "type":"text", "id":"wgsc_aips", "need_check":true, "maxlength":4095};
...
var isFirstClient = ((wgs_clientlist_data.length == 0) ? true : false);
if(isFirstClient)
$btn_container_apply.html("<#4306#>");
var $action_loading_container = $("<div>").addClass("action_container loading").appendTo($content_container);
$("<div>").addClass("action_container_hint").html(htmlEnDeCode.htmlEncode("* <#3852#>")).hide().appendTo($content_container);
...
isValid_wgs_addr = {'isError': true, 'errReason': address + " <#2625#>"};
return false;
}
...
isValid_wgsc_addr = {'isError': true, 'errReason': address + " <#2625#>"};
return false;
}
if(valid_is_IP_format(address, "IPv4")){
...
var wgs_lanaccess_parm = {"title":"<#976#>", "type":"switch", "id":"wgs_lanaccess"};
...
$Add_Client_Title_obj.find("#vpns_clientlist_title").html("<#3892#> (<#2772#>" + htmlEnDeCode.htmlEncode(wgs_clientlist_limit) + ")");
...
show_customize_alert("<#4437#>".replace("#MAXNUM", wgs_clientlist_limit));
return false;
}
...
var wgs_dns_parm = {"title":"<#3023#> DNS", "type":"switch", "id":"wgs_dns"};
Get_Component_Switch(wgs_dns_parm).appendTo($detail_adv);
var wgs_nat6_parm = {"title":"<#1996#> - IPv6", "type":"switch", "id":"wgs_nat6"};
Get_Component_Switch(wgs_nat6_parm).appendTo($detail_adv);
var wgs_psk_parm = {"title":"<#3797#>", "type":"switch", "id":"wgs_psk"};
...
var wgs_priv_parm = {"title":"<#1815#>", "id":"wgs_priv", "text":"-"};
Get_Component_Pure_Text(wgs_priv_parm).appendTo($detail_adv);
var wgs_pub_parm = {"title":"<#3166#>", "id":"wgs_pub", "text":"-"};
