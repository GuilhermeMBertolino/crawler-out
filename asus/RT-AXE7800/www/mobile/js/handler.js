...
{
$("#settingProfileBar").showSelectorHint("<#3312#>");
return false;
}
...
httpUserInput.showTextHint("<#884#>");
return false;
}
/* check password */
if(!use_defpass){
...
httpPassInput.showTextHint("<#593#>");
return false;
}
...
if(!confirm("<#412#>")){
httpPassInput.showTextHint("<#1367#>");
return false;
}
}
}
...
$("#verify_pincode_status").html("<#2920#>");
...
break;
case '4':
$("#verify_pincode_status").html("<#2858#>");
break;
case '6':
$("#verify_pincode_status").html("<#2918#>");
break;
case '-1':
$("#verify_pincode_status").html("<#2893#>");
break;
case '-2':
case '-10':
$("#verify_pincode_status").html("<#2891#>");
break;
default:
break;
}
...
$("#verify_pincode_status").html("<#2629#>");
$("#verify_pincode_status").css("display", "");
return false;
}
else{
...
$("#new_pincode_status").html("<#2864#>");
else
$("#new_pincode_status").html("<#2629#>");
...
if(!confirm("<#412#>")){
$("#wireless_key_" + band).showTextHint("<#1367#>");
return false;
}
}
return true;
...
if(get_wl_unit_by_band("6G") != "" && qisPostData.smart_connect_x == '1'){
if(qisPostData.smart_connect_selif_x == '3' || qisPostData.smart_connect_selif_x == '7'){
qisPostData.wl0_auth_mode_x = "psk2"; // 6g is not in smart connect band
qisPostData.wl0_mfp = "0";
}
else{
qisPostData.wl0_auth_mode_x = "psk2sae"; // 6g is in smart connect band
qisPostData.wl0_mfp = "1";
}
}
else if(get_wl_unit_by_band("6G") != "" && qisPostData.smart_connect_x == '3'){
...
if(get_wl_unit_by_band("6G") != "" && qisPostData.smart_connect_x == '1'){
if(qisPostData.smart_connect_selif_x == '3'){
qisPostData.wl1_auth_mode_x = "psk2"; // 6g is not in smart connect band
qisPostData.wl1_mfp = "0";
}
else{
qisPostData.wl1_auth_mode_x = "psk2sae"; // 6g is in smart connect band
...
if(get_wl_unit_by_band("6G1") == "2" || get_wl_unit_by_band("6G2") == "2"){
...
if(get_wl_unit_by_band("6G1") == "3" || get_wl_unit_by_band("6G2") == "3"){
qisPostData.wl3_auth_mode_x = "sae";
qisPostData.wl3_crypto = "aes";
qisPostData.wl3_mfp = "2";
}
else{
...
$("#wan_pppoe_passwd").showTextHint("<#797#>");
...
apply.EULA = function(){
const yearChecked = $("#ASUS_EULA_enable").prop("checked");
if (!yearChecked) {
alert("<#137#>");
$("label[for=ASUS_EULA_enable]").css({"color": top.webWrapper ? "red" : "#ff5722"});
return false;
}
$("#ppApply").html(Get_Component_btnLoading);
setTimeout(function () {
...
$("#login_name #http_username_title").html("<#2449#>");
var find_local_login_pw = str_find_st.replace("%@", "<#574#>");
...
var str_HowFindPassword_Local = str_HowFindPassword.replace("%@", "<#574#>");
...
if(isSupport("concurrep"))
$("#siteSurvey_page").find(".titleSub").html("<#4389#>");
else
$("#siteSurvey_page").find(".titleSub").html("<#4057#>");
...
if(isSupport("RPMesh"))
$("#papList_page").find(".titleSub").html("<#4389#>");
else
$("#papList_page").find(".titleSub").html("<#4057#>");
genPAPList(systemVariable.papList, systemVariable.multiPAP.wlcOrder);
...
*/
if(isSupport("defpsk")){
var find_WiFi_pw = str_find_st.replace("%@", "<#575#>");
...
var str_HowFindPassword_WiFi = str_HowFindPassword.replace("%@", "<#575#>");
...
var applyBtn = (systemVariable.isNewFw == 2) ? "<#304#>" : "<#1791#>";
...
if(isSwMode("RP"))
$("#summary_page").find("#stepText2").html("<#553#>");
...
$("#summary_page").find("#stepText1").html("<#551#> <#550#>");
$("#summary_page").find("#stepText2").html("<#553#>");
}
if(isSupport("lantiq")){
...
httpApi.checkCap("http://<#949#>", function(){
setTimeout(function(){
if(isPage("amasconncap_page")) window.location.href = "http://<#949#>/cfg_onboarding.cgi?flag=AMesh&id=" + systemVariable.macAddr.split(":").join("");
}, 3000);
...
goTo.EULA = function(){
if(systemVariable.eulaStatus == "1"){
goTo.PP();
return
}
const policyContainer = document.getElementById('policyContainer');
const ppAbort = document.getElementById('ppAbort');
const ppApply = document.getElementById('ppApply');
const eulaApply = document.getElementById('eulaApply');
ppAbort.textContent = `<#1770#>`;
ppApply.textContent = `<#1761#>`;
eulaApply.textContent = `<#1761#>`;
ppAbort.style.display = 'none';
ppApply.style.display = 'none';
eulaApply.style.display = '';
ppAbort.classList.add('disabled');
ppApply.classList.add('disabled');
eulaApply.classList.add('disabled');
const policyPageTitle = document.querySelector('#policy_page .titleMain');
const pageDesc = document.querySelector('#policy_page .pageDesc');
goTo.loadPage("policy_page", false, "pop");
policyContainer.innerHTML = '';
const policyComponent = new QisPolicyComponent({policy: 'EULA', applyFunc: apply.EULA});
policyContainer.appendChild(policyComponent.render());
policyPageTitle.innerHTML = ASUS_POLICY.Content.EULA.Title;
pageDesc.innerHTML = ASUS_POLICY.Dict.QisDesc;
policyComponent.setApplyBtn(eulaApply);
$(policyComponent.getAgeCheckbox).checkboxradio();
}
goTo.PP = function () {
goTo.loadPage("policy_page", false, "pop");
const policyContainer = document.getElementById('policyContainer');
...
ppApply.textContent = `<#1761#>`;
eulaApply.textContent = `<#1761#>`;
ppAbort.style.display = '';
ppApply.style.display = '';
eulaApply.style.display = 'none';
ppAbort.classList.add('disabled');
ppApply.classList.add('disabled');
eulaApply.classList.add('disabled');
const policyComponent = new QisPolicyComponent({policy: 'PP', applyFunc: apply.PP});
policyContainer.appendChild(policyComponent.render());
const policyPageTitle = document.querySelector('#policy_page .titleMain');
const pageDesc = document.querySelector('#policy_page .pageDesc');
policyPageTitle.textContent = ASUS_POLICY.Content.PP.Title;
pageDesc.textContent = ASUS_POLICY.Dict.QisDesc;
policyComponent.setAbortBtn(ppAbort);
policyComponent.setApplyBtn(ppApply);
}
...
var auth_change_str = "* <#1026#> <#1117#>".replace("WPA3-personal", current_auth).replace("WPA2/WPA3-personal", changeTo_auth);
...
var $ssid = $("<div>").html(htmlEnDeCode.htmlEncode("<#663#>: " + function(){
...
var $key = $("<div>").html(htmlEnDeCode.htmlEncode("<#664#>: " + wl_item.key_new));
...
$("#SDN_Scenarios_page .titleMain").html(((isSupport("mtlancfg") && isSupport("BUSINESS")) ? "<#2407#>": "<#2408#>"));
