...
if(get_wl_unit_by_band("5G2") != ''){
document.querySelector('label[for="wireless_checkbox"]').innerHTML = '<#773#>';
wlArrayRet.push({"title":"5 GHz-1", "ifname":get_wl_unit_by_band('5G'), "suffix": "_5G-1"});
wlArrayRet.push({"title":"5 GHz-2", "ifname":get_wl_unit_by_band('5G2'), "suffix": "_5G-2"});
}
else{
document.querySelector('label[for="wireless_checkbox"]').innerHTML = '<#774#>';
wlArrayRet.push({"title":"5 GHz", "ifname":get_wl_unit_by_band('5G'), "suffix": "_5G"});
wlArrayRet.push({"title":"6 GHz", "ifname":get_wl_unit_by_band('6G'), "suffix": "_6G"});
}
...
$obj.showTextHint('<#434#> ' + min + ' <#435#> ' + max + '.');
}
})
if($(".hint").length > 0) return true;
}
...
switch(state) {
case "down":
$("#LED_state").html("<#1028#> : Link down (DSL LED Off)<br>");
$("#LED_state").show();
break;
case "wait":
case "wait for init":
$("#LED_state").html("<#1028#> : Wait for init (DSL LED Flashing)<br>");
$("#LED_state").show();
break;
case "init":
case "initializing":
$("#LED_state").html("<#1028#> : Initializing (DSL LED Flashing)<br>");
$("#LED_state").show();
break;
case "up":
$("#LED_state").html("<#1028#> : Link up (DSL LED On)<br>");
$("#LED_state").show();
break;
default:
$("#LED_state").hide();
...
.append($("<div>").addClass("inputTitle").html(wl.title + " <#663#>"))
...
$("<div>")
.addClass("inputTitleContainer")
.append($("<div>").addClass("inputTitle").html(wl.title + " <#664#>"))
...
$("#gdContainer").append($("<div>").attr({"class": "GD-wait"}).append($("<div>").attr({"id": "GD-status"}).html("<#2018#>")))
...
testResult.errReason = '<#438#> ["]';
return testResult;
}
else if(str.charAt(str.length - 1) == '"'){
testResult.isError = true;
testResult.errReason = '<#440#> ["]';
return testResult;
}
else{
var invalid_char = [];
...
testResult.isError = true;
testResult.errReason = "<#439#> '" + invalid_char.join('') + "' !";
}
return testResult;
}
};
...
$("#site2site_wlSet #s2s_wlSet_desc").html(specific_wl_text.text + " <#376#>");
}
else{
$("#site2site_wlSet #s2s_wlSet_desc").html("<#376#>");
}
...
var str_local_login_desc = stringSafeGet("<#458#>");
var str_find_st = stringSafeGet("<#377#>");
var str_HowFindPassword = stringSafeGet("<#378#>");
