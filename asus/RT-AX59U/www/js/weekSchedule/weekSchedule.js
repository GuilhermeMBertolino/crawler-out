...
$("#" + weekScheduleApi.obj_id + "").find("#timezone_hint").html("* <#2741#>");
}
else
return;
}
...
if(_type == "start")
$time_setting_title.html("<span><#4428#></span>");
else
$time_setting_title.html("<span><#4429#></span>");
var get_time_setting_component = function(_type, _unit){
var time_unit_parm = {
"text":{"hour":"<#2421#>", "minute":"<#2817#>"},
...
show_valid_hint("* <#2202#>", $(this));
return false;
}
if((end_hour*60+end_min) > 1440){//24:00
show_valid_hint("* <#4429#> : <#4445#>", $(this));
return false;
}
if((start_hour*60+start_min) == (end_hour*60+end_min)){
...
$all_day_text.html("<#4427#>");
$edit_hour_minute_bg.append($("<div>").addClass("horizontal_line"));
var $routine_schedule_title = $("<div>").html("<#4441#>").appendTo($edit_hour_minute_bg);
...
return $("<div>").addClass("offtime_schedule_text dis").html("<#1626#> " + weekScheduleApi.text_Offtime_Sche);
},
...
$("<div>").addClass("btn_icon_bg tips_icon").attr("title", "<#657#>").unbind("click").click(function(e){
e = e || event;
e.stopPropagation();
...
weekScheduleApi.text_Offtime_Sche = "<#4435#>";
else if(weekScheduleApi.PC_schedule_mode == "M")
weekScheduleApi.text_Offtime_Sche = "<#4438#>";
}
},
...
$no_data_bg.append($("<div>").addClass("no_data_text").html("<#2550#>"));
return $no_data_bg;
},
...
.replace("MON", "<#1798#>").replace("TUE", "<#1802#>").replace("WED", "<#1803#>")
.replace("THU", "<#1801#>").replace("FRI", "<#1797#>").replace("SAT", "<#1799#>")
.replace("SUN", "<#1800#>");
else
display_title = week_group.replace("Daily", "<#4449#>").replace("Weekdays", "<#4448#>").replace("Weekend", "<#4447#>");
...
var $content_bg = $("<div>");
$("<div>").addClass("edit_offtime_title").html("<#4434#>").appendTo($content_bg);
...
$content_bg.append("<br>");
$("<div>").addClass("change_mode_hint").html("<#4443#>").appendTo($content_bg);
...
$("<div>").html("<#4437#>").appendTo($mode_desc);
$("<div>").html("<#4436#>").appendTo($mode_desc);
...
$("<div>").html("<#4440#>").appendTo($mode_desc);
$("<div>").html("<#4439#>").appendTo($mode_desc);
...
$("<div>").addClass("button_gen edit_done_btn").html("<#1770#>").appendTo($content_bg).unbind("click").click(function(e){
e = e || event;
e.stopPropagation();
...
var $action_cancel = $("<input/>").attr({"id":"action_cancel", "type":"button", "value":"<#287#>"}).addClass("button_gen");
...
var $start_bg = $("<div>").addClass("start_bg");
$("<div>").addClass("icon").appendTo($start_bg);
$("<div>").addClass("btn").html("<#790#>").unbind("click").click(function(e){
e = e || event;
e.stopPropagation();
$(this).closest("#tutorial_guide_bg").fadeOut(function(){
$(this).remove();
...
$("<div>").addClass("menu_title").html("<#1324#>").appendTo($left_bg);
