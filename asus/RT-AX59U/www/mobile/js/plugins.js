...
if(isSupport('wifi6e')){
document.querySelector('label[for="wireless_checkbox"]').innerHTML = '<#771#>';
...
$obj.showTextHint('<#432#> ' + min + ' <#433#> ' + max + '.');
}
})
if($(".hint").length > 0) return true;
}
...
switch(state) {
case "down":
$("#LED_state").html("<#1025#> : Link down (DSL LED Off)<br>");
$("#LED_state").show();
break;
case "wait":
case "wait for init":
$("#LED_state").html("<#1025#> : Wait for init (DSL LED Flashing)<br>");
$("#LED_state").show();
break;
case "init":
case "initializing":
$("#LED_state").html("<#1025#> : Initializing (DSL LED Flashing)<br>");
$("#LED_state").show();
break;
case "up":
$("#LED_state").html("<#1025#> : Link up (DSL LED On)<br>");
$("#LED_state").show();
break;
default:
$("#LED_state").hide();
...
.append($("<div>").addClass("inputTitle").html(wl.title + " <#660#>"))
...
$("<div>")
.addClass("inputTitleContainer")
.append($("<div>").addClass("inputTitle").html(wl.title + " <#661#>"))
...
$("#gdContainer").append($("<div>").attr({"class": "GD-wait"}).append($("<div>").attr({"id": "GD-status"}).html("<#2015#>")))
...
testResult.isError = true;
testResult.errReason = "<#437#> '" + invalid_char.join('') + "' !";
}
return testResult;
}
};
...
$("#site2site_wlSet #s2s_wlSet_desc").html(specific_wl_text.text + " <#374#>");
}
else{
$("#site2site_wlSet #s2s_wlSet_desc").html("<#374#>");
}
...
var str_local_login_desc = stringSafeGet("<#456#>");
var str_find_st = stringSafeGet("<#375#>");
var str_HowFindPassword = stringSafeGet("<#376#>");
