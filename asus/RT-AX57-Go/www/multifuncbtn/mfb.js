...
const vpn_title_text = (()=>{return (isSupport("vpn_fusion") ? `<#4265#>` : `<#3809#>`);})();
let btnsw_list = [
...
$("<div>").addClass("item_tag link").html("<#1688#>").appendTo($item_tag_cntr);
}
...
show_customize_alert(stringSafeGet(`<#2321#>`));
return false;
}
else{
...
let vpnc_btn_parm = {"id":"vpnc_btn", "text":"<#1553#>"};
...
$("<div>").addClass("profile_hint").html(stringSafeGet(`<#2321#>`)).appendTo($vpnc_group);
}
...
$("<div>").addClass("default_conn_text").html(`<#3787#>`).appendTo($text_container);
...
$profile_item_cntrs.filter("[data-btnsw-onoff="+nv_btnsw_onoff+"]").find(".item_tag_container").append($("<div>").addClass("item_tag link").html("<#1688#>"));
});
}
