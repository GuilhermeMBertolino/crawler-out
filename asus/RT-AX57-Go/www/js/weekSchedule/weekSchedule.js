...
$("#" + weekScheduleApi.obj_id + "").find("#timezone_hint").html("* <#2663#>");
}
else
return;
}
...
if(_type == "start")
$time_setting_title.html("<span><#4345#></span>");
else
$time_setting_title.html("<span><#4346#></span>");
var get_time_setting_component = function(_type, _unit){
var time_unit_parm = {
"text":{"hour":"<#2343#>", "minute":"<#2739#>"},
...
show_valid_hint("* <#2125#>", $(this));
return false;
}
if((end_hour*60+end_min) > 1440){//24:00
show_valid_hint("* <#4346#> : <#4362#>", $(this));
return false;
}
if((start_hour*60+start_min) == (end_hour*60+end_min)){
...
$all_day_text.html("<#4344#>");
$edit_hour_minute_bg.append($("<div>").addClass("horizontal_line"));
var $routine_schedule_title = $("<div>").html("<#4358#>").appendTo($edit_hour_minute_bg);
...
return $("<div>").addClass("offtime_schedule_text dis").html("<#1549#> " + weekScheduleApi.text_Offtime_Sche);
},
...
$("<div>").addClass("btn_icon_bg tips_icon").attr("title", "<#582#>").unbind("click").click(function(e){
e = e || event;
e.stopPropagation();
...
weekScheduleApi.text_Offtime_Sche = "<#4352#>";
else if(weekScheduleApi.PC_schedule_mode == "M")
weekScheduleApi.text_Offtime_Sche = "<#4355#>";
}
},
...
$no_data_bg.append($("<div>").addClass("no_data_text").html("<#2472#>"));
return $no_data_bg;
},
...
.replace("MON", "<#1721#>").replace("TUE", "<#1725#>").replace("WED", "<#1726#>")
.replace("THU", "<#1724#>").replace("FRI", "<#1720#>").replace("SAT", "<#1722#>")
.replace("SUN", "<#1723#>");
else
display_title = week_group.replace("Daily", "<#4366#>").replace("Weekdays", "<#4365#>").replace("Weekend", "<#4364#>");
...
var $content_bg = $("<div>");
$("<div>").addClass("edit_offtime_title").html("<#4351#>").appendTo($content_bg);
...
$content_bg.append("<br>");
$("<div>").addClass("change_mode_hint").html("<#4360#>").appendTo($content_bg);
...
$("<div>").html("<#4354#>").appendTo($mode_desc);
$("<div>").html("<#4353#>").appendTo($mode_desc);
...
$("<div>").html("<#4357#>").appendTo($mode_desc);
$("<div>").html("<#4356#>").appendTo($mode_desc);
...
$("<div>").addClass("button_gen edit_done_btn").html("<#1693#>").appendTo($content_bg).unbind("click").click(function(e){
e = e || event;
e.stopPropagation();
...
var $start_bg = $("<div>").addClass("start_bg");
$("<div>").addClass("icon").appendTo($start_bg);
$("<div>").addClass("btn").html("<#714#>").unbind("click").click(function(e){
e = e || event;
e.stopPropagation();
$(this).closest("#tutorial_guide_bg").fadeOut(function(){
$(this).remove();
...
$("<div>").addClass("menu_title").html("<#1247#>").appendTo($left_bg);
