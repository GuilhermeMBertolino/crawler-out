...
document.querySelector('label[for="wireless_checkbox"]').innerHTML = '<#696#>';
wlArrayRet.push({"title":"5 GHz", "ifname":get_wl_unit_by_band('5G'), "suffix": "_5G"});
wlArrayRet.push({"title":"6 GHz", "ifname":get_wl_unit_by_band('6G'), "suffix": "_6G"});
}
else{
...
else if(based_modelid === 'GT-AXE16000' || based_modelid === 'GT-BE98' || based_modelid === 'GT-BE98_PRO'){
...
_profile = ["", "", "", "", "", "", "", "", "", ""];
this.band = (function(){
if((isSupport('wifi6e') || isSupport('wifi7')) && _profile[0] === '6G'){
return "6GHz";
}
else if((isSupport('wifi6e') || isSupport('wifi7')) && _profile[0] === '5G'){
return "5GHz";
}
else{
...
else if((isSupport('wifi6e') || isSupport('wifi7')) && _profile[0] === '6G'){
return 2;
}
else if((isSupport('wifi6e') || isSupport('wifi7')) && _profile[0] === '5G'){
return 1;
...
$obj.showTextHint('<#357#> ' + min + ' <#358#> ' + max + '.');
}
})
if($(".hint").length > 0) return true;
}
...
switch(state) {
case "down":
$("#LED_state").html("<#948#> : Link down (DSL LED Off)<br>");
$("#LED_state").show();
break;
case "wait":
case "wait for init":
$("#LED_state").html("<#948#> : Wait for init (DSL LED Flashing)<br>");
$("#LED_state").show();
break;
case "init":
case "initializing":
$("#LED_state").html("<#948#> : Initializing (DSL LED Flashing)<br>");
$("#LED_state").show();
break;
case "up":
$("#LED_state").html("<#948#> : Link up (DSL LED On)<br>");
$("#LED_state").show();
break;
default:
$("#LED_state").hide();
...
.append($("<div>").addClass("inputTitle").html(wl.title + " <#585#>"))
...
$("<div>")
.addClass("inputTitleContainer")
.append($("<div>").addClass("inputTitle").html(wl.title + " <#586#>"))
...
$("#gdContainer").append($("<div>").attr({"class": "GD-wait"}).append($("<div>").attr({"id": "GD-status"}).html("<#1938#>")))
...
break;
case "SDN_Scenario":
matchingResult = (isSupport("mtlancfg") && isSupport("BUSINESS")) ? true : false;
break;
...
testResult.errReason = '<#361#> ["]';
return testResult;
}
else if(str.charAt(str.length - 1) == '"'){
testResult.isError = true;
testResult.errReason = '<#363#> ["]';
return testResult;
}
else{
var invalid_char = [];
...
testResult.isError = true;
testResult.errReason = "<#362#> '" + invalid_char.join('') + "' !";
}
return testResult;
}
};
...
$("#site2site_wlSet #s2s_wlSet_desc").html(specific_wl_text.text + " <#299#>");
}
else{
$("#site2site_wlSet #s2s_wlSet_desc").html("<#299#>");
}
...
var str_local_login_desc = stringSafeGet("<#381#>");
var str_find_st = stringSafeGet("<#300#>");
var str_HowFindPassword = stringSafeGet("<#301#>");
...
$('<script />', { type : 'text/javascript', src : "./SDN/Captive_Portal/FreeWiFi_template.js", group: "SDN_need_file"}).appendTo($('head'));
}
setTimeout(function(){
init_sdn_all_list();
},500);
