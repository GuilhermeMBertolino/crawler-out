...
var led_table = ["<#1629#>", "<#3164#>", "<#2812#>", "<#2426#>"];
...
var code = '';
if(system.INTELplatform){
code += '<div class="info-title"><#4407#></div>';
...
code += '<div class="info-block"><div class="info-title"><#2657#></div><div class="info-content">'+ _lanIP +'</div></div>';
code += '<div class="info-block"><div class="info-title"><#3065#></div><div class="info-content">'+ variable.secret_code +'</div></div>';
if(variable.serial_no != ''){
code += '<div class="info-block"><div class="info-title"><#3296#></div><div class="info-content">'+ variable.serial_no +'</div></div>';
}
...
<div class="info-title">${wl_nband_title[get_wl_unit_by_band(bandName[i])]} <#465#></div>
...
var modeDesc = ['<#4240#>', '<#4242#>', '<#4244#>', '', '<#1630#>'];
code += '<div class="division-block"><#4233#></div>';
...
code += '<div class="info-content"><#2255#> '+ yadns_clients[i] +'</div>';
code += '</div>';
}
}
...
var code = '<div class="division-block"><#840#></div>';
code += '<div>';
for(i=0;i<core_num;i++){
...
var code = '<div class="division-block"><#842#></div>';
code += '<div>';
code += '<div>';
...
code += '<div class="ram-text-width"><#845#></div>';
code += '</div>';
code += '<div class="display-flex flex-a-center ram-content">';
...
$("<div>").css({"margin-bottom":"6px"}).html("<#3097#> <#3098#>").appendTo($hint_text_bg);
...
$("<div>").css({"margin-bottom":"6px"}).html("<#3099#> :").appendTo($hint_text_bg);
var $help_item = $("<ol>").css({"margin":"0", "padding":"0", "margin-left":"24px"}).appendTo($hint_text_bg);
$("<li>").css({"color":"#64737c"}).html("<#3104#>".replace("#ETHCABLE", "Cat 5e")).appendTo($help_item);
$("<li>").css({"color":"#64737c"}).html("<#3100#>").appendTo($help_item);
$("<li>").css({"color":"#64737c"}).html("<#3101#>").appendTo($help_item);
$("<li>").css({"color":"#64737c"}).html("<#3102#>").appendTo($help_item);
$("<li>").css({"color":"#64737c"}).html("<#3103#>").appendTo($help_item);
var $action_bg = $("<div>").css({"margin-top":"18px", "text-align":"right"}).appendTo($popup_hint_bg);
var $action_ok = $("<input/>").attr({"id":"action_ok", "type":"button", "value":"<#1783#>"}).addClass("button_gen");
...
$("#phy_ports").append('<div class="division-block" style="position:relative;"><span><#836#></span><div class="port_status_notice_icon"></div></div>');
...
$("<div>").addClass("warn").html("<#3008#>").appendTo($port_status_icon_desc);/* untranslated */
$("<div>").addClass("unplug").html("<#839#>").appendTo($port_status_icon_desc);
if(show_notice){
if(top.webWrapper){
...
code += '<div class="port-block-width table-content"><#837#></div>';
code += '<div class="port-block-width table-content"><#834#></div>';
code += '</div>';
...
var code = '<div class="division-block"><#4484#></div>';
code += '<div>';
code += '<div class="display-flex flex-a-center table-header">';
code += '<div class="port-block-width table-content"><#834#></div>';
code += '<div class="port-block-width table-content">Tx (<#2495#>)</div>';
code += '<div class="port-block-width table-content">Rx (<#2495#>)</div>';
code += '</div>';
...
<div id="status_tab" class="tab-block tab-click" onclick="switchTab(this.id)"><#834#></div>
