...
var str_vpnc_ip_allowed = stringSafeGet("<#3881#>");
var str_vpnc_keepalive = stringSafeGet("<#3883#>");
var wgs_faq_href = {
...
var $faq_setup_wgs = $("<a/>").attr({"target":"_blank", "href":wgs_faq_href.setup_wgs}).html("<#3864#>");
$("<div>").addClass("step_text faq hyperlink").append($faq_setup_wgs).appendTo($step_text_container);
var $faq_site_to_site = $("<a/>").attr({"target":"_blank", "href":wgs_faq_href.site_to_site}).html("<#3865#>");
$("<div>").addClass("step_text faq hyperlink").append($faq_site_to_site).appendTo($step_text_container);
var $faq_setup_wgce = $("<a/>").attr({"target":"_blank", "href":wgs_faq_href.setup_wgc}).html("<#3866#>");
...
$("<div>").addClass("title").html("<#3897#>").appendTo($popup_title_container);
var $close_btn = $("<div>").addClass("vpn_icon_all_collect close_btn");
$close_btn.appendTo($popup_title_container);
$close_btn.unbind("click").click(function(e){
e = e || event;
...
if(show_config){
var wgs_export_switch_parm = {"options": [{"text":"QR code","value":"1"},{"text":"<#1632#>","value":"2"}]};/* untranslated */
...
$("<div>").addClass("notice_title").html("<#2519#>").appendTo($notice_container);
...
$("<div>").addClass("qr_text").html(htmlEnDeCode.htmlEncode("<#4507#>")).appendTo($qr_code_cntr);
return $container
}
...
$("<div>").addClass("text").html(htmlEnDeCode.htmlEncode("<#1632#>")).appendTo($export_btn);
...
var wgsc_name_parm = {"title":"<#3699#>", "type":"text", "id":"wgsc_name", "need_check":true, "maxlength":64};
if(!show_config){
...
var wgsc_aips_parm = {"title":str_vpnc_ip_allowed+" (<#2724#>)", "type":"text", "id":"wgsc_aips", "need_check":true, "maxlength":4095};
...
if(show_config){
var wgsc_update_key_parm = {"title":"Renew Key", "id":"wgsc_update_key", "btn_text":"<#297#>"};/* untranslated */
...
var $btn_container_apply = $("<div>").addClass("btn_container apply").appendTo($action_container).html("<#286#>");
var isFirstClient = ((wgs_clientlist_data.length == 0) ? true : false);
if(isFirstClient)
$btn_container_apply.html("<#4325#>");
var $action_loading_container = $("<div>").addClass("action_container loading").appendTo($content_container);
$("<div>").addClass("action_container_hint").html(htmlEnDeCode.htmlEncode("* <#3856#>")).hide().appendTo($content_container);
...
$(_obj).find(".status_icon").removeClass("connected").attr({"title":"<#310#>"});
...
$(_obj).find("[data-wgsc-unit='"+value.index+"']").find(".status_icon").addClass("connected").attr({"title":"<#279#>"});
}
}
}
});
...
if($wgs_addr.val() == ""){
$wgs_addr.show_validate_hint("<#416#>");
$wgs_addr.focus();
return false;
}
...
isValid_wgs_addr = {'isError': true, 'errReason': address + " <#2629#>"};
return false;
}
...
isValid_wgs_addr = {'isError': true, 'errReason': address + " <#430#>"};
return false;
}
});
if(isValid_wgs_addr.isError){
...
if($wgs_port.val() == ""){
$wgs_port.show_validate_hint("<#416#>");
$wgs_port.focus();
return false;
}
...
if($wgs_alive.val() == ""){
$wgs_alive.show_validate_hint("<#416#>");
$wgs_alive.focus();
return false;
}
...
if($wgsc_name.val() == ""){
$wgsc_name.show_validate_hint("<#416#>");
$valid_des.focus();
return false;
}
...
if($wgsc_addr.val() == ""){
$wgsc_addr.show_validate_hint("<#416#>");
$wgsc_addr.focus();
return false;
}
...
isValid_wgsc_addr = {'isError': true, 'errReason': address + " <#2629#>"};
return false;
}
if(valid_is_IP_format(address, "IPv4")){
...
isValid_wgsc_addr = {'isError': true, 'errReason': address + " <#430#>"};
return false;
}
});
if(isValid_wgsc_addr.isError){
...
if($wgsc_aips.val() == ""){
$wgsc_aips.show_validate_hint("<#416#>");
$wgsc_aips.focus();
return false;
}
...
isValid_wgsc_aips = {'isError': true, 'errReason': address + " <#430#>"};
return false;
}
});
if(isValid_wgsc_aips.isError){
...
if($wgsc_caips.val() == ""){
$wgsc_caips.show_validate_hint("<#416#>");
$wgsc_caips.focus();
return false;
}
...
isValid_wgsc_caips = {'isError': true, 'errReason': address + " <#430#>"};
return false;
}
});
if(isValid_wgsc_caips.isError){
...
var wgs_lanaccess_parm = {"title":"<#977#>", "type":"switch", "id":"wgs_lanaccess"};
Get_Component_Switch(wgs_lanaccess_parm).appendTo($detail_general);
var wgs_addr_parm = {"title":"<#3862#>", "type":"text", "id":"wgs_addr", "need_check":true, "maxlength":63};
Get_Component_Input(wgs_addr_parm).appendTo($detail_general);
var wgs_port_parm = {"title":"<#3863#>", "type":"text", "id":"wgs_port", "need_check":true, "maxlength":5};
...
$Add_Client_Title_obj.find("#vpns_clientlist_title").html("<#3896#> (<#2776#>" + htmlEnDeCode.htmlEncode(wgs_clientlist_limit) + ")");
...
show_customize_alert("<#4457#>".replace("#MAXNUM", wgs_clientlist_limit));
return false;
}
...
var wgs_dns_parm = {"title":"<#3027#> DNS", "type":"switch", "id":"wgs_dns"};
Get_Component_Switch(wgs_dns_parm).appendTo($detail_adv);
var wgs_nat6_parm = {"title":"<#1997#> - IPv6", "type":"switch", "id":"wgs_nat6"};
Get_Component_Switch(wgs_nat6_parm).appendTo($detail_adv);
var wgs_psk_parm = {"title":"<#3801#>", "type":"switch", "id":"wgs_psk"};
...
var wgs_priv_parm = {"title":"<#1816#>", "id":"wgs_priv", "text":"-"};
Get_Component_Pure_Text(wgs_priv_parm).appendTo($detail_adv);
var wgs_pub_parm = {"title":"<#3170#>", "id":"wgs_pub", "text":"-"};
Get_Component_Pure_Text(wgs_pub_parm).appendTo($detail_adv);
var wgs_update_key_parm = {"title":"Renew Key", "id":"wgs_update_key", "btn_text":"<#297#>"};/* untranslated */
