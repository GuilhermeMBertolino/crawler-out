...
tooltipXOffset: 10,
tooltipTemplate: "<@if (label){@><@=label@>: <@}@><@= value @> <@= unit @>", //for Pie Chart
...
cornerRadius: this.options.tooltipCornerRadius,
text: template(this.options.tooltipTemplate_pie, Element),
chart: this.chart
}).draw();
}
else{ //for pie chart
...
type_flag = "router";
index = top5_client_array.indexOf(this.activeElements[0].target_id);
}
else{
type_flag = "app";
...
circumference : (this.options.animateRotate) ? 0 : this.calculateCircumference(segment.percent),
label : segment.label,
...
segment.transition({
circumference : this.calculateCircumference(segment.percent),
outerRadius : this.outerRadius,
innerRadius : (this.outerRadius/100) * this.options.percentageInnerCutout
},animDecimal);
...
return (Math.pow(mouseX-this.x-15, 2) < Math.pow(this.radius + this.hitDetectionRadius/5,2));
}
});
flow_flag =this; // to destroy previous chart when redraw
this.datasets = [];
if (this.options.showTooltips){
helpers.bindEvents(this, this.options.tooltipEvents, function(evt){
