...
document.querySelector('label[for="wireless_checkbox"]').innerHTML = '<#775#>';
wlArrayRet.push({"title":"5 GHz", "ifname":get_wl_unit_by_band('5G'), "suffix": "_5G"});
wlArrayRet.push({"title":"6 GHz", "ifname":get_wl_unit_by_band('6G'), "suffix": "_6G"});
}
...
switch(state) {
case "down":
$("#LED_state").html("<#1029#> : Link down (DSL LED Off)<br>");
$("#LED_state").show();
break;
case "wait":
case "wait for init":
$("#LED_state").html("<#1029#> : Wait for init (DSL LED Flashing)<br>");
$("#LED_state").show();
break;
case "init":
case "initializing":
$("#LED_state").html("<#1029#> : Initializing (DSL LED Flashing)<br>");
$("#LED_state").show();
break;
case "up":
$("#LED_state").html("<#1029#> : Link up (DSL LED On)<br>");
$("#LED_state").show();
break;
default:
$("#LED_state").hide();
...
$("<div>")
.addClass("inputTitleContainer")
.append($("<div>").addClass("inputTitle").html(wl.title + " <#665#>"))
...
$("<div>")
.addClass("inputTitleContainer")
.append($("<div>").addClass("inputTitle").html(wlArray.title + " <#664#>"))
.appendTo(__container)
$("<div>")
.addClass("inputContainer")
.append($("<input>")
...
$("<div>")
.addClass("inputTitleContainer")
.append($("<div>").addClass("inputTitle").html(wlArray.title + " <#665#>"))
...
$("#gdContainer").append($("<div>").attr({"class": "GD-wait"}).append($("<div>").attr({"id": "GD-status"}).html("<#2019#>")))
...
actionScript.push("restart_net_and_phy");
}
...
if(systemVariable.isDefault && systemVariable.sdn_idx > 0){
actionScript.push("restart_sdn "+systemVariable.sdn_idx+"");
}
...
else if(get_wl_unit_by_band("5G2")){
bandCount = (get_wl_unit_by_band("6G") != "") ? "29" : "13";
}
else{
bandCount = (get_wl_unit_by_band("6G") != "") ? "19" : "3";
}
...
let sdn_access_rl = (()=>{
if(systemVariable.opMode != systemVariable.originOpMode)
return (httpApi.nvramDefaultGet(["sdn_access_rl"]).sdn_access_rl).replace(/&#60/g, "<").replace(/&#62/g, ">")
else
return decodeURIComponent(httpApi.nvramCharToAscii(["sdn_access_rl"]).sdn_access_rl);
})();
post_sdnObj.sdn_access_rl = `${sdn_access_rl}<0>${new_sdn_idx}`;
