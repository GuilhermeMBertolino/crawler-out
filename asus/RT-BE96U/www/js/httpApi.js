...
result.status_text = "<#3404#>";
}
else{//lb
result.status = (httpApi.isConnected(wan_index)) ? "connected" : "disconnected";
result.status_text = (result.status == "connected") ? "<#279#>" : "<#310#>";
}
}
else{
result.status = (httpApi.isConnected(wan_index)) ? "connected" : "disconnected";
...
result.proto_text = ((modem_operation != "") ? modem_operation : "<#2910#>");
}
}
}
return result;
