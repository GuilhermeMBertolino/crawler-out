...
$("#" + weekScheduleApi.obj_id + "").find("#timezone_hint").html("* <#2749#>");
}
else
return;
}
...
if(_type == "start")
$time_setting_title.html("<span><#4453#></span>");
else
$time_setting_title.html("<span><#4454#></span>");
var get_time_setting_component = function(_type, _unit){
var time_unit_parm = {
"text":{"hour":"<#2429#>", "minute":"<#2826#>"},
...
show_valid_hint("* <#4454#> : <#4470#>", $(this));
return false;
}
if((start_hour*60+start_min) == (end_hour*60+end_min)){
...
$all_day_text.html("<#4452#>");
$edit_hour_minute_bg.append($("<div>").addClass("horizontal_line"));
var $routine_schedule_title = $("<div>").html("<#4466#>").appendTo($edit_hour_minute_bg);
...
return $("<div>").addClass("offtime_schedule_text dis").html("<#1630#> " + weekScheduleApi.text_Offtime_Sche);
},
...
$("<div>").addClass("btn_icon_bg tips_icon").attr("title", "<#661#>").unbind("click").click(function(e){
e = e || event;
e.stopPropagation();
...
weekScheduleApi.text_Offtime_Sche = "<#4460#>";
else if(weekScheduleApi.PC_schedule_mode == "M")
weekScheduleApi.text_Offtime_Sche = "<#4463#>";
}
},
...
$no_data_bg.append($("<div>").addClass("no_data_text").html("<#2558#>"));
return $no_data_bg;
},
...
.replace("MON", "<#1802#>").replace("TUE", "<#1806#>").replace("WED", "<#1807#>")
.replace("THU", "<#1805#>").replace("FRI", "<#1801#>").replace("SAT", "<#1803#>")
.replace("SUN", "<#1804#>");
else
display_title = week_group.replace("Daily", "<#4474#>").replace("Weekdays", "<#4473#>").replace("Weekend", "<#4472#>");
...
var $content_bg = $("<div>");
$("<div>").addClass("edit_offtime_title").html("<#4459#>").appendTo($content_bg);
...
$content_bg.append("<br>");
$("<div>").addClass("change_mode_hint").html("<#4468#>").appendTo($content_bg);
...
$("<div>").html("<#4462#>").appendTo($mode_desc);
$("<div>").html("<#4461#>").appendTo($mode_desc);
...
$("<div>").html("<#4465#>").appendTo($mode_desc);
$("<div>").html("<#4464#>").appendTo($mode_desc);
...
$("<div>").addClass("button_gen edit_done_btn").html("<#1774#>").appendTo($content_bg).unbind("click").click(function(e){
e = e || event;
e.stopPropagation();
...
var $start_bg = $("<div>").addClass("start_bg");
$("<div>").addClass("icon").appendTo($start_bg);
$("<div>").addClass("btn").html("<#794#>").unbind("click").click(function(e){
e = e || event;
e.stopPropagation();
$(this).closest("#tutorial_guide_bg").fadeOut(function(){
$(this).remove();
...
$("<div>").addClass("menu_title").html("<#1328#>").appendTo($left_bg);
