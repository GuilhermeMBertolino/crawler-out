...
if(based_modelid == "RT-AC95U" || based_modelid == "RT-AX95Q" || based_modelid == "XT8PRO" || based_modelid == "BT12" || based_modelid == "BT10" || based_modelid == "BQ16" || based_modelid == "BQ16_PRO" || based_modelid == "XT8_V2" || based_modelid == "RT-AXE95Q" || based_modelid == "ET8PRO" || based_modelid == "ET8_V2" || based_modelid == "RT-AX82_XD6" || based_modelid == "RT-AX82_XD6S" || based_modelid == "XD4S" || based_modelid == "RT-AX53U" || based_modelid == "XD6_V2" || based_modelid == "RT-AX59U" || based_modelid == "RT-AX52"){
if($("#wans_lanport1 option[value='4']").length > 0)
$("#wans_lanport1 option[value='4']").remove();
if($("#wans_lanport2 option[value='4']").length > 0)
$("#wans_lanport2 option[value='4']").remove();
...
function is_special_lan(wans_lanport_v){//The special lan port can be configured as single wan.
var special_lan = "";
$.each(eth_wan_list, function(key) {
var wan_obj = eth_wan_list[key];
...
function is_original_eth_lan(port_val){//Check if the wan port is the original ethernet LAN port, such as 2.5G WAN/LAN1
var is_original_eth_lan = false;
$.each(eth_wan_list, function(key){
if(key == port_val){
var wan_obj = eth_wan_list[key];
if(wan_obj.hasOwnProperty("wans_lanport")){
...
else if(wans_caps.indexOf("sfp+") >= 0)
...
if(based_modelid == "RT-AC88Q" || based_modelid == "BRT-AC828" || based_modelid == "RT-AD7200") {
if(!noiptv_support) {
if(document.form.wans_primary.value != "lan" && orig_switch_wantag != "none") {
var confirm_flag = confirm(stringSafeGet("<#1956#>"));
if(confirm_flag) {
...
return confirm(stringSafeGet("<#1957#>"));
};
if(wans_flag == 1) {
...
conflict_func = conflict_func + ", <#2948#>";
}
else
conflict_func = "<#2948#>";
}
...
if(conflict_func.length > 0){
conflict_func = conflict_func + ", IPTV";
}
else
conflict_func = "IPTV";
}
...
if(conflict_func.length == 0)
conflict_func = "<#2948#>";
else if(conflict_func.length > 0 && conflict_func.indexOf("<#2948#>") == -1)
conflict_func += ", <#2948#>";
...
conflict_func += ", IPTV";
if(conflict_ports.length == 0)
conflict_ports = "2.5G WAN/LAN1";
else if(conflict_ports.indexOf("2.5G WAN/LAN1") == -1)
conflict_ports += ", 2.5G WAN/LAN1";
}
}
}
...
var hint_str = "<#3106#>";
var confirm_str = hint_str.replace("%1$@", conflict_ports).replace("%2$@", "<#491#>").replace("%3$@", conflict_func);
if(confirm(confirm_str)){
if(conflict_func.indexOf("<#2948#>") != -1){
document.form.lacp_enabled.disabled = false;
document.form.lacp_enabled.value = "0";
if(is_GTBE_series){
$('<input>').attr({
...
if(conflict_func.indexOf("IPTV") != -1){
document.form.switch_stb_x.disabled = false;
document.form.switch_stb_x.value = "0";
document.form.switch_wantag.disabled = false;
document.form.switch_wantag.value = "none";
...
if(obj.value == "lan"){
if($("#wans_primary option[value='lan']").length > 0 && document.form.wans_lanport1){
...
if(obj.value == "lan"){
if($("#wans_second option[value='lan']").length > 0 && document.form.wans_lanport2){
...
var replace_html = "";
var new_html_str = "";
replace_html = '<input type="text" name="wandog_interval" class="short_input input_3_table" maxlength="2" value="<% nvram_get("wandog_interval"); %>" onblur="update_consume_bytes();" onKeyPress="return validator.isNumber(this, event);" placeholder="5" autocorrect="off" autocapitalize="off">';
...
replace_html = '<input type="text" name="wandog_maxfail" class="short_input input_3_table" maxlength="2" value="<% nvram_get("wandog_maxfail"); %>" onKeyPress="return validator.isNumber(this, event);" placeholder="5" autocorrect="off" autocapitalize="off">';
if(wans_flag && (document.form.wans_mode.value == "fo" || document.form.wans_mode.value == "fb")){
$("#fo_detection_count_hd").html("<#1955#>");
new_html_str = "<#1953#>".replace("$WANDOG_MAXFAIL", replace_html);
$("#wandog_maxfail_setting").html(new_html_str);
}
else{
var new_str = stringSafeGet("<#1954#>");
new_html_str = new_str.replace("$WANDOG_MAXFAIL", replace_html);
$("#fo_detection_count_hd").html("<#2976#>");
$("#wandog_maxfail_setting").html(new_html_str);
}
...
consume_warning_str = "<#308#> "+consume_bytes+" <#309#>";
...
<tr><td colspan="2"><#3406#></td></tr><tr><td colspan="2"><#3407#>&nbsp;:&nbsp;<span id="str_detect_time"></span>&nbsp;<#3279#>.</td></tr>
<tr>
<th style="width:30%;"><#3414#></th>
<td>
<input type="text" name="detect_interval" class="input_3_table" maxlength="1" value=""; placeholder="5" autocorrect="off" autocapitalize="off" onKeyPress="return validator.isNumber(this, event);" onblur="update_str_time();" style="width: 38px; margin: 0px;">&nbsp;&nbsp;<#3279#>
</td>
</tr>
<tr>
<th><#3416#></th>
...
<span id="detect_tail_msg">&nbsp;( Detection Time: <span id="detection_time_value"></span>&nbsp;&nbsp;<#3279#>)</span>
</td>
</tr>
</table>
<div style="padding-bottom:10px;width:100%;text-align:center;">
<input id="yesButton" class="button_gen" type="button" value="<#277#>" onclick="change_detect_settings();">
<input id="noButton" class="button_gen" type="button" value="<#276#>" onclick="remain_origins();">
</div>
</div>
...
<div>&nbsp;</div>
<div class="formfonttitle"><#491#> - <#314#></div>
<div style="margin:10px 0 10px 5px;" class="splitLine"></div>
<div class="formfontdesc"><#1947#><a id="dualwan_faq" href="" target="_blank" style="margin-left:5px; text-decoration: underline;"><#314#> FAQ</a></div>
<div id="wan_aggre_desc" class="formfontdesc" style="color:#FFCC00; display:none;"><#3971#></div>
...
if(wans_caps.search("wan2") >= 0 && wans_caps.indexOf("sfp+") == -1)
document.form.wans_mode.value = "lb";
else
...
<input type="radio" name="wans_usb_bk" class="input" value="1" <% nvram_match("wans_usb_bk", "1", "checked"); %>><#277#>
<input type="radio" name="wans_usb_bk" class="input" value="0" <% nvram_match("wans_usb_bk", "0", "checked"); %>><#276#>
</div>
<span id="usb_tethering_hint" style="display: none;"><#1992#></span>
</td>
</tr>
<tr id="wans_mode_tr">
<th><#1964#></th>
...
<option value="fo"><#1965#></option>
<option value="lb" <% nvram_match("wans_mode", "lb", "selected"); %>><#1966#></option>
</select>
<span id="wans_mode_fo" style="margin-left:5px; color:#FFF; display:none;"><#1965#></span>
<span id="fb_span" style="display:none"><input type="checkbox" id="fb_checkbox"><#1949#></span>
<script>
...
<div id="lb_note" style="color:#FFCC00; display:none;"><#1962#></div>
<div id="lb_note2" style="color:#FFCC00; display:none;"><#1963#></div>
</td>
</tr>
...
<option value="1" <% nvram_match("wans_standby", "1", "selected"); %>><#4095#></option>
<option value="0" <% nvram_match("wans_standby", "0", "selected"); %>><#4096#></option>
</select>
</td>
</tr>
<tr>
...
&nbsp; : &nbsp;
<input type="text" maxlength="1" class="short_input input_3_table" name="wans_lb_ratio_1" value="" onkeypress="return validator.isNumber(this,event);" autocorrect="off" autocapitalize="off"/>
</td>
</tr>
<tr class="ISPProfile">
<th><#1959#></th>
...
<input type="radio" value="1" name="wans_isp_unit" class="content_input_fd" onClick="change_isp_unit(this.value);"><#1982#>
<input type="radio" value="2" name="wans_isp_unit" class="content_input_fd" onClick="change_isp_unit(this.value);"><#1989#>
</td>
</tr>
<tr class="ISPProfile">
<th><#1958#></th>
...
<thead>
<tr>
<td colspan="2"><#1981#><div style="font-weight: normal; font-style: italic; margin-top: 5px;"><#2045#></div></td>
</tr>
</thead>
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(26,3);"><#3415#></a></th>
<td id="retry_intervale_setting"><#349#>
<div><span id="consume_bytes_warning" style=""></span></div>
</td>
</tr>
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(26,5);"><div id="fo_detection_count_hd"><#1955#></div></a></th>
<td id="wandog_maxfail_setting"></td>
</tr>
<tr id="wandog_fb_count_tr">
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(26,6);"><div id="fb_detection_count_hd"><#1952#></div></a></th>
<td id="wandog_fbcount_setting"><#1951#></td>
</tr>
<tr>
<th><#2967#></th>
<td>
...
<img id="pull_arrow" class="pull_arrow" height="14px;" src="/images/unfold_more.svg" style="position:absolute;*margin-left:-3px;*margin-top:1px;" onclick="pullPingTargetList(this);" title="<#3290#>" onmouseover="over_var=1;" onmouseout="over_var=0;">
...
<th><#1987#></th>
<td>
<input type="radio" value="1" name="wans_routing_enable" onClick="enable_lb_rules(this.value)" class="content_input_fd" <% nvram_match("wans_routing_enable", "1", "checked"); %>><#277#>
<input type="radio" value="0" name="wans_routing_enable" onClick="enable_lb_rules(this.value)" class="content_input_fd" <% nvram_match("wans_routing_enable", "0", "checked"); %>><#276#>
</td>
</tr>
</table>
