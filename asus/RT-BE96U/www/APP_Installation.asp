...
<link rel="icon" href="images/favicon.png"><title><#948#> - <#477#></title>
...
function initial(){
default_apps_array = [["AiDisk", "aidisk.asp", "<#1098#>", "Aidisk_png", ""],
["<#3297#>", "mediaserver.asp", "<#3658#>", "server_png", ""],
["<#2969#>", "PrinterServer.asp", "<#2968#>", "PrinterServer_png", ""],
["3G/4G", "Advanced_Modem_Content.asp", "<#2438#>", "modem_png", ""],
["<#3487#>", "Advanced_TimeMachine.asp", "<#3492#>", "TimeMachine_png", "1.0.0.1"],
...
default_apps_array[1].splice(2,1,"<#2811#>");
}
...
if(default_apps_array.getIndexByValue2D("<#2969#>") != -1)
default_apps_array = default_apps_array.del(default_apps_array.getIndexByValue2D("<#2969#>")[0]);
}
...
if(default_apps_array.getIndexByValue2D("<#3487#>") != -1)
default_apps_array = default_apps_array.del(default_apps_array.getIndexByValue2D("<#3487#>")[0]);
}
...
document.getElementById("apps_state_desc").innerHTML = "<#3685#>";
...
document.getElementById("apps_state_desc").innerHTML = "<#3678#>";
...
document.getElementById("apps_state_desc").innerHTML = "<#3684#>";
...
document.getElementById("apps_state_desc").innerHTML = "[" + cookie.get("apps_last") + "] " + "<#2019#> <b>" + Math.round(installPercent) +"</b> <span style='font-size: 16px;'>%</span>";
...
document.getElementById("apps_state_desc").innerHTML = "<#3696#>";
else{
if(apps_depend_action_target != "terminated" && apps_depend_action_target != "error"){
if(apps_depend_action_target == "")
document.getElementById("apps_state_desc").innerHTML = "<b>[" + cookie.get("apps_last") + "] " + "<#2019#> </b>";
else
document.getElementById("apps_state_desc").innerHTML = "<b>[" + cookie.get("apps_last") + "] " + "<#2019#> </b>"
+"<br> <span style='font-size: 16px;'> <#2019#>："+apps_depend_do+"</span>"
...
document.getElementById("apps_state_desc").innerHTML = "<#599#>";
}
}
...
document.getElementById("apps_state_desc").innerHTML = "<#3651#>";
}
else if(apps_state_remove != 2 && apps_state_remove != ""){
errorcode = "apps_state_remove = " + apps_state_remove;
document.getElementById("apps_state_desc").innerHTML = "<#3649#>";
}
...
document.getElementById("apps_state_desc").innerHTML = "<#2019#>";
}
...
document.getElementById("apps_state_desc").innerHTML = "<#1534#>";
}
...
document.getElementById("apps_state_desc").innerHTML = "<#3686#>";
...
document.getElementById("apps_state_desc").innerHTML = "<b>[" + cookie.get("apps_last") + "] " + "<#2019#> </b>"
+"<br> <span style='font-size: 16px;'> <#2019#>："+_apps_depend_do+"</span>"
...
function show_apps(){
if(re_mode != "1")
document.getElementById("usbHint").innerHTML = "<#3193#>";
var counter = 0;
appnum = 0;
if(apps_array == "" && (appnet_support || appbase_support)){
apps_array = [["downloadmaster", "", "", "no", "no", "", "", "<#1899#>", "downloadmaster_png", "", "", ""],
...
if(i ==3){
htmlcode += '<div class="app_desc">' + default_apps_array[i][2] + ' <a href="https://www.asus.com/event/networks_3G4G_support/" target="_blank" style="text-decoration:underline;"><#3432#></a></div>\n';
}
else{
...
htmlcode += '<span class="app_action" onclick="apps_form(\'enable\',\''+ apps_array[i][0] +'\',\'no\');"><#4096#></span>\n';
else
htmlcode += '<span class="app_action" onclick="apps_form(\'enable\',\''+ apps_array[i][0] +'\',\'yes\');"><#4095#></span>\n';
if(sw_mode == 3 || link_internet == "2")
htmlcode += '<span class="app_action" onclick="apps_form(\'update\',\''+ apps_array[i][0] +'\',\'\');">Check update</span>\n'; /* untranslated */
if(apps_array[i][0] == "downloadmaster"){
htmlcode += '<span class="app_action" onclick="divdisplayctrl(\'none\', \'none\', \'none\', \'\');"><#1778#></span>\n';
}
else if(apps_array[i][0] == "fileflex")
htmlcode += '<span class="app_action" onclick="location.href=\'fileflex.asp\';"><#1778#></span>\n';
if( cookie.get("apps_last") == apps_array[i][0] &&
hasNewVer(apps_array[i]) &&
(sw_mode == 3 || link_internet == "2"))
htmlcode += '</div><div style="color:#FC0;margin-top:10px;"><span class="app_action" onclick="apps_form(\'upgrade\',\''+ apps_array[i][0] +'\',\'\');"><#3650#></span>\n';
else if(cookie.get("apps_last") == apps_array[i][0])
htmlcode += "</div><div style=\"color:#FC0;margin-top:10px;margin-left:10px;\"><span class=\"app_no_action\" onclick=\"\"><#2778#></span>\n";
}
else{
...
htmlcode += '<span class="app_action" onclick="_appname=\''+apps_array[i][0]+'\';check_usb_app_dev();"><#2018#></span>\n'; /* untranslated */
else
htmlcode += '<span class="app_action" onclick="apps_form(\'install\',\''+ apps_array[i][0] +'\',\''+ partitions_array[i] +'\');"><#2018#></span>\n'; /* untranslated */
}
...
if(accessableSize > 1)
htmlcode += '<div class="app_desc"><#1535#>: <b>'+ accessableSize+" GB" + '</b></div>';
else
htmlcode += '<div class="app_desc"><#1535#>: <b>'+ accessableSize+" GB <span style=\'color:#FFCC00\'>(Disk quota can not less than 1GB)" + '</span></b></div>';
htmlcode += '<div class="app_desc"><#3514#>: <b>'+ totalSize+" GB" + '</b></div>';
...
if(re_mode == "1")
htmlcode += '<tr height="360px"><td colspan="2" class="perNode_nohover"><span class="app_name" style="line-height:100%"><#2998#></span></td></tr>\n';
else
htmlcode += '<tr height="360px"><td colspan="2" class="nohover"><span class="app_name" style="line-height:100%"><#2998#></span></td></tr>\n';
}
document.getElementById("partition_div").innerHTML = htmlcode;
document.getElementById("usbHint").innerHTML = "<#1900#> :";
});
}
...
document.getElementById("quick_dmlink").onclick = function(){alert("<#1897#>");return false;}
}
document.getElementById("return_btn").style.display = "";
}
if(flag4 == "none")
...
<span style="margin-left:15px;" id="apps_state_desc"><#1515#></span>
...
<table>
<tr>
<td><div class="top-heading" style="cursor:pointer;margin-top:10px;height:20px;" id="quick_dmlink"><a id="realLink" href="" target="_blank"><b style="text-decoration:underline;color:#FC0;font-size:16px;"><#1902#></b></></a></div></td>
...
<br>
<li>
<a id="faq" href="" target="_blank" style="text-decoration:underline;font-size:14px;font-weight:bolder;color:#FFF"><#1905#> FAQ</a>
</li>
<li style="margin-top:10px;">
<a id="faq2" href="" target="_blank" style="text-decoration:underline;font-size:14px;font-weight:bolder;color:#FFF"><#1905#> Tool FAQ</a>
</li>
<li style="margin-top:10px;">
<a id="DMUtilityLink" href="http://dlcdnet.asus.com/pub/ASUS/wireless/RT-AC5300/UT_Download_Master_2228_Win.zip" style="text-decoration:underline;font-size:14px;font-weight:bolder;color:#FFF"><#1898#></a>
</li>
</ul>
</td>
</tr>
