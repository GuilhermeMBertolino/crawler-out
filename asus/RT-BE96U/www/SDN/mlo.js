...
$("<div>").addClass("desc").html(`<#3985#>`).appendTo($feature_desc_cntr);
...
$("<div>").addClass("profile_btn_container").html(`<#2471#>`.replace(``,'').replace(` :`,''))
...
let $container = $("<div>");
if(view_mode == "popup"){
Get_Component_Popup_Profile_Title(`<#4095#> MLO`).appendTo($container)
...
Get_Component_Profile_Title(`<#4095#> MLO`).appendTo($container);
let $popup_content_container = $("<div>").addClass("popup_content_container");
$popup_content_container.appendTo($container);
let $feature_desc_cntr = $("<div>").addClass("feature_desc_container").appendTo($popup_content_container);
$("<div>").addClass("title").html(`<#3991#>`).appendTo($feature_desc_cntr);
$("<div>").addClass("desc").html(
`<#3988#>`
).appendTo($feature_desc_cntr);
...
$("<div>").addClass("title").attr({"data-container":"profile_title"}).html(`<#1172#>`).appendTo($title_cntr);
...
let wifi_auth_parm = {"title":"<#4028#>", "text":`WPA3-Personal`};
Get_Component_Pure_Text(wifi_auth_parm).appendTo($content_container);
let backhaul_pwd_parm = {"title":"<#665#>", "id":"backhaul_pwd"};
Get_Component_Pure_Text(backhaul_pwd_parm).appendTo($content_container);
let hide_ssid_parm = {"title":"<#4118#>", "type":"switch", "id":"hide_ssid"};
...
let $btn_container_apply = $("<div>").attr({"id":"apply_btn"}).addClass("btn_container apply").appendTo($action_container).html("<#286#>");
