...
code += '<span title="Change to default client icon" onclick="card_setDefaultIcon();"><#1769#></span>';
code += '<span id="card_changeIconTitle" title="Change client icon" style="margin-left:10px;" onclick="card_show_custom_image();"><#1763#></span>';
code += '</div>';
}
code += '</td>';
code += `
...
code += "<span onmouseover='return overlib(\"" + htmlEnDeCode.htmlEncode(block_internet_hint) +"\");' onmouseout='return nd();'><#1649#></span>";
code += '</div>';
...
code += '<div id="card_internetTimeScheduling" class="internetTimeEdit" style="float:right;margin-right:10px;" title="<#3486#>"></div>';
code += '</div>';
code += '<div class="clientList_line"></div>';
code += '<div id="div_card_ipmac_binding" style="height:33px;width:100%;margin:5px 0;">';
code += '<div style="width:65%;float:left;line-height:33px;">';
code += "<span onmouseover='return overlib(\"" + htmlEnDeCode.htmlEncode(ip_binding_hint) +"\");' onmouseout='return nd();'><#1652#></span>";
code += '</div>';
...
switch(rssi_t) {
case 1:
connectModeTip = "<#816#>: <#571#>\n";
break;
case 2:
connectModeTip = "<#816#>: <#572#>\n";
break;
case 3:
connectModeTip = "<#816#>: <#573#>\n";
break;
case 4:
connectModeTip = "<#816#>: <#574#>\n";
break;
}
if(isSupport("stainfo")) {
if(clientInfo.curTx != "")
...
connectModeTip += "<#980#>: " + clientInfo.wlConnectTime + "";
}
}
if(sw_mode != 4){
...
document.getElementById("card_client_ipMethod").style.display = "";
document.getElementById("card_client_ipMethod").innerHTML = "<#1657#>";
...
document.getElementById("card_client_login").innerHTML = "<#1655#>";
}
if(clientInfo.isPrinter) {
document.getElementById("card_client_printer").style.display = "";
document.getElementById("card_client_printer").innerHTML = "<#1656#>";
}
...
var opModeDes = ["none", "<#953#>", "<#558#>", "<#536#>", "<#545#>"];
...
if(confirm(stringSafeGet("<#1653#>".replace("64", manually_dhcp_maximum)))) {
...
if(confirm(stringSafeGet("<#1650#>".replace("16", parentctrl_maximum)))) {
...
document.getElementById("card_changeIconTitle").innerHTML = "<#1766#>";
}
else {
slideFlag = true;
slideUp("card_custom_image", 500);
document.getElementById("card_changeIconTitle").innerHTML = "<#1763#>";
}
}
}
...
confirmFlag = confirm(stringSafeGet("<#1648#>"));
if(confirmFlag) {
hide_edit_client_block();
if(document.getElementById("edit_client_block") != null) {
...
code += '<thead><tr>';
code += '<td colspan="4">Client upload icon&nbsp;(<#2776#>&nbsp;100)</td>'; /*untranslated*/
code += '</tr></thead>';
code += '<tr>';
code += '<th width="45%"><#3042#></th>';
code += '<th width="30%"><#3040#></th>';
code += '<th width="15%"><#1647#></th>';
code += '<th width="10%"><#1767#></th>';
code += '</tr>';
code += '</table>';
...
code += '<input class="button_gen" type="button" onclick="uploadIcon_confirm();" value="<#1783#>">';
...
code +='<tr><td class="hintColor" colspan="4"><#2558#></td></tr>';
...
const networkLabel = isSupport("mtlancfg") ? "<#2965#>" : "<#3905#>";
return `
<div class='switch_block_main'>
<div class='switch_block'>
<div class='segmented_picker_label'><#3411#></div>
</div>
...
<div class='block_filter_name'><#1505#></div>
</div>
<div class='segmented_picker_option ${isAllMode ? '' : 'active'}' style='cursor:pointer' onclick='changeClientListViewMode();'>
...
switch (clienlistViewMode) {
case "All" :
code += create_clientlist_card("<#1660#>", "all");
break;
case "ByInterface" :
code += create_clientlist_card("<#3511#>", "wired");
...
for(var i=1; i<isSupport("mssid_count")+1; i++){
code += create_clientlist_card("<#376#> - " + i, "gn" + i);
}
}
}
break;
...
code += `</div>`;
if(!top.isIE8)
code += `<div class='div-export'><div class='btn-export' onclick='exportClientListLog();'><#1632#></div></div>`;
code += "</td></tr></tbody>";
code += "</table>";
...
document.getElementById("all_expander").innerHTML = "[ <#1662#> ]";
}
}
else {
$.each(isWL_map, function(index, value){
...
document.getElementById("wired_expander").innerHTML = "[ <#1662#> ]";
}
}
else{
if(!sorter["wl"+index+"_display"]){
document.getElementById("clientlist_wl"+index+"_list_Block").style.display = "none";
document.getElementById("wl"+index+"_expander").innerHTML = "[ <#1662#> ]";
}
}
});
if(smart_connect_version != ""){
...
document.getElementById("sc_expander").innerHTML = "[ <#1662#> ]";
}
}
if(isSupport("amas")){
if(isSupport("mtlancfg")){
...
document.getElementById("sdn"+sdn_all_rl.sdn_rl.idx+"_expander").innerHTML = "[ <#1662#> ]";
}
});
}
else{
...
document.getElementById("gn"+i+"_expander").innerHTML = "[ <#1662#> ]";
}
}
}
}
...
<th width=${obj_width[0]} onclick='sorter.addBorder(this);sorter.doSorter(9, "num", "${objID}");' style='cursor:pointer;'><#3905#></th>
<th width=${obj_width[1]}><#1647#></th>
<th width=${obj_width[2]} class='client-list-sm text-left' onclick='sorter.addBorder(this);sorter.doSorter(2, "str", "${objID}");' style='cursor:pointer;'><#1646#></th>
<th width=${obj_width[3]} class='client-list-xl text-left' onclick='sorter.addBorder(this);sorter.doSorter(3, "num", "${objID}");' style='cursor:pointer;'><#3730#></th>
<th width=${obj_width[4]} class='client-list-xl' onclick='sorter.addBorder(this);sorter.doSorter(4, "str", "${objID}");' style='cursor:pointer;'><#3040#></th>
...
<th width=${obj_width[7]} onclick='sorter.addBorder(this);sorter.doSorter(8, "num", "${objID}");' style='cursor:pointer;'><#980#></th>
</tr>
</thead>`;
clientListCode += `<tbody>`;
if (sortArray.length == 0) {
clientListCode += `<tr id='tr_${objID}'><td class='hintColor' colspan='${wl_colspan}'><#2558#></td></tr>`;
} else {
...
<div style="font-size: 0.75em;" title="<#2608#>">${clientlist_sort[j].ip6_prefix}</div>
...
<div class='client_info_detail_block'>
<div class='client_info_detail_title'><#980#></div>
<div>${accessTime}</div>
</div>
...
document.getElementById(_obj).innerText = "<#3014#>";
}
else {
slideFlag = true;
slideUp(_controlObj, 100);
document.getElementById(_obj).innerText = "<#3015#>";
}
}
}
...
htmlCode += "<div id='" + _containerID + "_clientlist_dropdown_expand' class='clientlist_dropdown_expand' onclick='expand_hide_Client(\"" + _containerID + "_clientlist_dropdown_expand\", \"" + _containerID + "_clientlist_offline\");' onmouseover='over_var=1;' onmouseout='over_var=0;'><#3015#></div>";
...
if(document.getElementById("" + _containerID + "_clientlist_dropdown_expand").innerText == "<#3015#>") {
...
{tag:"#CLIENTLIST_ELECTRONIC_DEVICES",text:"<#1663#>"},
{tag:"#CLIENTLIST_DESKTOP",text:"<#1664#>"},
{tag:"#CLIENTLIST_WINDOWS_DESKTOP",text:"<#1665#>"},
{tag:"#CLIENTLIST_LINUX_DESKTOP",text:"<#1666#>"},
{tag:"#CLIENTLIST_LINUX_DEVICE",text:"<#1667#>"},
{tag:"#CLIENTLIST_SMARTPHONE",text:"<#1668#>"},
{tag:"#CLIENTLIST_IPHONE",text:"<#1669#>"},
{tag:"#CLIENTLIST_ASUS_SMARTPHONE",text:"<#1670#>"},
{tag:"#CLIENTLIST_WINDOWS_PHONE",text:"<#1671#>"},
{tag:"#CLIENTLIST_ANDROID_PHONE",text:"<#1672#>"},
{tag:"#CLIENTLIST_PAD",text:"<#1673#>"},
{tag:"#CLIENTLIST_IPAD",text:"<#1674#>"},
{tag:"#CLIENTLIST_ASUS_PAD",text:"<#1675#>"},
{tag:"#CLIENTLIST_ANDROID_TABLET",text:"<#1676#>"},
{tag:"#CLIENTLIST_REPEATER",text:"<#1677#>"},
{tag:"#CLIENTLIST_NAS_SERVER",text:"<#1678#>"},
{tag:"#CLIENTLIST_NB",text:"<#1679#>"},
{tag:"#CLIENTLIST_WINDOWS_NB",text:"<#1680#>"},
{tag:"#CLIENTLIST_MACBOOK",text:"<#1681#>"},
{tag:"#Clientlist_ASUS_NB",text:"<#1682#>"},
{tag:"#CLIENTLIST_SD_CARD",text:"<#1683#>"},
{tag:"#CLIENTLIST_ROG_DEVICE",text:"<#1684#>"},
{tag:"#CLIENTLIST_SCANNER",text:"<#1685#>"},
{tag:"#CLIENTLIST_APPLE_DEVICE",text:"<#1686#>"},
{tag:"#CLIENTLIST_ANDROID_DEVICE",text:"<#1687#>"},
{tag:"#CLIENTLIST_WINDOWS_DEVICE",text:"<#1688#>"},
{tag:"#CLIENTLIST_ASUS_DEVICE",text:"<#1689#>"},
{tag:"#CLIENTLIST_MEDIA_ENTERTAINMENT",text:"<#1690#>"},
{tag:"#CLIENTLIST_ELECTRONIC_EQUIPMENT",text:"<#1691#>"},
{tag:"#CLIENTLIST_AIR_CONDITIONER",text:"<#1692#>"},
{tag:"#CLIENTLIST_REFRIGERATOR",text:"<#1693#>"},
{tag:"#CLIENTLIST_WEIGHT_SCALE",text:"<#1694#>"},
{tag:"#CLIENTLIST_ELECTRIC_POT",text:"<#1695#>"},
{tag:"#CLIENTLIST_MICROWAVE_OVEN",text:"<#1696#>"},
{tag:"#CLIENTLIST_AIR_PURIFIER",text:"<#1697#>"},
{tag:"#CLIENTLIST_FAN",text:"<#1698#>"},
{tag:"#CLIENTLIST_DEHUMIDIFIER",text:"<#1699#>"},
{tag:"#CLIENTLIST_WASHING_MACHINE",text:"<#1700#>"},
{tag:"#CLIENTLIST_WATER_HEATER",text:"<#1701#>"},
{tag:"#CLIENTLIST_ELECTRIC_KETTLE",text:"<#1702#>"},
{tag:"#CLIENTLIST_SMART_BULB",text:"<#1703#>"},
{tag:"#CLIENTLIST_SMART_TV",text:"<#1704#>"},
{tag:"#CLIENTLIST_CLEANING_ROBOT",text:"<#1705#>"},
{tag:"#CLIENTLIST_SECURITY",text:"<#1706#>"},
{tag:"#CLIENTLIST_SEISMOGRAPH",text:"<#1707#>"},
{tag:"#CLIENTLIST_SMART_LOCK",text:"<#1708#>"},
{tag:"#CLIENTLIST_WEARABLES",text:"<#1709#>"},
{tag:"#CLIENTLIST_SMART_BRACELET",text:"<#1710#>"},
{tag:"#CLIENTLIST_WATCH",text:"<#1711#>"},
{tag:"#CLIENTLIST_WALL_SWITCH",text:"<#1712#>"},
{tag:"#CLIENTLIST_WINDOW_SENSOR",text:"<#1713#>"},
{tag:"#CLIENTLIST_TEMPERATURE_HUMIDITY_SENSOR",text:"<#1714#>"},
{tag:"#CLIENTLIST_BODY_SENSOR",text:"<#1715#>"},
{tag:"#CLIENTLIST_SMART_PLUG",text:"<#1716#>"},
{tag:"#CLIENTLIST_ROBOT",text:"<#1717#>"},
{tag:"#PRINTER",text:"<#1656#>"},
{tag:"#ROUTER",text:"<#1839#>"},
{tag:"#NAS",text:"<#1841#>"},
{tag:"#IPCAM",text:"<#1842#>"}
];
...
$upload_category.append($("<div>").addClass("category_title").html("<#3023#>"));
