...
$("#dhcpdns_hint_dnspriv").html("<span><#1847#></span>");
...
$("#dotPresets").children().remove().end().append("<option value='-1'><#824#></option>");
...
document.getElementById("yadns_hint").innerHTML = "<span><#4224#></span>";
if(dnspriv_support){
document.getElementById("yadns_hint_dnspriv").style.display = "";
document.getElementById("yadns_hint_dnspriv").innerHTML = "<span><#4224#></span>";
}
}
}
if(gobi_support){
document.getElementById("page_title").innerHTML = "<#3902#>";
document.getElementById("wan_inf_th").innerHTML = "<#3897#>";
}
...
{tag:"#ADGUARD_1",text:"<#391#>"},
{tag:"#ADGUARD_2",text:"<#392#>"},
{tag:"#CLOUDFLARE_1",text:"<#393#>"},
{tag:"#CLOUDFLARE_2",text:"<#394#>"},
{tag:"#CLOUDFLARE_3",text:"<#395#>"}
];
...
else if(obj.options[obj.selectedIndex].text == "<#2901#>"){
...
var msg_iptv = "<#3958#>".replace(/LAN-*\D* 4/, wanAggr_p2_name(cur_wanports_bond));
}
else{
...
document.getElementById("check_mac").innerHTML="<#446#>";
return false;
}else if(flag ==2){
var childsel=document.createElement("div");
childsel.setAttribute("id","check_mac");
childsel.style.color="#FFCC00";
obj.parentNode.appendChild(childsel);
document.getElementById("check_mac").innerHTML="<#407#>";
return false;
}else{
...
if(!is_ipv4 && !is_ipv6) {
alert(document.form.dnspriv_server_0.value + "<#428#>");
document.form.dnspriv_server_0.focus();
return false;
}
...
code +='<tr><td style="color:#FFCC00;" colspan="5"><#2550#></td></tr>';
else{
...
var DNSListTableCategory=["<#397#>", "<#398#>", "<#399#>", "Safe", "<#400#>"];
var code="";
code += "<div style='margin-top:15px;margin-left:15px;float:left;font-size:15px;color:#93A9B1;'><#396#></div>";
...
code += "<thead><tr height='28px'><td id='td_all_list_title' colspan='4'><#1765#>";
code += "</td></tr></thead>";
code += "<tr id='tr_default_title' height='40px'>";
code += "<th class='IE8HACK' width='5%'></th>";
code += "<th class='IE8HACK' width='15%'><#713#></th>";
code += "<th class='IE8HACK' width='80%'><#2429#></th></tr>";
code += "<tr id='tr_auto_option' height='40px'>";
code += "<td><input name='DNS_service_opt' id='dns_auto' type='radio' class='input'></td>";
code += "<td>ISP</td>";
code += "<td style='padding:10px;text-align:left;'><#386#></td></tr>";
code += "</table>";
code += "</td></tr>";
...
code += "<th class='IE8HACK' width='5%'></th>";
code += "<th class='IE8HACK' width='15%'><#713#></th>";
code += "<th class='IE8HACK' width='15%'><#2429#></th>";
code += "<th class='IE8HACK' width='65%'><#1829#></th></tr>";
...
var description_temp = DNSService[idx].Description+" <a href='"+DNSService[idx].url+"' target='_blank' style='text-decoration:underline;'> <b><#2758#></b></a>";
...
code += "<tr><td>";
code += "<table width='100%' border='1' align='center' cellpadding='0' cellspacing='0' class='FormTable_table' style='margin-top:15px;'>";
code += "<thead><tr height='28px'><td colspan='4'><#471#>";
code += "</td></tr></thead>";
...
code += "<input class='button_gen' type='button' onclick='closeDNSListView()' value='<#287#>'>";
code += "<input class='button_gen' type='button' onclick='Update_DNS_service()' style='margin-left:15px;' value='<#1780#>'>";
code += "</div>";
...
DSN_status_info="<b>Default status :</b> <#386#>";
}
else{
DNS_list_index=DNSList_match(document.form.wan_dns1_x.value, document.form.wan_dns2_x.value);
if(DNS_list_index >= 0){
DSN_status_info = "<b><#335#>:</b> "+DNSService[DNS_list_index].FilterMode; /* Untranslated */
DSN_status_info += "<br><b><#1616#>:</b> "+DNSService[DNS_list_index].DNSService; /* Untranslated */
DSN_status_info += "<br><b><#585#>:</b> "+document.form.wan_dns1_x.value+", "+document.form.wan_dns2_x.value; /* Untranslated */
}
else{
DSN_status_info = "<b><#585#>:</b> "+document.form.wan_dns1_x.value; /* Untranslated */
...
function update_map(){
var msg = stringSafeGet("<#2798#>")+" <#1099#>";
if(confirm(msg)){
$.ajax({
url: "/s46reset.cgi",
success: function( response ) {
...
var aftrArray = [
["<#2605#> (dgw.xpass.jp)", "dgw.xpass.jp"], ["transix (gw.transix.jp)", "gw.transix.jp"]
];
...
document.getElementById("wan_s46_dslite_svc_span").innerHTML = (s46_dslite_svc_str=="xpass")? "<#2605#>":s46_dslite_svc_str;
}
setTimeout("update_info();", 1500);
}
}
...
<div>&nbsp;</div>
<div class="formfonttitle"><#489#> - <#490#></div>
<div style="margin:10px 0 10px 5px;" class="splitLine"></div>
<div id="page_title" class="formfontdesc" style="margin-bottom:0px;"><#2750#></div>
...
<tr>
<th id="wan_inf_th"><#3936#></th>
<td align="left">
<select class="input_option" name="wan_unit" onchange="change_wan_unit(this);"></select>
</td>
</tr>
</table>
<div id="basic_setting_desc" class="formfontdesc" style="margin-bottom:0px; margin-top: 15px;"><#3891#></div>
...
<option value="dhcp" <% nvram_match("wan_proto", "dhcp", "selected"); %>><#255#></option>
<option value="static" <% nvram_match("wan_proto", "static", "selected"); %>><#256#></option>
...
<option value="v6plus" <% nvram_match("wan_proto", "v6plus", "selected"); %>><#2589#></option>
<option value="ocnvc" <% nvram_match("wan_proto", "ocnvc", "selected"); %>><#2588#></option>
...
<div id="s46reset_btn" style="margin-left: 5px;"><input type="button" class="button_gen" onclick="update_map();" value="<#2797#>"></div>
...
<th><#2000#></th>
<td>
<input type="radio" name="wan_enable" class="input" value="1" <% nvram_match("wan_enable", "1", "checked"); %>><#275#>
<input type="radio" name="wan_enable" class="input" value="0" <% nvram_match("wan_enable", "0", "checked"); %>><#274#>
</td>
</tr>
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,22);"><#1993#></a></th>
<td>
<input type="radio" name="wan_nat_x" class="input" value="1" onclick="change_nat(1);" <% nvram_match("wan_nat_x", "1", "checked"); %>><#275#>
<input type="radio" name="wan_nat_x" class="input" value="0" onclick="change_nat(0);" <% nvram_match("wan_nat_x", "0", "checked"); %>><#274#>
</td>
</tr>
<tr id="nat_type_tr" style="display:none;">
<th><#2326#>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<a id="faq_nat" href="" target="_blank" style="font-family:Lucida Console;text-decoration:underline;">FAQ</a></th>
<td>
<select name="nat_type" class="input_option">
<option value="0" <% nvram_match("nat_type", "0", "selected"); %>><#2327#></option>
<option value="1" <% nvram_match("nat_type", "1", "selected"); %>><#2328#></option>
</select>
</td>
</tr>
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,23);"><#1544#></a>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<a id="faq" href="" target="_blank" style="font-family:Lucida Console;text-decoration:underline;">UPnP&nbspFAQ</a></th>
<td>
<input type="radio" name="wan_upnp_enable" class="input" value="1" onclick="return change_common_radio(this, 'LANHostConfig', 'wan_upnp_enable', '1')" <% nvram_match("wan_upnp_enable", "1", "checked"); %>><#275#>
<input type="radio" name="wan_upnp_enable" class="input" value="0" onclick="return change_common_radio(this, 'LANHostConfig', 'wan_upnp_enable', '0')" <% nvram_match("wan_upnp_enable", "0", "checked"); %>><#274#>
</td>
</tr>
<tr style="display:none;">
<th><#3954#></th>
<td>
<input type="radio" name="bond_wan_radio" class="input" value="1" onclick="return change_common_radio(this, 'LANHostConfig', 'bond_wan', '1')" <% nvram_match("bond_wan", "1", "checked"); %>><#275#>
<input type="radio" name="bond_wan_radio" class="input" value="0" onclick="return change_common_radio(this, 'LANHostConfig', 'bond_wan', '0')" <% nvram_match("bond_wan", "0", "checked"); %>><#274#>
<div id="wanAgg_desc" style="color:#FFCC00;"><#3953#></div>
...
<th><#4071#></th>
<td>
<input type="radio" name="ewan_dot1q" class="input" value="1" onclick="change_dsl_dhcp_enable();" <% nvram_match("ewan_dot1q", "1", "checked"); %>><#275#>
<input type="radio" name="ewan_dot1q" class="input" value="0" onclick="change_dsl_dhcp_enable();" <% nvram_match("ewan_dot1q", "0", "checked"); %>><#274#>
</td>
</tr>
<tr>
<th>VLAN ID</th>
...
<th><#4071#></th>
<td>
<input type="radio" name="wan_dot1q" class="input" value="1" onclick="return change_common_radio(this, 'IPConnection', 'wan_dot1q', 1);" <% nvram_match("wan_dot1q", "1", "checked"); %>><#275#>
<input type="radio" name="wan_dot1q" class="input" value="0" onclick="return change_common_radio(this, 'IPConnection', 'wan_dot1q', 0);" <% nvram_match("wan_dot1q", "0", "checked"); %>><#274#>
</td>
</tr>
<tr>
<th>VLAN ID</th>
...
<th><#2751#></th>
<td>
<input type="radio" name="wan_dhcpenable_x" class="input" value="1" onclick="change_wan_dhcp_enable(0);" <% nvram_match("wan_dhcpenable_x", "1", "checked"); %>><#275#>
<input type="radio" name="wan_dhcpenable_x" class="input" value="0" onclick="change_wan_dhcp_enable(0);" <% nvram_match("wan_dhcpenable_x", "0", "checked"); %>><#274#>
</td>
</tr>
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,1);"><#384#></a></th>
...
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,2);"><#406#></a></th>
...
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,3);"><#404#></a></th>
...
<option value="0" <% nvram_match("wan_s46_dslite_mode", "0", "selected"); %>><#241#></option>
<option value="1" <% nvram_match("wan_s46_dslite_mode", "1", "selected"); %>><#2784#></option>
</td>
</tr>
...
<img id="pull_arrow" height="14px;" src="/images/arrow-down.gif" style="position:absolute;*margin-left:-3px;*margin-top:1px;" onclick="pullaftrList(this);" title="<#3282#>" onmouseover="over_var=1;" onmouseout="over_var=0;">
...
<th><#2751#></th>
<td>
<input type="radio" name="wan_s46_dhcpenable_x" class="input" value="1" onclick="change_wan_dhcp_enable(0);" <% nvram_match("wan_dhcpenable_x", "1", "checked"); %>><#275#>
<input type="radio" name="wan_s46_dhcpenable_x" class="input" value="0" onclick="change_wan_dhcp_enable(0);" <% nvram_match("wan_dhcpenable_x", "0", "checked"); %>><#274#>
</td>
</tr>
<tr>
...
<a id="s46_ipaddr_hint" class="hintstyle" href="javascript:void(0);" onClick="openHint(7,1);" style="display:none"><#384#></a>
</th>
...
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(-1,-1);"><#2571#></a></th>
...
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(-1,-1);"><#2593#></a></th>
...
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,12);"><#585#></a></th>
<td>
<div id="DNS_status"></div>
<div style="margin:10px 0 5px 5px;" class="splitLine_dns"></div>
<div class="assign_dns"><#1902#></div>
<div style="margin:-38px 0 5px 0;text-align:right;"><input type="button" class="button_gen" onclick="Assign_DNS_service()" value="<#286#>"></div>
</td>
</tr>
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,42);">Forward local domain queries to upstream DNS</a></th><td>
<input type="radio" value="1" name="dns_fwd_local" <% nvram_match("dns_fwd_local", "1", "checked"); %> /><#275#>
<input type="radio" value="0" name="dns_fwd_local" <% nvram_match("dns_fwd_local", "0", "checked"); %> /><#274#>
</td>
</tr>
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,43);">Enable DNS Rebind protection</a></th><td>
<input type="radio" value="1" name="dns_norebind" <% nvram_match("dns_norebind", "1", "checked"); %> /><#275#>
<input type="radio" value="0" name="dns_norebind" <% nvram_match("dns_norebind", "0", "checked"); %> /><#274#>
</td>
</tr>
...
<input type="radio" value="1" name="dnssec_enable" onclick="showhide('dnssec_strict_tr',1);" <% nvram_match("dnssec_enable", "1", "checked"); %> /><#275#>
<input type="radio" value="0" name="dnssec_enable" onclick="showhide('dnssec_strict_tr',0);" <% nvram_match("dnssec_enable", "0", "checked"); %> /><#274#>
</td>
</tr>
...
<input type="radio" value="1" name="dnssec_check_unsigned_x" <% nvram_match("dnssec_check_unsigned_x", "1", "checked"); %> /><#275#>
<input type="radio" value="0" name="dnssec_check_unsigned_x" <% nvram_match("dnssec_check_unsigned_x", "0", "checked"); %> /><#274#>
</td>
</tr>
...
<option value="0" <% nvram_match("dns_priv_override", "0", "selected"); %>><#241#></option>
<option value="1" <% nvram_match("dns_priv_override", "1", "selected"); %>><#275#></option>
<option value="2" <% nvram_match("dns_priv_override", "2", "selected"); %>><#274#></option>
</select>
</td>
</tr>
<tr style="display:none">
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,35);"><#3924#></a></th>
<td align="left">
<select id="dnspriv_enable" class="input_option" name="dnspriv_enable" onChange="change_dnspriv_enable(this.value);">
<option value="0" <% nvram_match("dnspriv_enable", "0", "selected"); %>><#950#></option>
...
<div id="dnsfilter_hint_dnspriv" style="display:none;"><span><#344#></span></div>
</td>
</tr>
<tr style="display:none">
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,36);"><#3912#></a></th>
<td>
<input type="radio" name="dnspriv_profile" class="input" value="1" onclick="return change_common_radio(this, 'IPConnection', 'dnspriv_profile', 1)" <% nvram_match("dnspriv_profile", "1", "checked"); %> /><#3914#>
<input type="radio" name="dnspriv_profile" class="input" value="0" onclick="return change_common_radio(this, 'IPConnection', 'dnspriv_profile', 0)" <% nvram_match("dnspriv_profile", "0", "checked"); %> /><#3915#>
</td>
</tr>
<tr style="display:none" id="dot_presets_tr">
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,41);"><div class="table_text"><#3910#></a></th>
<td>
...
<thead>
<tr>
<td colspan="5"><#3916#>&nbsp;(<#2768#>&nbsp;8)</td>
</tr>
</thead>
<tr>
<th><a href="javascript:void(0);" onClick="openHint(7,37);"><div class="table_text"><#384#></div></a></th>
<th><a href="javascript:void(0);" onClick="openHint(7,38);"><div class="table_text"><#3918#></div></a></th>
<th><a href="javascript:void(0);" onClick="openHint(7,39);"><div class="table_text"><#3920#></div></a></th>
<th><a href="javascript:void(0);" onClick="openHint(7,40);"><div class="table_text"><#3922#></div></a></th>
<th><#2767#></th>
</tr>
<tr>
...
<thead>
<tr><td colspan="2"><#2569#></td></tr>
</thead>
<tr>
<th width="40%"><#1850#> (<#2974#> 60):</th>
...
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,29);"><#3105#></a></th>
<td align="left">
<select class="input_option" name="wan_auth_x" onChange="change_wan_type(document.form.wan_proto.value);">
<option value="" <% nvram_match("wan_auth_x", "", "selected"); %>><#950#></option>
<option value="8021x-md5" <% nvram_match("wan_auth_x", "8021x-md5", "selected"); %>>802.1x MD5</option>
</select></td>
</tr>
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,4);"><#3690#></a></th>
...
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,5);"><#579#></a></th>
<td>
...
<div style="margin-top:1px;"><input type="checkbox" name="show_pass_1" onclick="pass_checked(document.form.wan_pppoe_passwd);"><#714#></div>
</td>
</tr>
<tr style="display:none">
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,6);"><#888#></a></th>
<td align="left">
<select class="input_option" name="wan_pppoe_auth">
<option value="" <% nvram_match("wan_pppoe_auth", "", "selected"); %>><#241#></option>
...
<tr style="display:none">
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,6);"><#3109#></a></th>
<td>
...
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,7);"><#3129#></a></th>
...
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,8);"><#3127#></a></th>
...
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,9);"><#3135#></a></th>
...
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,10);"><#3113#></a></th>
...
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,18);">Host-Uniq (<#2417#>)</a></th>
...
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,17);"><#3133#></a></th>
<td>
<select name="wan_pptp_options_x" class="input_option">
<option value="" <% nvram_match("wan_pptp_options_x", "","selected"); %>><#241#></option>
<option value="-mppc" <% nvram_match("wan_pptp_options_x", "-mppc","selected"); %>><#2987#></option>
...
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,31);"><#3117#></a></th>
<td>
<select name="wan_ppp_echo" class="input_option" onChange="ppp_echo_control();">
<option value="0" <% nvram_match("wan_ppp_echo", "0","selected"); %>><#1625#></option>
...
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,32);"><#3120#></a></th>
...
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,33);"><#3122#></a></th>
...
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,34);"><#3124#></a></th>
...
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,18);"><#3115#></a></th>
...
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,19);"><#258#></a></th>
<td>
...
<tr id="vpn_dhcp">
<th><#3136#></th>
<td><input type="radio" name="wan_vpndhcp" class="input" value="1" onclick="return change_common_radio(this, 'IPConnection', 'wan_vpndhcp', 1)" <% nvram_match("wan_vpndhcp", "1", "checked"); %> /><#275#>
<input type="radio" name="wan_vpndhcp" class="input" value="0" onclick="return change_common_radio(this, 'IPConnection', 'wan_vpndhcp', 0)" <% nvram_match("wan_vpndhcp", "0", "checked"); %> /><#274#>
</td>
</tr>
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,15);"><#3116#></a></th>
<td>
...
<tr>
<th ><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,16);"><#3119#></a></th>
<td>
...
<input type="button" class="button_gen" onclick="showMAC();" value="<#261#>">
</td>
</tr>
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(7,30);"><#1845#></a></th>
<td>
<select name="wan_dhcp_qry" class="input_option">
<option value="0" <% nvram_match(" wan_dhcp_qry", "0","selected"); %>><#1848#></option>
<option value="1" <% nvram_match(" wan_dhcp_qry", "1","selected"); %>><#1846#></option>
<option value="2" <% nvram_match(" wan_dhcp_qry", "2","selected"); %>><#4300#></option>
</select>
</td>
</tr>
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick=""><#2017#></a></th>
<td>
<input type="radio" name="ttl_inc_enable" class="input" value="1" <% nvram_match("ttl_inc_enable", "1", "checked"); %>><#275#>
<input type="radio" name="ttl_inc_enable" class="input" value="0" <% nvram_match("ttl_inc_enable", "0", "checked"); %>><#274#>
</td>
</tr>
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick=""><#3389#></a></th>
<td>
<input type="radio" name="ttl_spoof_enable" class="input" value="1" <% nvram_match("ttl_spoof_enable", "1", "checked"); %>><#275#>
<input type="radio" name="ttl_spoof_enable" class="input" value="0" <% nvram_match("ttl_spoof_enable", "0", "checked"); %>><#274#>
</td>
</tr>
</table>
<div class="apply_gen" style="height:auto">
<input class="button_gen" id="apply_btn" onclick="applyRule();" type="button" value="<#284#>"/>
</div>
</td>
</tr>
</td>
