...
html += '</a>';
html += '<div class="clients" id="ameshNumber" style="cursor:pointer;"><#1140#>: <span>0</span></div>';
...
document.getElementById('NM_connect_title').innerHTML = "<#3016#> :";
}
else{
...
document.getElementById("first_wan_title").innerHTML = "<#2013#>:";
...
if(gobi_support)
document.getElementById("first_wan_title").innerHTML = "<#2901#>:";
else
document.getElementById("first_wan_title").innerHTML = "<#499#>:";
}
if(dualwan_second_if == 'wan')
document.getElementById("second_wan_title").innerHTML = "<#2013#>:";
...
if(gobi_support)
document.getElementById("second_wan_title").innerHTML = "<#2901#>:";
else
document.getElementById("second_wan_title").innerHTML = "<#499#>:";
}
}
...
if( ddns_enable == '0'){
document.getElementById("ddnsHostName").innerHTML = '<a style="color:#FFF;text-decoration:underline;" href="/Advanced_ASUSDDNS_Content.asp?af=ddns_enable_x"><#1629#></a>';
$('#copyDdns').hide();
}else if(ddnsName == '')
document.getElementById("ddnsHostName").innerHTML = '<a style="color:#FFF;text-decoration:underline;" href="/Advanced_ASUSDDNS_Content.asp?af=DDNSName"><#3339#></a>';
else if(ddnsName == isMD5DDNSName())
document.getElementById("ddnsHostName").innerHTML = '<a style="color:#FFF;text-decoration:underline;" href="/Advanced_ASUSDDNS_Content.asp?af=DDNSName"><#3339#></a>';
else{
if(ddns_server_x == "WWW.DNSOMATIC.COM"){
document.getElementById("ddnsHostName").innerHTML = '<a style="color:#FFF;text-decoration:underline;" href="https://dnsomatic.com/" target="_blank"><#1629#></a>';
$('#copyDdns').hide();
}
else{
...
if(sw_mode == "3"){
showtext(document.getElementById("NM_connect_status"), "<div style='margin-top:10px;'><#4082#></div>");
}
else if(sw_mode == "2"){
showtext(document.getElementById("NM_connect_title"), "<div style='margin-top:10px;'><#842#>:</div><br>");
}
else
return 0;
}
...
document.getElementById("clientNumber").innerHTML = "<#2251#>: <span id='_clientNumber'>" + num + "</span>";
}
...
dec_html_code += '<div class="style1"><strong id="diskUnmount'+ device.usbPath +'"><#1856#></strong></div>\n';
}
...
document.getElementById("deviceDec_" + device.usbPath).innerHTML = '<div style="margin:10px;"><#1766#></div>';
}
...
document.getElementById("changeIconTitle").innerHTML = "<#1759#>";
}
}
...
if(bwdpi_support) {
document.getElementById("time_scheduling_title").innerHTML = "<#3477#>";
}
...
connectModeTip += "<#976#>: " + clientObj.wlConnectTime + "";
}
}
if(sw_mode != 4){
...
document.getElementById('client_login').innerHTML = "<#1651#>";
}
if(clientObj.isPrinter) {
document.getElementById('client_printer').style.display = "";
document.getElementById('client_printer').innerHTML = "<#1652#>";
}
...
<div style="margin:200px auto; width:300px; background-color:#006699; color:#FFFFFF; line-height:150%; border:3px solid #FFF; padding:5px;"><#527#></p></div>
</div>
</noscript>
...
<div style="background: #121C1E;font-size: 16px;padding: 10px 20px;margin: 0 5px;border-radius: 8px;min-width: 80px;text-align: center;cursor:pointer" onclick="hide_notice()"><#287#></div>
<div style="background: #121C1E;font-size: 16px;padding: 10px 20px;margin: 0 5px;border-radius: 8px;min-width: 80px;text-align: center;cursor:pointer" onclick="notice_apply();"><#284#></div>
</div>
</div>
...
<thead>
<tr>
<td colspan="4">Client upload icon&nbsp;(<#2768#>&nbsp;100)</td>
</tr>
</thead>
<tr>
<th width="45%"><#1642#></th>
...
<input class="button_gen" type="button" onclick="btUserIconCancel();" value="<#287#>">
<input class="button_gen" type="button" onclick="btUserIconEdit();" value="<#1779#>">
...
<span title="Change to default client icon" class="IE8HACK" onclick="setDefaultIcon();"><#1765#></span>
<span id="changeIconTitle" class="IE8HACK" title="Change client icon" style="margin-left:10px;" onclick="show_custom_image();"><#1759#></span>
</div>
</td>
...
<span onmouseover="return overlib('Enable this button to block this device to access internet.');" onmouseout="return nd();"><#1645#></span></div>
<div class="left" style="cursor:pointer;float:right;" id="radio_BlockInternet_enable"></div>
</div>
<div class="clientList_line"></div>
<div style="height:32px;width:100%;margin:5px 0;">
<div style="width:65%;float:left;line-height:32px;">
<span id="time_scheduling_title" onmouseover='return overlib("<#3025#>");' onmouseout="return nd();"><#564#></span>
</div>
<div align="center" class="left" style="cursor:pointer;float:right;" id="radio_TimeScheduling_enable"></div>
<div id="internetTimeScheduling" class="internetTimeEdit" style="float:right;margin-right:10px;" title="<#3477#>" onclick="redirectTimeScheduling(document.getElementById('macaddr_field').value);" ></div>
</div>
<div class="clientList_line"></div>
<div style="height:32px;width:100%;margin:5px 0;">
<div style="width:65%;float:left;line-height:32px;">
<span onmouseover="return overlib('Enable this button to bind specific IP with MAC Address of this device.');" onmouseout="return nd();"><#1648#></span></div>
...
<input class="button_gen" type="button" onclick="edit_delete();" id="deleteBtn" value="<#1763#>" style="display:none;">
<input class="button_gen" type="button" id="blockBtn" value="<#1546#>" title="<#1597#>" style="display:none;">
<script>
...
<input class="button_gen" type="button" onclick="closeClientDetailView();" value="<#287#>">
<input id="edit_confirm" class="button_gen" type="button" onclick="edit_confirm();" value="<#284#>">
...
<div id="first_wan_title"><#1978#>:</div>
...
<div id="second_wan_title"><#1985#>:</div>
...
<div>
<span id="NM_connect_title" style="font-size:12px;font-family: Verdana, Arial, Helvetica, sans-serif;"><#3409#>:</span>
<br>
<strong id="NM_connect_status" class="index_status" style="font-size:14px;"><#730#>...</strong>
</div>
...
<strong id="ddnsHostName" class="index_status" style="font-size:14px;"><#621#></strong>
...
<span style="font-size:14px;font-family: Verdana, Arial, Helvetica, sans-serif;"><#2466#>:</span>
...
<span style="font-size:14px;font-family: Verdana, Arial, Helvetica, sans-serif;"><#3271#>: </span>
<br/>
<strong id="wl_securitylevel_span" class="index_status"></strong>
<img id="iflock">
</div>
<div id="mbModeContext" style="display:none">
<span style="font-size:14px;font-family: Verdana, Arial, Helvetica, sans-serif;"><#507#>: </span>
<br/>
<br/>
<strong class="index_status"><#542#></strong>
</div>
</td>
</tr>
<tr>
...
<input type="button" class="button_gen" value="<#3701#>" style="margin-top:15px;" onClick="pop_clientlist_listview(true)">
