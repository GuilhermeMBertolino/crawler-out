...
result.status_text = "<#3395#>";
}
else{//lb
result.status = (httpApi.isConnected(wan_index)) ? "connected" : "disconnected";
result.status_text = (result.status == "connected") ? "<#277#>" : "<#308#>";
}
}
else{
result.status = (httpApi.isConnected(wan_index)) ? "connected" : "disconnected";
...
result.proto_text = ((modem_operation != "") ? modem_operation : "<#2901#>");
}
}
}
return result;
