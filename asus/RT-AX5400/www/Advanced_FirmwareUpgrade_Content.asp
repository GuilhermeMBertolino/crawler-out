...
document.getElementById('dstzone').innerHTML = "<#370#>";
}
}
...
$(".current_fw_release_note").click({"model_name": "<#945#>", "fwver": FWString}, show_current_release_note);
}
...
$("#FWString").append("<span class='label-fw_rbk'><#2277#></span>");
...
html += "<td>";
html += '<div>';
html += '<input type="button" id="update" name="update" class="button_gen" onclick="show_offline_msg(true);" value="<#2769#>" />';
html += '<div><input type="button" id="amas_update" class="button_gen" style="margin:-33px 0px 0px 200px;display:none;" onclick="cfgsync_firmware_upgrade();" value="<#1788#>"/><div>';
html += '</div>';
html += '<div id="check_beta_div"><input type="checkbox" name="check_beta" id="amas_beta" onclick="change_beta_path()" value="" <% nvram_match("webs_update_beta", "1", "checked"); %>/><#2255#></div>'; // Untranslated
html += '<div id="linkpage_div" class="button_helplink" style="margin-left:200px;margin-top:-38px;display:none;">';
html += '<a id="linkpage" target="_blank"><div style="padding-top:5px;"><#2769#></div></a>';
html += '</div>';
...
html = "";
html += "<tr>";
html += "<td class='aimesh_node_category_bg' colspan='2'><#1212#></td>";
html += "</tr>";
html += "<tr>";
html += "<th>";
html += "<#945#>";
...
if(rbkfw_support){
html += "<span class='aimesh_fw_rbk' style='display:none;'><#2277#></span>";
}
html += "</div>";
html += "<div id='amesh_manual_upload_fw'>";
html += "<#2262#> : ";
html += "<span class='aimesh_fw_update_offline' style='margin-left:0px;' onclick='open_AiMesh_router_fw_upgrade();'><#1789#></span>";
html += "</div>";
...
$("#amas_" + mac_id + "").children().find(".checkFWCurrent").click({"model_name": "<#945#>", "fwver": FWString}, show_current_release_note);
}
...
if(!have_node) {
html += "<tr>";
html += "<td class='aimesh_node_category_bg' colspan='2'><#1140#></td>";
html += "</tr>";
}
html += "<tr>";
html += "<th>";
...
html += "<br>";
html += "<#1176#> : " + htmlEnDeCode.htmlEncode(alias);
html += "</th>";
...
if(rbkfw_support){
$("#amas_" + mac_id + "").children().find(".aimesh_fw_rbk_node").html("<#2277#>");
...
document.getElementById('update_states').innerHTML="<#2465#>&nbsp;<#2266#>";
...
document.getElementById('update_scan').style.display="none";
document.getElementById('update_states').innerHTML="<#361#><br><#2257#>";
document.getElementById('update').disabled = false;
}
else{
document.getElementById('update_scan').style.display="none";
...
contentA: "<#350#><br>",
contentC: "<br><#1023#> <#468#>",
left_button: (flag==2)? "<#301#>":"<#287#>",
left_button_callback: function(){confirm_cancel();},
left_button_args: {},
right_button: (flag==2)? "<#302#>":"<#1788#>",
...
document.getElementById('update_states').innerHTML="<#408#>";
}
}
...
document.getElementById('update_scan').style.display="none";
document.getElementById('update_states').style.display="";
document.getElementById('update_states').innerHTML="<#1733#>";
return false;
}
if(download_info > 0){
document.getElementById('update_states').style.display="";
document.getElementById('update_states').innerHTML="<#1639#>";
...
document.getElementById('loading_block3').innerHTML = "<div><#364#></div>";
}
else{
dead++;
}
...
document.getElementById("drword").innerHTML = "<#1733#>";
return false;
}
},
success: function(){
...
document.getElementById("drword").innerHTML = "&nbsp;&nbsp;&nbsp;<#2258#>...";
setTimeout("isDownloading();", 1000);
}
}
else if(cfg_check == "0") {
...
document.getElementById("drword").innerHTML = "<#361#><br><#2257#>";
return false;
}
...
document.getElementById("drword").innerHTML = "&nbsp;&nbsp;&nbsp;<#2258#>..."+fwdl_percent;
setTimeout("isDownloading();", 1000);
}
...
document.getElementById("drword").innerHTML = "Memory space is NOT enough to upgrade on internet. Please wait for rebooting.<br><#2257#>"; /* untranslated */ //Untranslated. fw_size_higher_mem
return false;
}
...
var date_format = "<#2273#> : " + year + "/" + month + "/" + date + " " + hour + ":" + minute;
return date_format;
}
...
$amesh_ok.attr({"type" : "button", "value" : "<#1779#>"});
...
$(".confirm_block").children().find("#status_iframe").contents().find("#amas_release_note_hint").val("<#2276#>");
}
...
notice = "<#2268#><br><br>If you want to revert both your main router and node(s) firmware to the previous version, please revert node(s) first before the main router."; //Untranslated
if(document.firmware_form.webs_update_enable.value == 1){
...
theadObj.innerHTML = "<tr height='28px'><td id='td_all_list_title' colspan='2'><#2278#></td></tr>";
...
divObj_bot.innerHTML = "<input class='button_gen' type='button' onclick='close_rbk_selector()' value='<#287#>'><input class='button_gen' type='button' onclick='rbk_fw_confirm()' style='margin-left:15px;' value='<#2277#>'>";
...
html += "<span class='aimesh_fw_update_offline' style='margin-left:0px;' onclick='show_offline_msg(false);'><#1653#></span>";
}
else {
html += "<span class='aimesh_fw_update_offline' style='margin-left:0px;' ><#1789#></span>";
}
}
else {
html += "<span class='aimesh_fw_update_offline' style='margin-left:0px;text-decoration:none;cursor:none;'><#2270#></span>";
}
return html;
}
...
<div>&nbsp;</div>
<div class="formfonttitle"><#506#> - <#510#></div>
<div style="margin:10px 0 10px 5px;" class="splitLine"></div>
<div class="formfontdesc"><strong><#2268#></strong>
<ol>
<li><#2263#></li>
<li><#368#></li>
<li id="fw_note2"><#2264#>&nbsp;<#2265#></li>
<li id="fw_note3"><#2256#></li>
</ol>
</div>
<br>
...
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(11, 14);"><#2253#></a></th>
<td>
...
<input type="button" id="sig_check" name="sig_check" class="button_gen" onclick="sig_version_check();" value="<#2769#>">
</div>
<div>
...
<input type="button" id="update" name="update" class="button_gen" onclick="detect_update();" value="<#2769#>" />
</div>
<div id="check_beta_div"><input type="checkbox" name="check_beta" id="path_beta" onclick="change_beta_path()" value="" <% nvram_match("webs_update_beta", "1", "checked"); %>/><#2255#></div>
<div id="linkpage_div" class="button_helplink" style="margin-left:200px;margin-top:-38px;display:none;">
<a id="linkpage" target="_blank"><div style="padding-top:5px;"><#2769#></div></a>
</div>
<div id="check_states">
...
<input type="button" name="upload" class="button_gen" onclick="submitForm()" value="<#1789#>" />
</td>
</tr>
</table>
