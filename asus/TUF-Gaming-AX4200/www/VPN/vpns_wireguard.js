...
var str_vpnc_ip_allowed = stringSafeGet("<#3872#>");
var str_vpnc_keepalive = stringSafeGet("<#3874#>");
var wgs_faq_href = {
...
var $faq_setup_wgs = $("<a/>").attr({"target":"_blank", "href":wgs_faq_href.setup_wgs}).html("<#3855#>");
$("<div>").addClass("step_text faq hyperlink").append($faq_setup_wgs).appendTo($step_text_container);
var $faq_site_to_site = $("<a/>").attr({"target":"_blank", "href":wgs_faq_href.site_to_site}).html("<#3856#>");
$("<div>").addClass("step_text faq hyperlink").append($faq_site_to_site).appendTo($step_text_container);
var $faq_setup_wgce = $("<a/>").attr({"target":"_blank", "href":wgs_faq_href.setup_wgc}).html("<#3857#>");
...
$("<div>").addClass("title").html("<#3888#>").appendTo($popup_title_container);
var $close_btn = $("<div>").addClass("vpn_icon_all_collect close_btn");
$close_btn.appendTo($popup_title_container);
$close_btn.unbind("click").click(function(e){
e = e || event;
...
$("<div>").addClass("notice_title").html("<#2511#>").appendTo($notice_container);
...
var wgsc_name_parm = {"title":"<#3690#>", "type":"text", "id":"wgsc_name", "need_check":true, "maxlength":64};
if(!show_config){
...
var wgsc_aips_parm = {"title":str_vpnc_ip_allowed+" (<#2716#>)", "type":"text", "id":"wgsc_aips", "need_check":true, "maxlength":4095};
...
var isFirstClient = ((wgs_clientlist_data.length == 0) ? true : false);
if(isFirstClient)
$btn_container_apply.html("<#4301#>");
var $action_loading_container = $("<div>").addClass("action_container loading").appendTo($content_container);
$("<div>").addClass("action_container_hint").html(htmlEnDeCode.htmlEncode("* <#3847#>")).hide().appendTo($content_container);
...
isValid_wgs_addr = {'isError': true, 'errReason': address + " <#2621#>"};
return false;
}
...
isValid_wgsc_addr = {'isError': true, 'errReason': address + " <#2621#>"};
return false;
}
if(valid_is_IP_format(address, "IPv4")){
...
$Add_Client_Title_obj.find("#vpns_clientlist_title").html("<#3887#> (<#2768#>" + htmlEnDeCode.htmlEncode(wgs_clientlist_limit) + ")");
...
var wgs_dns_parm = {"title":"<#3018#> DNS", "type":"switch", "id":"wgs_dns"};
...
var wgs_psk_parm = {"title":"<#3792#>", "type":"switch", "id":"wgs_psk"};
...
var wgs_pub_parm = {"title":"<#3161#>", "id":"wgs_pub", "text":"-"};
