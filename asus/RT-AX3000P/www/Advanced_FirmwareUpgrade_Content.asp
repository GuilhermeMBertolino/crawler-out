...
$("#FWString").click({"model_name": "<#861#>", "fwver": FWString}, show_current_release_note);
}
if(amesh_support && (isSwMode("rt") || isSwMode("ap")) && ameshRouter_support) {
var have_node = false;
...
html += "<td>";
html += '<div>';
html += '<input type="button" id="update" name="update" class="button_gen" onclick="show_offline_msg(true);" value="<#2657#>" />';
html += '<div><input type="button" id="amas_update" class="button_gen" style="margin:-33px 0px 0px 200px;display:none;" onclick="cfgsync_firmware_upgrade();" value="<#1695#>"/><div>';
html += '</div>';
html += '<div id="linkpage_div" class="button_helplink" style="margin-left:200px;margin-top:-38px;display:none;">';
html += '<a id="linkpage" target="_blank"><div style="padding-top:5px;"><#2657#></div></a>';
html += '</div>';
...
html = "";
html += "<tr>";
html += "<td class='aimesh_node_category_bg' colspan='2'><#1127#></td>";
html += "</tr>";
html += "<tr>";
html += "<th>";
html += "<#861#>";
...
html += "<div id='current_version'><#103#> : <span class='checkFWCurrent'>" + FWString + "</span></div>";
html += "<div id='amesh_manual_upload_fw'>";
html += "<#2162#> : ";
html += "<span class='aimesh_fw_update_offline' style='margin-left:0px;' onclick='open_AiMesh_router_fw_upgrade();'><#1696#></span>";
html += "</div>";
html += "<div id='checkNewFW' class='checkNewFW' style='display:none;'><#104#> : <span class='checkFWResult'></span></div>";
html += "</td>";
html += "</tr>";
$("#fw_version_tr").before(html);
if(afwupg_support && webs_update_enable_orig == 1){
$("#amas_" + mac_id + "").children().find(".checkFWCurrent").addClass("aimesh_fw_release_note");
$("#amas_" + mac_id + "").children().find(".checkFWCurrent").click({"model_name": "<#861#>", "fwver": FWString}, show_current_release_note);
}
...
if(!have_node) {
html += "<tr>";
html += "<td class='aimesh_node_category_bg' colspan='2'><#1055#></td>";
html += "</tr>";
}
html += "<tr>";
html += "<th>";
...
html += "<br>";
html += "<#1091#> : " + htmlEnDeCode.htmlEncode(alias);
html += "</th>";
html += "<td id='amas_" + mac_id + "' current_online='" + online + "'>";
html += "<div id='current_version'><#103#> : <span class='checkFWCurrent'>" + fwver + "</span></div>";
...
document.getElementById('update_states').innerHTML="<#2363#>&nbsp;<#2166#>";
...
document.getElementById('update_scan').style.display="none";
document.getElementById('update_states').innerHTML="<#279#><br><#2157#>";
document.getElementById('update').disabled = false;
}
else{
document.getElementById('update_scan').style.display="none";
...
contentA: "<#268#><br>",
contentC: "<br><#939#> <#386#>",
left_button: (flag==2)? "<#219#>":"<#206#>",
left_button_callback: function(){confirm_cancel();},
left_button_args: {},
right_button: (flag==2)? "<#220#>":"<#1695#>",
...
document.getElementById('update_states').innerHTML="<#326#>";
}
}
...
document.getElementById('update_scan').style.display="none";
document.getElementById('update_states').style.display="";
document.getElementById('update_states').innerHTML="<#1640#>";
return false;
}
if(download_info > 0){
document.getElementById('update_states').style.display="";
document.getElementById('update_states').innerHTML="<#1549#>";
...
document.getElementById('loading_block3').innerHTML = "<div><#282#></div>";
}
else{
dead++;
}
...
document.getElementById("drword").innerHTML = "<#1640#>";
return false;
}
},
success: function(){
...
document.getElementById("drword").innerHTML = "&nbsp;&nbsp;&nbsp;<#2158#>...";
setTimeout("isDownloading();", 1000);
}
}
else if(cfg_check == "0") {
...
document.getElementById("drword").innerHTML = "<#279#><br><#2157#>";
return false;
}
...
document.getElementById("drword").innerHTML = "&nbsp;&nbsp;&nbsp;<#2158#>..."+fwdl_percent;
setTimeout("isDownloading();", 1000);
}
...
document.getElementById("drword").innerHTML = "Memory space is NOT enough to upgrade on internet. Please wait for rebooting.<br><#2157#>"; /* untranslated */ //Untranslated. fw_size_higher_mem
return false;
}
...
var date_format = "<#2172#> : " + year + "/" + month + "/" + date + " " + hour + ":" + minute;
return date_format;
}
...
$(".confirm_block").children().find("#status_iframe").contents().find("#amas_release_note_hint").val("<#2175#>");
}
...
$("#amas_" + mac_id + "").children("#current_version").html("<#103#> : <span class='checkFWCurrent'>" + fwver + "</span>");
}
if(afwupg_support && online == 1 && webs_update_enable_orig == 1){
$("#amas_" + mac_id + "").children().find(".checkFWCurrent").addClass("aimesh_fw_release_note");
...
html += "<span class='aimesh_fw_update_offline' style='margin-left:0px;' onclick='show_offline_msg(false);'><#1560#></span>";
}
else {
html += "<span class='aimesh_fw_update_offline' style='margin-left:0px;' ><#1696#></span>";
}
}
else {
html += "<span class='aimesh_fw_update_offline' style='margin-left:0px;text-decoration:none;cursor:none;'><#2169#></span>";
}
return html;
}
...
<div>&nbsp;</div>
<div class="formfonttitle"><#424#> - <#428#></div>
<div style="margin:10px 0 10px 5px;" class="splitLine"></div>
<div class="formfontdesc"><strong><#2167#></strong>
<ol>
<li><#2163#></li>
<li><#286#></li>
<li id="fw_note2"><#2164#>&nbsp;<#2165#></li>
<li id="fw_note3"><#2156#></li>
</ol>
</div>
<br>
...
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(11, 14);"><#2153#></a></th>
<td>
...
<input type="button" id="sig_check" name="sig_check" class="button_gen" onclick="sig_version_check();" value="<#2657#>">
</div>
<div>
...
<input type="button" id="update" name="update" class="button_gen" onclick="detect_update();" value="<#2657#>" />
</div>
<div id="linkpage_div" class="button_helplink" style="margin-left:200px;margin-top:-38px;display:none;">
<a id="linkpage" target="_blank"><div style="padding-top:5px;"><#2657#></div></a>
</div>
<div id="check_states">
...
<input type="button" name="upload" class="button_gen" onclick="submitForm()" value="<#1696#>" />
</td>
</tr>
</table>
