...
var str_vpnc_ip_allowed = stringSafeGet("<#3871#>");
var str_vpnc_keepalive = stringSafeGet("<#3873#>");
var wgs_faq_href = {
...
var $faq_setup_wgs = $("<a/>").attr({"target":"_blank", "href":wgs_faq_href.setup_wgs}).html("<#3854#>");
$("<div>").addClass("step_text faq hyperlink").append($faq_setup_wgs).appendTo($step_text_container);
var $faq_site_to_site = $("<a/>").attr({"target":"_blank", "href":wgs_faq_href.site_to_site}).html("<#3855#>");
$("<div>").addClass("step_text faq hyperlink").append($faq_site_to_site).appendTo($step_text_container);
var $faq_setup_wgce = $("<a/>").attr({"target":"_blank", "href":wgs_faq_href.setup_wgc}).html("<#3856#>");
...
$("<div>").addClass("title").html("<#3887#>").appendTo($popup_title_container);
var $close_btn = $("<div>").addClass("vpn_icon_all_collect close_btn");
$close_btn.appendTo($popup_title_container);
$close_btn.unbind("click").click(function(e){
e = e || event;
...
if(show_config){
var wgs_export_switch_parm = {"options": [{"text":"QR code","value":"1"},{"text":"<#1628#>","value":"2"}]};/* untranslated */
...
$("<div>").addClass("notice_title").html("<#2510#>").appendTo($notice_container);
...
$("<div>").addClass("qr_text").html(htmlEnDeCode.htmlEncode("<#4482#>")).appendTo($qr_code_cntr);
return $container
}
...
$("<div>").addClass("text").html(htmlEnDeCode.htmlEncode("<#1628#>")).appendTo($export_btn);
...
var wgsc_name_parm = {"title":"<#3689#>", "type":"text", "id":"wgsc_name", "need_check":true, "maxlength":64};
if(!show_config){
...
var wgsc_aips_parm = {"title":str_vpnc_ip_allowed+" (<#2715#>)", "type":"text", "id":"wgsc_aips", "need_check":true, "maxlength":4095};
...
if(show_config){
var wgsc_update_key_parm = {"title":"Renew Key", "id":"wgsc_update_key", "btn_text":"<#295#>"};/* untranslated */
...
var $btn_container_apply = $("<div>").addClass("btn_container apply").appendTo($action_container).html("<#284#>");
var isFirstClient = ((wgs_clientlist_data.length == 0) ? true : false);
if(isFirstClient)
$btn_container_apply.html("<#4300#>");
var $action_loading_container = $("<div>").addClass("action_container loading").appendTo($content_container);
$("<div>").addClass("action_container_hint").html(htmlEnDeCode.htmlEncode("* <#3846#>")).hide().appendTo($content_container);
...
$(_obj).find(".status_icon").removeClass("connected").attr({"title":"<#308#>"});
...
$(_obj).find("[data-wgsc-unit='"+value.index+"']").find(".status_icon").addClass("connected").attr({"title":"<#277#>"});
}
}
}
});
...
if($wgs_addr.val() == ""){
$wgs_addr.show_validate_hint("<#414#>");
$wgs_addr.focus();
return false;
}
...
isValid_wgs_addr = {'isError': true, 'errReason': address + " <#2620#>"};
return false;
}
...
isValid_wgs_addr = {'isError': true, 'errReason': address + " <#428#>"};
return false;
}
});
if(isValid_wgs_addr.isError){
...
if($wgs_port.val() == ""){
$wgs_port.show_validate_hint("<#414#>");
$wgs_port.focus();
return false;
}
...
if($wgs_alive.val() == ""){
$wgs_alive.show_validate_hint("<#414#>");
$wgs_alive.focus();
return false;
}
...
if($wgsc_name.val() == ""){
$wgsc_name.show_validate_hint("<#414#>");
$valid_des.focus();
return false;
}
...
if($wgsc_addr.val() == ""){
$wgsc_addr.show_validate_hint("<#414#>");
$wgsc_addr.focus();
return false;
}
...
isValid_wgsc_addr = {'isError': true, 'errReason': address + " <#2620#>"};
return false;
}
if(valid_is_IP_format(address, "IPv4")){
...
isValid_wgsc_addr = {'isError': true, 'errReason': address + " <#428#>"};
return false;
}
});
if(isValid_wgsc_addr.isError){
...
if($wgsc_aips.val() == ""){
$wgsc_aips.show_validate_hint("<#414#>");
$wgsc_aips.focus();
return false;
}
...
isValid_wgsc_aips = {'isError': true, 'errReason': address + " <#428#>"};
return false;
}
});
if(isValid_wgsc_aips.isError){
...
if($wgsc_caips.val() == ""){
$wgsc_caips.show_validate_hint("<#414#>");
$wgsc_caips.focus();
return false;
}
...
isValid_wgsc_caips = {'isError': true, 'errReason': address + " <#428#>"};
return false;
}
});
if(isValid_wgsc_caips.isError){
...
var wgs_lanaccess_parm = {"title":"<#973#>", "type":"switch", "id":"wgs_lanaccess"};
...
$Add_Client_Title_obj.find("#vpns_clientlist_title").html("<#3886#> (<#2767#>" + htmlEnDeCode.htmlEncode(wgs_clientlist_limit) + ")");
...
show_customize_alert("<#4432#>".replace("#MAXNUM", wgs_clientlist_limit));
return false;
}
...
var wgs_dns_parm = {"title":"<#3017#> DNS", "type":"switch", "id":"wgs_dns"};
Get_Component_Switch(wgs_dns_parm).appendTo($detail_adv);
var wgs_nat6_parm = {"title":"<#1993#> - IPv6", "type":"switch", "id":"wgs_nat6"};
Get_Component_Switch(wgs_nat6_parm).appendTo($detail_adv);
var wgs_psk_parm = {"title":"<#3791#>", "type":"switch", "id":"wgs_psk"};
...
var wgs_priv_parm = {"title":"<#1812#>", "id":"wgs_priv", "text":"-"};
Get_Component_Pure_Text(wgs_priv_parm).appendTo($detail_adv);
var wgs_pub_parm = {"title":"<#3160#>", "id":"wgs_pub", "text":"-"};
Get_Component_Pure_Text(wgs_pub_parm).appendTo($detail_adv);
var wgs_update_key_parm = {"title":"Renew Key", "id":"wgs_update_key", "btn_text":"<#295#>"};/* untranslated */
