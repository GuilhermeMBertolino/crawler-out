...
$("#" + weekScheduleApi.obj_id + "").find("#timezone_hint").html("* <#2666#>");
}
else
return;
}
...
if(_type == "start")
$time_setting_title.html("<span><#4352#></span>");
else
$time_setting_title.html("<span><#4353#></span>");
var get_time_setting_component = function(_type, _unit){
var time_unit_parm = {
"text":{"hour":"<#2346#>", "minute":"<#2743#>"},
...
show_valid_hint("* <#2127#>", $(this));
return false;
}
if((end_hour*60+end_min) > 1440){//24:00
show_valid_hint("* <#4353#> : <#4369#>", $(this));
return false;
}
if((start_hour*60+start_min) == (end_hour*60+end_min)){
...
$all_day_text.html("<#4351#>");
$edit_hour_minute_bg.append($("<div>").addClass("horizontal_line"));
var $routine_schedule_title = $("<div>").html("<#4365#>").appendTo($edit_hour_minute_bg);
...
return $("<div>").addClass("offtime_schedule_text dis").html("<#1551#> " + weekScheduleApi.text_Offtime_Sche);
},
...
$("<div>").addClass("btn_icon_bg tips_icon").attr("title", "<#583#>").unbind("click").click(function(e){
e = e || event;
e.stopPropagation();
...
weekScheduleApi.text_Offtime_Sche = "<#4359#>";
else if(weekScheduleApi.PC_schedule_mode == "M")
weekScheduleApi.text_Offtime_Sche = "<#4362#>";
}
},
...
$no_data_bg.append($("<div>").addClass("no_data_text").html("<#2475#>"));
return $no_data_bg;
},
...
.replace("MON", "<#1723#>").replace("TUE", "<#1727#>").replace("WED", "<#1728#>")
.replace("THU", "<#1726#>").replace("FRI", "<#1722#>").replace("SAT", "<#1724#>")
.replace("SUN", "<#1725#>");
else
display_title = week_group.replace("Daily", "<#4373#>").replace("Weekdays", "<#4372#>").replace("Weekend", "<#4371#>");
...
var $content_bg = $("<div>");
$("<div>").addClass("edit_offtime_title").html("<#4358#>").appendTo($content_bg);
...
$content_bg.append("<br>");
$("<div>").addClass("change_mode_hint").html("<#4367#>").appendTo($content_bg);
...
$("<div>").html("<#4361#>").appendTo($mode_desc);
$("<div>").html("<#4360#>").appendTo($mode_desc);
...
$("<div>").html("<#4364#>").appendTo($mode_desc);
$("<div>").html("<#4363#>").appendTo($mode_desc);
...
$("<div>").addClass("button_gen edit_done_btn").html("<#1695#>").appendTo($content_bg).unbind("click").click(function(e){
e = e || event;
e.stopPropagation();
...
var $action_cancel = $("<input/>").attr({"id":"action_cancel", "type":"button", "value":"<#212#>"}).addClass("button_gen");
...
var $start_bg = $("<div>").addClass("start_bg");
$("<div>").addClass("icon").appendTo($start_bg);
$("<div>").addClass("btn").html("<#716#>").unbind("click").click(function(e){
e = e || event;
e.stopPropagation();
$(this).closest("#tutorial_guide_bg").fadeOut(function(){
$(this).remove();
...
$("<div>").addClass("menu_title").html("<#1249#>").appendTo($left_bg);
