...
result.status_text = "<#3321#>";
}
else{//lb
result.status = (httpApi.isConnected(wan_index)) ? "connected" : "disconnected";
result.status_text = (result.status == "connected") ? "<#202#>" : "<#233#>";
}
}
else{
result.status = (httpApi.isConnected(wan_index)) ? "connected" : "disconnected";
...
result.proto_text = ((modem_operation != "") ? modem_operation : "<#2827#>");
}
}
}
return result;
