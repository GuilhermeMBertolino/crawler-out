...
var led_table = ["<#1550#>", "<#3081#>", "<#2729#>", "<#2343#>"];
...
var code = '';
if(system.INTELplatform){
code += '<div class="info-title"><#4306#></div>';
...
code += '<div class="info-block"><div class="info-title"><#2574#></div><div class="info-content">'+ _lanIP +'</div></div>';
code += '<div class="info-block"><div class="info-title"><#2982#></div><div class="info-content">'+ variable.secret_code +'</div></div>';
if(variable.serial_no != ''){
code += '<div class="info-block"><div class="info-title"><#3213#></div><div class="info-content">'+ variable.serial_no +'</div></div>';
}
...
<div class="info-title">${wl_nband_title[get_wl_unit_by_band(bandName[i])]} <#388#></div>
...
var modeDesc = ['<#4139#>', '<#4141#>', '<#4143#>', '', '<#1551#>'];
code += '<div class="division-block"><#4132#></div>';
...
code += '<div class="info-content"><#2176#> '+ yadns_clients[i] +'</div>';
code += '</div>';
}
}
...
var code = '<div class="division-block"><#764#></div>';
code += '<div>';
code += '<div>';
code += '<div class="display-flex flex-a-center ram-content">';
code += '<div class="ram-text-width"><#765#></div>';
code += '<div class="ram-text-width"><#766#></div>';
code += '<div class="ram-text-width"><#767#></div>';
code += '</div>';
code += '<div class="display-flex flex-a-center ram-content">';
...
$("<div>").css({"margin-bottom":"6px"}).html("<#3014#> <#3015#>").appendTo($hint_text_bg);
...
$("<div>").css({"margin-bottom":"6px"}).html("<#3016#> :").appendTo($hint_text_bg);
var $help_item = $("<ol>").css({"margin":"0", "padding":"0", "margin-left":"24px"}).appendTo($hint_text_bg);
$("<li>").css({"color":"#64737c"}).html("<#3021#>".replace("#ETHCABLE", "Cat 5e")).appendTo($help_item);
$("<li>").css({"color":"#64737c"}).html("<#3017#>").appendTo($help_item);
$("<li>").css({"color":"#64737c"}).html("<#3018#>").appendTo($help_item);
$("<li>").css({"color":"#64737c"}).html("<#3019#>").appendTo($help_item);
$("<li>").css({"color":"#64737c"}).html("<#3020#>").appendTo($help_item);
var $action_bg = $("<div>").css({"margin-top":"18px", "text-align":"right"}).appendTo($popup_hint_bg);
var $action_ok = $("<input/>").attr({"id":"action_ok", "type":"button", "value":"<#1704#>"}).addClass("button_gen");
...
$("#phy_ports").append('<div class="division-block" style="position:relative;"><span><#758#></span><div class="port_status_notice_icon"></div></div>');
...
$("<div>").addClass("warn").html("<#2925#>").appendTo($port_status_icon_desc);/* untranslated */
$("<div>").addClass("unplug").html("<#761#>").appendTo($port_status_icon_desc);
if(show_notice){
if(top.webWrapper){
...
var code = '<div class="division-block"><#758#></div>';
code += '<div>';
code += '<div class="display-flex flex-a-center table-header">';
code += '<div class="port-block-width table-content"><#759#></div>';
code += '<div class="port-block-width table-content"><#756#></div>';
code += '</div>';
...
var code = '<div class="division-block"><#4383#></div>';
code += '<div>';
code += '<div class="display-flex flex-a-center table-header">';
code += '<div class="port-block-width table-content"><#756#></div>';
code += '<div class="port-block-width table-content">Tx (<#2412#>)</div>';
code += '<div class="port-block-width table-content">Rx (<#2412#>)</div>';
code += '</div>';
...
<div id="status_tab" class="tab-block tab-click" onclick="switchTab(this.id)"><#756#></div>
