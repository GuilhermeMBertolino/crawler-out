...
infolist = new Array();
infolist.push(["<#750#> ...","No_selected"]);
infolist.push(["<#2033#>","phone"]);
infolist.push(["QQ","qq"]);
infolist.push(["<#3897#>","wechat"]);
...
ptypelist = new Array();
ptypelist.push(["<#750#> ...", "No_selected"]);
ptypelist.push(["<#2038#>", "Setting_Problem"]);
ptypelist.push(["<#1982#>", "Connection_or_Speed_Problem"]);
ptypelist.push(["<#1974#>", "Compatibility_Problem"]);
ptypelist.push(["<#2042#>", "Suggestion"]);
ptypelist.push(["<#2053#>", "Technical_Support"]);
ptypelist.push(["<#924#>", "Other_Problem"]);
...
desclist.push(["<#424#>/<#3606#>","USB dongle"]);
url_group.push(["Modem"]);
desclist.push(["<#1826#>","DM"]);
...
desclist.push(["<#773#>","System Log"]);
url_group.push(["System"]);
desclist.push(["<#2888#>","Network Tools"]); //25
url_group.push(["Status_"]);
desclist.push(["<#3112#>","Rescue"]);
url_group.push(["Rescue"]);//false value
desclist.push(["<#1978#>","Other Devices"]);
url_group.push(["Other_Device"]);//false value
desclist.push(["<#3896#>","Fail to access"]);
url_group.push(["GUI"]);//false value
desclist.push(["<#435#>","FW update"]);
url_group.push(["FirmwareUpgrade"]);
if(isSupport("Instant_Guard")){
desclist.push(["<#4380#>","Instant Guard"]); //30
url_group.push(["Instant_Guard"]);
}
}
else if(ptype == "Connection_or_Speed_Problem"){
desclist.push(["<#1983#>","Wireless speed"]);
desclist.push(["<#1984#>","Wired speed"]);
desclist.push(["<#1986#>","Unstable connection"]);
desclist.push(["<#1985#>","Router reboot"]);
desclist.push(["<#1987#>","Wireless disconnected"]);
}
else if(ptype == "Compatibility_Problem"){
desclist.push(["<#1977#>","modem"]);
desclist.push(["<#1979#>","other router"]);
desclist.push(["<#1975#>","OS or Application"]);
desclist.push(["<#1980#>","printer"]);
desclist.push(["<#1981#>","USB modem"]);
desclist.push(["<#1976#>","external hardware disk"]);
desclist.push(["<#1978#>","network devices"]);
}
else if(ptype == "Suggestion"){
desclist.splice(0,1);
desclist.push(["<#2045#>","Translation"]);
desclist.push(["<#2046#>","UI/UX"]);
desclist.push(["<#2043#>","Current Feature"]);
desclist.push(["<#2044#>","New Feature Request"]);
}
else if(ptype == "Technical_Support"){
desclist.splice(0,1);
desclist.push(["<#2051#>","tech_ASUS"]);
desclist.push(["<#2049#>","tech_Amazon"]);
desclist.push(["<#2052#>","tech_iOS"]);
desclist.push(["<#2050#>","tech_Android"]);
}
else{ //Other_Problem
desclist.splice(0,1);
desclist.push(["<#924#>","others"]);
}
...
timelist.push(["<#750#> ...","No_selected"]);
timelist.push(["<#2061#>","Just Now"]);
...
timelist.push(["<#2065#>","Today"]);
timelist.push(["<#2066#>","Recently"]);
timelist.push(["<#2067#>","I don’t recall"]);
...
bandlist.push(["<#1426#>","All"]);
bandlist.push(["<#2022#>","I am not sure"]);
bandlist.push(["<#2023#>","Issue with the main router"]);
bandlist.push(["<#2024#>","Issue with node(s)"]);
...
unstablelist.push(["<#750#> ...","No_selected"]);
unstablelist.push(["<#2020#>","All WiFi"]);
...
unstablelist.push(["<#2021#>","Both WiFi and WAN"]);
unstablelist.push(["<#2022#>","I am not sure"]);
unstablelist.push(["<#2023#>","Issue with the main router"]);
unstablelist.push(["<#2024#>","Issue with node(s)"]);
...
if(value) {
if(allUsbStatus.search("storage") == "-1"){
alert("USB disk required in order to store the debug log, please plug-in a USB disk to <#870#> and Enable DSL Line Diagnostic again.");/*untranslated*/
...
var remaining_time_str = "<#2853#> : ";
if(dblog_remaining == 0) {
remaining_time_str += "0" + " " + "(Prepare data...)"; //Untranslated
return remaining_time_str;
}
if(days)
remaining_time_str += days + " <#1729#> ";
if(hours)
remaining_time_str += hours + " <#2346#> ";
if(minutes)
remaining_time_str += minutes + " <#2743#> ";
if(seconds)
remaining_time_str += seconds + " <#3196#> ";
return remaining_time_str;
};
...
var dblog_service_mapping = ["", "WiFi", "<#1826#>", "<#806#>", "AiMesh"];
...
selectOption = {"1 <#2346#>" : hour_to_sec(1)};
}
if(usb_support && $("input[name=dblog_tousb_cb]").prop("checked")) {
baseOption = { "12 <#2346#>" : hour_to_sec(12), "1 <#1729#>" : hour_to_sec(24), "2 <#1729#>" : hour_to_sec(48), "3 <#1729#>" : hour_to_sec(72) };
}
else {
baseOption = { "6 <#2346#>" : hour_to_sec(6), "12 <#2346#>" : hour_to_sec(12), "24 <#2346#>" : hour_to_sec(24) };
}
...
$(".dblog_service_item.wifi").after(gen_appendix_option(16, "<#2058#>", "dhd"));
}
$("input[name=dblog_service_list]").prop("checked", true);
...
if($("input[name=dblog_tousb_cb]").prop("checked")) {
if(media_support)
$(".dblog_service_item.all").after(gen_service_option(4, "<#806#>", "noUSB"));
if(!nodm_support)
$(".dblog_service_item.all").after(gen_service_option(2, "<#1826#>", "noUSB"));
}
}
...
var auth_status_hint = "<#1453#>";
document.form.fb_email.value = "";
switch(_status) {
case "0" :
auth_status_hint = "<#568#>";
break;
case "1" :
auth_status_hint = "<#1452#>";
...
<tr>
<td>
<div class="drword" id="drword" style="height:110px;"><#392#> <#708#>...
<br/>
<br/>
</div>
</td>
...
<div>&nbsp;</div>
<div class="formfonttitle"><#431#> - <#2731#></div>
<div style="margin:10px 0 10px 5px;" class="splitLine"></div>
<div id="fb_desc0" class="formfontdesc" style="display:none;"><#2003#></div>
<div id="fb_desc1" class="formfontdesc" style="display:none;"><#2004#></div>
<div id="fb_desc_disconnect" class="formfontdesc hint-color" style="display:none;"><#2005#> <a class="hint-color" href="mailto:router_feedback@asus.com?Subject=<%nvram_get("productid");%>" target="_top">router_feedback@asus.com</a></div>
...
<th><#2009#> *</th>
<td>
<input type="checkbox" class="input" name="attach_syslog" id="attach_syslog_id"><label for="attach_syslog_id"><#773#></label>&nbsp;&nbsp;&nbsp;
<input type="checkbox" class="input" name="attach_cfgfile" id="attach_cfgfile_id"><label for="attach_cfgfile_id"><#2037#></label>&nbsp;&nbsp;&nbsp;
<span id="attach_iptables_span" style="color:#FFFFFF;"><input type="checkbox" class="input" name="attach_iptables" id="attach_iptables_id"><label for="attach_iptables_id"><#2017#></label></span>
<span id="attach_modem_span" style="color:#FFFFFF;"><input type="checkbox" class="input" name="attach_modemlog" id="attach_modemlog_id"><label for="attach_modemlog_id"><#1968#></label></span>
<input type="checkbox" class="input" name="attach_wlanlog" id="attach_wlanlog_id"><label for="attach_wlanlog_id"><#2057#></label>
</td>
</tr>
<tr>
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(25,11);"><#1999#> *</a></th>
<td>
...
<br>
<span id="storage_ready" class="hint-color" style="display:none;">* <#3604#></span>
<span id="be_lack_storage" class="hint-color" style="display:none;">* <#2915#></span>
</td>
</tr>
...
<option value="3600">1 <#2346#></option>
<option value="18000">5 <#2346#></option>
<option value="43200">12 <#2346#></option>
<option value="86400">24 <#2346#></option>
<option value="172800">48 <#2346#></option>
</select>
</td>
</tr>
<tr class="dblog_support_class">
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(34, 1);"><#2000#></a></th>
<td>
...
<label class="storeUSBHint"><input type="checkbox" name="dblog_tousb_cb" value="1" onclick="diag_change_storeUSB();" checked><#2001#></label>
<span class="noUSBHint">* <#2915#></span>
</div>
<div class="dblog_enabled_status">
<span>* <#1992#></span>
<br>
<span class="dblog_stop_text" onclick="dblog_stop();"><#1991#></span>
</div>
</td>
</tr>
...
<div class="dblog_disabled_status">
<label class="dblog_service_item all"><input type="checkbox" name="dblog_service_list_all" onclick="diag_change_service_list_all();"><#1426#></label>
</div>
<div class="dblog_enabled_status">
...
<option value="No_selected"><#750#> ...</option>
<option value="Stable_connection"><#2039#></option>
<option value="Occasional_interruptions"><#2031#></option>
<option value="Frequent_interruptions"><#2016#></option>
<option value="Unavailable"><#2054#></option>
</select>
</td>
</tr>
<tr>
...
<th><a class="hintstyle" href="javascript:void(0);" onClick=""><#2068#></a></th>
<td>
...
<th><a class="hintstyle" href="javascript:void(0);" onClick=""><#2060#></a></th>
<td>
...
<th><a class="hintstyle" href="javascript:void(0);" onClick=""><#2019#></a></th>
<td>
...
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(34,2);"><#1451#></a></th>
<td>
...
<tr style="display:none;">
<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(34,3);"><#2048#></a></th>
<td>
...
<div id="eula_content" style="margin-left: 20px;"><#2055#></div>
</div>
<input class="btn_subusage button_gen" name="btn_send" onclick="applyRule()" type="button" value="<#1557#>"/>
</div>
</td>
</tr>
<tr>
...
<strong class="warning_title"><#2193#></strong>
<ul>
<li><#2030#><br><a id="call_link" style="font-weight: bolder;text-decoration:underline;cursor:pointer;" href="" target="_blank">https://www.asus.com/support/CallUs/</a></li>
</ul>
</div>
</td>
</tr>
