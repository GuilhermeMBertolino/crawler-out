...
state_reload() {
	...
	case $uplink in
	eth)
		if [ "$prev_uplink" != "eth" -a "$QCA_UAP_MIAMI" = "1" ]; then
			log "[state_reload][$prev_uplink -> eth] reload hostapd"
			touch /var/run/reload_hostapd
			[ "$prev_uplink" != "eth" ] && touch /var/run/update_hostapd
		fi
		...
		;;
	wds)
		if [ "$prev_uplink" != "wds" -a "$QCA_UAP_MIAMI" = "1" ]; then
			wds_ch=$(iwlist $vport chan | sed -n 's/^.*(Channel \([0-9]*\).*/\1/p')
			wds_bw=$(iwpriv $vport get_chwidth | awk -F : '{print $2}')
			wds_mode=$(iwpriv $vport get_desmode | awk -F : '{print $2}')
			log "[state_reload][$prev_uplink -> wds] reload hostapd"
			touch /var/run/reload_hostapd
			echo $wds_ch > /var/run/wds_ch
			echo $wds_bw > /var/run/wds_bw
			echo $wds_mode > /var/run/wds_mode
		fi
...
configure_vap() {
	...
	downlink_state=$3
	if [ "${MTK_UAP}" = "0" ]; then
		dumtxvap=$(cat /var/run/dummy_mbssid_vaps)
	fi
	...
	if [ -n "$dumtxvap" ]; then
		if [ "$service_state" = "down" ]; then
			stop_vap="$stop_vap $dumtxvap"
		else
			run_vap="$run_vap $dumtxvap"
		fi
	fi
	...
	if [ -f /var/run/reload_hostapd ]; then
		reload_hostapd
	else
...
list_first_vaps_ifs() {
	...
	do
		for vap in $(cat /var/run/${i}_devnames | grep -v vwire | grep -v scan | grep -v apcl)
		do
			if [ "$vap" ]
			then
				local fchar
...
check_cert_fingerprint() {
    ...
    local server=$(echo "$2" | sed -n  's#^https://\([^/]*\)/.*#\1#p')
    [ -n "${server}" ] || return 1
    local fingerprint=$(openssl s_client -connect "${server}" \
        < /dev/null 2>/dev/null \
        | openssl x509 -fingerprint -noout \
        | cut -d= -f2 \
        | sed 's/://g')
	local rc=$?
	[ "${rc}" == "0" -a -n "${fingerprint}" ] || return 1
    [ "${fingerprint}" == "${cert_fingerprint}" ] || return 1
}

upload_support_file() {
	[ $# -eq 3 ] || return 1
	[ -z "$(pidof support)" ] || return 1
	local token=$1
    local cert_fingerprint=$(echo "$2" | tr '[a-f]' '[A-F]')
	local upload_url=$3
	local support_dir=$(mktemp -p /tmp/ support-XXXXXXXXXX)
	local support_file="${support_dir}.tgz"

    if ! check_cert_fingerprint "${cert_fingerprint}" "${upload_url}"; then
        error_log "Invalid server certificate"
        return 1
    fi

	/sbin/support -d "${support_dir}"
	tar -C "${support_dir}" -czf "${support_file}" .
	rm -rf "${support_dir}"

	local http_code=$(curl -k 2>/dev/null \
			-w '%{http_code}' \
	...
	if [ "${rc}" != "0" -o "${http_code}" != "204" ]; then
		echo "Failed to upload support file. Http code: ${http_code}" | logger
...
support_tar() {
	local support_dir=$1
	[ -z $support_dir ] && support_dir="/tmp/support"
...
set-etherlight-mode)
	exit_if_busy $cmd $*
	exit_if_state_lock_failed $cmd $*
	_set_etherlight_mode "$1"
	state_unlock
	;;
set-etherlight-device-type)
	exit_if_busy $cmd $*
	exit_if_state_lock_failed $cmd $*
	pkill_generic led_locate.sh -f

	led_device_type_mode=3
	_set_etherlight_mode $led_device_type_mode
	_set_port_bitmap_color $led_device_type_mode $@
...
set-port-locate)
	...
	led_port_locate_mode=4
	_set_port_locate $@
	_set_port_bitmap_color $led_port_locate_mode $@
...
apply-config)
	...
	before_mca_pid=$(cat_file /var/run/mcad.0.pid)
	full_provision=0
	need_cfg_save_file=/var/run/need_cfg_save
	lock_count=30
	...
	after_mca_pid=$(cat_file /var/run/mcad.0.pid)
...
cfg_save_check)
	...
	if [ -f /var/run/need_cfg_save ]; then
		...
		if [ -s /etc/persistent/cfg/mgmt ]; then
			...
			rc=$?
			if [ $rc -ne 0 ]; then
				error_log "cfg_save_check: Failed to write configuration: $rc"
				ls -alR /etc/persistent/ |logger
			fi
		elif [ "$(cat /tmp/system.cfg |grep mgmt.is_default |awk -F= '{print $2}')" != "true" ]; then
			mkdir -p /tmp/run
			cfgmtd -r -p /tmp/run -f /tmp/run/read.cfg
			if [ $? -eq 0 ]; then
				log "cfg_save_check: try to recover mgmt"
				ls -alR /tmp/run/persistent/ |logger
				if [ -s /tmp/run/persistent/cfg/mgmt ]; then
					cp /tmp/run/persistent/cfg/* /etc/persistent/cfg/
					cp /tmp/run/persistent/blebr* /etc/persistent/
					cat /etc/persistent/cfg/mgmt |grep cfgversion |logger
					cat /etc/persistent/cfg/mgmt |grep servers |logger
					rm -rf /tmp/run/persistent /tmp/run/read.cfg
				fi
			fi
		fi
