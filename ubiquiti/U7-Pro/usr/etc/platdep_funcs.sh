...
_upgrade() {
	## Copy the firmware to eMMC, and reboot

	# Few things have already been checked.
	# 1. Firmware is valid, and for this platform
	# 2. Any checksums have been validated, when upgrading via NET
	# 3. Firmware will always be /tmp/fwupdate.bin

	cp /tmp/fwupdate.bin /var/log/fwupdate.bin
	sync
	...
	# Lets avoid module cleanup to reduce panic or deadlock risk, plus
	# this is not needed since we are not upgrading until after the reboot
	# do_cleanup_modules

	do_unmount_logs

	# The upgrade will be performed within preinit on the next boot
	reboot -f
...
list_phys_ifs() {
	...
	do
		for vap in $(cat /var/run/${i}_devnames | grep -v vwire | grep -v scan)
		do
			if [ "$vap" ]
			then
	...
	for ath in `cat /var/run/$1_devnames`; do
		...
		for i in `seq 1 5`; do
			...
			if [ ! -z "$ifup" ]; then
				...
				local init_cmd="iwpriv $radio minrssi 70; iwpriv $ath kickall 1"
