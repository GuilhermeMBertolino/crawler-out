	...
	if [ "$force_hostapd_attach" -eq 1 ]; then
		...
		then
			...
			if [ "$map" -ge 3 -a $band -ne 3 ]; then
				run_wpa_cli $ifname "$WPAD_VARRUN/wpa_supplicant-$ifname" "DPP_BOOTSTRAP_GEN $dpp_type $dpp_curve $dpp_key"
			else
				run_wpa_cli $ifname  "$WPAD_VARRUN/wpa_supplicant-$ifname" "DPP_BOOTSTRAP_GEN $dpp_type $dpp_curve  $channel mac=$(cat /sys/class/net/$ifname/address | sed 's/://g') $dpp_key"
			fi
