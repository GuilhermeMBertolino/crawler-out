...
hostapd_set_bss_options() {
	...
	local tmp_vif="$2"
	local flag1=1
	local hidden beacon_prot owrt_versn
	local auth_algs
	...
	if (awk -v a="$owrt_versn" -v b="19.07" 'BEGIN{print(a>=b)}'); then
		flag1=0
		for temp in $vif; do
	...
	config_get_bool noauth_pasn_activated "$vif" noauth_pasn_activated 1
	...
	append hostapd_cfg "noauth_pasn_activated=$noauth_pasn_activated" "$N"
	...
	[ -f $WPAD_VARRUN/hostapd-$phy/$ifname ] && rm $WPAD_VARRUN/hostapd-$phy/$ifname
	...
	if [ $mld_netdev == 1 ]; then
		...
		ap-sdxpinn*)
			...
			bridge=$(echo "$bridge_tmp" | cut -d' ' -f1)
		;;
		*)
	...
	*)
	...
	then
		...
		if [ "${ieee80211w}" -eq 2 ] || [ "${ieee80211w}" -gt 0 -a "$mld" ]
		then
			add_sha256=1
		fi
...
hostapd_setup_vif() {
	...
	local no_nconfig
	local ifname device channel hwmode htmode dpp_operating_class ccfs puncture_bitmap
	local fst_disabled
	local fst_iface1
	local fst_iface2
	local fst_group_id
	...
	if [ "$driver" == "nl80211" ]; then
		...
		# these are in the right orders (most generic at the end)
			...
			*bea:HT40+|*bea:HT40-|*bea:HT40|*bea:EHT40+|*bea:EHT40-|*bea:EHT40)
				...
				append hostapd_cfg "eht_oper_chwidth=0" "$N"
				append hostapd_cfg "eht_oper_centr_freq_seg0_idx=$idx" "$N"
				append hostapd_cfg "puncture_bitmap= $puncture_bitmap" "$N"
				;;
			*bea:HT80|*bea:EHT80)
				nl_hwmode=a
				append hostapd_cfg "ieee80211ac=1" "$N"
...
EOF
	...
	if [ $wps_vap_tie_dbdc -ne 0 ]; then
		...
		if [ "$force_hostapd_attach" -eq 1  ]; then
			...
			wpa_cli -g $WPAD_VARRUN/hostapd/global raw ADD bss_config=$ifname:/var/run/hostapd-$ifname.conf
			[ $? -eq 0 ] || return
			...
			[ -n "$ifname" ] && run_hostapd_cli $ifname "$WPAD_VARRUN/hostapd-$device" "close_log"
			if [ -n "$wps_possible" -a -n "$config_methods" ]; then
				pid=$WPAD_VARRUN/hostapd_cli-$ifname.pid
				run_hostapd_cli_daemon $ifname "$WPAD_VARRUN/hostapd-$device" "-P $pid -a /lib/wifi/wps-hostapd-update-uci -B"
			...
			then
				...
				if [ "$map" -eq 0 ]; then
					run_hostapd_cli $ifname "$WPAD_VARRUN/hostapd-$device" "-P $pid -a /lib/wifi/dpp-hostapd-update-uci -B"
				fi

				run_hostapd_cli $ifname "$WPAD_VARRUN/hostapd-$device" "DPP_CONTROLLER_STOP"

				hostapd_cli -i $ifname -p $WPAD_VARRUN/hostapd-$device dpp_bootstrap_remove \*
				hostapd_cli -i $ifname -p $WPAD_VARRUN/hostapd-$device dpp_pkex_remove \*
				hostapd_cli -i $ifname -p $WPAD_VARRUN/hostapd-$device dpp_configurator_remove \*
				if [ "${type}" == "pbbs" ]; then
					echo "Push Button based bootstrapping generation"
				else
					if [ "$map" -ge 3 -a $band -ne 3 ]; then
						run_hostapd_cli $ifname  "$WPAD_VARRUN/hostapd-$device" "DPP_BOOTSTRAP_GEN $dpp_type $dpp_curve $dpp_key $supported_curves"
					else
						if [ $dpp_urihost_ifname ]; then
							run_hostapd_cli $ifname  "$WPAD_VARRUN/hostapd-$device" "DPP_BOOTSTRAP_GEN $dpp_type $dpp_curve $channel mac=$(cat /sys/class/net/$ifname/address | sed 's/://g') $dpp_key host=$(ifconfig $dpp_urihost_ifname | grep "inet addr" | awk -F ':' '{print $2}' | awk '{print $1}') $supported_curves"
						else
							run_hostapd_cli $ifname  "$WPAD_VARRUN/hostapd-$device" "DPP_BOOTSTRAP_GEN $dpp_type $dpp_curve  $channel mac=$(cat /sys/class/net/$ifname/address | sed 's/://g') $dpp_key $supported_curves"
						fi
					fi
				fi

				if [ "$dpp_mud_url" ]; then
					run_hostapd_cli $ifname "$WPAD_VARRUN/hostapd-$device" "SET dpp_mud_url $dpp_mud_url"
				fi
				...
				if [ "$mdns_action" == "advertise" ]; then
					...
					if [ "$mdns_role" == "controller" ]; then
						res=$(hostapd_cli -i $ifname -p $WPAD_VARRUN/hostapd-$device dpp_bootstrap_gen $dpp_type $dpp_curve)
						pkhash=echo $(hostapd_cli -i $ifname -p $WPAD_VARRUN/hostapd-$device dpp_bootstrap_info $res) | sed -r 's/.*pkhash=(\S+).*/\1/'
				...
				if [ -z "$dpp_controller" ] && [ "$map" -eq 0 ]; then
					if [ "$dpp_auth_role" == "initiator" ]; then
						...
						if [ "${type}" == "qrcode" ]
						then
							run_hostapd_cli $ifname  "$WPAD_VARRUN/hostapd-$device" "dpp_auth_init peer=1 role=enrollee $neg_freq $tcp_addr"
						elif [ "${type}" == "pbbs" ]; then
							run_hostapd_cli $ifname "$WPAD_VARRUN/hostapd-$device" "dpp_configurator_add $dpp_curve"
						else
							run_hostapd_cli $ifname  "$WPAD_VARRUN/hostapd-$device" "dpp_pkex_add own=1 init=1 $pkex_ver $pkex_identifier role=enrollee $pkex_code $tcp_addr"
						fi
					else
						#Responder configuration
						if [ "dpp_over_tcp" -ne 0 ]; then
							run_hostapd_cli $ifname "$WPAD_VARRUN/hostapd-$device" "DPP_CONTROLLER_START qr=single"
						fi

						if [ "${type}" == "qrcode" ]; then
							run_hostapd_cli $ifname  "$WPAD_VARRUN/hostapd-$device" "dpp_listen $dpp_freq role=enrollee"
						else
							run_hostapd_cli $ifname  "$WPAD_VARRUN/hostapd-$device" "dpp_pkex_add own=1 $pkex_ver $pkex_identifier role=enrollee $pkex_code"
						fi

						if [ $dpp_chirp -eq 1 ]; then
							run_hostapd_cli $ifname "$WPAD_VARRUN/hostapd-$device" "dpp_chirp own=1 iter=10 listen=$dpp_freq"
						fi
