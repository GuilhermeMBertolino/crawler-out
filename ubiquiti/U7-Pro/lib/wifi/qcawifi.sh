...
g_platform_features="enable_daemon_support enable_cold_boot_support enable_hds_support enable_regdb_support enable_qdss_tracing"
g_pci_targets="qcn9000 qcn6122 qcn9160 qcn9224"
...
get_config_file_path() {
	...
	*) return ;;
	...
	#                 cfg/default is in /
	# for SDX65/75 boards, ini files are in /etc/misc/ipq/ini and /etc/misc/ipq/ini/internal
...
update_global_daemon_coldboot_qdss_support_variables() {
	...
	if [ -d /sys/module/cnss2 ]
	then
		g_cnss2_module_param_path="/sys/module/cnss2/parameters"
	elif [ -d /sys/module/ipq_cnss2 ]
	then
		g_cnss2_module_param_path="/sys/module/ipq_cnss2/parameters"
	fi
...
start_cnssdaemon() {
	...
	do
		...
		do
			daemon=`grep -i $board_name_prefix $fw_ini_file | grep "enable_daemon_support" | grep -i $target | grep -i $slot | awk -F'=' '{print $2}' | grep -c 1`
			if [ $daemon != 0 ]
			then
				args=$args" -i "$target"_"$slot
			fi
		done
	done
...
function update_ini_nss_info()
{
	[ -f /lib/wifi/wifi_nss_hk_olnum ] && { \
		...
		if [ -e /sys/firmware/devicetree/base/MP_256 ]; then
			...
			update_internal_ini QCN9160_i.ini dp_nss_comp_ring_size 0x2000
...
function update_ini_napi_scale_factor()
{
	...
	ap-al*|ap-mi*)
		...
		[ -f /ini/internal/QCN9224_i.ini ] && {
			update_internal_ini QCN9224_i.ini dp_napi_scale_factor 2
		}
...
function update_ini_for_512MP_dp_tx_desc()
{
	...
	if [ $1 == "QCN9000_i.ini" ] || [ $1 == "QCN6122_i.ini" ] ||
	   [ $1 == "QCN9160_i.ini" ]; then
...
function update_ini_for_1G_MP_P_build()
{
    local board_name
...
load_qcawifi() {
	...
	[ -n "$enable_cfg80211" ] && {
		echo "qcawifi configuration is disable" > /dev/console;
		return 1;
	}
...
enable_qcawifi() {
	...
	case "$board_name" in
	ap-hk*|ap-ac*|ap-cp*|ap-oak*|ap-mp*|*emu*|ap-al*)
	...
	*)
	...
	if [ -e /sys/firmware/devicetree/base/MP_256 ]; then
		...
		*)
		...
		# Force all the radios in NSS offload mode disabled on 1G and 512M profile
		case "$board_name" in
		*hk*|*ac*|*cp*|*oa*|*mp*)
			update_ini_file nss_wifi_olcfg 0
		;;
...
detect_qcawifi() {
	...
	if [ -e /sys/firmware/devicetree/base/MP_256 ]; then
		...
		update_ini_for_lowmem QCN9160_i.ini
	fi
...
EOF
	...
	if [ ! -f /sys/firmware/devicetree/base/MP_256 ]; then
		if [ $num_chains -le 2 ]; then
			...
			update_ini_for_mon_buf_ring_chainmask_2 QCN9160_i.ini
			update_ini_for_mon_buf_ring_chainmask_2 QCA9574_i.ini
			...
			update_ini_for_monitor_buf_ring QCN9160_i.ini
			update_ini_for_monitor_buf_ring QCA9574_i.ini
		fi
	fi
	...
	if [ -e /sys/firmware/devicetree/base/MP_512 ]; then
		...
		if [ $is_e_build -ne 1 ]; then
			...
			update_ini_for_512MP_E_build QCN9160_i.ini
			update_ini_for_512MP_E_build QCA9574_i.ini
		fi
	fi
	...
	case "$board_name" in
		ap*)
			if [ ! -f /sys/firmware/devicetree/base/MP_256 ] && [ ! -f /sys/firmware/devicetree/base/MP_512 ]; then
				...
				update_ini_for_512MP_dp_tx_desc QCN6122_i.ini
				update_ini_for_512MP_dp_tx_desc QCN9160_i.ini
				update_ini_for_1G_MP_P_build QCA5018_i.ini
				update_ini_for_1G_MP_P_build QCN6122_i.ini
				update_ini_for_1G_MP_P_build QCN9160_i.ini
			fi
			;;
		*)
	esac
