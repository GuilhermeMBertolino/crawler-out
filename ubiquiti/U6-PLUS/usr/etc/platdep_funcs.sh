...
LOG_EMMC_LOCATION='/var/log'
LOG_EMMC_LOCATION_SENT='/var/log/crash_log-sent'
WIFICOREDUMP_LOG_LIMIT=10
...
save_emmc_log() {
	...
	local WIFICOREDUMP_CNT=$2
	local TARFILE=$3
	local WIFICOREDUMP_LOG_LIMIT=10
	sync

	mkdir -p ${LOG_EMMC_LOCATION}
	mkdir -p ${LOG_EMMC_LOCATION_SENT}
	if [ "$2" = "10" ]
	then
		cat /proc/meminfo > ${LOG_EMMC_LOCATION_SENT}/meminfo
		mpstat -A 2 2 > ${LOG_EMMC_LOCATION_SENT}/mpstat
		top -n 2 -d 2 > ${LOG_EMMC_LOCATION_SENT}/top
		info > ${LOG_EMMC_LOCATION_SENT}/info
		cp -f /etc/OSDA.bin ${LOG_EMMC_LOCATION_SENT}/
		cp -f /etc/WIFI.bin ${LOG_EMMC_LOCATION_SENT}/
		cp -f /etc/ULM2.bin ${LOG_EMMC_LOCATION_SENT}/
		cp -f /etc/WROM.bin ${LOG_EMMC_LOCATION_SENT}/
		cp -f /etc/RBUF.bin ${LOG_EMMC_LOCATION_SENT}/
		cp -f /etc/ULM3.bin ${LOG_EMMC_LOCATION_SENT}/
		cp -f /etc/CORE.bin ${LOG_EMMC_LOCATION_SENT}/
		cp -f /etc/EMID.bin ${LOG_EMMC_LOCATION_SENT}/
		cp -f ${LOG_EMMC_LOCATION}/messages.old ${LOG_EMMC_LOCATION_SENT}/messages.old
		cp -f ${LOG_EMMC_LOCATION}/messages ${LOG_EMMC_LOCATION_SENT}/messages
		rm -f ${LOG_EMMC_LOCATION_SENT}/wifidump.tgz
	fi

	cat ${LOG_EMMC_LOCATION}/messages.old ${LOG_EMMC_LOCATION}/messages > ${LOG_EMMC_LOCATION}/crash_log_${TIMESTAMP}_${WIFICOREDUMP_CNT}

	# remove old logs if we have more than a specified limit
	rm -f `ls -rtd ${LOG_EMMC_LOCATION}/crash_log_* | head -n -${WIFICOREDUMP_LOG_LIMIT}`

	sync
}

read_emmc_log() {
	crash_log_file=${LOG_EMMC_LOCATION_SENT}/messages
	if [ -s "${crash_log_file}" ]
	then
		full_log_file=${LOG_EMMC_LOCATION_SENT}/wifidump.tgz
		tar -czf ${full_log_file} ${LOG_EMMC_LOCATION_SENT}/messages ${LOG_EMMC_LOCATION_SENT}/messages.old ${LOG_EMMC_LOCATION_SENT}/OSDA.bin ${LOG_EMMC_LOCATION_SENT}/WIFI.bin ${LOG_EMMC_LOCATION_SENT}/ULM2.bin  ${LOG_EMMC_LOCATION_SENT}/WROM.bin ${LOG_EMMC_LOCATION_SENT}/RBUF.bin ${LOG_EMMC_LOCATION_SENT}/ULM3.bin ${LOG_EMMC_LOCATION_SENT}/CORE.bin ${LOG_EMMC_LOCATION_SENT}/EMID.bin ${LOG_EMMC_LOCATION_SENT}/meminfo ${LOG_EMMC_LOCATION_SENT}/mpstat ${LOG_EMMC_LOCATION_SENT}/top ${LOG_EMMC_LOCATION_SENT}/info 
		logger "hexdumping file:${full_log_file}"
		hexdump -ve '32/1 "%02x " "\n"' ${full_log_file} |while read line
		do
			logger "${line}"
		done
		logger "end of hexdump:${full_log_file}"
		md5sum ${full_log_file}  |logger

		echo "FWCmdTimeout: md5sum of messages"
		md5sum ${LOG_EMMC_LOCATION_SENT}/messages
		cat ${LOG_EMMC_LOCATION_SENT}/messages
		echo "FWCmdTimeout: md5sum of log file"
		md5sum ${full_log_file}
		base64 ${full_log_file}

		mv ${LOG_EMMC_LOCATION}/crash_log_* ${LOG_EMMC_LOCATION_SENT} >/dev/null
		rm ${LOG_EMMC_LOCATION_SENT}/messages

		# remove old logs if we have more than a specified limit
		rm -f `ls -rtd ${LOG_EMMC_LOCATION_SENT}/crash_log_* | head -n -${WIFICOREDUMP_LOG_LIMIT}`
	else
		oopsdump | if [ $? -eq 0 ]
		then
			tail -c 1000k
