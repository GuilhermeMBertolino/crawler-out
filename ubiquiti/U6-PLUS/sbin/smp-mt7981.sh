...
get_if_info()
{
	...
	do
		if echo "$vif" | grep -qE "eth0|ra[0-9]|rai[0-9]|apclii0"; then
	...
	do
		...
		if echo $v | egrep -q "wmac"; then
			WIFI_RADIO1=1
		fi
		if echo $v | egrep -q "wbsys"; then
            WIFI_RADIO1=1
        fi

		if echo $v | egrep -q "pci0000"; then
			WIFI_RADIO2=1
		fi

		if echo $v | egrep -q "pci0001"; then
			WIFI_RADIO3=1
		fi
	done;
