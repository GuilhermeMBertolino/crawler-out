...
guest_update_interface_ebtables() {
	...
	if [ -f "$MARK_FILE" ]; then
		...
		guest_error "Cannot update ebtables for $IFACE, $MARK_FILE doesn't exist"
	fi
	...
	while true; do
		if [ -z "$PORTAL" ]; then
			guest_debug "Couldn't find a portal to $ACTION ebtable jump for $IFACE"
			break
		fi
		...
		local GUESTIN
		if ! GUESTIN="$(ebtable_chain_containing nat "-j ${AUTH_CHAIN}\>")"; then
		...
		local GUESTOUT=$(echo "$GUESTIN" | sed -n 's/IN/OUT/p')
		if [ -z "$GUESTOUT" ]; then
