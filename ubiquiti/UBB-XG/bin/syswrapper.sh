...
. /usr/lib/ubnt/common.sh
. /usr/lib/ubnt/platdep_funcs.sh
...
state_reload() {
	...
	if [ -f /etc/do-firmware-update ]; then
		set_led upgrade
		return
	fi
	...
	if [ "$default" == "true" ]; then
        ...
        if [ -f /var/run/ledbar.default.color ]; then
            set_led color "$(cat /var/run/ledbar.default.color)"
        else
    		set_led color 3
        fi
	fi
...
do_update_uplink() {
    ...
    sleep 2
    iface_bond=$(vppctl show l2fib verbose | grep $(arping -c 5 -f -I $(ip route show match 0/0 | awk '{print $5, $3}') | awk '/reply from/ {print $5 }' | tr -d '][' | sed 's/\b\(\w\)\b/0\1/g') | awk '{print $9}')
    iface_hw=$(vppctl show hardware ${iface_bond} | awk '/'${iface_bond}'/ { print $4 }')
    iface=$(vppctl show bond details | awk '/BondEthernet/ {print} /number of active/ { getline; print }' | awk '/'${iface_hw}'/ { getline; print $1}' | sed 's/^vpp-//')
    rm /var/run/vapusage.*
    case ${iface} in
	bond0)
	    active_wireless=$(cat /var/run/radio.active)
...
ip-changed)
	update_ip $*
	# update lldp
	/usr/etc/init.d/plugin start lldp
	;;
ip-fallback)
	sleep 30
...
service-restarted)
	...
	RESTART_COUNT="$6"
	exit_if_busy $cmd $*
	cat /var/log/messages | grep "$PROCESS\[$PID\]" | sensitive_data_filter > /var/log/last_app_crash

	# Replace newlines with \n and trim the process log to last 8kB
	CRASH_LOG=`sed ':a; N; $!ba; s/\n/\\n/g' /var/log/last_app_crash | tail -c 8192`
	# Background in case script is called without working mcad
	mca-custom-alert.sh -k event_string -v EVT_AP_RestartProc -k caller -v "$CALLER" -k proc -v "$PROCESS" -k exit_status -v "$EXIT_CODE" -k restart_count -v "$RESTART_COUNT" -k crash_log -v "$CRASH_LOG" &
	;;
*)
	exit 1
	;;
