...
MT7622()
{
	...
	if [ "$num_of_wifi" = "0" ]; then
		...
		dbg "MT7622 with $NUM_OF_WIFI Wi-Fi bands"
	fi
}
...
MT7623()
{
	...
	if [ "$num_of_wifi" = "0" ]; then
		...
		dbg "MT7623 with $NUM_OF_WIFI Wi-Fi bands"
	fi
}
...
get_wifi_if_name()
{
	...
	# generate rps list for rai0~rai7
	gen_vifs_to_rps_if "wifi2_prefix" 8
...
scan_wifi_num()
{
	if [ -d "/sys/class/net/$wifi3" ]; then
		NUM_OF_WIFI=3
	elif [ -d "/sys/class/net/$wifi2" ];then
		NUM_OF_WIFI=2
	elif [ -d "/sys/class/net/$wifi1" ];then
		NUM_OF_WIFI=1
	else
		NUM_OF_WIFI=0
	fi

	dbg "# NUM_OF_WIFI=$NUM_OF_WIFI"
}
