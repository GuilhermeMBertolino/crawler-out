...
exit_if_default() {
    ...
    [ "$isDefault" = "unknown" ] && error_out "not sending event in unknown state"
}
...
json_output="{"
while getopts ":k:v:j:hdf:" opt; do
    case $opt in
        k)
            num_key=$((num_key+1))
        ...
        v|j)
            ...
            json_output="${json_output}\"${JSON_KEY}\":${quotes}${JSON_VALUE}${quotes},"
            ;;
        h)
            usage
            exit 0
...
mca-ctrl -t event -f ${json_tmp_file} -q
